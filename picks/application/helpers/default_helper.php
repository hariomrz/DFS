...
function get_exclude_class_methods(){
    $class = array();
    $method = array("lobby/get_sports_list","lobby/get_lobby_fixture","lobby/get_lobby_filter","lobby/get_fixture_contest","contest/get_contest_detail");
    $event_arr = array();
...
function generate_entity_uid($name=""){
    $len = 10;
    $name_str = "";
    if($name != ""){
        $words = explode(" ",trim($name));
        foreach($words as $w) {
          $name_str .= mb_substr($w, 0, 1);
        }
    }
    $len = $len - strlen($name_str);
    $str_data = '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
    $str_result = substr(str_shuffle($str_data),0, $len);
    $entity_key = strtoupper($name_str.$str_result);
    return $entity_key;
}
...
function reset_contest_prize_data($contest)
{
    ...
    $prize_data = $contest['prize_distibution_detail'];
    if(!is_array($contest['prize_distibution_detail'])){
...
function get_picks_master_data($season_data,$picks_master_data )
{
        
        
        $records = array();
        // $records["picks_data"]['question']  = !empty($season_data['question'])?$season_data['question']: $picks_master_data['questions'];
        // $records["picks_data"]['correct']   = !empty($season_data['correct'])?$season_data['correct']: $picks_master_data['correct'];
        // $records["picks_data"]['wrong']     = !empty($season_data['wrong'])?$season_data['wrong']: $picks_master_data['wrong'];
...
function get_media_setting($type){
    $media_arr = array();
    $media_arr['jersey'] = array("min_h"=>"36","max_h"=>"250","min_w"=>"36","max_w"=>"200","size"=>"2","type"=>array("jpg","png","jpeg","gif"),"path"=>"upload/jersey");
    $media_arr['flag'] = array("min_h"=>"42","max_h"=>"250","min_w"=>"42","max_w"=>"250","size"=>"2","type"=>array("jpg","png","jpeg"),"path"=>"upload/flag");
     $media_arr['option_image'] = array("min_h"=>"200","max_h"=>"200","min_w"=>"200","max_w"=>"200","size"=>"2","type"=>array("jpg","png","jpeg"),"path"=>"upload/picks");
    $media_arr['explaination_image'] = array("min_h"=>"200","max_h"=>"200","min_w"=>"520","max_w"=>"520","size"=>"2","type"=>array("jpg","png","jpeg"),"path"=>"upload/picks");
