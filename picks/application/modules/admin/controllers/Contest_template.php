...
class Contest_template extends Common_Api_Controller {
	...
	/**
     * Used for getting all master data
     * @param []
     * @return json array
     */
	...
	{		
		...
		if($allow_picks == 1) {
			...
			$allow_pick_data = $this->app_config['allow_picks']['custom_data'];
		...
		$result = array(
					...
					'allow_pick_data'           => $allow_pick_data
				);		
    ...
    */
    ...
    /**
     * get Master group data
     * []
     * @return json array
     */
	public function get_group_list_post(){
		$this->load->model('Contest_template_model');
        $post_data = $this->input->post();
		$response = $this->Contest_template_model->get_group_list($post_data);
	...
	/**
    * [get_fixture_template_post description]
    ...
    */
    public function get_fixture_template_post()
	{
		...
		$this->form_validation->set_rules('season_id', 'Season Ids', 'trim|required');
		...
		$season_ids = $post_data['season_id'];
