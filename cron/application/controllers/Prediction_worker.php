...
class Prediction_worker extends MY_Controller
{
    ...
    public function process_prediction_refund() {
        ...
        $callback = function($msg)
        {
            try{
                ...
                // var_dump($data);
                // die('dfd');
                $this->Prediction_model->refund_coins($data);
            }
            //catch exception
            catch(Exception $e) 
            {
    ...
    public function process_prediction() {
        ...
        $callback = function($msg)
        {
            try{
                ...
                $this->Prediction_model->$function_name($data);
            }
            //catch exception
            catch(Exception $e) 
            {
    ...
    public function process_prediction_feed()
    {
        $queue_name = "prediction_feed";
        $connection = new AMQPStreamConnection(MQ_HOST, MQ_PORT, MQ_USER, MQ_PASSWORD);
        $channel = $connection->channel();
        $channel->queue_declare($queue_name, false, true, false, false);
        echo ' [*] Waiting for messages. To exit press CTRL+C '.ENVIRONMENT, "\n";
