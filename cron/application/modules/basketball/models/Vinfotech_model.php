...
class Vinfotech_model extends MY_Model
{
	...
	{
		parent::__construct();
        $this->api = $this->get_basketball_config_detail('vinfotech');
		...
		if (!empty($current_game))
		{
			...
			foreach ($current_game as $season_game)
			{
				...
				{	
					...
					foreach ($match_array['response']['data'] as $key => $value) 
					{
						...
						$score_data[] = array(
	                        ...
	                        "field_goals_attempted"				=> $value['field_goals_attempted'],
	                        "field_goals_missed"					=> $value['field_goals_missed'],
	                        "free_throws_made"						=> $value['free_throws_made'],
	                        "free_throws_attempted"						=> $value['free_throws_attempted'],
	                        "free_throws_missed"				=> $value['free_throws_missed'],
	                        "rebounds"				=> $value['rebounds'],
	                        "assists"					=> $value['assists'],
	                        "steals"			=> $value['steals'],
	                        "blocked_shots"					=> $value['blocked_shots'],
					...
					{
						//echo "<pre>";print_r($score_data);die;
						$this->replace_into_batch(GAME_STATISTICS_BASKETBALL, array_values($score_data));
		    	...
		    	if(!empty($match_info))
				{
					...
					echo "<br> Scores updated for the match : ".$season_game_uid." <br>";
				}
	...
	public function calculated_fantasy_score($sports_id)
	{
		...
		if (!empty($current_game))
		{
			...
			foreach ($current_game as $season_game)
			{   
				$this->calculate_basketball_fantasy_points($sports_id,$season_game,$formula);	   
			}// End of Team loop  
     ...
     * @return array
     */
	public function calculate_basketball_fantasy_points($sports_id,$season_game,$formula)
	{
		...
		if (!empty($game_stats))
		{
            ...
            {
                //echo "<pre>";print_r($player_stats);die;
                $normal_score       = 0;
                ...
                {
                    $normal_score = $normal_score + ($player_stats['field_goals_missed'] * $formula['normal']['FIELD_GOALS_MISSED'] );
                    $normal_testing_var['FIELD_GOALS_MISSED'] = ( $player_stats['field_goals_missed'] * $formula['normal']['FIELD_GOALS_MISSED'] );
                }
                //free_throws_missed : FREE_THROWS_MISSED
                if ($player_stats['free_throws_missed'] > 0)
                {
                    $normal_score = $normal_score + ($player_stats['free_throws_missed'] * $formula['normal']['FREE_THROWS_MISSED'] );
                    $normal_testing_var['FREE_THROWS_MISSED'] = ( $player_stats['free_throws_missed'] * $formula['normal']['FREE_THROWS_MISSED'] );
                }

                //rebounds : REBOUNDS
                if ($player_stats['rebounds'] > 0)
                {
                    $normal_score = $normal_score + ($player_stats['rebounds'] * $formula['normal']['REBOUNDS'] );
                    $normal_testing_var['REBOUNDS'] = ( $player_stats['rebounds'] * $formula['normal']['REBOUNDS'] );
                }
                //assists 
                if ($player_stats['assists'] > 0)
                {
                    $normal_score = $normal_score + ($player_stats['assists'] * $formula['normal']['ASSISTS'] );
                    $normal_testing_var['ASSISTS'] = ( $player_stats['assists'] * $formula['normal']['ASSISTS'] );
                }
                
                //blocked_shots
                if ($player_stats['blocked_shots'] > 0)                   
                {                           
                    $normal_score = $normal_score + ($player_stats['blocked_shots'] * $formula['normal']['BLOCKED_SHOT'] );
                    $normal_testing_var['BLOCKED_SHOT'] = ( $player_stats['blocked_shots'] * $formula['normal']['BLOCKED_SHOT'] );
                }
                //steals
                if ($player_stats['steals'] > 0)
                {
                    $normal_score = $normal_score + ($player_stats['steals'] * $formula['normal']['STEALS'] );
                    $normal_testing_var['STEALS'] = ( $player_stats['steals'] * $formula['normal']['STEALS'] );
                }
                //turnovers
                if ($player_stats['turnovers'] > 0)
                {
                    $normal_score = $normal_score + ($player_stats['turnovers'] * $formula['normal']['TURNOVERS'] );
                    $normal_testing_var['TURNOVERS'] = ( $player_stats['turnovers'] * $formula['normal']['TURNOVERS'] );
                }

                //each points
                if ($player_stats['points'] > 0)
                {
                    $normal_score = $normal_score + ($player_stats['points'] * $formula['normal']['EACH_POINT'] );
                    $normal_testing_var['EACH_POINT'] = ( $player_stats['points'] * $formula['normal']['EACH_POINT'] );
                }
				...
				//booster
				$booster_arr = array();
				$booster_arr = $this->get_booster_break_down($all_player_testing_var[$player_stats['player_id']]['normal']);
	...
	private function get_booster_break_down($final_break_down){
		$booster_arr = array("DICKEYS_MIDDLE_RING"=>"0","MISS_BUTLERS_TURNOVER"=>"0","GRAND_THEFT_BALLER"=>"0","BLOCK_PARTY"=>"0","CAPTAIN"=>"0");
		if(isset($final_break_down["EACH_POINT"])){
			$booster_arr["DICKEYS_MIDDLE_RING"] = $final_break_down["EACH_POINT"];
		}

        if(isset($final_break_down["TURNOVERS"])){
			$booster_arr["MISS_BUTLERS_TURNOVER"] = $final_break_down["TURNOVERS"];
		}

        if(isset($final_break_down["STEALS"])){
			$booster_arr["GRAND_THEFT_BALLER"] = $final_break_down["STEALS"];
		}

        if(isset($final_break_down["BLOCKED_SHOT"])){
			$booster_arr["BLOCK_PARTY"] = $final_break_down["BLOCKED_SHOT"];
		}

		  /* if(isset($final_break_down["RECEIVING_TOUCHDOWNS"])){
					$booster_arr["HOT_BENCH"] = $final_break_down["RECEIVING_TOUCHDOWNS"];
				}
		*/

        if(isset($final_break_down["ASSISTS"])){
			$booster_arr["CAPTAIN"] = $booster_arr["CAPTAIN"] + $final_break_down["ASSISTS"];
		}
        if(isset($final_break_down["EACH_POINT"])){
			$booster_arr["CAPTAIN"] = $booster_arr["CAPTAIN"] + $final_break_down["EACH_POINT"];
		}
        if(isset($final_break_down["REBOUNDS"])){
			$booster_arr["CAPTAIN"] = $booster_arr["CAPTAIN"] + $final_break_down["REBOUNDS"];
		}
		return $booster_arr;
	}
	...
	private function get_game_statistics_by_season_id($season_id,$sports_id,$league_id)
    {
        $sql = $this->db->select("GSB.*,PT.position", FALSE)
                ->from(GAME_STATISTICS_BASKETBALL . " AS GSB")
