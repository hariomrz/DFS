...
class Livestockfantasy_model extends MY_Model {
    ...
    */
    ...
    public function get_my_joined_contests($post_data) {
        ...
        COUNT(DISTINCT C.contest_id) as contest_count, COUNT(DISTINCT LMC.lineup_master_id) as team_count,LMC.lineup_master_contest_id,LMC.total_score,LMC.game_rank,LM.lineup_master_id,LM.team_name,C.contest_id, C.contest_title,C.brokerage", false)
     ...
     * @return array
     */
    public function get_lineup_details($post_data) {
        $result = $this->db->select("LM.lineup_master_id,LM.collection_id,CM.scheduled_date,CM.is_lineup_processed,0 as deadline_time,LMC.total_score,LMC.lineup_master_contest_id,LM.user_id,LM.user_name,LM.team_name,LM.team_data,CM.category_id,CM.published_date,CM.end_date,CM.name as collection_name,LMC.game_rank,CM.score_updated_date,LM.team_short_name,LM.remaining_cap")
                ->from(LINEUP_MASTER_CONTEST . " LMC")
                ...
                ->where("LMC.lineup_master_id", $post_data['lineup_master_id'])
    ...
    public function get_portfolio_score($post_data)
    {
        $result = $this->db->select('LMC.total_score,LM.team_data as stock_data')
                    ->from(LINEUP_MASTER_CONTEST. " LMC")
    ...
    */
    public function get_all_stocks($post_data)
    {
        ...
        (
            TRUNCATE((CASE WHEN C.scheduled_date <= "'.$current_date.'" AND C.end_date >= "'.$current_date.'"  THEN ((S.last_price-SH.close_price)/SH.close_price)*100  
                 WHEN C.end_date < "'.$current_date.'" THEN ((SH1.close_price-SH.close_price)/SH.close_price)*100 
    ...
    public function get_user_fixture_contest($status) {
        ...
        LMC.lineup_master_contest_id,LMC.total_score,LMC.game_rank,LMC.is_winner,LMC.prize_data,
        LM.lineup_master_id,LM.user_id,LM.team_name,
        IFNULL(LMCC.lm_count, '0') as user_joined_count,MG.group_name,LMC.last_score,LMC.percent_change,LMC.last_percent_change,CM.scheduled_date,CM.end_date,CM.collection_id,C.brokerage";
        $this->db->select($select, false)
                ->from(LINEUP_MASTER_CONTEST . ' LMC')
                ->join(LINEUP_MASTER . ' LM', 'LMC.lineup_master_id = LM.lineup_master_id', 'INNER')
                ->join(CONTEST . ' C', 'C.contest_id = LMC.contest_id', 'INNER')
    ...
    function get_pending_user_trade($where=array())
    {
        $result = $this->db->select("SUM(lot_size) as lot_size_pending,stock_id,sum(trade_value) as total_trade_value,status")
                        ->from(USER_TRADE)
                        ->where($where)
                        ->group_by('stock_id')
                        ->get()->result_array();
    ...
    public function join_game($contest, $post_data) {
        ...
        if(!empty($contest_data) && $contest_data['total_user_joined'] < $contest_data['size']){
            ...
            $portfolio_arr = array();
            $portfolio_arr['collection_id'] = $contest['collection_id'];
            $portfolio_arr['user_id'] = $this->user_id;
            $portfolio_arr['user_name'] = $this->user_name;
            $portfolio_arr['team_name'] = "Portfolio";
            $portfolio_arr['team_short_name'] = "P";
            $portfolio_arr['added_date'] = format_date();
            $portfolio_arr['modified_date'] = format_date();

            $lineup_master_id = $this->save_new_lineup($portfolio_arr);

            $lineup_master_contest = array();
            $lineup_master_contest['lineup_master_id'] = $lineup_master_id;
            $lineup_master_contest['contest_id']  = $contest["contest_id"];
            $lineup_master_contest['total_score'] = $this->salary_cap;
            $lineup_master_contest['last_score']  = $this->salary_cap;
    ...
    public function get_user_transaction($post_data)
    { 
        $lineup_master_id = $post_data['lineup_master_id'];      
           $results = $this->db->select('UT.transaction_id,UT.trade_value,UT1.brokerage,UT.closing_balance,UT.type,UT.modified_date as added_date,S.display_name as name,S.logo,IF(UT.brokerage =0, 0, 1) as is_brokerage,UT.lot_size,UT.status')
                ->from(USER_TRADE.' UT')
                ->join(USER_TRADE.' UT1',"UT.transaction_id=UT1.parent_id",'LEFT')
                ->join(STOCK.' S',"S.stock_id=UT.stock_id") 
                ->where('UT.lineup_master_id',$lineup_master_id)
                ->get()->result_array();
            return $results;  
    }

    function get_user_lineup($post_data=array()) {
       $results = $this->db->select('LM.*,LMC.total_score')
            ->from(LINEUP_MASTER.' LM')
            ->join(LINEUP_MASTER_CONTEST.' LMC',"LMC.lineup_master_id = LM.lineup_master_id")
            ->where('LM.user_id',$this->user_id)
            ->where('LM.collection_id',$post_data['collection_id'])
            ->where('LM.lineup_master_id',$post_data['lineup_master_id'])
            ->get()->row_array();
            return $results;          
    }
