...
class Live_stock_fantasy_model extends MY_Model {
    ...
    public function get_trade_history($post_data)
    {
      return   $this->db->select("UT.stock_id,UT.lot_size,UT.trade_value,UT1.brokerage,UT.price,UT.type,UT.status,UT.added_date,S.display_name")
                 ->from(USER_TRADE. " UT")
                 ->join(USER_TRADE. " UT1","UT.transaction_id=UT1.parent_id","LEFT")
                 ->join(STOCK. " S","S.stock_id=UT.stock_id")
                 ->where("UT.lineup_master_id",$post_data['lineup_master_id'])
                 ->where("UT.contest_id",$post_data['contest_id'])
                 ->get()->result_array();
    }
}