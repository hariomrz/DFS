...
class Live_stock_fantasy extends Common_Api_Controller {
    public $market_id = 1;
    public $stock_type=4;
	public function __construct() {
		parent::__construct();
		$_POST = $this->post();
		$this->load->model('admin/Live_stock_fantasy_model');
        $this->contest_categoty_map= array();
		$this->contest_categoty_map[1] = array('func' => 'get_daily_category_dates');
        define('CONTEST_PUBLISH_TIME',"11:00:00");
    ...
    */
    public function publish_fixture_post()
    {
        ...
        $this->form_validation->set_rules('published_date', 'PublishedDate', 'trim|required');
        ...
        $insert_data['collection_data']['is_lineup_processed'] = 1;
	...
	{      
        $scheduled_date = date('Y-m-d',strtotime($post['scheduled_date']));
        if(!$scheduled_date)
        {
        ...
        $start_time = $post['scheduled_date'];
        $end_time =   $post['end_date'];

        $custom_data = !empty($this->app_config['allow_live_stock_fantasy'])?$this->app_config['allow_live_stock_fantasy']['custom_data']:array();
        ...
        if($minutes < $min_candle)
        {
            $msg = "Candle time should not be less than #min#";
    ...
    {
        $collection_exits= $this->Live_stock_fantasy_model->check_fixture_exists($category_id,$start_time,$end_time);
    ...
    {
        $industry_list= $this->Live_stock_fantasy_model->get_industry_list();
        // print_r($collection_exits);
        // die('fd');
    ...
    function get_candle_time_list_post()
    {
        ...
        $times= $this->Live_stock_fantasy_model->get_candle_time_list($post_data['fixture_date']);
    ...
    function stock_closing_rate_for_time_post()
    {
		...
		$result = $this->Live_stock_fantasy_model->stock_closing_rate_for_time($schedule_date);
    ...
    function update_stock_rate_post()
    {
        ...
        $data['stocks'] = $stocks;
        $flag = $this->Live_stock_fantasy_model->update_time_price($data);
   ...
   {    
       ...
       $collection_data =  $this->Live_stock_fantasy_model->get_collection_stocks($postdata);
                     ...
                     print_r($result);die;*/
                      ...
                      if(!empty($stock_socket_enable)){
                        ...
                        foreach($history_data as $value) {              
                             ...
                             if($history_data) {
                              $this->Live_stock_fantasy_model->update_candle_price($history_data);
                             }  
   ...
   /**
    * Get lineup trade history
    * @param lineupmaster_id,contest_id
    * @return json array
    */
    public function get_user_trade_history_post(){
        $this->form_validation->set_rules('lineup_master_id', 'Lineup Master Id', 'trim|required');
        ...
        $post_data = $this->input->post();
        $history = $this->Live_stock_fantasy_model->get_trade_history($post_data);
