...
class Contest extends Common_Api_Controller {
    ...
    function get_my_lobby_fixtures_post() {
...
//           Completed-> Recent Completed Matches should show first. => DESC
    ...
    public function get_user_joined_fixture_by_status_post() {
        ...
        if (!empty($result)) {
            foreach ($result as $fixture) {
                ...
                $fixture['won_marchandise_list'] = filter_blank_values($fixture['won_marchandise_list']);
                $fixture['won_rank_list'] = filter_blank_values($fixture['won_rank_list']);
    ...
    public function get_user_contest_by_status_post() {
        ...
        $this->load->model("contest/Contest_model");
        $contest_list = $this->Contest_model->get_user_fixture_contest($collection_id, $status);
    ...
    public function validation_for_join_game($contest, $post_data) {
        ...
        $contest_joined = $this->Contest_model->get_single_row("lineup_master_contest_id", LINEUP_MASTER_CONTEST, array("contest_id" => $post_data['contest_id'], "lineup_master_id" => $post_data['lineup_master_id']));
        ...
        $user_lineup = $this->Contest_model->get_single_row("lineup_master_id,collection_id", LINEUP_MASTER, array("lineup_master_id" => $post_data['lineup_master_id'], "user_id" => $this->user_id));
        ...
        if ($contest['collection_id'] != $user_lineup['collection_id'] )  {
        ...
        if ($contest['prize_type'] == 2 || $contest['prize_type'] == 3) { // for Conins 
            ...
            // $banned_state = get_banned_state();
            // $banned_state_ids = array_keys($banned_state);
            // if(INT_VERSION != "1" && $this->currency_type != "2" && $this->entry_fee > 0){
            ...
            if ($is_valid || $contest['entry_fee'] == 0) {
                ...
                if (isset($joined['joined_count']) && $contest['entry_fee'] > 0) {
                    ...
                    if (empty($join_result) || $join_result['result'] == 0) {
                        ...
                        $this->Contest_model->remove_joined_game($contest, $joined['lineup_master_contest_id']);
                ...
                if (isset($joined['joined_count']) && $joined_status == 1) {
                    ...
                    $user_ct_cache = "user_ct_".$contest["collection_id"]."_".$this->user_id;
    ...
    public function get_contest_invite_code_post() {
        ...
        //check for existence
        $this->load->model("contest/Contest_model");
        $exist = $this->Contest_model->get_single_row("contest_id,code", INVITE, array("contest_id" => $contest['contest_id'], "user_id" => 0));
        if (empty($exist)) {
...
// die('fdf');
    ...
    public function check_eligibility_for_contest_post() 
    {
        ...
        $row = $this->Contest_model->get_single_row("contest_id", INVITE, $where);
    ...
    public function get_public_contest_post() {
        ...
        if (!$collection_details) {
            $collection_details = $this->Contest_model->get_single_row('name,scheduled_date,category_id',COLLECTION,array('collection_id' => $collection_id));
    ...
    public function get_user_contest_join_count_post() {
        ...
        $team_name = '';
        $this->load->model("contest/Contest_model");
        $result = $this->Contest_model->get_user_contest_join_count($post_data);
    ...
    public function switch_team_contest_post() {
        ...
        if ($is_valid) {
            ...
            $this->api_response_arry['message'] = $this->contest_lang['portfolio_switch_success'];
            $this->api_response();
        } else {
            $this->api_response_arry['response_code'] = rest_controller::HTTP_INTERNAL_SERVER_ERROR;
    ...
    public function get_new_contest_leaderboard_post()
    {
        ...
        $top_three= $this->Contest_model->get_new_contest_leaderboard_top_three($post );
        ...
        if($this->user_id)
        {
            $own_result= $this->Contest_model->get_new_contest_leaderboard($post ,$this->user_id);
        ...
        $result = $this->Contest_model->get_new_contest_leaderboard($post );
        ...
        $contest_details= $this->Contest_model->get_single_row('prize_distibution_detail, collection_id',CONTEST,array('contest_id' => $post['contest_id']) );
        ...
        if(!empty($contest_details['collection_id'])) {
            $collection_details= $this->Contest_model->get_single_row('IFNULL(score_updated_date,"") as score_updated_date',COLLECTION,array('collection_id' => $contest_details['collection_id']) );
        ...
        if(!empty($team_data['pl'])){
            foreach ($team_data['pl'] as $stock_id=>$score_data) {
                ...
                if(!empty($player_info)) {
                    ...
                    $stock_predict_accuracy_percent =  $this->Contest_model->get_single_row('accuracy_percent',LINEUP,array('stock_id'=>$score_data['stock_id'],'lineup_master_id'=>$score_data['lineup_master_id']));
    ...
    public function get_user_switch_team_list_post() {
        ...
        $post_data['user_id'] = $this->user_id;
        $this->load->model("contest/Contest_model");
        $team_data = $this->Contest_model->get_collection_contest_free_teams($post_data);
    ...
    public function validation_for_multiple_join_game($contest, $post_data) {
        ...
        //check for valid line up of user
        $user_lineup = $this->Contest_model->get_single_row("count(lineup_master_id) as total,GROUP_CONCAT(DISTINCT collection_id) as collection_id", LINEUP_MASTER, array("lineup_master_id IN(".implode(',',$lineup_master_ids).")" => NULL, "user_id" => $this->user_id));
    ...
    public function get_lineup_score_calculation_post()
    {
        ...
        $row = $this->Contest_model->get_single_row('collection_id,published_date,end_date,scheduled_date',COLLECTION,array('collection_id' => $post_data['collection_id']));

        $post_data['published_date'] = $row['published_date'];
        $post_data['end_date'] = $row['end_date'];
        $post_data['scheduled_date'] = $row['scheduled_date'];
        $result = $this->Contest_model->get_lineup_score_calculation($post_data);
        $this->api_response_arry['response_code'] = rest_controller::HTTP_OK;
        $this->api_response_arry['data'] = $result;
        $this->api_response();
    }
    ...
    public function validate_contest_promo_code_post() {
        ...
        $contest_id = $post_data['contest_id'];
        $this->load->model("Contest_model");
        $contest_info = $this->Contest_model->get_contest_detail($post_data);
        if (empty($contest_info)) {
            $this->api_response_arry['response_code'] = rest_controller::HTTP_INTERNAL_SERVER_ERROR;
            $this->api_response_arry['message'] = $this->contest_lang['invalid_contest'];
            $this->api_response();
    ...
    public function get_contest_leaderboard_post()
    {
        ...
        $result = $this->Contest_model->get_contest_leaderboard($post);
        ...
        $contest_details= $this->Contest_model->get_prize_and_updated_date($post['contest_id']);
