...
class PDF extends FPDF
{
    ...
    {  
        if ($this->page == 1)
        {
            ...
            if(!empty($data) && $data['stock_type'] == 3){
                $data['season_scheduled_date'] = $data['scheduled_date'];
                $data['collection_name'] = date('d-M h:i',strtotime($data['scheduled_date'])).'-' .date('h:i A',strtotime($data['end_date']));
