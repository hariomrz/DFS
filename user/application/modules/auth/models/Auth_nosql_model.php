...
class Auth_nosql_model extends NOSQL_Model {
     ...
     * @param int $is_systemuser
     * @return string
     */
    public function send_otp($data, $send_email_otp = FALSE, $is_systemuser = 0) {
        if (!empty($data['phone_no'])) {
            $phone_no = $data['phone_no'];
        } else {
            $phone_no = strtolower($data['email']);
        }

        $current_date = format_date();
        $otpData = array();
        if (isset($is_systemuser) && $is_systemuser == 1) {
            $otp = "1234";
        } else {
