...
var stock_conn = require('../db_stock_config')  
var lineupModel  = require('../models/stockLineupModel')
var query = util.promisify(stock_conn.query).bind(stock_conn);
...
exports.lineup_move = async function(req,res)
{
     var start = new Date();
	var end = 0;
     var collection_master_data= await lineupModel.get_current_live_collection();
          ...
          var lineup_team_data = [];
			     _.map(oneChunk,(oneTeam,ti)=>{
					...
					var team_stocks=  JSON.parse(oneTeam.team_data);
                         ...
                         let b_length = team_stocks.b.length;
                         if(typeof team_stocks.b === 'object')
                         {
                              b_length = Object.keys(team_stocks.b).length;
                         }
                         lineup_players_count = 0;
                         console.log('tc',tc,team_stocks.b,typeof(team_stocks.b));
				 	if(team_stocks.b && b_length>0)
				 	{
						
                              
				 		_.map(team_stocks.b,(player,pi)=>{
                                   console.log('tc',tc,player,pi);
				 			if(lineup_players_count<= tc)
				 			{
                                        ...
                                        {
                                             stock_id = pi;
                                             user_lot_size = player;
                                        }

								var captain = 0;
								if(stock_id==team_stocks.c_id)
								{
									captain=1;
								}

                                        if(stock_id==team_stocks.vc_id)
								{
									captain=2;
								}
								...
								stock_id_arr[stock_id].stock_id,
						          0,
								captain,
                                        1,//buy
                                        user_lot_size,
                         ...
                         let s_length = team_stocks.s.length;
                         if(typeof team_stocks.s === 'object')
                         {
                             s_length = Object.keys(team_stocks.s).length;
                         }
                         if(team_stocks.s && s_length>0)
                         {
                             
                              _.map(team_stocks.s,(player,pi)=>{
                                  if(lineup_players_count<= tc)
                                  {
