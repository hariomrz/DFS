 ...
 */ 
var predictionModel  = require('../models/fixedOpenPredictorModel')
		  ...
		  function firstStep(done) {
			...
			(pm_res,done)=>{
				...
				predictionModel.get_prediction_details(req,function(pm_err,pm_res){
					...
					{
						helper.res.response_code = 500
						helper.res.global_error ='Sorry, this prediction is closed. Predict other questions.'
					  res.status(500).send(helper.res)
					  return;
					}
					else{
		  ...
		  function secondStep(pm_res, done) {
		   ...
		   //make entry for prediction
			...
			(pm_res,done)=>{
				...
				predictionModel.update_prediction_master(req,function(pm_err,pm_res){
					helper.res.response_code = 200
