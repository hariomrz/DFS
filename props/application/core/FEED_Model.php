    ...
    */
    ...
    public function get_season_match_for_score($sports_id)
    {
        ...
        $this->db->select("S.home_id,S.away_id,S.year,S.season_id,S.season_game_uid, S.league_id,S.type, S.feed_date_time, S.scheduled_date, S.format, L.sports_id,L.league_abbr,T1.team_uid AS home_uid,T2.team_uid AS away_uid")
            ...
            //->where("L.status", '1')
            ->where("S.props_status", "0")
            ->where_in("S.status", array(0,1,3))
            //->where("S.league_id", '218')
    ...
    {
        $current_date_time = format_date();
        $past_date_time = date("Y-m-d H:i:s", strtotime(format_date()." -7 days"));
        $this->db->select("S.season_game_uid,S.season_id,S.league_id,S.scheduled_date, S.format,L.sports_id,S.status")
                ...
                ->where("S.props_status", '0')
                ->where("S.scheduled_date <='".$current_date_time."'")
                ->where("S.scheduled_date >='".$past_date_time."'")
                ->where("L.sports_id", $sports_id);
        //$this->db->where("S.season_id", 611);
        $this->db->group_by("S.season_id");
        $sql = $this->db->get();
    ...
    {
        $this->db->select("prop_id,fields_name")
                ->from(MASTER_PROPS)
                ->where("sports_id", $sports_id);
        $sql = $this->db->get();
        $props = $sql->result_array();
        $props = array_column($props, "prop_id","fields_name");
        return $props;
    }

    public function get_available_matches($sports_id)
    {
        $matches = $this->db->select("S.season_game_uid,S.home_id,S.away_id")
                    ->from(SEASON." AS S")
                    ...
                    ->result_array();
        if(empty($matches)){
            return array("matchids"=>[],"teamids"=>[]);
        }

        $rs['matchids'] = array_column($matches, "season_game_uid");
        $homeids = array_column($matches, "home_id");
        $awayids = array_column($matches, "away_id");
        $rs['teamids'] =  array_unique(array_merge($homeids,$awayids));
        return $rs;
    }
