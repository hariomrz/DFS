...
set_time_limit(0);
class Vinfotech_basketball_model extends FEED_Model
{
	private $api;
	public $db;
	...
	function __construct()
	{
		...
		$this->api = $this->get_sports_config_detail('basketball','vinfotech');
	}
			...
			foreach ($league_data as $league)
			{
				...
				if(!empty($season_array['response']['data']))
				{
					...
					foreach ($season_array['response']['data'] as $key => $value) 
					{
						...
						//$value['season_scheduled_date'] = date("Y-m-d H:i:s", strtotime($value['season_scheduled_date']." +90 days"));
    ...
    {
    	$url = "http://".ML_SERVER_API_URL."/cron/basketball_props/".$season_game_uid;
    	//echo $url ;die;
 		...
 		if(!empty($current_game))
 		{
 			foreach ($current_game as $key => $value) 
 			{
 				...
 				$season_status = $value['status'];
 				 $this->db->select("L.lineup_id,L.user_team_id,L.season_prop_id,L.type,L.value,SP.prop_id,SP.points,SP.player_id,MP.fields_name,SB.points,SB.rebounds,SB.assists,SB.minutes")
	                ->from(LINEUP." AS L")
	                ...
	                ->join(STATS_BASKETBALL." AS SB","SB.player_id = SP.player_id AND SB.season_id = SP.season_id AND SB.season_id = ".$season_id,"LEFT")
		        ...
		        if(!empty($lineups))
		        {
		        	...
		        	foreach ($lineups as $key => $lp) 
		        	{
		        		...
		        		$stats = 0;
		        		//Points
		        		if($lp['minutes'] >= '1' && $lp['fields_name'] == 'points' && $lp['points'] > $prop_value && $lp['type'] == "1"){
		        			$status = 1;
		        			$stats = $lp['points'];
		        		}elseif($lp['minutes'] >= 1 && $lp['fields_name'] == 'points' && $lp['points'] < $prop_value && $lp['type'] == "2"){
		        			$status = 1;
		        			$stats = $lp['points'];
		        		}elseif($lp['minutes'] >= 1 && $lp['fields_name'] == 'points'){
		        			$status = 2;
		        			$stats = $lp['points'];
		        		}elseif($lp['fields_name'] == 'points'){
		        			$status = 3;
		        		}
		        		...
		        		//rebounds
		        		if($lp['minutes'] >= 1 && $lp['fields_name'] == 'rebounds' && $lp['rebounds'] > $prop_value && $lp['type'] == "1"){
		        			$status = 1;
		        			$stats = $lp['rebounds'];
		        		}elseif($lp['minutes'] >= 1 && $lp['fields_name'] == 'rebounds' && $lp['rebounds'] < $prop_value && $lp['type'] == "2"){
		        			$status = 1;
		        			$stats = $lp['rebounds'];
		        		}elseif($lp['minutes'] >= 1 && $lp['fields_name'] == 'rebounds'){
		        			$status = 2;
		        			$stats = $lp['rebounds'];
		        		}elseif($lp['fields_name'] == 'rebounds'){
		        			$status = 3;
		        		}
