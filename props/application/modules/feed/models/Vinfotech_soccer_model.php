...
class Vinfotech_soccer_model extends FEED_Model
{
    ...
    {
    	$url = "http://".ML_SERVER_API_URL."/cron/soccer_props/".$season_game_uid;
    	//echo $url ;die;
 		...
 		if(!empty($current_game))
 		{
 			foreach ($current_game as $key => $value) 
 			{
 				...
 				$season_status = $value['status'];
 				 $this->db->select("L.lineup_id,L.user_team_id,L.season_prop_id,L.type,L.value,SP.prop_id,SP.points,SP.player_id,SS.goals,SS.assists,SS.saves,SS.passes_completed,SS.tackles_won,SS.chancecreated,SS.clearance,SS.minutes,SS.shots_on_goal,MP.fields_name")
	                ->from(LINEUP." AS L")
	                ...
	                ->join(STATS_SOCCER." AS SS","SS.player_id = SP.player_id AND SS.season_id = SP.season_id AND SS.season_id = ".$season_id,"LEFT")
		        ...
		        if(!empty($lineups))
		        {
		        	...
		        	foreach ($lineups as $key => $lp) 
		        	{
		        		...
		        		$stats = 0;
		        		//Goals
		        		if($lp['minutes'] >= 1 && $lp['fields_name'] == 'goals' && $lp['goals'] > $prop_value && $lp['type'] == "1"){
		        			$status = 1;
		        			$stats = $lp['goals'];
		        		}elseif($lp['minutes'] >= 1 && $lp['fields_name'] == 'goals' && $lp['goals'] < $prop_value && $lp['type'] == "2"){
		        			$status = 1;
		        			$stats = $lp['goals'];
		        		}elseif($lp['minutes'] >= 1 && $lp['fields_name'] == 'goals'){
		        			$status = 2;
		        			$stats = $lp['goals'];
		        		}elseif($lp['fields_name'] == 'goals'){
		        			$status = 3;
		        		}

		        		//Assists
		        		if($lp['minutes'] >= 1 && $lp['fields_name'] == 'assists' && $lp['assists'] > $prop_value && $lp['type'] == "1"){
		        			$status = 1;
		        			$stats = $lp['assists'];
		        		}elseif($lp['minutes'] >= 1 && $lp['fields_name'] == 'assists' && $lp['assists'] < $prop_value  && $lp['type'] == "2"){
		        			$status = 1;
		        			$stats = $lp['assists'];
		        		}elseif($lp['minutes'] >= 1 && $lp['fields_name'] == 'assists'){
		        			$status = 2;
		        			$stats = $lp['assists'];
		        		}elseif($lp['fields_name'] == 'assists'){
		        			$status = 3;
		        		}

		        		//Shots On Target
		        		if($lp['minutes'] >= 1 && $lp['fields_name'] == 'shots_on_goal' && $lp['shots_on_goal'] > $prop_value && $lp['type'] == "1"){
		        			$status = 1;
		        			$stats = $lp['shots_on_goal'];
		        		}elseif($lp['minutes'] >= 1 && $lp['fields_name'] == 'shots_on_goal' && $lp['shots_on_goal'] < $prop_value && $lp['type'] == "2"){
		        			$status = 1;
		        			$stats = $lp['shots_on_goal'];
		        		}elseif($lp['minutes'] >= 1 && $lp['fields_name'] == 'shots_on_goal'){
		        			$status = 2;
		        			$stats = $lp['shots_on_goal'];
		        		}elseif($lp['fields_name'] == 'shots_on_goal'){
		        			$status = 3;
		        		}

		        		//Goalie saves
		        		if($lp['minutes'] >= 1 && $lp['fields_name'] == 'saves' && $lp['saves'] > $prop_value && $lp['type'] == "1"){
		        			$status = 1;
		        			$stats = $lp['saves'];
		        		}elseif($lp['minutes'] >= 1 && $lp['fields_name'] == 'saves' && $lp['saves'] < $prop_value && $lp['type'] == "2"){
		        			$status = 1;
		        			$stats = $lp['saves'];
		        		}elseif($lp['minutes'] >= 1 && $lp['fields_name'] == 'saves'){
		        			$status = 2;
		        			$stats = $lp['saves'];
		        		}elseif($lp['fields_name'] == 'saves'){
		        			$status = 3;
		        		}

		        		//Passes Completed
		        		if($lp['minutes'] >= 1 && $lp['fields_name'] == 'passes_completed' && $lp['passes_completed'] > $prop_value && $lp['type'] == "1"){
		        			$status = 1;
		        			$stats = $lp['passes_completed'];
		        		}elseif($lp['minutes'] >= 1 && $lp['fields_name'] == 'passes_completed' && $lp['passes_completed'] < $prop_value && $lp['type'] == "2"){
		        			$status = 1;
		        			$stats = $lp['passes_completed'];
		        		}elseif($lp['minutes'] >= 1 && $lp['fields_name'] == 'passes_completed'){
		        			$status = 2;
		        			$stats = $lp['passes_completed'];
		        		}elseif($lp['fields_name'] == 'passes_completed'){
		        			$status = 3;
		        		}

		        		//Tackles Won
		        		if($lp['minutes'] >= 1 && $lp['fields_name'] == 'tackles_won' && $lp['tackles_won'] > $prop_value && $lp['type'] == "1"){
		        			$status = 1;
		        			$stats = $lp['tackles_won'];
		        		}elseif($lp['minutes'] >= 1 && $lp['fields_name'] == 'tackles_won' && $lp['tackles_won'] < $prop_value &&  $lp['type'] == "2"){
		        			$status = 1;
		        			$stats = $lp['tackles_won'];
		        		}elseif($lp['minutes'] >= 1 && $lp['fields_name'] == 'tackles_won'){
		        			$status = 2;
		        			$stats = $lp['tackles_won'];
		        		}elseif($lp['fields_name'] == 'tackles_won'){
		        			$status = 3;
		        		}

		        		//clearance
		        		if($lp['minutes'] >= 1 && $lp['fields_name'] == 'clearance' && $lp['clearance'] > $prop_value && $lp['type'] == "1"){
		        			$status = 1;
		        			$stats = $lp['clearance'];
		        		}elseif($lp['minutes'] >= 1 && $lp['fields_name'] == 'clearance' && $lp['clearance'] < $prop_value &&  $lp['type'] == "2"){
		        			$status = 1;
		        			$stats = $lp['clearance'];
		        		}elseif($lp['minutes'] >= 1 && $lp['fields_name'] == 'clearance'){
		        			$status = 2;
		        			$stats = $lp['clearance'];
		        		}elseif($lp['fields_name'] == 'clearance'){
		        			$status = 3;
		        		}
