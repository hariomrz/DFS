...
import { Button, Modal, Tabs, Tab, Table, ProgressBar, Panel, ModalHeader, ModalBody, Col, Row, Card, Input, FormGroup, Label } from 'reactstrap';
...
export default class PromoteContest extends React.Component {
  ...
  handleContestNotificationType = (e) => {
    ...
    // let value = e.target.value;
    // let id = e.target.id;
    // var contestPromoteParam = _.cloneDeep(this.state.contestPromoteParam);
    // contestPromoteParam[MODULE_C.fixtureChannelMap[id]] = value=='false'?true:false;
    // contestPromoteParam[id] = value=='false'?true:false;

    // this.setState({contestPromoteParam : contestPromoteParam},()=>
    // {
    //   console.log("this.state.contestPromoteParam:",this.state.contestPromoteParam);
  ...
  render() {
    ...
    const { IsFixturePromoteShow, IsContestPromoteHide, hidemodal } = this.props;

    const { contestPromoteParam, notificationData, cd_balance, ContestData, userSegmentParam, total_users, } = this.state;
    return (
            ...
            <Row className="usertime">
              <Col lg={12}>
                ...
                <div className="prize-tag">
                  Prize Pool <span className="rupees-icon">{NC.CURRENCY}</span> {ContestData.prize_pool}
                </div>
              </Col>
