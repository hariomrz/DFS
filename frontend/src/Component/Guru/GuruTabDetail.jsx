...
import { Utilities, _isUndefined, _isEmpty, _Map, _sumBy, _cloneDeep, _filter, _omit } from '../../Utilities/Utilities';
...
import GuruFieldViewModal from './GuruFieldViewModal';

const NewPlayerCard = lazy(() => import('../../Modals/NewPlayerCard'));
export default class GuruTabDetail extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            playerDetails: {},
            leagueId: this.props.FixturedContest ? this.props.FixturedContest.is_network_contest && this.props.FixturedContest.is_network_contest == 1 ? this.props.LobyyData.league_id : this.props.FixturedContest.league_id : '',
    ...
    componentDidMount(){
        //alert(this.props.FixturedContest.league_id)
    }
     /**
     * 
    ...
    checkPlayerExclude = (player,e) => {
        ...
        if(!this.state.showFillterModal){
            let TempArr = this.state.rosterListGuru;
            const { ExcluderosterListGuru } = this.state
            _Map(TempArr, (it, ix) => {
                if (it.player_uid == player.player_uid) {
                    if (_isUndefined(it['is_excluded']) || it['is_excluded'] == 0) {
                        it['is_excluded'] = 1
                        this.setState({
                            ExcluderosterListGuru: [...ExcluderosterListGuru, player]
                        })
                    } else {
                        this.setState({
                            ExcluderosterListGuru: _filter(ExcluderosterListGuru, o => o.player_uid != player.player_uid)
                        })
                        it['is_excluded'] = 0
                    }
                }
        ...
        this.setState({rosterListGuru:newList,SelectedPlayerPosition:"ALL"},async()=>{
            ...
            if (api_response_data.response_code == WSC.successCode) {
                this.setState({ isGenrateApi: true }, () => {
                    ...
                    this.setState({ lineupArr: initialArray, generate_lineup: api_response_data.data.lineup }, () => {
                ...
                Utilities.showToast(api_response_data.message, 5000);
                this.setState({ isGenrateApi: true })
            }
        })
    ...
    setLocationStateData() {
        if (this.props) {
            ...
            const { FixturedContest, league_id, SelectedPlayerPosition, PositionOrder, LobyyData, collection_master_id,
                from, rootDataItem, isFromMyTeams, ifFromSwitchTeamModal, isFrom, isClone, isCollectionEnable, team, isReverseF } = this.props;
            this.setState({
                leagueId: this.props.FixturedContest ? this.props.FixturedContest.is_network_contest && this.props.FixturedContest.is_network_contest == 1 ? this.props.LobyyData.league_id : this.props.FixturedContest.league_id : league_id,
                ...
                if(!_isEmpty(this.props.FixturedContest) || !_isEmpty(this.props.LobyyData)){
    ...
    checkExcluded = () => {
        const { rosterListGuru } = this.state;
        this.setState({
            ExcluderosterListGuru: _filter(rosterListGuru, 'is_excluded')
        })
    }
    GoToFieldView = () => {
        ...
        this.setState({
            GenerateGuruData: {
                ...
                resetIndex: 1,
                generate_lineup: this.state.generate_lineup
            }
        }, () => {
            this.setState({
                showFiledView: true
    ...
    render() {
        ...
        const HeaderOption = {
            ...
            resetIndex: this.props.resetIndex ? this.props.resetIndex : 0,
            showFilterByTeam: false,
            themeHeader: false
        }
        ...
        const GuruFieldViewProps = {
            ...this.props,
            GenerateGuruData,
            HideFieldView: this.HideFieldView,
            showFieldV: showFiledView,
            hideFieldV: () => {}
        }
            ...
            <MyContext.Consumer>
                {(context) => (
                    ...
                    <div className={"web-container guru-roster roster-web-container fixed-sub-header web-container-fixed white-bg " + ((SELECTED_GAMET == GameType.MultiGame && LobyyData && match_list.length > 1) ? ' lineup-with-collection' : '') + `${this.state.activeClass}`}>
                        ...
                        <div style={{paddingTop:0,position:'unset'}} className={"roster-header " + (soff > 100 ? 'fixed-v' : 'fixed-v')}>
                        ...
                        <div style={{marginTop:0}} className={"table-rosters " + (AppSelectedSport == SportsIDs.baseball ? ' sports-baseball' : ' ')} id="tableLineupPlayer" >
                            <InfiniteScroll
                        ...
                        {
                            showFiledView &&
                            <GuruFieldViewModal {...GuruFieldViewProps}/>
                        }
                    </div>
