...
import { LiveFantatsyLiveContest, LiveFantatsyUpcomingContest, LiveFantatsyCompletedContest } from '../LiveFantasy';
import { my_contest_config } from '../../JsonFiles';
import { joinContest, getUserLineUps, getMyCollectionLF, joinContestLF } from '../../WSHelper/WSCallings';
...
import LiveOverSocket from './LiveOverSocket';
...
/**
  ...
  * @author Vinfotech
*/
export default class LFMycontest extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            isLoaderShow: false,
    ...
    */
    goToLobby = () => {
        ...
        let contestListingPath = '/' + Utilities.getSelectedSportsForUrl().toLowerCase() + '/contest-listing/' + FixtureData.collection_id + '/' + home + "-vs-" + away + "-" + dateformaturl + "?sgmty=" + btoa(Constants.SELECTED_GAMET)
    ...
    */
    createTeamAndJoin = (dataFromConfirmFixture, dataFromConfirmLobby) => {
        ...
        if (lengthMatchList == 0) {
            this.props.history.push({ pathname: '/lineup/' + urlData.home.toLowerCase() + "-vs-" + urlData.away.toLowerCase() + "-" + dateformaturl, state: { FixturedContest: dataFromConfirmFixture, LobyyData: this.state.LobyyData ? this.state.LobyyData : dataFromConfirmLobby, resetIndex: 1, collection_master_id: urlData.collection_id, current_sport: Constants.AppSelectedSport,isReverseF :dataFromConfirmFixture.is_reverse || false, isSecIn: dataFromConfirmFixture.is_2nd_inning == 1 } })
        }
    ...
    */
    ...
    getMyCollectionsList = async (status) => {
        ...
        var responseJson = await getMyCollectionLF(param);
    ...
    openLeaderboard = (e, childItem, rootItem) => {
        ...
        this.props.history.push({
            pathname: '/' + Utilities.getSelectedSportsForUrl().toLowerCase() + '/lf-leaderboard',
            state: {
                rootItem: rootItem,
        ...
        if (sideView == false || isEdit == true) {
            if (isEdit == false) {
                ...
                if (urlData.home) {
                    ...
                    this.props.history.push({ pathname: fieldViewPath.toLowerCase(), state: { team: teamitem, contestItem: contestItem, rootitem: rootitem, isEdit: isEdit, from: 'MyContest', isFromtab: isFromtab, isFrom: 'MyContest', resetIndex: 1, isPlayingAnnounced: rootitem.playing_announce ,isReverseF: contestItem.is_reverse || false,isFromUpcoming: true,isSecIn : contestItem.is_2nd_inning == 1 ? true : false,isNF: isNFContest} });
                }
                else {
                    let pathurl = Utilities.replaceAll(urlData.collection_name, ' ', '_');
    ...
    goToBoosterScreen = (fixtureData, contestData, teamData) => {
        this.props.history.push({
            pathname: `/booster-collection/${fixtureData.collection_id}/${Utilities.getSelectedSportsForUrl().toLowerCase()}/${teamData.lineup_master_id ? teamData.lineup_master_id : '0'}`
    ...
    */
    ...
    openScoreCard=(e,item,status)=>{
        ...
        let collection_master_id = item.collection_id
        this.props.history.push({
    ...
    goToBench=(item,childItem,teamItem)=>{
        ...
        let CMID = item.collection_id ? item.collection_id : childItem.collection_id;
    ...
    */
    render() {
        ...
        return (
            <MyContext.Consumer>
                {(context) => (
                    <div className={"web-container my-contest-style tab-two-height web-container-fixed" + (this.state.selectedTab == Constants.CONTEST_COMPLETED ? ' bg-white' : '') + (Constants.SELECTED_GAMET == Constants.GameType.MultiGame ? ' multigame-my-contest' : '')}>
                                    ...
                                    <Col className="top-tab-margin" xs={12}>
                                        <Tab.Content animation>
                                            <Tab.Pane eventKey={Constants.CONTEST_LIVE}>
                                                <LiveFantatsyLiveContest {...this.props} liveContestList={this.state.liveContestList} ContestDetailShow={this.ContestDetailShow} openLeaderboard={this.openLeaderboard} allowCollection={this.state.allowCollection} goToChatMyContest={this.goToChatMyContest} TourList={TourList} MerchandiseList={MerchandiseList} isTLoading={isTLoading} openScoreCard={this.openScoreCard} showStats={this.showStats} isLoaderShow={this.state.isLoaderShow} />
                                            ...
                                            <Tab.Pane eventKey={Constants.CONTEST_UPCOMING}>
                                                ...
                                                <LiveFantatsyUpcomingContest {...this.props} collectionMasterId={this.state.collectionMasterId} upcomingContestList={this.state.upcomingContestList} removeFromList={this.removeFromList}
                                                    ...
                                                    goToChatMyContest={this.goToChatMyContest} TourList={TourList} MerchandiseList={MerchandiseList} isTLoading={isTLoading} isLoaderShow={this.state.isLoaderShow} goToBoosterScreen={this.goToBoosterScreen} goToBench={this.goToBench} />
