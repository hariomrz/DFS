...
export default class PFMatchCard extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    render() {
        const { item, gotoDetails,gotoLeaderBoard,isFromFreeToPlayLandingPage,isFrom, onLBClick, isSecondInning,gotoGameCenter,sports_id} = this.props;
        let sponserImage = item.sponsor_logo && item.sponsor_logo!=null ? item.sponsor_logo : 0

        return (
            <MyContext.Consumer>
                {(context) => (
                    <div className={"fixture-card-wrapper cursor-pointer xhighlighted-card " + (this.props.fixtureCardLg ? ' fixture-card-wrapper-lg' : '')} onClick={(event) => this.fixtureCardClick(event)}>
                        <div className={"fixture-card-body display-table" + (isSecondInning ? ' sec-innig' : '')}>
                            ...
                            <PFMatchInfo isSecondInning={isSecondInning} item={item} timerCallback={this.state.timerCallback} isFrom={isFrom} sports_id={sports_id} />
                        </div>
