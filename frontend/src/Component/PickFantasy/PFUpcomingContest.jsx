...
import {
    GetPFUserContestByStatus, getPrizeInWordFormat
...
export default class PFUpcomingContest extends React.Component {
    ...
    getMyContestList(item, idx, isFromProps) {
        ...
        if (item.season_id == expandedItem && !isFromProps) {
    ...
    gotoGameCenter = (event, data) => {
        ...
        let gameCenter = '/game-center/' + data.season_id;
    ...
    render() {
        ...
        let { expandedItem, isRFEnable, isLoaderShow, isBenchEnable } = this.state;
                            ...
                            _Map(this.state.upcomingContestList, (item, idx) => {
                                return (
                                    <div key={idx} className={"contest-card upcoming-contest-card-new mt10" + (Constants.SELECTED_GAMET == Constants.GameType.MultiGame && item.match_list && item.match_list.length > 1 ? ' contest-card-with-collection' : '')}>
                                                                ...
                                                                </p>
                                                                {
                                                                    (item['2nd_inning_date'] && item.status == 1) ? <div>
                                                                        ...
                                                                        Utilities.showCountDown(item) ?
                                                                            ...
                                                                            :
                                                                            <span className="time-line-date"> <MomentDateComponent data={{ date: item.scheduled_date, format: "D MMM - hh:mm A" }} /> </span>
                                                                }
                                                            </React.Fragment>
                                                        }
                                        ...
                                        {Constants.SELECTED_GAMET == Constants.GameType.DFS && item.is_gc == 1 && Utilities.getMasterData().allow_gc == 1 && expandedItem == item.season_id &&
                                        ...
                                        }
                                        {
                                            ...
                                            _Map(item.contest, (childItem, idx) => {
                                                ...
                                                return (
                                                    <div key={idx} className={"contest-card-body xmb20 ml15 mr15 mt10 picks-upcoming-view " + (idx !== 0 ? "mt15" : '')}>
                                                        <div className="contest-card-body-header cursor-pointer pt0 " onClick={() => ContestDetailShow(childItem, item)}>
                                                            ...
                                                            <div className="contest-progress-block">
                                                                <div className="progress-bar-default">
                                                                    ...
                                                                    <div className="progress-bar-value">
                                                                        <span className="user-joined">{Utilities.numberWithCommas(parseFloat(childItem.total_user_joined))}</span>
                                                                        <span className="total-entries"> / {childItem.size} Entries</span><span className="min-entries">min {childItem.minimum_size}</span>
                                                                    </div>
                                                                ...
                                                                </div>
                                                                {
                                                                    ...
                                                                    <button style={isRookie ? { overflow: 'visible', top: -2 } : {}} onClick={(event) => getUserLineUpListApi(event, item, childItem, "teamItem", true)} className="btn btn-primary pull-right width100" disabled={!((parseInt(childItem.user_joined_count) < parseInt(childItem.multiple_lineup)) && (parseInt(childItem.size) > parseInt(childItem.total_user_joined)))}>
                                                                        {/* {AppLabels.JOIN}  */}
                                                                        {
                                                        ...
                                                        <ul className="contest-listing upcoming">
                                                            {
                                                                _Map(childItem.teams, (teamItem, idx) => {
                                                                    let showBooster = childItem.is_2nd_inning == '0' && childItem.is_network_contest != 1 && Utilities.getMasterData().booster == 1 && item.booster != '' && Constants.SELECTED_GAMET == Constants.GameType.DFS;
                                                                    let showBench = isBenchEnable && childItem.is_2nd_inning == '0' && childItem.is_network_contest != 1 && Constants.SELECTED_GAMET == Constants.GameType.DFS && (item.playing_announce == 0 || (item.playing_announce == 1 && teamItem.bench_applied == "1"));
                                                                    let showMore = (showBooster || showBench) || false;
                                                                    let showBenchErr = isBenchEnable && childItem.is_2nd_inning == '0' && childItem.is_network_contest != 1 && Constants.SELECTED_GAMET == Constants.GameType.DFS && item.playing_announce == 0 && teamItem.bench_applied != "1";

                                                                    let showBoosterErr = childItem.is_2nd_inning == '0' && childItem.is_network_contest != 1 && Utilities.getMasterData().booster == 1 && item.booster != '' && Constants.SELECTED_GAMET == Constants.GameType.DFS && teamItem.booster_id && parseInt(teamItem.booster_id) == 0;
                                                                    return (
                                                                        <li key={idx}>
                                                                            ...
                                                                            <div className="cell-block contest-details-right">
                                                                                {
                                                                                    ...
                                                                                    <img style={{ marginTop: -20 }} src={Images.PL_LOGO_SMALL} alt=''></img>
                                                                                }
                                                                                {
                                                                                ...
                                                                                <a href onClick={() => openLineup(item, childItem, teamItem, true, Constants.CONTEST_UPCOMING,false)}>
                                                                                    <i className="icon-edit-line"></i>
                                                                                    <span className='fs8 mt5'>{AppLabels.EDIT_TEAM}</span>
                                                                                </a>
                                                                                <>
                                                                                    <a href className="visible-for-mobile"
                                                                                    onClick={() => openLineup(item, childItem, teamItem, false, Constants.CONTEST_UPCOMING, true)}
                                                                                    >
                                                                                        <i className="icon-search-bg"></i>
                                                                                        <span className='fs8 mt5'>{AppLabels.VIEW_TEAM}</span>
                                                                                    </a>

                                                                                    <a href className="visible-for-desktop" 
                                                                                    onClick={() => openLineup(item, childItem, teamItem, false, Constants.CONTEST_UPCOMING, true)}
                                                                                    >
                                                                                        <i className="icon-search-bg"></i>
                                                                                        <span className='fs8 mt5'>{AppLabels.VIEW_TEAM}</span>
