...
class StockFixtureCard extends Component {
        ...
        return (
            <div className="tour-body upcoming">
                ...
                <div className="tour-info-text">
                    <div className="tour-nm">{msg.length > 0 ? msg[0] : ''}<time>{Utilities.getFormatedDate({ date: date, format: "DD MMM - hh:mm a" })}</time>{msg.length > 0 ? msg[1] : ''}</div>
                     <a href  onClick={showHTPModal}>{AL.HOW_TO_PLAY_FREE}</a>
                </div>
            </div>
        )
    }
            ...
            <>
                {
                    Utilities.showCountDown({ game_starts_in: game_starts_in }) ?
                        <div className="countdown time-line">
                            <CountdownTimer timerCallback={() => console.log('timerCallback')} deadlineTimeStamp={game_starts_in} />
                        </div>
                        :
                        parseInt(item.is_live || '0') === 1 ?
                            ...
                            parseInt(item.status || '0') > 1 &&
                                <span className="com-status">
                                    <MomentDateComponent data={{ date: item.end_date, format: "DD MMM" }} />
    ...
    renderWinningSec = (item, name) => {
        return (
            <div className="tour-footer tour-footer-winning-sec">
                <div className="league-name">{name} {AL.CONTEST_TEXT} ({item.total_contest})</div>
                    ...
                    {AL.WINNINGS}
                    {
                        item.prize_type.toString() === "0"
                            ...
                            item.prize_type.toString() === "1" ?
                                <span>
                                    {Utilities.getMasterData().currency_code} {Utilities.getPrizeInWordFormat(item.total_prize_pool)}
                                </span>
                                :
                                item.prize_type.toString() === "2" ?
                                    <span>
                                        <img alt='' style={{ marginBottom: '2px' }} src={Images.IC_COIN} width="12px" height="12px" /> {item.total_prize_pool}
                                    </span>
                                    :
    ...
    renderJoinedSec = (item) => {
        return (
            <div className="tour-footer tour-footer-winning-sec">
                <div className="league-name">{item.team_count} {AL.PORTFOLIOS}</div>
                <div className={"rank-sec winning-sec"}>
    ...
    render() {
        const {
            item, isFrom, btnAction,showHTPModal
        } = this.props.data;
        let cDate = new Date()
        ...
        let name = item.category_id.toString() === "1" ? AL.DAILY : item.category_id.toString() === "2" ? AL.WEEKLY : AL.MONTHLY;
        let icon = item.category_id.toString() === "1" ? Images.stock_24 : item.category_id.toString() === "2" ? Images.stock_mon : Images.stock_cal;
        let icon_g = item.category_id.toString() === "1" ? Images.daily_g : item.category_id.toString() === "2" ? Images.weekly_g : Images.monthly_g;
        let game_starts_in = Date.parse(sDate)
        return (
            <MyContext.Consumer>
                {(context) => (
                    
                    <div onClick={()=> (isOpenAt && isFrom !== 'LSlider') ? null : btnAction()} className="tour-card cursor-pointer">
                        {(isFrom === 'Lobby' || isFrom === 'LobbyCM') && <img className="bg-graph" src={icon_g} alt="" />}
                        {
                            isOpenAt && isFrom !== 'LSlider'
                                ...
                                <>
                                    <div className="tour-body">
                                        ...
                                        <div className="tour-info-text">
                                            <div className="tour-nm">{item.collection_name && item.collection_name != '' ? item.collection_name : name} {SELECTED_GAMET == GameType.StockFantasy ? AL.STOCK_FANTASY : ''}</div>
                                            {
                                                isFrom === 'LobbyCM' &&
                                                <div class="countdown time-line"><span><strong>16</strong><span>:</span><strong>48</strong><span>:</span><strong>05</strong><span></span></span></div>
                                            }
                                            {this.renderTimeView(item, game_starts_in)}
                                            {
                                                ...
                                                <a href className="tour-btn" onClick={btnAction}>
                                                    {
                                                        parseInt(item.is_live || '0') === 1 || parseInt(item.status || '0') > 1 ? AL.VIEW : AL.EDIT
                                                    }
                                                </a>
                                            }
                                            <div className="sch-date">
                                                <MomentDateComponent data={{ date: item.season_scheduled_date, format: "D MMM hh:mm a " }} />
                                                {
                                                    ...
                                                    :
                                                    <MomentDateComponent data={{ date: item.end_date, format: " - D MMM hh:mm a " }} />
                                                }
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    {
                                        (isFrom === 'Lobby' || isFrom === 'LobbyCM') ? this.renderWinningSec(item, name) : this.renderJoinedSec(item)
                                    }
                                </>
                        }
                        {
