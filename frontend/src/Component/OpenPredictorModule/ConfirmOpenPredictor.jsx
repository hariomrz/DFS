...
class ConfirmOpenPredictor extends Component {
  ...
  UNSAFE_componentWillMount() {
    ...
    getUserBalance().then((responseJson) => {
      if (responseJson.response_code === WSC.successCode) {
        ...
        WSManager.setAllowedBonusPercantage(
          responseJson.data.allowed_bonus_percantage,
        )
  ...
  onChange = (e) => {
    if (e.target.value <= this.state.maxCoin) {
      ...
      this.setState(
        { bidAmount: this.state.bidAmount, refreshField: false },
        () => {
  ...
  render() {
    ...
    let isBtnEnbl =
      cpData.entry_type == 1
        ? cpData.entry_fee <= coin_bal
        : bidAmount >= minCoin && bidAmount <= coin_bal
          ...
          >
              ...
              <Modal.Body className=''>
                  <div className="container">
                      <div className="title-pp open-pred-pre">{AL.PLACE_PRE}</div>
                      <div className="desc-pp confirm-open">{AL.PRE_MSG}</div>
                      <div className="your-pre">
                          ...
                          <span className='open-pre'>{AL.YOUR_PRE}</span>
                          <span className="option-pre open-pre-option">{cpData.option_predicted.option}</span>
                          {
                              <div className="match-timing ">
                                  {
                                      Utilities.showCountDown({ game_starts_in: game_starts_in })
                                          ?
                                          <div className="countdown time-line open-pre-match">
                                              {
                      ...
                      {this.state.refreshField &&
                          <div>
                              ...
                              >
                                  ...
                                  <span  className="enter-coins-text open-enter-coin-text">
                                  {placeholderText}
                                  </span>
                              </FormGroup>
                          </div>
                      }
                      <div className="available-bal "><span className='pre open-available-bal'>{AL.AVAIL_BAL}</span><span className="bal open-available-bal"><img src={Images.IC_COIN} alt="" />{Utilities.numberWithCommas(coin_bal)}</span></div>
                      {
                      ...
                      <div onClick={(e) => this.submitPrediction()} className={'make-prediction-btn' + ((isBtnEnbl && !isLoading) ? ' active' : '')}>
                          <div className='make-prediction open-make-prediction'>{AL.MAKE_PREDICTION}</div>
                      </div>
                  </div>
...
export default ConfirmOpenPredictor
