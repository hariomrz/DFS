...
class ContestDetailModal extends React.Component {
        ...
        this.setState({ maxcurrentStatus: isSelected }, () => {
            ...
            if (this.state.ContestDetail.guaranteed_prize == '1' && this.state.ContestDetail.is_tie_breaker == '0') {
                if (this.state.maxcurrentStatus) {
                    this.setState({ newPrizeDistributionList: this.state.isPickFantasy ? JSON.parse(this.state.ContestDetail.prize_distibution_detail) : this.state.ContestDetail.prize_distibution_detail })

                }
                else {
                    this.setState({ newPrizeDistributionList: this.state.isPickFantasy ? JSON.parse(this.state.ContestDetail.current_prize) : this.state.ContestDetail.current_prize })
    ...
    */
    ...
    ContestDetail = async (data) => {
        ...
        if (api_response_data) {
            ...
            if (!isNetworkGame && Constants.SELECTED_GAMET == Constants.GameType.DFS && api_response_data.is_booster && api_response_data.is_booster == '1' && api_response_data.is_reverse == '0' && !this.props.isSecIn) {
            ...
            this.setState({
                ...
                prizeDistributionDetail: this.state.isPickFantasy ? JSON.parse(api_response_data.prize_distibution_detail) : api_response_data.prize_distibution_detail,
                prizeList: this.state.isPickFantasy ? JSON.parse(api_response_data.prize_distibution_detail) : api_response_data.prize_distibution_detail,
                ...
                if (api_response_data.is_tie_breaker == '1' && api_response_data.guaranteed_prize == '1') {
                    this.setState({ maxcurrentStatus: false, newPrizeDistributionList: this.state.isPickFantasy ? JSON.parse(api_response_data.current_prize) : Constants.SELECTED_GAMET == Constants.GameType.LiveStockFantasy ? api_response_data.prize_distibution_detail : api_response_data.current_prize })
                }
                    ...
                    if (this.state.contestStatus == Constants.CONTEST_UPCOMING || this.state.contestStatus == undefined) {
                        this.setState({ maxcurrentStatus: true, newPrizeDistributionList: this.state.isPickFantasy ? JSON.parse(api_response_data.prize_distibution_detail) : api_response_data.prize_distibution_detail })

                    }
                    else {
                        this.setState({ maxcurrentStatus: false, newPrizeDistributionList: this.state.isPickFantasy ? JSON.parse(api_response_data.current_prize) : api_response_data.current_prize })
                ...
                let prizeDisArray = this.state.isPickFantasy ? JSON.parse(api_response_data.prize_distibution_detail) : api_response_data.prize_distibution_detail;
    ...
    getMiniLeagueDetails = async (mini_league_uid) => {
        ...
        if (api_response_data) {
            this.setState({
                miniLeagueprizeList: this.state.isPickFantasy ? JSON.parse(api_response_data.data.prize_distibution_detail) : api_response_data.data.prize_distibution_detail,
    ...
    getPrizeAmount = (prize_data) => {
        let prizeAmount = this.getWinCalculation(this.state.isPickFantasy ? JSON.parse(prize_data.prize_distibution_detail) : prize_data.prize_distibution_detail);
        return (
            <React.Fragment>
                {
    ...
    */
    ...
    aadharConfirmation = () => {
        if (this.props.profileShow && this.props.profileShow.aadhar_status == "0" && this.props.profileShow.aadhar_id) {
    ...
    render() {
        ...
        const { ContestDetail, normal_scoring_rules, bonus_scoring_rules, economy_scoring_rules, pitching_scoring_rules, hitting_scoring_rules, strike_scoring_rules, prizeDistributionDetail, playerCount, joinBtnVisibility, userList, sportsSelected, allowCollection, showCollectionInfo, showError, isPrivateContest, showMaxToggle, activeSTIDx, isRFEnable, isXPEnable, newPrizeDistributionList } = this.state;
                            ...
                            <Modal.Header className={LobyyData ? (LobyyData.match_list && LobyyData.match_list.length > 1 ? 'header-with-collection' : '') : (ContestDetail.match_list && ContestDetail.match_list.length > 1 ? 'header-with-collection' : '')}>
                                <Modal.Title >
                                    ...
                                    <div className="match-heading header-content">
                                        {(Constants.SELECTED_GAMET != Constants.GameType.MultiGame && !this.props.isStockF && MULTI_VALIDATION != 2) &&
                                ...
                                {console.log('Connn', ContestDetail)}
                                    ...
                                    <div className="header-section-contest-entry" style={{ marginTop: MULTI_VALIDATION == 2 ? '0' : '15px' }}>
                                        <div className="center-alignment">
                                            ...
                                            <span className={`entry-fee-btn ${Constants.SELECTED_GAMET == Constants.GameType.StockPredict && !Utilities.minuteDiffValueStock({ date: ContestDetail.game_starts_in }, -5) ? ' disabled' : ''}`}
                                                onClick={(Utilities.getMasterData().a_aadhar == 1 && WSManager.loggedIn() && ContestDetail.entry_fee != '0'  ) ? (profileShow && profileShow.aadhar_status == "1" || OpenContestDetailFor.entry_fee == '0') ? () => onJoinBtnClick(ContestDetail) : () => this.aadharConfirmation() : () => onJoinBtnClick(ContestDetail)}>
                                                {
                                ...
                                <Tabs id={'contest-detail-tab'} onSelect={this.ontabSelect} defaultActiveKey={this.props.activeTabIndex} >
                                    ...
                                    <Tab eventKey={2} title={AppLabels.INFO}>
                                        <div className="info-section">
                                            {
                                                ContestDetail.is_booster == '1' && ContestDetail.is_reverse == '0' && !this.props.isSecIn &&
                                                ...
                                                <div className='private-info-box'>
                                                    <div className='private-contest-detail-box'>
                                                        ...
                                                        <div className='creator-info'>
                                                            {(ContestDetail.creator_details && user_data) ?
                                                                ((ContestDetail.creator_details.user_id === user_data.user_id) ? 'You' : ContestDetail.creator_details.user_name)
                                                                : ContestDetail.creator_details ? ContestDetail.creator_details.user_name
                                                                    : ''
                                                            }
                                                            {ContestDetail.creator_details && <img src={ContestDetail.creator_details.image ? Utilities.getThumbURL(ContestDetail.creator_details.image) : Images.DEFAULT_AVATAR} alt="" />}
                                                        </div>
                                                    </div>
                                            ...
                                            <div className="header-section">
                                                {(Constants.SELECTED_GAMET == Constants.GameType.DFS || MULTI_VALIDATION == 1) && isRFEnable && ContestDetail.is_reverse == '1' &&
                                                    <div className="rev-fan-header-des">
                                                        <div className="upper-wrap"><img src={Images.REVERSE_FANTASY_ICON} alt="" /> {AppLabels.REVERSE_FANTASY}</div>
                                                        <div className="lower-wrap">{AppLabels.REVERSE_FANTASY_JUST_OPP}</div>
                                                    </div>
                                                }
                                            ...
                                            {!this.props.isStockF && !isPickFantasy && <div className="contest-info">
                                                <p>
                                                ...
                                                <Panel id="collapsible-panel-example-1" defaultExpanded>
                                                    ...
                                                    <Panel.Collapse>
                                                        <Panel.Body>
                                                            ...
                                                            {!isPickFantasy && !this.props.isStockF && Constants.SELECTED_GAMET != Constants.GameType.LiveStockFantasy && <Table responsive>
                                                                <thead>
                                                                    <tr>
                                                                        ...
                                                                        <Table responsive>
                                                                            <tbody>
                                                                                ...
                                                                                {Utilities.getMasterData().vc_point > 0 &&
                                                                                    (sportsSelected != SportsIDs.badminton) &&
                                                                                    <tr>
                                                                                        <td>{AppLabels.SATELLITE_STOCK}</td>
                                                                                        <td className="text-right right-text">
                                                                                            {
                                                                                                masterDataResponse != null &&
                                                                                                <>
                                                            ...
                                                            {
                                                                !isPickFantasy && Constants.SELECTED_GAMET != Constants.GameType.LiveStockFantasy && <Table responsive>
                                                                    <thead>
                                                                        <tr>
                                                            ...
                                                            {/* } */}
                                                            {
                                                                !isPickFantasy && hitting_scoring_rules.length > 0 && Constants.SELECTED_GAMET != Constants.GameType.LiveStockFantasy &&
                                                                <Table responsive>
                                                                    <thead>
                                                                        <tr>
                                                            ...
                                                            {
                                                                !isPickFantasy && strike_scoring_rules.length > 0 && Constants.SELECTED_GAMET != Constants.GameType.LiveStockFantasy &&
                                                                <Table responsive>
                                                                    <thead>
                                                                        <tr>
                                                                                    ...
                                                                                    <td className="text-right team-joined">{item.user_join_count != -1 && item.user_join_count}
                                                                                        {
                                                                                            this.props.isStockF || this.props.isStockPF ?
                                                                                                <span>{item.user_join_count != -1 && (item.user_join_count > 1 ? ' ' + AppLabels.PORTFOLIOS : ' ' + AppLabels.PORTFOLIO)}</span>
                                                                                                :
