...
import {Utilities, checkBanState, _filter, parseURLDate, isDateTimePast, convertToTimestamp} from '../Utilities/Utilities';
import CustomHeader from '../components/CustomHeader';
import { Sports } from "../JsonFiles";
import { Helmet } from "react-helmet";
import MetaData from "../helper/MetaData";
import { getPublicContestDetail,getPublicContestDetailMultiGame,getUserTeams,getMultigameUserTeams ,joinContest, getPublicContestNetworkfantasy,getPublicContestDetailLF, getUserAadharDetail} from '../WSHelper/WSCallings';
...
export default class Contest extends React.Component {
    ...
    getPublicContest(data) {
        ...
        let methodApi = this.state.is_network_contest == 1 ? getPublicContestNetworkfantasy : Constants.SELECTED_GAMET == Constants.GameType.MultiGame ? getPublicContestDetailMultiGame : Constants.SELECTED_GAMET == Constants.GameType.LiveFantasy ?getPublicContestDetailLF:getPublicContestDetail ;
        methodApi(param).then((apiResponseData) => {
            if (apiResponseData.response_code == WSC.successCode) {
                ...
                if(responseJson.match) {
                    ...
                    responseJson = {..._apiResponseData, ...match[0], match_list: match, current_prize: JSON.parse(current_prize), prize_distibution_detail: JSON.parse(prize_distibution_detail), 
                        game_starts_in: convertToTimestamp(responseJson.season_scheduled_date), playing_announce: match[0].playing_announce}
                }
                ...
                if(isDateTimePast(responseJson.season_scheduled_date)) {
                    Utilities.showToast("Match already started. You can't save your team.", 3000);
                }
            }
        })
    }
    ...
    onSubmitBtnClick = (data) => {
        ...
        // else {
            if(this.state.userTeamListSend.length > 0){
                ...
                this.setState({LobyyData: urlData})
                ...
                if (!WSManager.loggedIn()) {
                    setTimeout(() => {
                        this.props.history.push({ pathname: '/signup', state: { lineupPath: lineupPath.toLowerCase(), joinContest: true, LobyyData: urlData, FixturedContest: this.state.contestData ,resetIndex: 1,collection_master_id:urlData.collection_master_id, current_sport: AppSelectedSport, isReverseF : this.state.showRF, isSecIn: this.state.isSecIn, isPlayingAnnounced: urlData.playing_announce } })
                        Utilities.showToast(AppLabels.Please_Login_Signup_First, 3000);
                    }, 10);
                } 
                else{
                ...
                if(WSManager.loggedIn()){
                    this.props.history.push({ pathname: lineupPath.toLowerCase(), state: { LobyyData: data, FixturedContest: this.state.contestData ,resetIndex: 1,collection_master_id:urlData.collection_master_id, current_sport: AppSelectedSport, isReverseF : this.state.showRF, isSecIn: this.state.isSecIn, isPlayingAnnounced: urlData.playing_announce } })
                }
                else{
                    this.props.history.push({ pathname: '/signup', state: { lineupPath: lineupPath.toLowerCase(), joinContest: true,LobyyData: data, FixturedContest: this.state.contestData ,resetIndex: 1,collection_master_id:urlData.collection_master_id, current_sport: AppSelectedSport, isReverseF : this.state.showRF, isSecIn: this.state.isSecIn, isPlayingAnnounced: urlData.playing_announce } })
                }
            }
        // }
      }
          ...
          if (dataFromConfirmPopUp.selectedTeam.lineup_master_id != null && dataFromConfirmPopUp.selectedTeam.lineup_master_id == "" || dataFromConfirmPopUp.selectedTeam == "") {
            Utilities.showToast(AppLabels.SELECT_NAME_FIRST, 1000);
              ...
              if (checkBanState(dataFromConfirmPopUp.FixturedContestItem, CustomHeader)) {
                  ...
                  if (currentEntryFee <= Number(dataFromConfirmPopUp.balanceAccToMaxPercent)) {
        ...
        if(checkBanState(dataFromConfirmFixture, CustomHeader)){
          ...
          if (urlData.home) {
              this.props.history.push({ pathname: '/lineup/' + urlData.home.toLowerCase() + "-vs-" + urlData.away.toLowerCase() + "-" + dateformaturl, state: { FixturedContest: dataFromConfirmFixture, 
                LobyyData: {...this.state.LobyyData, game_starts_in: convertToTimestamp(this.state.LobyyData.season_scheduled_date)},
                resetIndex: 1, current_sport: Constants.AppSelectedSport, isSecIn: this.state.isSecIn ,isReverseF: this.state.showRF, isPlayingAnnounced: urlData.playing_announce } })
          }
          else {
    ...
    CallJoinGameApi(dataFromConfirmPopUp) {
        ...
        joinContest(param).then((responseJson) => {
            if (responseJson.response_code == WSC.successCode) {
                ...
                console.log(dataFromConfirmPopUp, '7. dataFromConfirmPopUp');
  ...
  seeMyContest = () => {
    ...
    render() {
        ...
        let is_tour_game = contestData.is_tour_game == 1 ? true : false;
        return (
            <MyContext.Consumer>
                {(context) => (
                    <div className={"web-container header-margin web-container-fixed share-contest-wrapper" + (Constants.SELECTED_GAMET == Constants.GameType.MultiGame && contestData && contestData.match_list && contestData.match_list.length > 1 ? ' share-collection-wrapper' : ' ')}>
                        ...
                        <div className="contest-card contest-card-wrapper">
                                    ...
                                    {Constants.SELECTED_GAMET != Constants.GameType.MultiGame &&
                                        <React.Fragment>
                                            ...
                                            <li className="progress-middle">
                                                <div {...{className: `team-content pb10 public-contest ${contestData.is_2nd_inning == 1 ? 'is_2nd_inning' : ''}`}}>
                                                    <p>{is_tour_game ? contestData.tournament_name : contestData.league_name}</p>
                                                    {
                                                        contestData.is_2nd_inning == 1 ?
                                                            <div className="sec-inning-label">
                                                                {AppLabels.JOIN_SEC_INNING}{" • "}<MomentDateComponent data={{ date: contestData.season_scheduled_date, format: "D MMM" }} />
                                                            </div>
                                                        :
                                                        <>
                                                            {
                                    ...
                                    <div className="contest-list-header">
                                        ...
                                        <div className="display-table">
                                            ...
                                            <div className="display-table-cell v-mid position-relative entry-criteria pl15" >
                                                ...
                                                disabled={isDateTimePast(contestData.season_scheduled_date) || contestData.total_user_joined == contestData.size}
                                                >
                                                {/* {AppLabels.JOIN}  */}
                                                {
                                                        ...
                                                        {contestData.currency_type == 1 &&
                                                            <>
                                                            {
                                                                ...
                                                                <React.Fragment>
                                                                    ...
                                                                    <span>{contestData.entry_fee}</span>
                                                                </React.Fragment>
                                                                :
                                                                <React.Fragment>
                                                                    <span>{AppLabels.FREE}</span>
                                                                </React.Fragment>
                                                            }
                                                            </>
                                                        }
                            ...
                            {showContestDetail &&
                                <ContestDetailModal profileShow={aadharData}
                                    showPCError={true} 
