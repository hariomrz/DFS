...
const MenuItem = ({ item, selected, actTab }) => {
    return (
...
export const Menu = (list, selected, actTab) =>
    list.map(el => {
        return <MenuItem actTab={actTab} item={el} key={el.category_id} selected={selected} />;
    });
...
class SetPrediction extends Component {
    ...
    getLiveFixtures = () => {
        ...
        WSManager.Rest(NC.baseURL + NC.FIXED_OP_GET_CATEGORY_LIST_BY_STATUS, params).then(Response => {
            if (Response.response_code == NC.successCode) {
                ...
                this.setState({
                    ...
                    this.menuItems = Menu(this.state.FixtureList, this.state.CATEGORY_ID, this.state.activeTab);
                    this.resetChanges()
                })
    ...
    getPredictionList = () => {
        ...
        WSManager.Rest(NC.baseURL + NC.FIXED_OP_GET_ALL_PREDICTION, params).then(Response => {
            if (Response.response_code == NC.successCode) {
                this.setState({
    ...
    createPrediction = () => {
        ...
        let CallUrl = NC.FIXED_OP_CREATE_PREDICTION
        if (EditPredictionMasterId) {
            params.prediction_master_id = EditPredictionMasterId
            CallUrl = NC.FIXED_UPDATE_PREDICTION
        }
    ...
    pausePlayPrediction(item, idxVal) {
        ...
        WSManager.Rest(NC.baseURL + NC.FIXED_OP_PAUSE_PLAY_PREDICTION, params).then(Response => {
            if (Response.response_code == NC.successCode) {
    ...
    updatePinPrediction(item, idxVal) {
        ...
        let TempPredictionList = this.state.PredictionList
        WSManager.Rest(NC.baseURL + NC.FIXED_OP_UPDATE_PIN_PREDICTION, params).then(Response => {
            if (Response.response_code == NC.successCode) {
    ...
    getPredictionUserList = () => {
        ...
        WSManager.Rest(NC.baseURL + NC.FIXED_OP_GET_PREDICTION_PARTICIPANTS, params).then(Response => {
            if (Response.response_code == NC.successCode) {
                this.setState({
    ...
    submitAnswer = () => {
        ...
        let URL = NC.FIXED_OP_UPDATE_PREDICTION_PROOF
        if (AlertMSg){
            params.prediction_option_id = preOptionId
            URL = NC.FIXED_OP_SUBMIT_PREDICTION_ANSWER
        }
    ...
    deletePrediction = () => {
        ...
        let TempPredictionList = this.state.PredictionList
        WSManager.Rest(NC.baseURL + NC.FIXED_OP_DELETE_PREDICTION, params).then(Response => {
            if (Response.response_code == NC.successCode) {
                _.remove(TempPredictionList, function (item, idx) {
                    return idx == idxVal
                })
    ...
    copyPredictionUrl = (item) => {
        ...
        el.value = NC.baseURL + NC.FixedPredictionShareUrl + item.category_id + '/' + btoa(item.prediction_master_id);
    ...
    onChangeImage = (event) => {
        ...
        WSManager.multipartPost(NC.baseURL + NC.FIXED_OP_DO_UPLOAD_PROOF_IMAGE, data)
            .then(Response => {
                if (Response.response_code == NC.successCode) {
                    this.setState({
            ...
            <div className="set-prediction">
                ...
                <Row className="mt-4">
                    <Col md={12}>
                        ...
                        <div onClick={() => this.props.history.push('/prize-open-predictor/category?tab=' + BackTo)} className="go-back float-right">{'<'} Back to category</div>
                    </Col>
                </Row>
                <Row>
                    <Col md={12}>
                                                                    ...
                                                                    <div className="ques-action">
                                                                        <UncontrolledDropdown direction="left">
                                                                            ...
                                                                            <DropdownMenu>
                                                                                ...
                                                                                {(!HelperFunction.getTimeDiff(item.deadline_date) && !item.onCompTimer) && <DropdownItem 
