...
class PredictionType extends Component {
    ...
    getPredictionModule = () => {
        WSManager.Rest(NC.baseURL + NC.FIXED_OP_GET_PREDICTION_STATUS, {}).then(Response => {
    ...
    updatePredictionModule = () => {
        ...
        WSManager.Rest(NC.baseURL + NC.FIXED_OP_UPDATE_PREDICTION_STATUS, params).then(Response => {
            if (Response.response_code == NC.successCode) {
                
                if (ModuleSetting == 0)
                    this.props.history.push('/prize-open-predictor/category')
