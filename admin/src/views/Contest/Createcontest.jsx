...
import { REVERSE_FANTASY_TT, SCRATCH_WIN_CHECK, CONTEST_TIE_BRE_TT, CONTEST_SPONSOR_TT, CONTEST_RECUR_TT, CONTEST_PIN, CONTEST_PRZ_DIS_TT, CONTEST_BONUS, SECOND_INNING } from '../../helper/Message';
class CreateContest extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      contestObject: {
        ...
        'sponsor_logo': '', 'sponsor_link': '', 'set_sponsor': '0', 'sponsor_contest_dtl_image': '', 'is_reverse': false, 'is_scratchwin': false, 'is_2nd_inning': false
      },
      IsValidate: false,
      IsFormValid: true,
      ...
      isDFS: LS.get('isMGEnable') ? LS.get('isMGEnable') : true,
    };
  }
  ...
  GetFixtureDetail = () => {
    ...
    WSManager.Rest(NC.baseURL + NC.DFS_GET_FIXTURE_DETAILS, param).then((responseJson) => {
      if (responseJson.response_code === NC.successCode) {
        ...
        if (!this.state.isDFS) {
          let tempArr = [];
          _.map(fixtureDetail, (fixtureitem) => {
          ...
          let detailData = this.state.fixtureDetail
          detailData['format'] = fixtureDetail.match[0].format

          this.setState({ fixtureDetail: detailData })
        }
      }
    ...
    WSManager.Rest(NC.baseURL + NC.GET_CONTEST_TEMPLATE_MASTER_DATA, { "sports_id": this.state.selected_sport }).then((responseJson) => {
      if (responseJson.response_code === NC.successCode) {
        ...
        responseJson.group_list.map(function (lObj, lKey) {
          if (lObj.group_id != HF.get_h2h_group_id())
          {
            groupList.push({ 
              value: lObj.group_id, label: lObj.group_name 
            });
          }
        });
        let merchandiseList = [];
  ...
  CreateContest = () => {
    ...
    if (is_valid == 1) {
      let collection_name = this.state.fixtureDetail.home + ' vs ' + this.state.fixtureDetail.away;
      contestObject['collection_name'] = collection_name;
      contestObject['collection_master_id'] = this.state.collection_master_id;
      ...
      contestObject['season_scheduled_date'] = (!this.state.isDFS) ? this.state.season_scheduled_date : this.state.fixtureDetail.season_scheduled_date;
      ...
      if (WSManager.validateFormFields("contest_contest_form")) {
        ...
        WSManager.Rest((this.state.isDFS) ? NC.baseURL + NC.CREATE_CONTEST : NC.baseURL + NC.COLLECTION_CREATE_CONTEST, params).then((responseJson) => {
          if (responseJson.response_code === NC.successCode) {
            notify.show(responseJson.message, "success", 5000);
            if (!this.state.isDFS) {
              this.props.history.push({ pathname: '/contest/collectioncontest/' + this.state.league_id + '/' + this.state.collection_master_id })
            } else {
              this.props.history.push({ pathname: '/contest/fixturecontest/' + this.state.collection_master_id + '/' + this.state.season_id + '/1' })
            }
          }
  ...
  calculateProfitData = () => {
    ...
    if (contestObject.prize_pool_type == 2) {
      ...
      // prize_pool = parseFloat(this.state.contestObject.entry_fee * contestObject.size);      
  ...
  render() {
    ...
    return (
      <div className="create-dfs-contest ftp-wrap animate-left">
        <Row className="mb-1">
          <Col xs="12" lg="12">
            <Card className="recentcom">
              <CardHeader className="contestcreate">
                <h5 className="DFScontest">Create DFS Contest</h5>
              </CardHeader>
              <CardBody className="contestcard">
                <Row>
                  <Col sm={4}>
                    <div class="form-group">
                      <label for="group_id" className="fixturevs">Select contest group</label>
                      <Select
                        className="gray-select-field"
                        id="group_id"
                        name="group_id"
                        placeholder="Select Group"
                        value={contestObject.group_id}
                        options={groupList}
                  ...
                  {
                    contestObject.entry_fee_type !== '2' &&
                    <Col sm={4} className="mt-4">
                      <div className="form-group gray-form-group">
                        <label for="max_bonus_allowed" className="fixturevs">Bonus Allowed %
                        ...
                        <input type="text" max={100} maxLength={3} disabled={contestObject.entry_fee_type == "0" || contestObject.entry_fee == "0"} className="contestname required" id="max_bonus_allowed" name="max_bonus_allowed" value={contestObject.max_bonus_allowed} onChange={(e) => this.handleFieldVal(e, 'max_bonus_allowed', 'max_bonus_allowed')} placeholder="Bonus Allowed %"></input>
                      </div>
                    </Col>
                  }
                  ...
                  {/* {
                    (HF.getMasterData().allow_reverse_contest == '1' && !contestObject.is_2nd_inning && this.state.isDFS) &&
                    <Col sm={4} className="mt-4">
                      <div className="form-group">
                        ...
                        <div className="autorecurrentdiv">
                          <Input
                            ...
                            id="is_reverse"
                            name="is_reverse"
                            value={contestObject.is_reverse}
                  ...
                  {
                    (HF.allowSecondInni() == '1' && !contestObject.is_reverse && selected_sport == '7' && (fixtureDetail.format == 1 || fixtureDetail.format == 3) && this.state.isDFS) &&
                    <Col sm={4} className="mt-4">
                      <div className="form-group">
                        <label htmlFor="is_2nd_inning" className="fixturevs">Second Inning
                        <span className="btn-information">
                            <img id="sinn" className="infobtn" src={Images.INFO} />
                            <Tooltip placement="left" isOpen={SecInniToolTip} target="sinn" toggle={this.SecInniToggle}>{SECOND_INNING}</Tooltip>
                          </span>
                        </label>
                        <div className="autorecurrentdiv">
                          <Input
                            ...
                            id="is_2nd_inning"
                            name="is_2nd_inning"
                            value={contestObject.is_2nd_inning}
                            onChange={(e) => this.handleCheckboxFieldVal(e, 'is_2nd_inning')}
                            className="custom-control-input" />
                          <label className="custom-control-label" htmlFor="is_2nd_inning"></label>
                        </div>
                      </div>
                    </Col>
                  }
                            ...
                            <div className="add-more-prizes" onClick={this.addRow}>
                              ...
                              <div className="add-prize-subtext">Site Rake is
                                <span style={{ width: "auto", display: 'inlineBlock', marginLeft: 3, marginRight: 3 }} className={contestObject.site_rake <= 0 ? "text-danger" : ""}>
                                  ...
                                  {
                                    (!contestObject.prize_pool || contestObject.prize_pool > 0) && contestObject.prize_pool_type == '1'
                                    // (!contestObject.prize_pool || contestObject.prize_pool > 0)
                                    // (contestObject.prize_pool > 0)
                                      ?
                                      contestObject.site_rake + "%"
                                      :
                                      ""
                                  }
                                </span>
