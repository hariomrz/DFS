...
class PredictionCreateCategory extends Component {
    ...
    onChangeImage = (event) => {
        ...
        WSManager.multipartPost(NC.baseURL + NC.OP_DO_UPLOAD, data)
            .then(Response => {
    ...
    createCategory = () => {
        ...
        let CallUrl = NC.OP_ADD_CATEGORY
        if (EditCategoryId) {
            params.category_id = EditCategoryId
            CallUrl = NC.OP_UPDATE_CATEGORY
        }
    ...
    getCategoryList = () => {
        ...
        let { PERPAGE, CURRENT_PAGE, SEASON_GAME_ID, activeTab } = this.state
        let params = {
            items_perpage: PERPAGE,
            current_page: CURRENT_PAGE
        }

        WSManager.Rest(NC.baseURL + NC.OP_GET_ALL_CATEGORY, params).then(Response => {
            if (Response.response_code == NC.successCode) {
                this.setState({
    ...
    deleteCategory = () => {
        ...
        WSManager.Rest(NC.baseURL + NC.OP_DELETE_CATEGORY, params).then(Response => {
            if (Response.response_code == NC.successCode) {
    ...
    render() {
        let { Total, PERPAGE, CURRENT_PAGE, ActionPopupOpen, Message, CategoryList, CategoryImageNameMsg, CategoryImage, CategoryName, CategoryNameMsg } = this.state
                ...
                <div className="promotion-feedback op-create-category animated fadeIn">
                    ...
                    <Row className="mt-4 op-heading">
                        <Col md={12}>
                            ...
                            <div onClick={() => this.props.history.push('/open-predictor/category')} className="go-back float-right">{'<'} Back to Category</div>
                        </Col>
                    </Row>
