...
import { CONTEST_PRZ_DIS_TT, CONTEST_TIE_BRE_TT, CONTEST_HEADER_NAME_TT, CONTEST_SPONSOR_TT, CONTEST_RECUR_TT, MODULE_NOT_ENABLE } from '../../helper/Message';
import { SP_GET_ALL_MASTER_DATA, LSF_CREATE_TEMPLATE } from '../../helper/WSCalling';
import { Base64 } from 'js-base64';
import SelectDate from "../../components/SelectDate";
class LSF_Createtemplate extends Component {
  constructor(props) {
    super(props);
    ...
    this.state = {
      ...
      contestTemplate: { 'category_id': [1], 'multiple_lineup': '1', 'entry_fee_type': '1', 'max_bonus_allowed': '0', 'prize_type': '1', 'prize_pool_type': '1', "master_contest_type_id": "1", "group_id": "1", "is_auto_recurring": false, 'site_rake': 0, 'site_rake_max': 0, 'minimum_size': '2', 'size': '2', 'custom_total_percentage': '100', 'custom_total_amount': '0', 'prize_value_type': '0', 'is_tie_breaker': false, 'sponsor_name': '', 'sponsor_logo': '', 'sponsor_link': '', 'set_sponsor': '0', 'sponsor_contest_dtl_image': '', 'is_reverse': false, 'is_scratchwin': false, 'brokerage': 0},
      IsValidate: false,
      IsFormValid: true,
      payout_data: [],
      rows: [{}],
  ...
  GetContestTemplateMasterData = () => {
    ...
    SP_GET_ALL_MASTER_DATA({ "sports_id": this.state.selected_sport }).then((responseJson) => {
      if (responseJson.response_code === NC.successCode) {
        responseJson = responseJson.data;
    ...
    if (is_valid == 1) {
      ...
      if (WSManager.validateFormFields("contest_template_form")) {
        if ((contestTemplate.set_sponsor == 1 && this.state.wrongLink) || contestTemplate.set_sponsor == 0) {
          ...
          LSF_CREATE_TEMPLATE(params).then((responseJson) => {
            if (responseJson.response_code === NC.successCode) {
              notify.show(responseJson.message, "success", 5000);
              this.props.history.push({ pathname: '/livestockfantasy/contesttemplate' })
              // this.setState({
              //   contestTemplate: {
              //     'category_id': [1],
              //     'entry_fee_type': '1',
              //     'prize_type': '1', 
              //     'prize_pool_type': '1',
              //     "master_contest_type_id": "1", 
              //     "group_id": "1", 
              //     "is_auto_recurring": false, 
              //     'minimum_size': '2', 
              //     'size': '2', 
              //     'custom_total_percentage': '100', 
  ...
  render() {
    ...
    const StartDateProps = {
      ...
      min_date: new Date(),
      max_date: null,
      sel_date: new Date(StartDate),
      date_key: 'StartDate',
      place_holder: 'From Date',
    }

    return (
      <div className="SpCreateTemplate xsf-create-template animate-left">
                    ...
                    <Col sm={4} className="">
                      <div className="form-group gray-form-group">
                        <label for="prize_pool" className="fixturevs">Brokerage (%)
                            ...
                            <Tooltip placement="right" isOpen={this.state.isShowBrkgToolTip} target="isShowBrkgToolTip" toggle={this.toggle4}>
                        ...
                        <input
                          ...
                          id="brokerage"
                          value={contestTemplate.brokerage}
...
export default LSF_Createtemplate;