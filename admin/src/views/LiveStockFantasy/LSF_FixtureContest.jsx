...
class LSF_FixtureContest extends Component {
    ...
    WSManager.Rest(NC.baseURL + API_URL, param).then((responseJson) => {
      if (responseJson.response_code === NC.successCode) {
        ...
        if (GroupIndex >= "0" && DeleteIndex >= "0") {
          ...
          this.props.history.push('/livestockfantasy/fixture?pctab=' + ActiveFxType + '&tab=' + ActiveTab)
        }
        notify.show(responseJson.message, "success", 5000);
      }
      else if (responseJson.response_code == NC.sessionExpireCode) {
  ...
  render() {
    ...
    return (
      <div className="animated fadeIn contest-group-wrapper SpFixtureContest LSFFixtureContest">
          ...
          <div className="fixture-contest">
            {
              ...
              <label className="backtofixtures" onClick={() => this.props.history.push('/livestockfantasy/fixture?tab=' + ActiveTab)}> {'<'} Back to candle</label>
            }
          </div>
        </Col>
                          ...
                          <div className="contest-list-header clearfix">
                            <div className="contest-heading">
                              <div className="action-head clearfix">
                                <div onClick={() => this.props.history.push('/livestockfantasy/contest_detail/' + contest.contest_id + '?tab=' + ActiveTab)} className="contest-name text-ellipsis">{contest.contest_name}</div>
                              </div>
...
export default LSF_FixtureContest;