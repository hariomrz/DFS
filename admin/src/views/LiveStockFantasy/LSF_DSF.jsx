...
import { Col, Row, Button, Table, Nav, NavItem, NavLink } from 'reactstrap';
...
import LSF_MatchAlertMsgModal from './LSF_MatchAlertMsgModal';
import LSF_FixtureCard from './LSF_FixtureCard';
// import SF_AddFixtureModal from './SF_AddFixtureModal';
import { LSF_getHoliday } from '../../helper/WSCalling';
import Pagination from "react-js-pagination";
class LSF_DSF extends Component {
  constructor(props) {
    super(props);
    let filter = {
      current_page: 1,
  ...
  componentDidMount() {
    ...
    this.setState({
      ...
      DataKey: (!_isEmpty(values) && (values.tab == '1')) ? 'upcoming_fixture' : 'live_fixture',
    }, () => {
  ...
  redirectToStock = (selectedObj) => {
    let { activeFxType, activeTab } = this.state
    this.props.history.push({ pathname: '/livestockfantasy/fixturecontest/' + activeTab + '/' + selectedObj.collection_id });
  }

  redirectToNewStockReview = (itemObj) => {
    this.props.history.push({ pathname: '/livestockfantasy/create-candle/' + itemObj.collection_id })
  }

  redirectToContestTemplate = (selectedObj) => {
    let { activeTab } = this.state
    let send_val = this.getUrlFxInput(selectedObj)
    this.props.history.push({ pathname: '/livestockfantasy/createtemplatecontest/' + activeTab + '/' + selectedObj.collection_id });
  }
  ...
  toggleTab(tab) {
    if (this.state.activeTab !== tab) {
      this.setState({
        ...
        fixtureList: [],
        DT_CURRENT_PAGE: 1,
        DataKey: tab == '1' ? 'upcoming_fixture' : 'live_fixture',
      }, function () {
        this.GetAllFixtureList();
      });
    }
  ...
  addFixtureNext = () => {
    ...
    WSManager.Rest(NC.baseURL + NC.SF_VALIDATE_FIXTURE, param).then((responseJson) => {
      if (responseJson.response_code === NC.successCode) {
        ...
        this.props.history.push({ pathname: '/livestockfantasy/verify-stocks/' + activeFxType + '/' + activeTab + '/' + FixtureDate + '/0/' + new_name })
      }
      else if (responseJson.response_code == NC.sessionExpireCode) {
        WSManager.logout();
        this.props.history.push('/login');
  ...
  render() {
    const { MiniLeActiveTab, activeFxType, activeTab, current_page, total, fixtureList, filter, DfsT_List, DfsT_Total, DfsT_Posting, DelayModalIsOpen, delayPosting, fixtureObjData, delay_hour, delay_minute, delay_message, HourMsg, MinuteMsg, msgModalIsOpen, msgFormValid, MsgItems, Message, fxPinModalOpen, fxPinPosting, fxPinMsg, addFxModalOpen, addFxPosting, CategoryList, FixtureDate, DateLength, WeekLength, MonthLength, NextIndex, PrevIndex, FixtureName, DataKey, Total, CURRENT_PAGE, PERPAGE } = this.state;
    ...
    return (
      <div className="animated fadeIn SpDsf">
        {msgModalIsOpen && <LSF_MatchAlertMsgModal {...MatchAlertMsgProps} />}
        {/* {fxPinModalOpen && <PromptModal {...fxPinModalProps} />} */}
        {/* {addFxModalOpen && <SF_AddFixtureModal {...addFixtureProps} />} */}
        <Row>
          <Col md={12}>
            <h2 className="h2-cls">Daily Stock Fantasy</h2>
          </Col>
        </Row>
        <hr />
        <Row>
            ...
            <div className="user-navigation">
              ...
              <Row className="sf-dfs-tabs">
                <Col md={12}>
                    ...
                    <Row>
                      <Col lg={12}>
                        <Button
                          ...
                          onClick={() => this.props.history.push('/livestockfantasy/create-candle/0')}
                        >
                          Create New Candle
                        </Button>
                      </Col>
                  ...
                  <Row className="SpRow">
                    <Col lg={12} className="SpContent">
                      {this.getSFCard(fixtureList[DataKey])}

                    </Col>
                  </Row>

                  {
                    _isEmpty(fixtureList[DataKey]) &&
                    <Col md={12}>
