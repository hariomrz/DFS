...
import { LSF_STOCK_CLOSING_RATE_FOR_TIME, SF_updatePriceStatus, LSF_UPDATE_STOCK_RATE, SP_GET_CANDLE_TIME_LIST } from '../../helper/WSCalling';
...
class LSF_MatchCloser extends Component {
    constructor(props) {
        super(props)
        this.state = {
    ...
    getStockList = () => {
        ...
        LSF_STOCK_CLOSING_RATE_FOR_TIME(params).then(ResponseJson => {
    ...
    closeFixture = () => {
        ...
        SF_updatePriceStatus({}).then(Response => {
            if (Response.response_code == NC.successCode) {
                this.props.history.push('/livestockfantasy/fixture?pctab=1&tab=1')
    ...
    updateResult = () => {
        ...
        LSF_UPDATE_STOCK_RATE(param).then(Response => {
            if (Response.response_code == NC.successCode) {
                notify.show(Response.message, "success", 5000)
                this.setState({
                    CloseFxModalOpen: false,
...
export default LSF_MatchCloser
