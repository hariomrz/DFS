...
import SP_MatchAlertMsgModal from './SP_MatchAlertMsgModal';
...
class SF_DSF extends Component {
  ...
  redirectToStock = (selectedObj) => {
    let { activeFxType, activeTab } = this.state
    this.props.history.push({ pathname: '/stockpredict/fixturecontest/' + activeTab + '/' + selectedObj.collection_id });
  }

  redirectToNewStockReview = (itemObj) => {
    this.props.history.push({ pathname: '/stockpredict/create-candle/' + itemObj.collection_id })
  }

  redirectToContestTemplate = (selectedObj) => {
    let { activeTab } = this.state
    let send_val = this.getUrlFxInput(selectedObj)
    this.props.history.push({ pathname: '/stockpredict/createtemplatecontest/' + activeTab + '/' + selectedObj.collection_id });
  }
  ...
  addFixtureNext = () => {
    ...
    WSManager.Rest(NC.baseURL + NC.SF_VALIDATE_FIXTURE, param).then((responseJson) => {
      if (responseJson.response_code === NC.successCode) {
        ...
        this.props.history.push({ pathname: '/stockpredict/verify-stocks/' + activeFxType + '/' + activeTab + '/' + FixtureDate + '/0/' + new_name })
      }
      else if (responseJson.response_code == NC.sessionExpireCode) {
        WSManager.logout();
        this.props.history.push('/login');
  ...
  render() {
    ...
    return (
      <div className="animated fadeIn SpDsf">
        {msgModalIsOpen && <SP_MatchAlertMsgModal {...MatchAlertMsgProps} />}
                    ...
                    <Row>
                      <Col lg={12}>
                        <Button
                          ...
                          onClick={() => this.props.history.push('/stockpredict/create-candle/0')}
                        >
                          Create New Candle
                        </Button>
                      </Col>
