...
import { SP_GET_FIXTURE_TEMPLATE, SP_CREATE_TEMPLATE_CONTEST } from "../../helper/WSCalling";
class SP_CreateTemplateContest extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    GetFixtureTemplates = () => {
        ...
        SP_GET_FIXTURE_TEMPLATE(params).then((responseJson) => {
    ...
    selectAllTemplate = () => {
        ...
        templateList.map((template) => (
            _.map(template.template_list, (templist) => {
                if (selectedTemplate.indexOf(templist.template_id) == -1 && templist.stock_type == 3) {
        ...
        if (window.confirm("Are you sure want to create contest of selected template ?")) {
            ...
            SP_CREATE_TEMPLATE_CONTEST(params).then((responseJson) => {
                if (responseJson.response_code === NC.successCode) {
                    ...
                    this.props.history.push({ pathname: '/stockpredict/fixturecontest/' + ActiveTab + '/' + collection_id });
                } else {
...
export default SP_CreateTemplateContest;