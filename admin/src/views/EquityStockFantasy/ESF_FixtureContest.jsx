...
class FixtureContest extends Component {
  ...
  GetFixtureContest = () => {
    ...
    WSManager.Rest(NC.baseURL + NC.ESF_GET_FIXTURE_CONTEST, params)
      .then((responseJson) => {
        if (responseJson.response_code === NC.successCode) {
  ...
  redirectToCreateContest = () => {
    ...
    this.props.history.push({ pathname: '/equitysf/createcontest/' + ActiveFxType + '/' + ActiveTab + '/' + fixtureDetail.scheduled_date + '/' + collection_id })
  }
  ...
  deleteContest = (e, contestObj, index) => {
    ...
    if (window.confirm("Are you sure want to delete this contest ?")) {
      ...
      WSManager.Rest(NC.baseURL + NC.ESF_DELETE_CONTEST, params).then((responseJson) => {
        if (responseJson.response_code === NC.successCode) {
          this.GetFixtureContest();
          notify.show(responseJson.message, "success", 5000);
        } else {
  ...
  cancelMatch = () => {
    ...
    if (API_FLAG == 1) {
      ...
      API_URL = NC.ESF_CANCEL_COLLECTION
    } else {
      param.contest_unique_id = CONTEST_U_ID
      API_URL = NC.ESF_CANCEL_CONTEST
    }
    ...
    WSManager.Rest(NC.baseURL + API_URL, param).then((responseJson) => {
      if (responseJson.response_code === NC.successCode) {
        ...
        if (GroupIndex >= "0" && DeleteIndex >= "0") {
          ...
          // this.props.history.push({ pathname: '/equitysf/fixture' })
  ...
  render() {
    ...
    return (
      <div className="animated fadeIn contest-group-wrapper esf-fixture-contest-main">
          ...
          <div className="fixture-contest">
            {
              ...
              <label className="backtofixtures" onClick={() => this.props.history.push('/equitysf/fixture?pctab=' + ActiveFxType + '&tab=' + ActiveTab)}> {'<'} Back to Fixtures</label>
            }
          </div>
        </Col>
                          ...
                          <div className="contest-list-header clearfix">
                            <div className="contest-heading">
                              <div className="action-head clearfix">
                                <div onClick={() => this.props.history.push('/equitysf/contest_detail/' + contest.contest_id+'?tab='+ActiveTab)} className="contest-name text-ellipsis">{contest.contest_name}</div>
                              </div>
