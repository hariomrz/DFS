...
import { REVERSE_FANTASY_TT, SCRATCH_WIN_CHECK, CONTEST_PRZ_DIS_TT, CONTEST_TIE_BRE_TT, CONTEST_HEADER_NAME_TT, CONTEST_SPONSOR_TT, CONTEST_RECUR_TT, CONTEST_BONUS, MODULE_NOT_ENABLE } from '../../helper/Message';
...
class Createtemplate extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      contestTemplate: { 'category_id': [], 'multiple_lineup': '1', 'entry_fee_type': '1', 'max_bonus_allowed': '0', 'prize_type': '1', 'prize_pool_type': '1', "master_contest_type_id": "1", "group_id": "1", "is_auto_recurring": false, 'site_rake': 0, 'site_rake_max': 0, 'minimum_size': '2', 'size': '2', 'custom_total_percentage': '100', 'custom_total_amount': '0', 'prize_value_type': '0', 'is_tie_breaker': false, 'sponsor_name': '', 'sponsor_logo': '', 'sponsor_link': '', 'set_sponsor': '0', 'sponsor_contest_dtl_image': '', 'is_reverse': false, 'is_scratchwin': false },
      IsValidate: false,
      IsFormValid: true,
      payout_data: [],
      rows: [{}],
      ...
      SWinToolTip: false,
      contest_template_id: (this.props.match.params.template_id) ? Base64.decode(this.props.match.params.template_id) : false,
      copyPosting: false,
    };
  }

  componentDidMount() {
    if (HF.allowEquityFantasy() != '1') {
  ...
  GetContestTemplateMasterData = () => {
    ...
    WSManager.Rest(NC.baseURL + NC.ESF_GET_CONTEST_TEMPLATE_MASTER_DATA, { "sports_id": this.state.selected_sport }).then((responseJson) => {
      if (responseJson.response_code === NC.successCode) {
        responseJson = responseJson.data;
        ...
        this.setState({
          prizeType: prizeTypeTmp, merchandiseObj: merchandiseObj,
  ...
  CreateContestTemplate = () => {
    ...
    // if (this.state.selectedLeague.length <= 0) {
    ...
    if (is_valid == 1) {
      ...
      contestTemplate['category_id'] = this.state.selectedLeague;
      contestTemplate['sports_id'] = this.state.selected_sport;
      contestTemplate['stock_type'] = "2";
      ...
      if (WSManager.validateFormFields("contest_template_form")) {
        if ((contestTemplate.set_sponsor == 1 && this.state.wrongLink) || contestTemplate.set_sponsor == 0) {
          ...
          WSManager.Rest(NC.baseURL + NC.ESF_CREATE_CONTEST_TEMPLATE, params).then((responseJson) => {
            if (responseJson.response_code === NC.successCode) {
              notify.show(responseJson.message, "success", 5000);
              this.props.history.push({ pathname: '/equitysf/contesttemplate' })
            }
            this.setState({ posting: false })
          })
        }else{
  ...
  calculateProfitData = () => {
    ...
    _.map(this.state.payout_data, (value, key) => {
      ...
      //Start price for merchandise
      // if(value.prize_type == "3" && value.min_value && value.max_value){
      //   min_total = parseFloat(min_total) + parseFloat(value.min_value);
      //   max_total = parseFloat(max_total) + parseFloat(value.max_value);
      // }
      //End price for merchandise
    });
  ...
  onChangeImage = (event) => {
    ...
    WSManager.multipartPost(NC.baseURL + NC.ESF_UPLOAD_CONTEST_TEMPLATE_SPONSER, data)
      .then(Response => {
  ...
  onChangeContDtlImage = (event) => {
    ...
    WSManager.multipartPost(NC.baseURL + NC.ESF_DO_UPLOAD_SPONSOR_CONTEST_DTL, data)
      .then(Response => {
        if (Response.response_code == NC.successCode) {
  ...
  GetContestTemplateDetails = () => {
    this.setState({ posting: true })
    WSManager.Rest(NC.baseURL + NC.ESF_GET_CONTEST_TEMPLATE_DETAILS, { "contest_template_id": this.state.contest_template_id }).then((responseJson) => {
      if (responseJson.response_code === NC.successCode) {
  ...
  setPreTemplateData = (routeState) => {
    ...
    tempCopyD.template_name = routeState.contest_name;
    tempCopyD.template_title = routeState.template_title;
    ...
    if (routeState.is_reverse == 1) {
      tempCopyD.is_reverse = true;
    }
    if (routeState.is_scratchwin == 1) {
      tempCopyD.is_scratchwin = true;
    }    
    setTimeout(() => {
      this.setState({
        contestTemplate: tempCopyD,
        payout_data: routeState.prize_distibution_detail,
        selectedLeague: routeState.template_categories,
  ...
  render() {
    ...
    return (
      <div className="esf-create-template create-template-gry-input ftp-wrap animate-left">
              ...
              <Card className="recentcom">
                ...
                <CardBody className="contestcard">
                  <Row>
                    ...
                    <Col sm={4}>
                      <div className="form-group gray-form-group">
                        <label htmlFor="template_name" className="fixturevs">Enter Template Name
                        </label>
                        <input className="contestname required" id="template_name" name="template_name" value={contestTemplate.template_name} onChange={(e) => this.handleFieldVal(e, 'template_name', 'template_name')} placeholder="Template Name here (eg. WIN 540)"></input>
                      </div>
                    </Col>
                    <Col sm={4}>
                      <div className="form-group gray-form-group">
                        <label htmlFor="template_name" className="fixturevs font-sm">Template Header Name(Optional)
                        ...
                        <input
                          ...
                          id="template_title"
                          name="template_title"
                          value={contestTemplate.template_title}
                          onChange={(e) => this.handleFieldVal(e, 'template_title', 'template_title')}
                          placeholder="Template Header Name"
                        ></input>
                      </div>
                    </Col>
                  </Row>
                      ...
                      <ul className="minmax-list">
                        <li className="minmax-item mr-3">
                          <div className="form-group minmax-size gray-form-group">
                            <input type="number" className="contestname min-max-size xminsize required" name="minimum_size" value={contestTemplate.minimum_size} onChange={(e) => (this.handleFieldVal(e, 'minimum_size', 'minimum_size'), this.calculatePrizePool(e, 'minimum_size'))} placeholder="Min. Size"></input>
                          </div>
                        </li>
                        <li className="minmax-item">
                          <div className="form-group minmax-size gray-form-group">
                            <input type="number" className="contestname min-max-size xmaxsize required" name="size" value={contestTemplate.size} onChange={(e) => (this.handleFieldVal(e, 'size', 'size'), this.calculatePrizePool(e, 'size'))} placeholder="Max. Size"></input>
                          </div>
                        </li>
                      </ul>
                    </Col>
                  ...
                  <Row className="mt-4">
                    ...
                    {/* {
                      HF.getMasterData().allow_reverse_contest == '1' &&
                      <Col sm={4}>
                        <div className="form-group">
                          <label htmlFor="is_reverse" className="fixturevs">Reverse Fantasy
                        <span className="btn-information">
                              <img id="rev-fant" className="infobtn" src={Images.INFO} />
                              <Tooltip placement="right" isOpen={RevFantToolTip} target="rev-fant" toggle={this.reverseFantTogg}>{REVERSE_FANTASY_TT}</Tooltip>
                            </span>
                          </label>
                          <div className="autorecurrentdiv">
                            <Input
                              checked={contestTemplate.is_reverse}
                              type="checkbox"
                              id="is_reverse"
                              name="is_reverse"
                              value={contestTemplate.is_reverse}
                              onChange={(e) => this.handleCheckboxFieldVal(e, 'is_reverse')}
                              className="custom-control-input" />
                            <label className="custom-control-label" htmlFor="is_reverse"></label>
                          </div>
                        </div>
                      </Col>
                    }
                    {
                      ...
                      <Col sm={4}>
                        <div className="form-group">
                          ...
                          <div className="autorecurrentdiv">
                            <Input
                              checked={contestTemplate.is_scratchwin}
                              type="checkbox"
                              id="is_scratchwin"
                              name="is_scratchwin"
                              value={contestTemplate.is_scratchwin}
                          ...
                          <Row>
                            <Col sm={6} md={6}>
                              <div className="form-group multiselect-wrapper spr-link-wdt">
                                <label htmlFor="category_id" className="fixturevs ">Sponsored Link</label>
