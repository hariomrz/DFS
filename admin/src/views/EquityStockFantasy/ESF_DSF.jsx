...
import ESF_MatchAlertMsgModal from './ESF_MatchAlertMsgModal';
import ESF_FixtureCard from './ESF_FixtureCard';
import ESF_AddFixtureModal from './ESF_AddFixtureModal';
import { ESF_getHoliday } from '../../helper/WSCalling';
class ESF_DSF extends Component {
  constructor(props) {
    super(props);
    let filter = {
      current_page: 1,
  ...
  GetAllFixtureList = () => {
    ...
    WSManager.Rest(NC.baseURL + NC.ESF_GET_FIXTURES, param).then((responseJson) => {
      if (responseJson.response_code === NC.successCode) {
  ...
  redirectToStock = (selectedObj) => {
    ...
    this.props.history.push({ pathname: '/equitysf/fixturecontest/' + activeFxType + '/' + activeTab + '/' + selectedObj.collection_id });
  }

  redirectToNewStockReview = (itemObj) => {
    let send_val = this.getUrlFxInput(itemObj)
    this.props.history.push({ pathname: '/equitysf/verify-stocks/' + this.state.activeFxType + '/' + this.state.activeTab + '/' + send_val + '/' + itemObj.collection_id + '/' + itemObj.name })
  }

  redirectToContestTemplate = (selectedObj) => {
    let { activeFxType, activeTab } = this.state
    let send_val = this.getUrlFxInput(selectedObj)
    this.props.history.push({ pathname: '/equitysf/createtemplatecontest/' + activeFxType + '/' + activeTab + '/' + send_val + '/' + selectedObj.collection_id });
  }
  ...
  updateMatchMsg = (flag) => {
    ...
    WSManager.Rest(NC.baseURL + NC.ESF_UPDATE_FIXTURE_CUSTOM_MESSAGE, param).then((responseJson) => {
      if (responseJson.response_code === NC.successCode) {
  ...
  addFixtureNext = () => {
    ...
    WSManager.Rest(NC.baseURL + NC.ESF_VALIDATE_FIXTURE, param).then((responseJson) => {
      if (responseJson.response_code === NC.successCode) {

        notify.show(responseJson.message, "success", 5000);
        let new_name = FixtureName ? FixtureName : 0;
        this.props.history.push({ pathname: '/equitysf/verify-stocks/' + activeFxType + '/' + activeTab + '/' + FixtureDate + '/0/' + new_name })
      }
      else if (responseJson.response_code == NC.sessionExpireCode) {
        WSManager.logout();
        this.props.history.push('/login');
  ...
  GetHolidayList = () => {
    ...
    ESF_getHoliday(params).then((responseJson) => {
      if (responseJson.response_code === NC.successCode) {
  ...
  render() {
    ...
    return (
      <div className="animated fadeIn esf-dsf-main">
        {msgModalIsOpen && <ESF_MatchAlertMsgModal {...MatchAlertMsgProps} />}
        {fxPinModalOpen && <PromptModal {...fxPinModalProps} />}
        {addFxModalOpen && <ESF_AddFixtureModal {...addFixtureProps} />}
        <Row>
          <Col md={12}>
