...
export default class ESF_ContestDetails extends Component {
    constructor(props) {
        super(props)
        this.state = {
            Total: 0,
    ...
    lineupDetailModal(ApiCall, lineup_master_contest_ids) {
        this.setState(prevState => ({
            isLineupModalOpen: !prevState.isLineupModalOpen
        }), () => {
            if (ApiCall) { this.getLinupDetails(lineup_master_contest_ids) }
            else
    ...
    getGameDetail = () => {
        ...
        WSManager.Rest(NC.baseURL + NC.ESF_GET_GAME_DETAIL, params).then(ResponseJson => {
            if (ResponseJson.response_code == NC.successCode) {
                this.setState({
    ...
    getGameLinupDetail = (gameDetail) => {
        ...
        WSManager.Rest(NC.baseURL + NC.ESF_GET_GAME_LINEUP_DETAIL, params).then(ResponseJson => {
            if (ResponseJson.response_code == NC.successCode) {
                this.setState({
    ...
    getLinupDetails = (lineup_master_contest_ids) => {
        let params = {
            lineup_master_contest_id: lineup_master_contest_ids
        }
        WSManager.Rest(NC.baseURL + NC.ESF_GET_LINEUP_DETAIL, params).then((ResponseJson) => {
    ...
    render() {
        const { gameDetail, LeagueDetail, MatchDetail, MatchList, SportDetail, UserData, prize_distibution_detail, GameLinupDetail, LinupUpData, CURRENT_PAGE, PERPAGE, Total, UserType, TotalMerchadiseDistri, AllowSystemUser, contest_template_id, activeTab, TemplateCategory } = this.state
                ...
                <div className="contest-d-main">
                    ...
                    <div className="details-box">
                        <Row className="box-items mt-3">
                            ...
                            <Col md={3} className={` ${isFromContest ? 'xmt-3' : ''}`}>
                            ...
                            </Col>
                            {
                                ...
                                <Col md={3} className="mt-3">
                                    ...
                                    <div className="user-value">
                                        ...
                                        {HF.getFormatedDateTime(gameDetail.scheduled_date, "D-MMM-YYYY hh:mm A ")}
                                    </div>
                                </Col>
                            }
                            <Col md={3} className="mt-3">
                                                ...
                                                return (
                                                    <tbody key={idx}>
                                                        <tr>
                                                            ...
                                                            <td>
                                                                {lineup.total_score.toString().includes("-") ? '-' : ''}
                                                                {HF.getCurrencyCode()}
                                                                {lineup.total_score.toString().replace("-", "")}                                                                
                                                            </td>
                                                            <td>
                                                                {
                                                                    lineup.is_winner == "1" ?
                                                            ...
                                                            <td onClick={() => this.lineupDetailModal(true, lineup.lineup_master_contest_id)}>
                                                    ...
                                                    <ul className="lineup-feelist">
                                                        ...
                                                        <li className="lineup-feeitem">
                                                            ...
                                                            <div className="font-weight-bold">
                                                                {!_isUndefined(LinupUpData.score) ? (LinupUpData.score.toString().includes("-") ? '-' : '') : ''}
                                                                {HF.getCurrencyCode()}
                                                                {!_isUndefined(LinupUpData.score) ? LinupUpData.score.toString().replace("-", "") : ''}
                                                            </div>
                                                        </li>
                                                    </ul>
                                                </Col>
                                                    ...
                                                    <Table>
                                                        <thead>
                                                            <tr>
                                                                ...
                                                                <th>Buy/Sell</th>
                                                                <th>Joining Price</th>
                                                                {activeTab != '1' && <th>Invested Amount</th>}
                                                                <th>
                                                                    {activeTab == '0' && 'Result Price'}
                                                                    {activeTab == '2' && 'Closing Price'}
                                                                </th>
                                                                {activeTab == '0' && <th>Current Amount</th>}
                                                                {activeTab == '1' && <th>Invested Amount</th>}
                                                                {activeTab == '2' && <th>Final Amount</th>}
                                                                
                                                                {activeTab != '1' && <th>Gain/Loss</th>}
                                                        ...
                                                        </thead>
                                                        {
                                                            _.map(LinupUpData.lineup, (lineup, idx) => {
                                                                ...
                                                                if (lineup.type == '2')
                                                                {
                                                                    if(invest_amt < usr_amount)
                                                                    {
                                                                        gain_loss = -Math.abs(gain_loss)
                                                                    }
                                                                    if(invest_amt >= usr_amount)
                                                                    {
                                                                        gain_loss = Math.abs(gain_loss)
                                                                ...
                                                                if (lineup.captain == '1')
                                                                {
                                                                    usr_amount = usr_amount * 2
                                                                }
                                                                else if (lineup.captain == '2')
                                                                {
                                                                    usr_amount = usr_amount * 1.5
                                                                }
                                                                return (
                                                                    <tbody key={idx}>
                                                                        <tr>
                                                                            ...
                                                                            <td className="pl-4">{lineup.user_lot_size}</td>

                                                                            <td>
                                                                                {lineup.type == '1' ? 'Buy' : lineup.type == '2' ? 'Sell' : '--'}
                                                                            </td>
                                                                            <td>{lineup.joining_rate}</td>
                                                                            {activeTab != '1' && <td>{invest_amt}</td>}
                                                                            <td>{activeTab != '1' && HF.convertTodecimal(lineup.result_rate, 2)}</td>
                                                                            <td>{usr_amount}</td>                                                                            
                                                                            
                                                                            {activeTab != '1' && <td>{gain_loss}</td>}
