...
import { ESF_uploadStockLogo, ESF_list, ESF_lot, ESF_delete, ESF_save, ESF_update, ESF_autoSuggetionList, ESF_getIndustryList, SP_GET_MASTER_DATA  } from '../../helper/WSCalling';
...
class ESF_ManageStock extends Component {
    constructor(props) {
        super(props)
        this.state = {
    ...
    getStockList = () => {
        ...
        ESF_list(params).then(ResponseJson => {
    ...
    getIndustry = () => {
        ESF_getIndustryList({}).then(ResponseJson => {
            if (ResponseJson.response_code == NC.successCode) {
    ...
    deleteStock = () => {
        ...
        ESF_delete(param).then((responseJson) => {
            if (responseJson.response_code === NC.successCode) {
    ...
    handleSelectChange = (data, name) => {
        if (!_isNull(data)) {
            this.setState({ [name]: data.value, CreatePosting: false })
        }
    }

    addStockModal = () => {
        let { StockIcon, CreatePosting, StockDispName, EditFlag, LotSize, SearchStock, SelectedIndustry, SelectedCap, IndustryOpt, CapOptions } = this.state
    ...
    addStockToggle = (index, item) => {       
        ...
        if (!_isUndefined(item)) {
            this.setState({
                ...
                SelectedIndustry: item.industry_id,
                SelectedCap: item.cap_type,// parseInt(item.cap_type),
            })
        }
    ...
    validForm = () => {
        let { LotSize, StockDispName, StockIconName, SearchStock, EditFlag, SelectedIndustry, SelectedCap } = this.state
        ...
        else if (_isEmpty(SelectedIndustry)) {
            let msg = 'Please select industry'
            notify.show(msg, 'error', 3000)
            r_flag = false
        }
        else if (_isEmpty(SelectedCap)) {
            let msg = 'Please select cap'
            notify.show(msg, 'error', 3000)
            r_flag = false
        }
    ...
    addStock = () => {
        ...
        let { StockDispName, LotSize, StockIconName, StockId, EditFlag, MasterStockId, SelectedIndustry, SelectedCap } = this.state
    ...
    onChangeImage = (event) => {
        ...
        ESF_uploadStockLogo(data).then(Response => {
    ...
    getStockSuggestion = () => {
        ...
        ESF_autoSuggetionList(params).then(ResponseJson => {
            if (ResponseJson.response_code == NC.successCode) {
    ...
    render() {
        let { StockList, Total, ListPosting, Keyword, SelectedLot, DeleteModalOpen, DeletePosting, addStockModalOpen, CURRENT_PAGE, PERPAGE, LotList } = this.state
        const Select_Props = {
            is_disabled: false,
            is_searchable: true,
            is_clearable: false,
        ...
        return (
            <div className="esf-manage-stocks">
...
export default ESF_ManageStock
