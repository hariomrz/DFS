...
export default class UserContestReport extends Component {
        ...
        WSManager.Rest(NC.baseURL + NC.GET_CONTEST_FILTER, params).then((responseJson) => {
            if (responseJson.response_code === NC.successCode) {
                ...
                this.setState({ 
                    ...
                    FeatureOptionsoriginal: TempFlist
                 }, () => {
                    if (this.state.SelectedLSports == 15) {
                        this.setState({
                            FeatureOptions: TempFlist.filter(obj => obj.value != 2)
                        })
                    } else {
                        this.setState({
                            FeatureOptions: TempFlist
    ...
    exportReport_Post = () => {
        ...
        const { Keyword, FromDate, ToDate, sortField, isDescOrder, SelectedLeague, SelectedCollection, sportsId, SelectedFeature } = this.state
        let params = {
            league_id: SelectedLeague.value,
    ...
    exportReport_Get = () => {
        ...
        var query_string = 'report_type=contest_report&csv=1&keyword=' + Keyword + '&from_date=' + tempFromDate + '&to_date=' + tempToDate + '&sort_order=' + sOrder + '&sort_field=' + sortField + '&league_id=' + LeagueId + '&collection_master_id=' + CollId + '&sports_id=' + sportsId + '&feature_type=' + SelectedFeature;
    ...
    handleTypeChange = (value, name) => {
        if (value != null) {
            this.setState({
                ...
                this.getReportUser()
                if(name == 'SelectedLSports') {
                    const { FeatureOptionsoriginal } = this.state
                    if(value.value == 15) {
                        this.setState({
                            FeatureOptions: FeatureOptionsoriginal.filter(obj => obj.value != 2)
                        })
                    } else {
                        this.setState({
                            FeatureOptions: FeatureOptionsoriginal
                        })
                    }
                    this.getLeagueFilter()
                }
    ...
    render() {
        const { posting, UserReportList, CURRENT_PAGE, PERPAGE, TotalUser, Keyword, isDescOrder, SelectedLeague, LeagueList, AllSportsList, SelectedLSports, CollectionList, SelectedCollection, sumJoinRealAmount, sumJoinWinningAmount, sumJoinBonusAmount, FromDate, ToDate, groupList, SelectedGroup, TotalUserReport, SelectedFeature, FeatureOptions } = this.state
        ...
        return (
            <Fragment>
                <div className="animated fadeIn mt-4 uc-report">
                    <Row>
                        <Col md={12}>
                            <h1 className="h1-cls">Contestss Report</h1>
                        </Col>
                    </Row>
                        <Row className="mt-4">
                            <Col md={2}>
                                                    ...
                                                    return (
                                                        <tbody key={idx}>
                                                            <tr>
                                                                ...
                                                                <td>{item.total_join_real_amount ? parseFloat(parseFloat(item.total_join_real_amount)+parseFloat(item.total_join_winning_amount)).toFixed(2) : '--'}</td>
                                                                ...
                                                                <td>{item.bot_entry_fee ? item.bot_entry_fee : '--'}</td>
                                                                ...
                                                                {/* <td>{item.profit_loss ? parseFloat(parseFloat(item.entry_real)-parseFloat(item.bot_entry_fee)-parseFloat(item.total_win_amount_to_real_user)).toFixed(2) : '--'}</td> */}
<td>{item.profit_loss ? parseFloat((parseFloat(item.total_join_real_amount)+parseFloat(item.total_join_winning_amount))-(parseFloat(item.bot_entry_fee))-parseFloat(item.total_win_amount_to_real_user)).toFixed(2) : '--'}</td>
                                                                <td>
                                                                    {/* {WSManager.getUtcToLocalFormat(item.season_scheduled_date, 'D-MMM-YYYY hh:mm A')} */}
                         {HF.getFormatedDateTime(item.schedule_date, 'D-MMM-YYYY hh:mm A')}
