...
class Createtemplate extends Component {
  ...
  CreateContestTemplate = () => {
    ...
    if (is_valid == 1) {
      ...
      contestTemplate['league_id'] = this.state.selectedLeague;
      contestTemplate['sports_id'] = this.state.selected_sport;
      contestTemplate['is_copy'] = this.state.isCopyTemplate;
      ...
      if (WSManager.validateFormFields("contest_template_form")) {
        if ((contestTemplate.set_sponsor == 1 && this.state.wrongLink) || contestTemplate.set_sponsor == 0) {
          ...
          WSManager.Rest(NC.baseURL + NC.CREATE_CONTEST_TEMPLATE, params).then((responseJson) => {
            if (responseJson.response_code === NC.successCode) {
              notify.show(responseJson.message, "success", 5000);
              this.props.history.push({ pathname: '/game_center/contesttemplate' })
            }
            this.setState({ posting: false })
          })
        } else {
  ...
  validatePrizeData = (e, index, merchandise) => {
    ...
    if (name == "max") {
      payout_data[index]['max'] = parseInt(value);
    }
  ...
  setPreTemplateData = (routeState) => {
    ...
    tempCopyD.template_name = routeState.template_name;
    ...
    if(routeState.prize_value_type == 1){
      tempCopyD.prize_value_type = '0';
      let contestTemplate = _.cloneDeep(this.state.contestTemplate);
      contestTemplate.prize_value_type = '0';
      this.setState({ contestTemplate: contestTemplate })
    }
    ...
    if (routeState.is_2nd_inning == 1) {
      tempCopyD['is_2nd_inning'] = true;
    }
    setTimeout(() => {
      this.setState({
              ...
              <Card className="recentcom pb-4">
                ...
                <CardBody className="contestcard">
                  ...
                  <Row className="mt-4">
                    ...
                    {/* {
                      (HF.getMasterData().allow_reverse_contest == '1' && selected_sport != '15' && !contestTemplate.is_2nd_inning && contestTemplate.group_id != HF.get_h2h_group_id()) &&
                      <Col sm={4}>
                        <div className="form-group">
                    ...
                    {
                      (HF.allowSecondInni() == '1' && selected_sport == '7' && !contestTemplate.is_reverse && contestTemplate.group_id != HF.get_h2h_group_id()) &&
                      <Col sm={4} className={`${contestTemplate.is_reverse ? 'mt-4' : ''}`}>
                        <div className="form-group">
                          <label htmlFor="is_2nd_inning" className="fixturevs">Second Inning
                        <span className="btn-information">
                              <img id="sinn" className="infobtn" src={Images.INFO} />
                              <Tooltip placement="right" isOpen={SecInniToolTip} target="sinn" toggle={this.SecInniToggle}>{SECOND_INNING}</Tooltip>
                            </span>
                          </label>
                          <div className="autorecurrentdiv">
                            <Input
                              checked={contestTemplate.is_2nd_inning}
                              type="checkbox"
                              id="is_2nd_inning"
                              name="is_2nd_inning"
                              value={contestTemplate.is_2nd_inning}
                              ...
                              <td className="percentage-td">
                                ...
                                {payout_data[idx].prize_type != 3 &&
                                  <Input
                                    ...
                                    name="amount"
                                    // value={payout_data[idx].amount}
                                    value={ this.state.isCopyTemplate && contestTemplate.prize_value_type === '1' ? payout_data[idx].per : payout_data[idx].amount}
                                    onChange={(e) => {
