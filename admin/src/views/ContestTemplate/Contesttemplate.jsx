...
class Contesttemplate extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      TotalTemplates: 0,
      SWinToolTip: false,
      AutoPublishModalIsOpen: false,
      CancelAutoPublishPosting: false,
      auto_published: '',
      contest_template_id: '' 
    };
  }
  ...
  GetContestTemplates = () => {
    ...
    WSManager.Rest(NC.baseURL + NC.GET_CONTEST_TEMPLATE_LIST, params).then((responseJson) => {
  ...
  deleteContestTemplate = (e, templateObj, index) => {
    ...
    if (window.confirm("Are you sure want to delete this template ?")) {
      ...
      WSManager.Rest(NC.baseURL + NC.DELETE_CONTEST_TEMPLATE, params).then((responseJson) => {
        if (responseJson.response_code === NC.successCode) {
          this.GetContestTemplates();
  ...
  AutoPublishModalToggle = (template) => {

    // console.log(template);return false;
    if (template.auto_published == 0) {
      var autoPublished = 1;      
    }else{
      var autoPublished = 0;
    }
    this.setState({
      AutoPublishModalIsOpen: true, 
      auto_published: autoPublished ,
      contest_template_id: template.contest_template_id      
    });
  }

  closeAutopublishPopup = () => {
    this.setState({
      AutoPublishModalIsOpen: false
    });
  }
  ...
  AtopublishModal = () => {
    return (
      <div>
        <Modal
          isOpen={this.state.AutoPublishModalIsOpen}         
        ...
        >
          ...
          <ModalBody>
            ...
            {console.log(this.state.auto_published,'auto_published')}
            <div className="inputform-box">
              {this.state.auto_published == 0 && < label > <span className="label-first">Are you sure want to remove from auto publish ?</span></label>}
              {this.state.auto_published == 1 && < label > <span className="label-first">Are you sure want to enable for auto publish ?</span></label>}
            </div>
          ...
          <ModalFooter>
            <Button
              color="secondary"
              onClick={this.UpdateAutoPublish}
              disabled={this.state.CancelAutoPublishPosting}
            >Yes</Button>{' '}
            <Button color="primary" onClick={this.closeAutopublishPopup}>No</Button>
          </ModalFooter>
        </Modal>
      </div>
    )
  ...
  UpdateAutoPublish = () => {
    let { CancelAutoPublishPosting } = this.state
    this.setState({ CancelAutoPublishPosting : false ,
      // AutoPublishModalIsOpen: false
});

    let param = {
      contest_template_id : this.state.contest_template_id,
      auto_published : this.state.auto_published
    };

    // console.log(param);

    WSManager.Rest(NC.baseURL + NC.UPDATE_CONTEST_AUTOPUBLISH, param).then((responseJson) => {
      if (responseJson.response_code === NC.successCode) {
        this.setState({
          CancelAutoPublishPosting: false,    
          AutoPublishModalIsOpen: false     
                          ...
                          <div className="contest-list-header">
                            <div className="contest-heading">
                              <div 
                                onClick={() => {
                                  this.props.history.push({
                                    pathname: '/contest_template_detail/' + template.contest_template_id,
                                    state: { templateGroup: item, template_list: template, 
                                      // leagueId: template.template_leagues['0'] 
                                    }
                                  })
                                }}
                              ...
                              <div className="clearfix">
                                <ul className="ul-action con-action-list">
                                {
                                  ...
                                  }
                                  {
                                    ...
                                    <li className="action-item">
                                      <i
                                        id={"swin_" + group_index + '_' + idx} className={`icon-SW contest-type ${(template.is_scratchwin == "1") ? '' : 'not-active'}`}>
                                 ...
                                 
                                  {
                                    <li
                                      ...
                                      onClick={() => { this.props.history.push({pathname:'copycreatecontesttemplate/' + Base64.encode(template.contest_template_id)})}}
                                    >
                                  ...
                                  {
                                    <li className="action-item">
                                      {/* <span class="icon-upload " onClick={() => this.AutoPublishModalToggle(template)}></span> */}
                                      <span title="Auto Publish"  className={`icon-upload ${(template.auto_published == "1") ? 'auto_publish_yes' : ''}`} onClick={() => this.AutoPublishModalToggle(template)}>
                                        
                                      </span>
                                      {/* <a onClick={() => this.AutoPublishModalToggle(template)}>A</a> */}
                                    </li>
                                  }
                                </ul>
                              </div>
