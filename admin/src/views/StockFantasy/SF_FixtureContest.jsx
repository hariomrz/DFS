...
class FixtureContest extends Component {
  ...
  GetFixtureDetail = () => {
    ...
    WSManager.Rest(NC.baseURL + NC.SF_GET_COLLECTION_DETAILS, param).then((responseJson) => {
  ...
  GetFixtureContest = () => {
    ...
    WSManager.Rest(NC.baseURL + NC.SF_GET_FIXTURE_CONTEST, params)
      .then((responseJson) => {
        if (responseJson.response_code === NC.successCode) {
  ...
  redirectToCreateContest = () => {
    ...
    this.props.history.push({ pathname: '/stockfantasy/createcontest/' + ActiveFxType + '/' + ActiveTab + '/' + fixtureDetail.scheduled_date + '/' + collection_id })
  }
    ...
    WSManager.Rest(NC.baseURL + API_URL, param).then((responseJson) => {
      if (responseJson.response_code === NC.successCode) {
        ...
        if (GroupIndex >= "0" && DeleteIndex >= "0") {
          ...
          // this.props.history.push({ pathname: '/stockfantasy/fixture' })
          this.props.history.push('/stockfantasy/fixture?pctab=' + ActiveFxType + '&tab=' + ActiveTab)
        }
        notify.show(responseJson.message, "success", 5000);
      }
      else if (responseJson.response_code == NC.sessionExpireCode) {
    ...
    return (
      <div>
        <Modal
          ...
          className="sf-cancel-match-modal"
        >
  ...
  render() {
    ...
    return (
      <div className="animated fadeIn contest-group-wrapper sf-fixture-contest-main">
          ...
          <div className="fixture-contest">
            {
              ...
              <label className="backtofixtures" onClick={() => this.props.history.push('/stockfantasy/fixture?pctab=' + ActiveFxType + '&tab=' + ActiveTab)}> {'<'} Back to Fixtures</label>
            }
          </div>
        </Col>
                          ...
                          <div className="contest-list-header clearfix">
                            <div className="contest-heading">
                              <div className="action-head clearfix">
                                <div onClick={() => this.props.history.push('/stockfantasy/contest_detail/' + contest.contest_id+'?tab='+ActiveTab)} className="contest-name text-ellipsis">{contest.contest_name}</div>
                              </div>
