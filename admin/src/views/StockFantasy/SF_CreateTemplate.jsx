...
class Createtemplate extends Component {
  ...
  GetCategoryList = () => {
    WSManager.Rest(NC.baseURL + NC.SF_GET_ALL_CATEGORY_LIST, { "sports_id": this.state.selected_sport }).then((responseJson) => {
  ...
  GetContestTemplateMasterData = () => {
    ...
    WSManager.Rest(NC.baseURL + NC.SF_GET_CONTEST_TEMPLATE_MASTER_DATA, { "sports_id": this.state.selected_sport }).then((responseJson) => {
      if (responseJson.response_code === NC.successCode) {
        responseJson = responseJson.data;
    ...
    if (is_valid == 1) {
      ...
      if (WSManager.validateFormFields("contest_template_form")) {
        if ((contestTemplate.set_sponsor == 1 && this.state.wrongLink) || contestTemplate.set_sponsor == 0) {
          ...
          WSManager.Rest(NC.baseURL + NC.SF_CREATE_CONTEST_TEMPLATE, params).then((responseJson) => {
            if (responseJson.response_code === NC.successCode) {
              notify.show(responseJson.message, "success", 5000);
              this.props.history.push({ pathname: '/stockfantasy/contesttemplate' })
            }
            this.setState({ posting: false })
          })
        }else{
  ...
  render() {
    ...
    return (
      <div className="sf-create-template create-template-gry-input ftp-wrap animate-left">
