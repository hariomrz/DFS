...
class SF_ContestTemplate extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  GetContestTemplates = () => {
    ...
    WSManager.Rest(NC.baseURL + NC.SF_GET_CONTEST_TEMPLATE_LIST, params).then((responseJson) => {
      if (responseJson.response_code === NC.successCode) {
        responseJson = responseJson.data;
        var totTemp = 0
  ...
  deleteContestTemplate = (e, templateObj, index) => {
    ...
    if (window.confirm("Are you sure want to delete this template ?")) {
      ...
      WSManager.Rest(NC.baseURL + NC.SF_DELETE_CONTEST_TEMPLATE, params).then((responseJson) => {
        if (responseJson.response_code === NC.successCode) {
          this.GetContestTemplates();
  ...
  applyTemplateToLeague = () => {
    ...
    if (window.confirm("Are you sure want to apply selected template on selected league ?")) {
      ...
      WSManager.Rest(NC.baseURL + NC.SF_APPLY_CONTEST_TEMPLATE_TO_CATEGORY, params).then((responseJson) => {
        if (responseJson.response_code === NC.successCode) {
          ...
          <Row className="dfsrow">
            ...
            <Button className='btn-secondary-outline' onClick={() => {
              this.props.history.push({
                pathname: '/stockfantasy/createcontesttemplate'
              })
                  ...
                  <div className="contest-group">
                    <div className="contest-list-wrapper">
                      <div className={selectedTemplate.indexOf(template.template_id) != -1 ? "contest-card more-contest-card contest-selected" : "contest-card more-contest-card"}>
                                ...
                                <ul className="ul-action con-action-list">
                                  ...
                                  } */}
                                  {
                                    <li
                                      ...
                                      onClick={() => { this.props.history.push({ pathname: '/stockfantasy/copycreatecontesttemplate/' + Base64.encode(template.template_id) }) }}
                                    >
                        ...
                        <div className="league-listing-wrapper bottom-card-detail">
                          <div className="league-listing-container">
...
export default SF_ContestTemplate;