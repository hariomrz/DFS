...
import { SF_getStockVerify, SF_publishFixture, SF_save, SF_update, SF_updateFixture } from '../../helper/WSCalling';
...
class SF_VerifyStocks extends Component {
    constructor(props) {
        super(props)
        this.state = {
    ...
    getStockList = () => {
        ...
        SF_getStockVerify(params).then(ResponseJson => {
    ...
    publishFixture = () => {
        ...
        if (collection_id == '0' && (p_arr.length < 10 || p_arr.length > 50)) {
            ...
            notify.show("Please select minimum 10 and maximum 50 stocks", "error", 3000)
            return false
        }
        else if (ret_flag) {
            this.setState({ PublishModalIsOpen: false })
        ...
        if (collection_id == '0') {
            URL = SF_publishFixture
            params.category_id = parseInt(ActiveFxType)
            params.value = fx_val
        } else {
            URL = SF_updateFixture
        ...
        URL(params).then(ResponseJson => {
            if (ResponseJson.response_code == NC.successCode) {
                ...
                if (collection_id == '0') {
                    this.props.history.push({ pathname: '/stockfantasy/createtemplatecontest/' + ActiveFxType + '/' + ActiveTab + '/' + FixtureValue + '/' + ResponseJson.data.collection_id });
                } else {
...
export default SF_VerifyStocks
