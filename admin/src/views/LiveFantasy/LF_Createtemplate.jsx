...
class LF_Createtemplate extends Component {
  constructor(props) {
    super(props);
    ...
    if (is_valid == 1) {
      ...
      if (WSManager.validateFormFields("contest_template_form")) {
        if ((contestTemplate.set_sponsor == 1 && this.state.wrongLink) || contestTemplate.set_sponsor == 0) {
          ...
          WSManager.Rest(NC.baseURL + NC.LF_CREATE_TEMPLATE, params).then((responseJson) => {
            if (responseJson.response_code === NC.successCode) {
              notify.show(responseJson.message, "success", 5000);
              this.props.history.push({ pathname: '/livefantasy/contesttemplate' })
            }
            this.setState({ posting: false })
          })
        } else {
  ...
  render() {
    ...
    return (
      <div className="fk-create-template fk-create-template-gry-input ftp-wrap animate-left">
        <form id="contest_template_form">
                  ...
                  <Row>
                    ...
                    {/* <Col sm={4}>
                      <div className="form-group multiselect-wrapper">
                        <label htmlFor="league_id" className="fixturevs ">Apply Template to</label>
                        <MultiSelect
                          className={
                            this.state.multiSelectClassName
                          }
                          id="league_id"
                          name="league_id"
                          placeholder="All leagues(s)"
                          overrideStrings={{
                            selectSomeItems: "Select leagues(s)"
                          }}
                          options={leagueList}
                          ...
                          onSelectedChanged={selectedLeague => this.setState({ selectedLeague })}
                        />
                      </div>
                    </Col> */}
                    <Col sm={4}>
          ...
          {contestTemplate.is_tie_breaker && <Row className="mb-1 fk-sponser-section">
            <Col xs="12" lg="12">
          ...
          <Row className="mb-1 fk-sponser-section">
            <Col xs="12" lg="12">
              <Card className="recentcom">
                <CardBody className="contestcard">
                  <Row>
...
export default LF_Createtemplate;