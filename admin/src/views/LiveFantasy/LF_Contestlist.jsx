...
class LF_Contestlist extends Component {
  constructor(props) {
    super(props);
    ...
    this.state = {
      ...
      contestParams: { 'sports_id': selected_sports_id, 'league_id': '', 'season_game_uid': '', 'collection_master_id': '', 'group_id': '', 'status': '', 'keyword': '', 'sort_field': 'season_scheduled_date', 'sort_order': 'DESC', currentPage: 1, pageSize: 100, pagesCount: 1, 'over': '' },
      leagueList: [],
      groupList: [],
      statusList: [],
      contestList: [],
    ...
    this.setState({
      ...
      if (dropName == 'league_id') {
        if (contestParams[dropName]) {
          ...
          this._getOverDropDown();
        } else {
          this.setState({ fixtureList: [], overDD: [] });
        }
      }
      this.SearchContest();
    });
  ...
  GetContestFilterData = () => {
    ...
    WSManager.Rest(NC.baseURL + NC.LF_GET_CONNTEST_FILTER, params).then((responseJson) => {
  ...
  GetAllFixtureList = () => {
    ...
    WSManager.Rest(NC.baseURL + NC.LF_GET_ALL_SEASON_SCHEDULE, param).then((responseJson) => {
      if (responseJson.response_code === NC.successCode) {
  ...
  GetContestList = () => {
    ...
    WSManager.Rest(NC.baseURL + NC.LF_GET_CONTEST_LIST, params).then((responseJson) => {
      if (responseJson.response_code === NC.successCode) {
  ...
  markPinContest = (e, contest, contest_index) => {
    ...
    if (window.confirm("Are you sure want to mark pin ?")) {
      ...
      let params = {
        ...
        'collection_master_id': contest.collection_master_id ? contest.collection_master_id : contest.collection_id ? contest.collection_id :''
      };
  ...
  _getOverDropDown = () => {
    ...
    WSManager.Rest(NC.baseURL + NC.LF_LEAGUE_OVER, params).then((responseJson) => {
      if (responseJson.response_code === NC.successCode) {
        var resd = responseJson.data.overs ? responseJson.data.overs : [];
        let t_over = []
        _.map(resd, function (item) {
          t_over.push({
            value: item,
            label: item
          });
        })
        this.setState({ overDD: t_over })
      }
    })
  }
  ...
  render() {
    ...
    return (
      <div className="animated fadeIn lf-contestlist-dashboard">
        <Col lg={12}>
        ...
        <Row>
          <Col xs="12" sm="12" md="12" className="contest-dashboard-dropdown">
            ...
            <FormGroup className="league-filter select-wrapper">
              <label className="filter-label">Select Over</label>
              <Select
                className=""
                id="over"
                name="over"
                placeholder="Select Over"
                value={this.state.contestParams.over}
                options={overDD}
                onChange={(e) => this.handleSelect(e, 'over')}
              />
            </FormGroup>
                          ...
                          <td className="contest-column">
                            ...
                            <div className="float-right">
                              ...
                              <div className="carddiv contest-listtable">
                                <span className="livcardh3dfs">
                                  Inn {item.inning} Over {item.overs}
                                </span>
                              </div>
                              <div className="carddiv contest-listtable">
                                <div>
