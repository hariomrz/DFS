...
class LF_DFS extends Component {
  ...
  redirectToSalaryReview = (selectedObj, event_type) => {
    ...
    if (selectedObj.is_published > 0 || event_type == 'live' || event_type == 'completed') {
      ...
      this.props.history.push({ pathname: '/livefantasy/overdetails/' + selectedObj.league_id + '/' + selectedObj.season_game_uid + '/' + tab, state: { isCollection: true } })


    } else {
      this.props.history.push({ pathname: '/livefantasy/oversetup/' + selectedObj.league_id + '/' + selectedObj.season_game_uid })
    }
  }
  ...
  //     pathname: '/livefantasy/createtemplatecontest/' + selectedObj.league_id + '/' + selectedObj.season_game_uid + '/2/1',
  ...
  saveDelayTime = () => {
    ...
    WSManager.Rest(NC.baseURL + NC.LF_UPDATE_FIXTURE_DELAY, params).then((responseJson) => {
      if (responseJson.response_code === NC.successCode) {
  ...
  updateMatchMsg = (flag) => {
    ...
    WSManager.Rest(NC.baseURL + NC.LF_UPDATE_FIXTURE_CUSTOM_MESSAGE, param).then((responseJson) => {
      if (responseJson.response_code === NC.successCode) {
        notify.show(responseJson.message, "success", 5000);
  ...
  getMerchandiseList = () => {
    ...
    WSManager.Rest(NC.baseURL + NC.LF_GET_ALL_MERCHANDISE, params).then(Response => {
  ...
  markPinFixture = () => {
    ...
    WSManager.Rest(NC.baseURL + NC.LF_PIN_FIXTURE, params).then((responseJson) => {
      if (responseJson.response_code === NC.successCode) {
                                    ...
                                    <Col lg={12} className="cardlivcol">
                                      {
                                        !_isEmpty(fixtureList.live_fixture)
                                          ?
                                          _Map(fixtureList.live_fixture, (item, index) => {
                                ...
                                <Col lg={12} className="cardupcomingcol">
                                  {
                                    !_isEmpty(fixtureList.upcoming_fixture)
                                      ?
                                      _Map(fixtureList.upcoming_fixture, (item, index) => {
                                        return (
