...
import { uploadGroupIcon, LF_GET_GROUP, LF_DELETE_GROUP, LF_UPDATE_GROUP, LF_CREATE_GROUP } from "../../helper/WSCalling";
...
class LF_ContestCategory extends Component {
    constructor(props) {
        super(props)
        this.state = {
            CURRENT_PAGE: 1,
    ...
    getGroup = () => {
        ...
        LF_GET_GROUP(params).then(ResponseJson => {
    ...
    handleInputChange = (event) =>{
        ...
        this.setState({ [name]: value },()=>{
            let valid = true                 
            if ((this.state.GroupName.length > 2) && (this.state.Description.length > 2) && !_.isEmpty(this.state.GroupIconName)){
                valid = false
            }
    ...
    addMoreModal = () => {
        ...
        return (
            <div>
                <Modal className="addmore-su-modal addcont-cat-mod" isOpen={this.state.addMoreModalOpen}
                    ...
                    <ModalBody>
                        ...
                        <Row>
                            <Col md={12}>
                                <label htmlFor="Redeem">Group Icon</label>
    ...
    deleteGroup = () => {
        ...
        LF_DELETE_GROUP(params).then(ResponseJson => {
            if (ResponseJson.response_code == NC.successCode) {
    ...
    createGroupFn = () => {
        ...
        LF_CREATE_GROUP(params).then(ResponseJson => {
            if (ResponseJson.response_code == NC.successCode) {
                this.addMoreToggle()
                this.getGroup()
                notify.show(ResponseJson.message, "success", 3000)
    ...
    updateGroupFn = () => {
        ...
        LF_UPDATE_GROUP(params).then(ResponseJson => {
            if (ResponseJson.response_code == NC.successCode) {
    ...
    onChangeImage = (event) => {
        ...
        uploadGroupIcon(data).then(Response => {
...
export default LF_ContestCategory