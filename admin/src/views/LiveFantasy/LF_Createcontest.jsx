...
import { CONTEST_TIE_BRE_TT, CONTEST_SPONSOR_TT, CONTEST_RECUR_TT, CONTEST_PIN, CONTEST_PRZ_DIS_TT, CONTEST_BONUS } from '../../helper/Message';
class LF_Createcontest extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      contestObject: {
        ...
        'sponsor_logo': '', 'sponsor_link': '', 'set_sponsor': '0', 'sponsor_contest_dtl_image': ''
      },
      IsValidate: false,
      IsFormValid: true,
  ...
  GetFixtureDetail = () => {
    let param = {
      ...
      "collection_id": this.state.P_COLLECTION_ID ?  this.state.P_COLLECTION_ID : ''  
    }
  ...
  GetContestTemplateMasterData = () => {
    this.setState({ posting: true })
    WSManager.Rest(NC.baseURL + NC.LF_GET_ALL_MASTER_DATA, { "sports_id": this.state.selected_sport }).then((responseJson) => {
      if (responseJson.response_code === NC.successCode) {
  ...
  CreateContest = () => {
    ...
    if (is_valid == 1) {
      ...
      contestObject['collection_id'] = this.state.P_COLLECTION_ID;
      ...
      if (WSManager.validateFormFields("contest_contest_form")) {
        ...
        WSManager.Rest(NC.baseURL + NC.LF_CREATE_CONTEST, params).then((responseJson) => {
          if (responseJson.response_code === NC.successCode) {
            notify.show(responseJson.message, "success", 5000);
            this.props.history.push({ pathname: '/livefantasy/fixturecontest/' + this.state.league_id + '/' + this.state.season_game_uid + '/2' + '/' + this.state.P_COLLECTION_ID })
          }
...
export default LF_Createcontest;