...
class Depositor_Leaderboard extends Component {
    constructor(props) {
        super(props)
        this.state = {
        ...
        getReferralRank(params).then(ResponseJson => {
            if (ResponseJson.response_code == NC.successCode) {
                this.setState({
                    ...
                    TotalDeposit: ResponseJson.data.total_deposit ? ResponseJson.data.total_deposit : 0,
                    ListPosting: false,
                })
            } else {
    ...
    exportUser = () => {
        ...
        var query_string = '&keyword=' + Keyword + '&from_date=' + tempFromDate + '&to_date=' + tempToDate + '&leaderboard=deposit&csv=true';
    ...
    render() {
        let { SelPeriod, UsersList, Total, ListPosting, CURRENT_PAGE, PERPAGE, Keyword, IsOrder, SortField, TotalDeposit, FromDate, ToDate, BackBtn } = this.state
                        ...
                        <Row>
                            <Col md={12}>
                                <div className="clearfix">
                                    <div className="ldr-title">Amount Deposited</div>
                        ...
                        <Row className="mt-30">
                            <Col md={12}>
                                <div className="ldr-tot-container">
                                    <div className="ldr-tot-heading">Total Deposit</div>
                                    <div className="ldr-tot-count">
                                        {TotalDeposit > 0 && HF.getCurrencyCode()}{HF.getNumberWithCommas(TotalDeposit)}
                                    </div>
                                </div>
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th
                                        ...
                                        onClick={() => this.sortLeaderboard('total_deposit', IsOrder)}
                                    >Amount <span className={(IsOrder && SortField === 'total_deposit') ? "arrow-up" : "arrow-down"}></span></th>
                                </tr>
                            </thead>
                            {
                                Total > 0 ?
                                        ...
                                        return (
                                            <tbody key={idx}>
                                                <tr>
                                                    ...
                                                    <td>{item.email}</td>
                                                    <td>{item.city}</td>
                                                    <td>{HF.getCurrencyCode()}{item.total_deposit}</td>
                                                </tr>
                                            </tbody>
                                        )
                                    })
...
export default Depositor_Leaderboard