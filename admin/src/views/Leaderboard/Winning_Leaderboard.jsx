...
class Winning_Leaderboard extends Component {
    constructor(props) {
        super(props)
        this.state = {
        ...
        getReferralRank(params).then(ResponseJson => {
            if (ResponseJson.response_code == NC.successCode) {
                this.setState({
                    ...
                    TotalWinning: ResponseJson.data.winning ? ResponseJson.data.winning : 0,
                    TotalFee: ResponseJson.data.fee ? ResponseJson.data.fee : 0,
                    TotalSiteRake: ResponseJson.data.site_rake ? ResponseJson.data.site_rake : 0,
                    TotalContest: ResponseJson.data.total_contest ? ResponseJson.data.total_contest : 0,
                    ListPosting: false,
                })
            } else {
    ...
    exportUser = () => {
        ...
        var query_string = '&keyword=' + Keyword + '&from_date=' + tempFromDate + '&to_date=' + tempToDate + '&leaderboard=winning&csv=true';
    ...
    render() {
        let { SelPeriod, UsersList, Total, ListPosting, CURRENT_PAGE, ITEMS_PERPAGE, Keyword, IsOrder, SortField, TotalFee, TotalSiteRake, FromDate, ToDate, BackBtn, TotalContest, PERPAGE } = this.state
                        ...
                        <Row>
                            <Col md={12}>
                                <div className="clearfix">
                                    <div className="ldr-title">Winnings</div>
                        ...
                        <Row className="mt-30">
                            <Col md={12}>
                                <div className="ldr-tot-container">
                                    <div className="ldr-tot-heading">Total Contest</div>
                                    <div className="ldr-tot-count">
                                        {
                                            (TotalContest > 0) ?
                                                HF.getNumberWithCommas(TotalContest)
                                                :
                                                '0'
                                        }
                                    </div>
                                ...
                                <div className="ldr-tot-container">
                                    ...
                                    <div className="ldr-tot-count">
                                        {HF.getCurrencyCode()}{HF.getNumberWithCommas(TotalFee)}
                                    </div>
                                </div>
                                <div className="ldr-tot-container">
                                    <div className="ldr-tot-heading">Site Rake</div>
                                    <div className="ldr-tot-count">
                                        {HF.getCurrencyCode()}{HF.getNumberWithCommas(TotalSiteRake)}
                                    </div>
                                </div>
                                <div className="ldr-tot-container float-right mt-3">
                                    <i
                                        className="export-list icon-export"
                                        onClick={e => this.exportUser()}
                                    ></i>
                                </div>
                            </Col>
                        </Row>
...
export default Winning_Leaderboard