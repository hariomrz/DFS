...
import Depositor_Leaderboard from './Depositor_Leaderboard';
...
class ReferralLeaderboard extends Component {
        ...
        getReferralRank(params).then(ResponseJson => {
            ...
            if (ResponseJson.response_code == NC.successCode) {
                this.setState({
                    ...
                    TotalReferral: ResponseJson.data.total_referral ? ResponseJson.data.total_referral : 0,
                    ListPosting: false,
                })
            } else {
    ...
    exportUser = () => {
        ...
        var query_string = '&keyword=' + Keyword + '&from_date=' + tempFromDate + '&to_date=' + tempToDate + '&leaderboard=referral&csv=true';
    ...
    render() {
        let { SelPeriod, FromDate, ToDate, UsersList, Total, ListPosting, CURRENT_PAGE, PERPAGE, Keyword, IsOrder, SortField, TotalReferral, BackBtn } = this.state
        let { dbCallback } = this.props
        var todaysDate = moment().format('D MMM YYYY');


        console.log('newdateeeeeee', new Date(FromDate))
        console.log('FromDate.........', new Date(ToDate))
                            ...
                            <Col md={12}>
                                <div className="ldr-tot-container">
                                    ...
                                    <div className="ldr-tot-count">
                                        {HF.getNumberWithCommas(TotalReferral)}
                                    </div>
                                </div>
