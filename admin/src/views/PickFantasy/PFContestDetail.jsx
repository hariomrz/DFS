...
export default class PFContestDetail extends Component {
    constructor(props) {
        super(props)
        this.state = {
            Total: 0,
    ...
    componentDidMount() {
        ...
        let url = window.location.href;
         if (url.includes('#')) {
           url = url.split('#')[1];
           const contest_unique_id = url.split('/')[3];
           this.setState({
                contest_unique_id: contest_unique_id
            }, ()=>this.getGameDetail(contest_unique_id)
           )
        }
    ...
    getGameDetail = (contest_unique_id) => {
        let params = {
            "contest_unique_id": contest_unique_id
        }
        WSManager.Rest(NC.baseURL + NC.PF_GET_GAME_DETAIL, params).then(ResponseJson => {
            if (ResponseJson.response_code == NC.successCode) {
                this.setState({
    ...
    getGameLinupDetail = (gameDetail) => {
        ...
        WSManager.Rest(NC.baseURL + NC.PF_GET_GAME_LINEUP_DETAIL, params).then(ResponseJson => {
            if (ResponseJson.response_code == NC.successCode) {
                this.setState({
                    GameLinupDetail: ResponseJson.data,
    ...
    getLinupDetails = (lineup, lineup_master_contest_ids) => {
        let params = {
            "user_contest_id": lineup_master_contest_ids,
        }
        WSManager.Rest(NC.baseURL + NC.PF_GET_LINEUP_DETAIL, params).then((ResponseJson) => {
            if (ResponseJson.response_code === NC.successCode) {
                this.setState({
                    LinupUpData: ResponseJson.data,
                    activeUserInfo: lineup
                },()=>{
                })
            }
        }).catch(error => {
    ...
    render() {
        const { gameDetail, LeagueDetail, MatchDetail, MatchList, SportDetail, UserData, prize_distibution_detail, GameLinupDetail, LinupUpData, CURRENT_PAGE, PERPAGE, Total, UserType, TotalMerchadiseDistri, AllowSystemUser, contest_template_id, selected_sport, BenchPly, BenchLoad, activeUserInfo } = this.state
                                    ...
                                    <div className="user-value">
                                        {
                                            ...
                                            <>
                                                {
                                                    HF.getTimeDiff(gameDetail.scheduled_date) ?  
                                                    <>
                                                        {gameDetail.status == '1' ? 'Cancel' : 'Coming'}
                                                    </> : 
                                                    <>
                            ...
                            {/* {
                                ...
                                <Col md={3} className="mt-3">
                                    ...
                                    <div className="user-value">
                                        ...
                                        {gameDetail.contest_mode_id == '2' && 'FAB 5'}
                                        {gameDetail.contest_mode_id == '3' && 'SUPER 7'}
                                    </div>
                                </Col>
                            } */}
                        </Row>
                                        ...
                                        </thead>
                                        {
                                            _.map(MatchList, (lineup, idx) => {
                                                return (
                                                    <tbody>
                                                        <tr>
                                                        <td className="text-center">{lineup.match}</td>
                                                ...
                                                return (
                                                    <tbody key={idx}>
                                                        ...
                                                        {lineup.is_systemuser == "1" &&
                                                            ...
                                                            <td onClick={() => this.lineupDetailModal(lineup, lineup.user_contest_id)}>
                                    ...
                                    <ModalBody className="p-0">
                                                ...
                                                <Col xs={12}>
                                                    <ul className="lineup-feelist">
                                                        <li className="lineup-feeitem">
                                                            ...
                                                            <div className="font-weight-bold">{activeUserInfo.team_name}</div>
                                                        </li>
                                                        <li className="lineup-feeitem">
                                                            <label>Username</label>
                                                            <div className="font-weight-bold">{activeUserInfo.user_name}</div>
                                                        </li>
                                                        <li className="lineup-feeitem">
                                                            <label>Total Score </label>
                                                            <div className="font-weight-bold">{activeUserInfo.total_score}</div>
                                                        </li>
                                                    </ul>
                                                </Col>
                                            </Row>
                                        ...
                                        <Col xs={12}>
                                            <Row className="rank-box">
                                                <Col xs={3}>
                                                    <h3 className="h3-cls">Rank {' '} {activeUserInfo.game_rank}</h3>
                                                </Col>
                                                <Col xs={9}>
                                            ...
                                            <Col md={12}>
                                                <div className="table-responsive common-table">
                                                    <Table>
                                                        <thead>
                                                            <tr>
                                                                <th className="pl-4">Questions</th>
                                                                <th>User Predict</th>
                                                                <th>Applied Booster</th>
                                                                ...
                                                                return (
                                                                    <tbody key={idx}>
                                                                        <tr>
                                                                            ...
                                                                            <td>{lineup.user_predict == "1" ? lineup.option_1 : lineup.user_predict == "2" ? lineup.option_2 : lineup.user_predict == "3" ?  lineup.option_3 :  lineup.user_predict == "4" ? lineup.option_4 : "--" }</td>
                                                                            <td>
                                                                                {lineup.is_captain != 1 && lineup.is_vc != 1 ? '--' :
                                                                                    <>
                                                                                        {lineup.is_captain == 1 && <span>Doubler</span> }
                                                                                        {lineup.is_vc == 1 && <span>{lineup.is_captain == 1 && ','} No Negative</span>}
                                                                                    </>
                                                                                }
                                                                            </td>
                                                                            <td className="pl-4">{lineup.correct_answer == "1" ? lineup.option_1 : lineup.correct_answer == "2" ? lineup.option_2 : lineup.correct_answer == "3" ?  lineup.option_3 :  lineup.correct_answer == "4" ? lineup.option_4 : "--" }</td>
                                                                            {/* <td>{lineup.score}</td> */}
                                                                            <td>{lineup.score}</td>
                                                                        </tr>
                                                                    </tbody>
