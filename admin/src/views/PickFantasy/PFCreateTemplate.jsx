...
class PFCreateTemplate extends Component {
  constructor(props) {
    super(props);
    ...
    if (is_valid == 1) {
      ...
      if (WSManager.validateFormFields("contest_template_form")) {
        if ((contestTemplate.set_sponsor == 1 && this.state.wrongLink) || contestTemplate.set_sponsor == 0) {
          ...
          WSManager.Rest(NC.baseURL + NC.PF_CREATE_CONTEST_TEMPLATE, params).then((responseJson) => {
            if (responseJson.response_code === NC.successCode) {
              notify.show(responseJson.message, "success", 5000);
              this.props.history.push({ pathname: '/picksfantasy/contest-template' })
            }
            this.setState({ posting: false })
          })
        } else {
                  ...
                  <Row>
                    <Col xs={12}>
                      <Table responsive className="prize-distribution-table">
                        ...
                        <tbody>
                          {payout_data && payout_data.map((item, idx) => (
                            <tr id="addr0" key={idx}>
...
export default PFCreateTemplate;
