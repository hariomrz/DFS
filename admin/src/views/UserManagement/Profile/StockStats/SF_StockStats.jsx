...
export default class SF_StockStats extends Component {
    ...
    lineupDetailModal(game, index) {
        this.setState({
            game_stock_type: game.stock_type,
        }, () => {
            if (game.user_id && game.contest_id)
                this.getGameLinupDetails(game.contest_id, game.user_id,game)
        })
        ...
        this.setState({
            ...
            game_contest_id: game.contest_id
            // game_stock_type: game.stock_type,
        })
    ...
    getGameLinupDetails = (contest_id, user_id,data) => {
        let params = {
            game_id: contest_id,
            user_id: user_id
        }
    ...
    getLinupDetails = (data) => {
        let { lineup_master_contest_id, game_stock_type } = this.state
        
        if(game_stock_type == '4'){

            let params = {
                contest_id: data.contest_id,
                lineup_master_id: data.lineup_master_id
            }
            let url = NC.LSF_GET_LINEUP_DETAIL
    
            if (game_stock_type == '2') {
                url = NC.ESF_GET_LINEUP_DETAIL
            }
            ...
            let url = NC.SF_GET_LINEUP_DETAIL
    ...
    getGameHistory() {
        const { PERPAGE, CURRENT_PAGE, FromDate, ToDate, SelectedGroup, SelectedCategory, Keyword, SelectedGameType } = this.state
        let params = {
            "keyword": Keyword,
    ...
    handleCollectionChange = (value, name) => {
        if (value != null)
            this.setState({ [name]: value ,showType: (value.value == 4 || value.value == 3) ? false : true}, this.getGameHistory)
    }
    ...
    exportStockStat = () => {
        let { FromDate, ToDate, SelectedGroup, SelectedCategory, Keyword, SelectedGameType } = this.state
        ...
        var query_string = 'keyword=' + Keyword + '&from_date=' + tempFromDate + '&to_date=' + tempToDate + '&sort_order=DESC&sort_field=scheduled_date' + '&group_id=' + group + '&category_id=' + '&role=2' + SelectedCategory + '&user_id=' + uid + '&stock_type=' + SelectedGameType.value;
    ...
    render() {
        const { lineup_master_contest_id, CURRENT_PAGE, PERPAGE, Total, LinupUpData, LinupUpRankWinning, GameHistoryData, FreeGraphOption, TotalContestGraph, SportPreferencesGraph, game_contest_name, game_entry_fee, game_prize_pool, game_fixture_title, game_added_date, indexVal, TotalUser, Keyword, CategoryOptions, SelectedCategory, FromDate, ToDate, groupList, SelectedGroup, game_prize_type, game_status, SelectedGameType, GameTypeOption, game_stock_type,showType } = this.state;
            ...
            <Fragment>
                <div className="stockstats">
                    <Row className="mt-4">
                        ...
                        {
                            (HF.allowStockFantasy() == '1' || HF.allowEquityFantasy() == '1') && showType &&
                            <Col md={2}>
                                <div className="search-box float-left w-100">
                                        ...
                                        return (
                                            <tbody key={idx}>
                                                <tr>
                                                    ...
                                                    <td>{game.status == '0' ? 'Live' : game.status == '3' ? 'Completed' : '--'}</td>
                                                    ...
                                                    {HF.allowStockFantasy() == '1' && HF.allowEquityFantasy() == '1' && <td>{this.getGameType(game.stock_type)}</td>}
                                                    {
                                            ...
                                            <Table>
                                                <thead>
                                                    {
                                                        ...
                                                        <tr>
                                                            ...
                                                            {/* {game_stock_type == '1' && <th>Lot size</th>} */}
                                                            {game_stock_type == '2' && <th>No. of Shares</th>}
                                                            <th>Buy/Sell</th>
                                                            {game_stock_type == '1' && <th>Joining rate</th>}
                                                            {game_stock_type == '2' && <th>Opening Price</th>}
                                                            <th>Invested Amount</th> 
                                                            <th>
                                                                {game_status == '0' && 'Result Rate'}
                                                                {(game_status == '2' || game_status == '3') && 'Closing Rate'}
                                                            </th>
                                                            {game_stock_type == '1' && <th>Points</th>}
                                                            {
                                                                game_stock_type == '2' &&
                                                                <Fragment>
                                                                    

                                                                    {game_status == '0' && <th>Current Amount</th>}
                                                                    {game_status == '1' && <th>Invested Amount</th>}
                                                                    {game_status == '3' && <th>Final Amount</th>}

                                                                    {game_status != '1' && <th>Gain/Loss</th>}                                                              
                                                                </Fragment>
                                                            }
                                                        </tr>
                                                ...
                                                </thead>
                                                {
                                                    _.map(LinupUpData, (lineup, idx) => {
                                                        ...
                                                        var gain_loss = '--'
                                                        if (game_status != '1') {
                                                            usr_amount = HF.convertTodecimal(parseFloat(lineup.user_lot_size) * parseFloat(lineup.result_rate), 2)

                                                            invest_amt = HF.convertTodecimal(parseFloat(lineup.user_lot_size) * parseFloat(lineup.joining_rate), 2)

                                                            gain_loss = HF.convertTodecimal(parseFloat(usr_amount) - parseFloat(invest_amt), 2)
                                                        }
                                                        if (game_status == '1') {
                                                            usr_amount = HF.convertTodecimal(parseFloat(lineup.user_lot_size) * parseFloat(lineup.joining_rate), 2)
                                                            ...
                                                            <tbody key={idx}>
                                                                {
                                                                    ...
                                                                    <tr>
                                                                        ...
                                                                        <td>{lineup.display_name ? lineup.display_name : '--'}</td>
                                                                        {game_stock_type == '2' && <td className="pl-4">{lineup.user_lot_size}</td>}
                                                                        
                                                                        <td>
                                                                            {lineup.type == '1' ? 'Buy' : lineup.type == '2' ? 'Sell' : '--'}
                                                                        </td>
                                                                        <td>{lineup.joining_rate}</td>
                                                                        {game_status != '1' && <td>{invest_amt}</td>}
                                                                        <td>
                                                                            {game_status != '1' && HF.convertTodecimal(lineup.result_rate, 2)}
                                                                        </td>
                                                                        {game_stock_type == '1' && <td>{lineup.score}</td>}
                                                                        {
                                                                            game_stock_type == '2' &&
                                                                            <Fragment>
                                                                                <td>{usr_amount}</td> 
                                                                                
                                                                                {game_status != '1' && <td>{gain_loss}</td>}                                                                           
                                                                            </Fragment>
                                                                        }
