...
!(function (app, angular) {
    app.controller('CrmTopFollowedListCtrl', CrmTopFollowedListCtrl);

    function CrmTopFollowedListCtrl($http, $q, $scope, $rootScope, $window, apiService, CommonService, UtilSrvc) {
       ...
       $scope.userObj={};
        ...
        var initialFilter = {
            OrderByField : 'TotalFollowers',
            OrderBy: "DESC"
        };
        $scope.filter = angular.copy(initialFilter);
        function getUserList(reqData) {
            
            $http.post(base_url + 'admin_api/admin_crm/get_top_followed', reqData).success(function (response) {
    ...
    CrmTopFollowedListCtrl.$inject = ['$http', '$q', '$scope', '$rootScope', '$window', 'apiService', 'CommonService',  'UtilSrvc'];
