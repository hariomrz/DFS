...
(function ($, _, undefined) {
    ...
    var defaultSettings = {
        ...
        templates       : {
            ...
            tagHighlight      : _.template('<strong><span class="user-$<%= moduleid %>-$<%= userid %>">$<%= idx %></span></strong>'),
    ...
    var TextNTags = function (editor) {
        ...
        beautified_text = beautified_text.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/'/g,'&#39;');
            _.each(settings.triggers, function (trigger) {
                var markup = templates.tagHighlight({idx: trigger.parserGroups.title, userid: trigger.parserGroups.id,moduleid:trigger.parserGroups.type, class_name: trigger.classes.tagHighlight});
        ...
        function onEditorKeyDown (e) {
            ...
            switch (e.keyCode) {
                ...
                case keys['DELETE']:
                    ...
                    //TO HANDEL DELETE KEY CASE
                    if(e.keyCode == keys['DELETE'] && sStart == sEnd && sStart >= 0){
                        sEnd += 1;
                    }
                    
                    if(sEnd > sStart) {
                        removedTag = removeTagsInRange(sStart, sEnd);
                        shiftTagsPosition(sStart, sStart - sEnd);
                        if(removedTag){
                            //addTag(removedTag[0],1);
                        }
                    }
                    return true;
        ...
        function addTag (tag,automatic) {
            if(currentTriggerChar == '' && automatic==1){
                currentTriggerChar = '@';
            }
            ...
            var objPropTransformer = transformObjectProperties(trigger.keys_map);
            var trigger = settings.triggers[currentTriggerChar],
                ...
                sStart = elEditor[0].selectionStart,
                tagStart = parseInt(sStart - currentTriggerChar.length - currentDataQuery.length)+increase,
                newCaretPosition = tagStart + localTag.title.length,
                left_text = plain_text.substr(0, tagStart),
                right_text = plain_text.substr(sStart);
                
                var lastChar = left_text.charAt(left_text.length-1);
                if(lastChar == '@')
                {
                    tagStart = parseInt(sStart - currentTriggerChar.length - currentDataQuery.length);
                    left_text = plain_text.substr(0, tagStart);
                }
            ...
            {
                if($(elEditor).attr('id') == 'PostContent')
                {
                    input = {GroupDescription:'',ModuleEntityGUID:tag.UserGUID,ModuleID:tag.type,Type:'User',name:tag.name,ProfilePicture:tag.ProfilePicture};
                    angular.element('#WallPostCtrl').scope().wallTagAdded(input);
                    angular.element('#WallPostCtrl').scope().$apply();
                }
            }
