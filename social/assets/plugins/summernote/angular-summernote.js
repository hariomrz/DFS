...
angular.module('summernote', [])
  ...
  .controller('SummernoteController', ['$scope', '$attrs', '$timeout', function($scope, $attrs, $timeout) {
    ...
    if (angular.isDefined($attrs.onImageUpload)) {
      ...
      summernoteConfig.callbacks.onImageUpload = function(files, editor$,welEditable) {
        sendFile(files, editor$,welEditable,$attrs);
    ...
    this.activate = function(scope, element, ngModel) {
      ...
      scope.SetLinkData = function(){
        var getLink=element.summernote('getEditorLink');
        if(getLink!=''){
          angular.element(document.getElementById('WallPostCtrl')).scope().parseLinkData(getLink);
          element.summernote('SetEditorLink','');
        }
      }
      var originalOnChange = summernoteConfig.callbacks.onChange;
      summernoteConfig.callbacks.onChange = function (contents) { 
          //var post_content = $(".note-editable").text().trim();
          var c = contents.replace('<br>','');
          c = c.replace('<p>','');
          c = c.replace('</p>','');
          c = c.replace('<span>','');
          c = c.replace('</span>','');
          ...
          else if($('#RatingCtrl').length>0)
          {
            var wall_scope = angular.element(document.getElementById('RatingCtrl')).scope();
          }
          if(c.length>0 || wall_scope.fileCount>0 || wall_scope.mediaCount)
          {
            wall_scope.noContentToPost = false;
          }
          else
          {
            wall_scope.noContentToPost = true;
          }
          scope.SetLinkData();
          $timeout(function () {
...
var imageAllowType = ['image/png', 'image/jpeg', 'image/JPEG', 'image/PNG', 'image/jpg', 'image/JPG'];
function sendFile  (files,editor, welEditable,$attrs) {
    var filecount = files.length;
    var processfilecount = 0;
   
    if(filecount > 3){
        alert('Please select max. 3 images at a time.', 'alert-danger');
        return false;
    }
    ...
    $.each(files, function (key,value) {
        ...
        if (file.type == '')
        {
            alert('Invalid file', 'alert-danger');
            return false;
        }
        else {
            if ($.inArray(file.type, imageAllowType) == -1) {
                alert('Invalid file', 'alert-danger');
                return false;
            }
        }

        //$('#ArticleEditor').addClass('loader');
        ...
        reader.onload = function (evt) {
            ...
            angular.element(document.getElementById('WallPostCtrl')).scope().summernoteUpload(evt,editor, welEditable,$attrs);
