...
class Fire_log{
        ...
        public function __construct()
	{
		...
		define( 'PARAM_DILEM', $this->CI->config->item( 'fire_log_param_dilem' ));
                ...
                $this->file_path = getcwd(). '/application/';
		...
		{
			$list = get_dir_file_info( $this->file_path . 'logs' );
			$filtered_list = array();
			...
			usort($filtered_list, function($a, $b) {return strcmp($b["name"], $a["name"]);});
		...
		public function get_file( $log_file )
		{
			...
			$path = $this->file_path . 'logs/' .$log_file;
		...
		{
			$file = $this->file_path . 'logs/' . $log_file;
			if( file_exists( $file ))
			{
				@unlink( $file );
			}
