...
class Skills_model extends Common_Model
{
    ...
    function save($skills, $module_id, $module_entity_id)
    {
        ...
        if (count($skills) > 0 && !empty($skills) && is_array($skills))
        {
            ...
            {
                $skill_id = isset($skill['ID']) ? $skill['ID'] : 0;
                if (!$skill_id)
                {
                    $skill_id = $this->get_skill_id($skill['Name']);
                }
                $status_id = (isset($skill['StatusID']) && !empty($skill['StatusID'])) ? $skill['StatusID'] : 2;
                $this->db->select("EntitySkillGUID");
                ...
                if ($query->num_rows() == 0 && $status_id == 2)
                {
                    ...
                    $insert_data[$key]['CreatedDate'] = $current_date;
                    $insert_data[$key]['ModifiedDate'] = $current_date;
            ...
            {
                $data = $this->db->insert_batch(ENTITYSKILLS, $insert_data);
            }
            if (count($entity_skills) > 0)
            {
                $this->db->where(array('ModuleID' => $module_id, 'ModuleEntityID' => $module_entity_id));
                $this->db->where_in('SkillID', $entity_skills);
                $this->db->delete(ENTITYSKILLS);
            }
     ...
     */
    function get_entity_skill_id($module_id, $module_entity_id, $skill_id)
    {
        ...
        if ($query->num_rows())
        {
            return array('EntitySkillID' => $query->row()->EntitySkillID, 'NotificationTypeID' => $notification_type_id_existing[$module_id], 'Type' => 'existing');
        }
        else
        {
            $this->db->insert(ENTITYSKILLS, array('EntitySkillGUID' => get_guid(), 'ModuleID' => $module_id, 'ModuleEntityID' => $module_entity_id, 'SkillID' => $skill_id, 'NoOfEndorsements' => 0, 'StatusID' => 1, 'ModifiedDate' => get_current_date('%Y-%m-%d %H:%i:%s'), 'CreatedDate' => get_current_date('%Y-%m-%d %H:%i:%s')));
    ...
    {
        $this->load->model(array('pages/page_model', 'group/group_model'));
        if ($module_id == 1)
        {
     ...
     * @param  [array] $skills
     */
    function save_endorsement($user_id, $module_id, $module_entity_id, $visitor_module_id, $visitor_module_entity_id, $skills)
    {
        $skills_list = array();
        if ($skills)
        {
            ...
            $new_temp_skill = array();
            $existing_temp_skill = array();
            foreach ($skills as $skill)
            {
                $skill_id = isset($skill['ID']) ? $skill['ID'] : '';
                if (empty($skill_id))
                {
                    $skill_id = $this->get_skill_id($skill['Name']);
                }

                $entity_skills = $this->get_entity_skill_id($module_id, $module_entity_id, $skill_id);
                $entity_skill_id = $entity_skills['EntitySkillID'];
                ...
                if ($entity_skills['Type'] == 'new')
                {
                    $new_temp_skill[] = $skill_id;
                    $new_temp_data[] = array('skill_id' => $skill_id, 'notification_type_id' => $notification_type_id);
                }
                else
                {
            ...
            {
                $this->endorsement_notification($new_temp_data, $new_temp_skill, $user_id, $module_id, $module_entity_id, $visitor_module_id, $visitor_module_entity_id);
            }
            if (!empty($existing_temp_data))
            {
    ...
    {
        $this->db->select('ES.EntitySkillGUID, ES.EntitySkillID, ES.SkillID, ES.NoOfEndorsements, S.Name');
        ...
        $this->db->join(SKILLSMASTER . " S", "S.SkillID = ES.SkillID");
        ...
        /* if ($filter == 1)
          ...
          $this->db->where('ES.NoOfEndorsements >= ', 1);
          } */
        if ($filter == 2)
        {
...
// $this->db->where('ES.NoOfEndorsements >= ',1);
        ...
        $this->db->order_by('ES.NoOfEndorsements', 'DESC');
        $query = $this->db->get();
...
// $this->db->select('IF(MD.ImageName="","soccer.svg",CONCAT("org_", MD.ImageName)) as ImageName', FALSE);
        ...
        $this->db->join(MEDIA . ' MD', 'MD.MediaID = S.MediaID', 'LEFT');
        $this->db->where('S.StatusID', '2');
        $this->db->where("S.Name LIKE '%" . $search_keyword . "%'", NULL, FALSE);
        
        $sub = $this->subquery->start_subquery('where_in');
        $sub->select('SkillID')->from(ENTITYSKILLS);
        $sub->where('ModuleID', $module_id);
        $sub->where('ModuleEntityID', $module_entity_id);
        $this->subquery->end_subquery('S.SkillID', FALSE);
...
// echo $this->db->last_query();die;
    ...
    public function update_skill_profile_count($skill_id, $count = 1)
    {
        ...
        $table_name = SKILLSMASTER;
        $condition = array("SkillID" => $skill_id);
    ...
    {
        $set_field = "NoOfEndorsements";
        $table_name = ENTITYSKILLS;
        $condition = array("EntitySkillID" => $entity_skill_id);
...
//echo $this->db->last_query();die;
     ...
     * @return [array/int]              [endorsement list/count]
     */
    function endorsement_list($entity_skill_id, $count_flag = FALSE, $page_no = '', $page_size = '', $visitor_module_id = '', $visitor_module_entity_id = '', $keyword = '', $Ignore_module_id = '', $Ignore_module_entity_id = '')
    {
        ...
        $this->db->select('ES.ModuleID, ES.ModuleEntityID, ES.CreatedDate');
        ...
        $this->db->where('ES.EntitySkillID', $entity_skill_id);

        if ($Ignore_module_id != '' && $Ignore_module_entity_id != '')
        {
            $this->db->where('ES.EntitySkillID !=', $Ignore_module_id);
            $this->db->where('ES.ModuleEntityID !=', $Ignore_module_entity_id);
        }
        ...
        if (empty($count_flag)) // check if array needed
        {
            ...
            if ($query->num_rows())
            {
                foreach ($query->result_array() as $result)
                {
                    ...
                    {
                        $can_endorse = FALSE;
//$data['ProfileTypeName'] = $this->user_model->get_profile_type_name($result['UserTypeID'], '');
                        $data['Location'] = $this->user_model->get_user_location($module_entity_id);
    ...
    function is_endorse($entity_skill_id, $visitor_module_id, $visitor_module_entity_id)
    {
        ...
        $this->db->from(ENTITYENDORSEMENT);
        $this->db->where(array('EntitySkillID' => $entity_skill_id, 'ModuleID' => $visitor_module_id, 'ModuleEntityID' => $visitor_module_entity_id));
    ...
    /**
     * [delete_skills Used to delete skill for an entity]
     * @param  [array] $skills         [Array of skills to be deleted]
     * @param  [int] $module_id        [Module ID]
     * @param  [int] $module_entity_id [Module Entity ID]
     * @return [boolean]               [true]
     */
    function delete_skills($skills, $module_id, $module_entity_id)
    {
        if (count($skills) > 0 && !empty($skills) && is_array($skills))
        {
            $entity_skills = array();
     ...
     * @param  [int] $skill_id              [description]
     * @param  [int] $module_id             [description]
     * @param  [int] $module_entity_id      [description]
     * @param  [int] $visitor_module_id        [description]
     * @param  [int] $visitor_module_entity_id [description]     
     * @return [type]                        [description]
     */
    function delete_endorsement($skill_id, $module_id, $module_entity_id, $visitor_module_id, $visitor_module_entity_id)
    {
        $skills_list = array();
        if ($skill_id)
        {
            ...
            {
                $row = $query->row();
                $entity_skill_id = $row->EntitySkillID;

                $this->db->where(array('ModuleID' => $visitor_module_id, 'ModuleEntityID' => $visitor_module_entity_id));
                $this->db->where('EntitySkillID', $entity_skill_id);
                $this->db->delete(ENTITYENDORSEMENT);
    ...
    {
        $this->db->select("SUM(ES.NoOfEndorsements) as NoOfEndorsements");
        ...
        if ($result)
        {
            $NoOfEndorsements = $result['NoOfEndorsements'];
        }
    ...
    function get_user_endorse_skill($module_id, $module_entity_id, $to_module_id, $to_module_entity_id, $page_no, $page_size)
    {
        ...
        $this->db->select("SM.Name,SM.SkillID,ES.EntitySkillID,ES.NoOfEndorsements");
        ...
        if ($query->num_rows() > 0)
        {
            foreach ($query->result_array() as $result)
            {
                ...
                $result['TotalEndorsement'] = $result['NoOfEndorsements'];
                $result['Endorsements'] = $this->endorsement_list($entity_skill_id, FALSE, 1, 5, $to_module_id, $to_module_entity_id, '', $module_id, $module_entity_id);
//print_r($result);die;
                unset($result['NoOfEndorsements']);
                $response[] = $result;
            }
