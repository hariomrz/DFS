...
class Roles_model extends Admin_Common_Model
{
    ...
    public function getApplicationPermissionsList($RoleId){  
        ...
        foreach($app_results as $application){            
            ...
            $sub->select('RR.RoleRightID AS RoleRightID',FALSE);
            $sub->from(ROLERIGHTS." AS RR ");
            $sub->where('RR.RoleID',$RoleId);
            $sub->where('RR.ApplicationID',$application['ApplicationID']);
            $sub->where('RR.RightID = AR.RightID');
            $this->subquery->end_subquery('RoleRightID');
            ...
            foreach($rights_results as $rolerights){
                ...
                $sub->where('RR.RoleID',$RoleId);
                $sub->where('RR.ApplicationID',$application['ApplicationID']);
                $sub->where('RR.RightID = RS.RightID');
                $this->subquery->end_subquery('RoleRightID');                
    ...
    function deleteRolePermissions($RoleId){ 
        if (CACHE_ENABLE) {
            ...
            initiate_worker_job('delete_user_rights_cache', array(''));
        }       
        $this->db->where('RoleID', $RoleId);
    ...
    public function getRightsPermissionsList($ApplicationID,$RightID){  
        ...
        $sub->select('RR.RoleRightID AS RoleRightID',FALSE);
        $sub->from(ROLERIGHTS." AS RR ");
        $sub->where('RR.RightID',$RightID);
        $sub->where('RR.ApplicationID',$ApplicationID);
        $sub->where('RR.RoleID = R.RoleID');
        $this->subquery->end_subquery('RoleRightID');
        $this->db->from(ROLES."  R ");
        $this->db->where('R.RoleStatusID',2);
        $query = $this->db->get();
    ...
    public function getRoleUsersList($role_id,$start_offset=0, $end_offset=""){  
        ...
        $sub = $this->subquery->start_subquery('select');
        $sub->select('GROUP_CONCAT(R1.Name SEPARATOR ",") as UserRoles',FALSE);
        $sub->from(ROLES." AS R1 ");
        $sub->join(USERROLES." AS UR1", ' UR1.RoleID = R1.RoleID','inner');
        $sub->where('UR1.UserID = U.UserID');
        $this->subquery->end_subquery('UserRoles');
