...
class Dashboard_model extends Admin_Common_Model {
     ...
     */
    public function update_entity($module_id, $module_entity_id, $entity_column_val = 1, $entity_column = 'Verified', $user_id = 0) {
        $update_table = ''; 
        $where_column = '';
        switch ($module_id) {
            case 1:
        ...
        if($module_id == 19 && $entity_column == 'StatusID') {
            ...
            $this->activity_model->removeActivity($module_entity_id, $user_id, $entity_column_val);
            return;
        }
        ...
        if($module_id == 19 && $entity_column == 'Verified') {
            $this->db->select('A.UserID, A.LocalityID, A.PostContent');
            ...
            if (!empty($activity_data)) {
                $notification_data = array('ActivityID' => $module_entity_id, 'UserID' => $activity_data['UserID'], 'SenderName' => $activity_data['SenderName'], 'NotificationTypeKey' => 'post_message', 'Mentions' => array(), 'LocalityID' => $activity_data['LocalityID'], 'PostContent' => $activity_data['PostContent']);
    ...
    */
    function hide_activity($activity_guid) {
        try {       
            $activity = $this->get_single_row('ActivityID, IsShowOnNewsFeed', ACTIVITY, array('ActivityGUID' => $activity_guid));
            if(!empty($activity)) {
