...
class Login_model extends Common_Model
{
    ...
    function check_user_auto_login($user_name, $user_id='') {
        ...
        $this->db->select(USERS . '.UserID,' . USERS . '.Language,' . USERS . '.UserGUID,' . USERLOGINS . '.LoginKeyword,' . USERS . '.FirstName,' . USERS . '.LastName,' . USERS . '.Email,' . USERS . '.ProfilePicture,' . USERS . '.StatusID,' . USERLOGINS . '.IsPasswordChange,'. USERS . '.IsProfileSetup');
        $this->db->from(USERS);
    ...
    function check_user_exist($source_id, $user_name, $password) {
        ...
        if ($source_id == 1) {
            $this->db->select(USERS . '.UserID,' . USERS . '.Language,' . USERS . '.UserGUID,' . USERLOGINS . '.LoginKeyword,' . USERS . '.FirstName,' . USERS . '.LastName,' . USERS . '.Email,' . USERS . '.ProfilePicture,' . USERS . '.StatusID,' . USERLOGINS . '.IsPasswordChange,'. USERS . '.IsProfileSetup,'. USERLOGINS . '.Password');
            $this->db->from(USERS);
            $this->db->join(USERLOGINS, USERLOGINS . '.UserID=' . USERS . '.UserID', 'left');
    ...
    function update_login_analytics($data)
    {
        ...
        $analytics['AgeGroupID']            = get_age_group_id($data['UserID']);
    ...
    function copy_login_analytics($login_session_key) {
        ...
        if (!empty($login_user_data) && isset($login_user_data['UserID'])) {
            ...
            $analytics['AgeGroupID']            = get_age_group_id($analytics['UserID']);
    ...
    public function confirm_email($user_guid) {
        ...
        $query = $this->db->select('Email, StatusID')->get_where(USERS, array('ActivationCode' => $user_guid));
        $return['msg'] = 2;
        if ($query->num_rows()) {
            $row = $query->row();
            $status = $row->StatusID;
    ...
    public function resend_otp($phone_no) {
        ...
        $record = $this->get_single_row("PhoneVerified", USERS, $condition);
