...
class Upload_file_model extends Common_Model {
    ...
    protected $allowed_video_max_size       = '40960'; //MB
    ...
    function uploadImage($Data,$generateThumb=1){     
        ...
        if ( ! $this->upload->do_upload($file_input)){
            ...
            if (strtolower(IMAGE_SERVER) == 'remote') {
                ...
                $s3             = new S3(AWS_ACCESS_KEY, AWS_SECRET_KEY);
                $is_s3_upload   = $s3->putObjectFile($s3_path, BUCKET, $s3_path, S3::ACL_PUBLIC_READ,$this->headers,$mime_type);
                @unlink($s3_path);
            }
            $image_type         = $ext;
...
/**
    ...
    function uploadImageInBg($data) {
        ...
        if(!empty($data)) {
            ...
            if(!empty($MediaGUID) && $totalSize)
            {
                $update_media =  array('Size' => $totalSize,'MediaSizeID'=>$this->getMediaSizeID($totalSize),'StatusID'=>2);
    ...
    public function generate_thumb($temp_file, $dir_name, $file_name, $thumb = array(), $zc = array(), $using_phpthumb = 0,$pathToFolder='',$type='',$is_original=0,$document_root='') {
        ...
        if($using_phpthumb==1)
        {
            ...
            foreach ($thumb as $i => $row) 
            {
               ...
               /* if(($h==500 && $height < $h) || ($w==750 && $width < $w))
                {
                    $file_full_path = $dir_name.'/'.$file_name;
                    $destination_path = $dir_name . '/' . $w . 'x' . $h . '/' . $file_name;
                    ...
                    {
                        $file_location = $document_root.ROOT_FOLDER. '/' .$dir_name.'/'.$file_name;
                        if(file_exists($file_location))
                        {
                            copy($this->image_server_path.$file_full_path, $destination_path);
                            $totalSize = $totalSize + ($size);
                        }
                    }
                }
    ...
    */
    function saveMedia($Media,$user_id=''){
        ...
        if($user_id){
            $this->subscribe_model->toggle_subscribe($user_id,'Media',$MediaID); 
        }
        return $MediaID;    
    }
    ...
    function updateProfilePicture($MediaGUID,$ProfilePicture,$user_id,$module_id,$ModuleEntityGUID,$from_admin=false){
        ...
        /* if($module_id == 3) {
            ...
            $email_id = $this->community_model->get_email_by_user_id($user_id);
            $data = array(
                'ImageUrl' => $this->image_server_path . 'upload/profile/' . $ProfilePicture,
                'UserEmailID' => $email_id,
                'ModuleID' => $module_id,
                'ModuleEntityID' => $module_id
            );
            $this->community_users_model->update_profile_picture($data);
        }
         * 
         */
    ...
    public function create_video_thumb($video) {
        ...
        foreach ($thumb_arr as $i => $row) {
            //screenshot size 
            $size = $row[0].'x'.$row[1];  //$size = '220x220';
    ...
    public function zencoder_create_job($video_url = '', $thumb_arr='')
    {
        ...
        $video_types = array('ogv','webm','mp4');//allowed video conversions
                ...
                if($query->num_rows())
                {
                    ...
                    if($row['MediaSectionID'] == 3 && $state == 'Finished' && $row['MediaSectionReferenceID'] && $row['StatusID'] != 3)
                    {
                        ...
                        if($activity_status != 3) {
                            ...
                            if(CACHE_ENABLE) {
                                sleep(5);
                                //$log = $this->cache->delete('activity_'.$MediaSectionReferenceID);
                                $this->cache->clean();
                            }
        ...
        if(isset($attachments) && !empty($attachments))
        {
            ...
            {                
                ...
                if($attachment['is_attachment'])
                {
                    ...
                    //$dir_name   = PATH_IMG_UPLOAD_FOLDER . $data['Type'];//'email_attachments';                    
                    ...
                    $OrgFullPath = $attachment['filepath'];
                    ...
                    $MimeType = mime_content_type($attachment['filepath']);
                    if (strtolower(IMAGE_SERVER) == 'remote') {               

                        $s3_path        = $dir_name .'/org_' . $attachment['filename'];
                        $s3             = new S3(AWS_ACCESS_KEY, AWS_SECRET_KEY);
                    ...
                    if($this->saveMedia($Media,$row['UserID']))
                    {
                        $this->checkMediaCounts($Media,true);                        
                    }                    
                }
