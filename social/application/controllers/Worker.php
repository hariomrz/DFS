...
class Worker extends CI_Controller {
    ...
    public function process_notification_gearman() {
        $worker = new GearmanWorker();
        $worker->addServer();
        ...
        $worker->addFunction("send_notification",function(GearmanJob $Job){
            ...
            $this->db->initialize();
            $this->load->model(array('notification_model'));
            $this->notification_model->send_notification($data); 
        });
        while ($worker->work());
    }
    
    public function process_image_thumb_gearman() {
        $worker = new GearmanWorker();
        $worker->addServer();
            ...
            if(!empty($data['method']))
            {
                switch ($data['method'])  {                   
                    ...
                    case 'send_post_notification': 
                            ...
                            $this->activity_model->send_post_notifications($data['UserID'], $data['PostContent'], $data['ActivityTypeID'], $data['ActivityID'], $data['ModuleID'], $data['ModuleEntityID'], $data['AfterProcess'], $data['PostAsModuleID'], $data['PostAsModuleEntityID'], $data['ExcludedUsers'], $data['PostType'], $data['NotifyAll']);
                        break;
            ...
            if(!empty($data['method']))
            {
                ...
                {        
                    ...
                    case 'add_update_relationship_score': 
                            ...
                            $this->db->initialize();
                            $data = $data['data']; 
                            add_update_relationship_score($data['UserID'], $data['ModuleID'], $data['ModuleEntityID'], $data['Score']);
                        break;
                    case 'save_log': 
                            $this->db->close();
                            $this->db->initialize();
