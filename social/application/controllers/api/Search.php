...
{   
    ...
    */
    public function GetSearchFilters_post()
    {
       ...
       /* $validation_rule[]  =   array('field' => 'FilterName', 'label' => 'Filter Name', 'rules' => '');
        ...
        if ($this->form_validation->run() == FALSE) 
        {
            ...
            /*Get Final Records*/
            $Records = $this->search_model->GetSearchFilters($this->UserID,'UserID',500);
            if($Records){
                $return['Data'] = $Records;
            }
       /* }*/
    ...
    public function tag_post()
    {
        ...
        {
            $search_keyword     = $data['SearchKeyword'];
            $return['Data'] = $this->search_model->get_tags($search_keyword);
        }
        $this->response($return);
    }
	 ...
	 * @return [json] [return json boject]
	 */
	public function user_post($limit=16)
    {
        /* Define variables - starts */
        $return         = $this->return;     
        $data           = $this->post_data;
        ...
        else 
        {
            $search_keyword     = $data['SearchKeyword'];
        	$show_friend        = isset($data['ShowFriend'])    ? $data['ShowFriend']  : 0 ;
            $search_type        = isset($data['SearchType'])    ? $data['SearchType']  : '' ;
        	if($limit==16)
            {
                $limit         = isset($data['PageSize']) ? $data['PageSize'] : 16 ;                
            }
            ...
            $workexp = isset($data['Workplace']) ? $data['Workplace'] : array();
            $education = isset($data['Education']) ? $data['Education'] : array();
            $cities = isset($data['Cities']) ? $data['Cities'] : array();

            $sort_by = isset($data['SortBy']) ? $data['SortBy'] : '' ;

            $advance_search = array('Location'=>$location,'AgeGroup'=>$age_group,'Gender'=>$gender,'Interest'=>$interest,'Skills'=>$skills,'Education'=>$education,'WorkExp'=>$workexp,'Cities'=>$cities,'FromSearch'=>'1');
            $data   = $this->friend_model->get_user_list($search_keyword, $user_id, $show_friend, array(), $limit, $offset,$advance_search,0,$sort_by,$search_type);

            if($data)
            {
                foreach($data as $k=>$v)
                {
                    $data[$k]['Name'] = $v['FirstName'].' '.$v['LastName'];
                }
            }
            $return['Data'] = $data;
            $total_records      = $this->friend_model->get_user_list($search_keyword, $user_id, $show_friend, array(), $limit, $offset,$advance_search,1,'',$search_type);
            /*if($total_records>100)
            {
                $total_records = $total_records/100;
                if(is_float($total_records))
                {
    ...
    public function get_city_list_post()
    {
        ...
        $return['Data'] = $this->search_model->get_city_list($keyword);
        $this->response($return);
    }
     ...
     * @return [json] [return json boject]
     */
    public function get_skills_post()
    {
        /* Defined varibles - starts */
        $return     = $this->return;
        $data       = $this->post_data;
        ...
        $keyword = isset($data['Keyword']) ? $data['Keyword'] : '' ;
        $return['Data'] = $this->search_model->get_skills($keyword);
        $this->response($return);
    }
     ...
     * @return [json] [return json boject]
     */
    public function get_interest_post()
    {
        /* Defined varibles - starts */
        $return     = $this->return;
        $data       = $this->post_data;
        ...
        $keyword = isset($data['Keyword']) ? $data['Keyword'] : '' ;
        $return['Data'] = $this->search_model->get_interest($keyword);
        $this->response($return);
    }
