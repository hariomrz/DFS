    ...
    public function send_native_invitations_post()
    {
        ...
        if (!empty($LoginSessionKey))
        {
            ...
            { /* Validation - ends */
                ...
                //If group id exists then add member in group
                /* if(!empty($res['old_email']) && $groupid>0)
                  {
                  foreach($res['old_email'] as $oldkey=>$oldvalue)
                  {
                  if(!$this->group_model->check_membership($groupid,$oldkey)){
                  $indata=array("GroupID"=>$groupid,"UserID"=>$oldkey,"JoinedAt"=>get_current_date('%Y-%m-%d %H:%i:%s'));

                  $this->group_model->add_members($indata);
                  }
                  }
                  } */
