<div  infinite-scroll="getUserOrientation()" infinite-scroll-distance="2" infinite-scroll-use-document-bottom="true" infinite-scroll-disabled="scroll_disable">
        ...
        <div class="col-xs-7">
            <div class="panel-body">
                <ul class="list-group list-group-thumb sm">
                    ...
                    <ng-include ng-if="((activityData.activity_log_details.ActivityTypeID != 23) && (activityData.activity_log_details.ActivityTypeID != 24) && (activityData.activity_log_details.ActivityTypeID != 25) && (activityData.activity_log_details.ActivityTypeID != 16))" src="partialPageUrl + '/dashboard/activityFeed/orientationContent.html'"></ng-include>
                    </li>
                </ul>
            </div>
        </div>
            ...
            <div class="panel-body">
                <div class="form-group no-bordered">
                    ...
                    <div class="text-field">
                        <textarea maxlength="200" data-req-maxlen="200" id="description-{{activityData.activity.ActivityID}}" ng-model="activityData.activity.Description" placeholder="Description">{{activityData.activity.Description}}</textarea>
                    </div>
                </div>

                <div class="form-group no-bordered">
                    <label class="control-label bolder">User Orientation</label>    
                    <select chosen data-disable-search="true" ng-model="activityData.activity.OrientationCategoryID" ng-options="g.Key as g.Name for g in UserOrientationOptions" title="Select" placeholder="Select" class="form-control">
                    </select>                
                </div>
                <!-- <div class="form-group no-bordered" ng-repeat="( orientationIndex, orientation ) in UserOrientationOptions">                    
                    <label class="checkbox" >
                        <input ng-checked="activityData.activity.OrientationCategoryID == orientation.Key" ng-click="check_user_orientation(activityData.activity, orientation.Key)" id="ont_{{activityData.activity.ActivityID}}_{{orientation.Key}}" value="{{orientation.Key}}" type="checkbox" class="check-content-filter">                
                        <span class="label bold" ng-bind="orientation.Name"></span>                
                    </label>                    
                </div> -->
