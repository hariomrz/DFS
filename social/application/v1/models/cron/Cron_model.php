...
class Cron_model extends Common_Model {
    ...
    function ward_engagement_old($no_of_days=1) {
        for ($d = 1; $d <= $no_of_days; $d++) {
            $today_date = get_current_date('%Y-%m-%d', $d);
            $this->db->select("COUNT(A.ActivityID) as TotalPost");
            $this->db->select('IFNULL(GROUP_CONCAT(A.ActivityID),0) as ActivityIDs', FALSE);
           ...
           // echo $this->db->last_query();die;
            if($query->num_rows()) {
                ...
                foreach ($query->result_array() as $result) {      
                    ...
                    $activity_id_str = $result['ActivityIDs'];  
                    if($activity_id_str) {
                        $activity_ids = explode(',', $activity_id_str);
                        $this->db->select("COUNT(PostCommentID) as TotalComment");
                        $this->db->from(POSTCOMMENTS);
                        $this->db->where('ParentCommentID', 0);
                        $this->db->where_in('EntityID', $activity_ids);
                        $this->db->where('EntityType', 'ACTIVITY');
                        $c_query = $this->db->get();
                       // echo $this->db->last_query();die;
                        if($c_query->num_rows()) {
                            $c_result = $c_query->row_array(); 
                            $ward_engagement['TotalComment'] = $c_result['TotalComment'];
                        }

                        $this->db->select("COUNT(PostLikeID) as TotalLike");
                        ...
                        if($l_query->num_rows()) {
                            ...
                            $ward_engagement['TotalPostLike'] = $l_result['TotalLike'];
                        }
                    }
                    $ward_engagements[] = $ward_engagement;
                }
    ...
    function ward_engagement($no_of_days=1) {
        log_message('error', 'V1 Ward Engagement => '.get_current_date('%Y-%m-%d %H:%i:%s'));
        for ($d = 1; $d <= $no_of_days; $d++) {
            $today_date = get_current_date('%Y-%m-%d', $d);
            $this->db->select("COUNT(A.ActivityID) as TotalPost");
            $this->db->select('AW.WardID');        
