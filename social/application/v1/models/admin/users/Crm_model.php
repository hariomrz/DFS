...
class Crm_model extends Admin_Common_Model {
    ...
    public function query_filter_users($filter, $citi_ids) {
        ...
        $ward_id = (int) isset($filter['WID']) ? $filter['WID'] : 1;
...
//                }
    ...
    function send_notification($data) {
        ...
        if ($isSms == 0) {            
            ...
            $user_ids_chunk = array_chunk($user_ids,3000);            
            ...
            if ($query->num_rows() > 0) {                
                foreach ($query->result_array() as $Notifications) {
                    ...
                    if(empty($activity_id)) {
                        ...
                        $message = $sender_name . " posted a new message.";                        
                    }
            ...
            /* if(!empty($activity_guid)) {
                ...
                if($activity_id) {
                    ...
                    if(empty($short_url)) {
                        $long_url = DOMAIN."/activity?guid=".$activity_guid;
                        $short_url = get_short_url($long_url);
                        if (CACHE_ENABLE) {
                            $this->cache->save('asl'.$activity_id, $short_url, CACHE_EXPIRATION);
                        }
                    }
                }
            } */        
            
            $users =  $this->get_users($data, false, false, 'U.PhoneNumber');           
            foreach ($users as $user) {
