...
class Activity_model extends Common_Model {
     ...
     * @return [Array/string]                    [ActivityIDs or details based on value of $only_id]
     */
    function get_newsfeed_announcements($current_user,$only_id=false) {        
        $role_id=2;
        $module_id = 3;
...
//        $friend_of_friends      = $this->user_model->get_friends_of_friend_list();
    ...
    function add_activity_ward($activity_id, $ward_ids) {
     ...
     * @return [Array]                    [Activity array]
     */
    function get_public_feed($activity_guid='',$module_id=0,$module_entity_id=0,$page_no=1,$page_size=10,$count_only=false, $rules = array(), $extra_params = array(),$post_type = 0) {
     ...
     * @return [array]          [block user list]
     */
    function get_blocked_user_list($user_id, $search_key, $PageNo, $PageSize) {
     ...
     */
    function removeActivity($activity_id, $user_id, $explicit_status_id = 0)
    {
     ...
     */
    function addComment($entity_guid, $Comment, $Media, $user_id, $is_media_exists = 0, $SourceID = 1, $DeviceID = 1, $entity_owner = 0, $entity_type = 'ACTIVITY', $postas_module_id = 3, $postas_module_entity_id=0, $parent_comment_id=0, $is_anonymous=0, $comment_id=0, $taged_user='') {        
        if(!$postas_module_entity_id) {
            $postas_module_entity_id=$user_id;
        }
    ...
    function send_comment_notification($data) {
        ...
        $post_as_module_id = $data['PostAsModuleEntityID'];
     ...
     * @return [type]               [description]
     */
    function updateCommentCount($entity_id, $entity_type, $Count = 1,$parent_comment_id=0)
    {
       ...
       // $friends = $friend_followers_list['Friends'];
        ...
        if ($query->num_rows())
        {
            ...
            foreach ($query->result_array() as $r)
            {
                ...
                $result[$i]['Occupation'] = $r['Occupation'];
                if(empty($r['LocalityID'])) {
                    $r['LocalityID'] = 0;
                }
                $result[$i]['Locality'] = array(
                    "Name" => $r['Name'], 
     ...
     * @return [type]               [description]
     */
    function update_like_count($entity_id, $entity_type, $count = 1)
    {
        $set_field = "NoOfLikes";
        $table_name = ACTIVITY;
        $condition = array("ActivityID" => $entity_id);
     ...
     * @return [type]               [description]
     */
    function update_dislike_count($entity_id, $entity_type, $count = 1)
    {
     ...
     */
    function blockUser($user_id, $entity_id, $module_id, $module_entity_id)
    {
        $this->db->where('EntityID', $entity_id);
        $this->db->where('ModuleID', $module_id);
        $this->db->where('ModuleEntityID', $module_entity_id);
