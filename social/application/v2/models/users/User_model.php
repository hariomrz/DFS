...
//            }
	...
	function profile_cache($user_id, $current_user_id = 0) {            
            $this->db->select("U.FirstName, U.LastName, CONCAT(IFNULL(U.FirstName,''), ' ',IFNULL(U.LastName,'')) as EntityName, U.Email, U.Gender, U.UserGUID, UD.TimeZoneID, UD.LocalityID, U.StatusID, U.CanCreatePoll");
...
//            else
    ...
    */
    ...
    function get_user_details($user_id) {
        ...
        if (empty($userdata)) {
            $this->db->select("U.FirstName, U.LastName, U.UserGUID, UD.LocalityID");            
            $this->db->select('IFNULL(UD.Occupation,"") as Occupation', FALSE);
            $this->db->from(USERS . ' U');
            $this->db->join(USERDETAILS . ' UD', 'UD.UserID = U.UserID', 'left');
            $this->db->where('U.UserID', $user_id);
