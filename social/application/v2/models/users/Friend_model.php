...
class Friend_model extends Common_Model {
    ...
    function get_newsfeed_tagging_records($search_key, $user_id, $type, $page_no = PAGE_NO, $page_size = PAGE_SIZE, $Status = '', $UID = '', $module_id = 0, $module_entity_id = 0, $AdvanceSearch = array(), $CountOnly = 0, $exclude_ids = array()) {
        ...
        $privacy_condition = "";
        if ($blocked_users) {
            $privacy_condition .= "u.UserID NOT IN (" . implode(',', $blocked_users) . ") AND ";
        }

        $exclude_ids[] = $UsrID;
        $search_key = $this->db->escape_like_str($search_key);
        $innerSql = ' 
				SELECT u.FirstName, IF(u.ProfilePicture="","user_default.jpg",u.ProfilePicture) as ProfilePicture, u.UserID, u.UserGUID, u.LastName, "3" as ModuleID
				FROM ' . USERS . ' u 								
				WHERE ' . $privacy_condition . ' u.StatusID NOT IN(3,4) AND u.UserID NOT IN (' . implode(',', $exclude_ids) . ') 
				AND (u.FirstName like "%' . $search_key . '%" or u.LastName like "%' . $search_key . '%" or concat(u.FirstName," ",u.LastName) like "%' . $search_key . '%")
			';
        /* $group_where = '';
