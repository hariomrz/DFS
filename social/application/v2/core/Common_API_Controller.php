...
class Common_API_Controller extends REST_Controller {
    ...
    */
    ...
    protected function _custom_prepare_basic_auth() {        
        ...
        $key_detail = array();
        try {
            $key_detail = $this->mongo_db->where(array( AUTH_KEY => $key))->find_one('active_user_login');
        } catch (Exception $e) {
            log_message("error", "Unable to connect to MongoDB: {$e->getMessage()}");
        }
        $logged_user_data = array();
        if(empty($key_detail)) {
            $logged_user_data = $this->login_model->active_login_auth($this->post_data);
            if($logged_user_data['ResponseCode']=='200') {
                $key_detail = $logged_user_data['Data'];
                $key_detail['RightIDs'] = $this->login_model->get_user_rights($logged_user_data['Data']['UserID']);                
                $key_detail[AUTH_KEY] = $key;
                $this->mongo_db->insert('active_user_login',$key_detail);
            }
        }
        
        if(!empty($key_detail)) {
            $this->IsApp                    = $key_detail['IsApp'];
            $this->UserID                   = $key_detail['UserID'];
            $this->LoggedInGUID             = $key_detail['LoggedInGUID'];
            $this->LoggedInName             = $key_detail['LoggedInName'];
            $this->LoggedInProfilePicture   = $key_detail['LoggedInProfilePicture'];
            $this->DeviceTypeID             = $key_detail['DeviceTypeID'];
            $this->SourceID                 = $key_detail['SourceID'];
            $this->UserTypeID                = $key_detail['UserTypeID'];
            $this->RoleID                   = isset($key_detail['RoleID']) ? $key_detail['RoleID'] : '';
            $this->RightIDs                 = $key_detail['RightIDs'];            
            $this->can_create_poll          = $key_detail['CanCreatePoll'];
            
	    if(empty($this->LocalityID)) {
                $this->LocalityID               = $key_detail['LocalityID'];
            }
            ...
            if($logged_in_date < $current_date) {
                ...
                $this->login_model->delete_mongo_db_record('active_user_login', array(AUTH_KEY => $key));
            }
