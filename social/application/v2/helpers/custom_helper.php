...
function SendPushMsg($entityID, $message, $Data = array()) {
    ...
    $obj = &get_instance();
    $Query = $obj->db->query("SELECT DeviceToken, DeviceTypeID FROM `ActiveLogins` WHERE UserID='$entityID' AND DeviceToken!='' GROUP BY DeviceToken, DeviceTypeID ORDER BY ActiveLoginID DESC ");
    if ($Query->num_rows() > 0) {
...
function push_notification_android($registatoin_ids = array(), $message = '', $badge = 1, $Data) {
    ...
    try {
        ...
        if($Data['ToUserID'] ==  1) {
            log_message('error', 'Reciever - '.$Data['ToUserID'].' Token - '.$registatoin_ids[0].' NotificationTypeID - '.$Data['NotificationTypeID']);
...
if (!function_exists('linkify')) {
   function linkify($value, $protocols = array('http', 'https'), array $attributes = array('class'=>'linkify')) {
