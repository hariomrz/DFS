...
class Admin_crm extends Admin_API_Controller {
    ...
    function send_notifications_post(){
        ...
        /* Gather Inputs - starts */
        $data = $this->post_data;
        $allUserSelected =   isset($data['allUserSelected']) ? $data['allUserSelected'] : '' ;
        $notification_text =   isset($data['notification_text']) ? $data['notification_text'] : '' ;
        $isSms =   isset($data['isSms']) ? $data['isSms'] : 0 ;
        $user_ids =   isset($data['users']) ? $data['users'] : 0 ;
        $subject ='Notification from admin ';
        initiate_worker_job('send_notification', array('useridsString'=>$user_ids, 'notification_text'=>$notification_text,'isSms'=>$isSms,'allUserSelected'=>$allUserSelected), '', 'notification');
