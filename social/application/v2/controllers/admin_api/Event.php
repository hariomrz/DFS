...
/*
* Controller to manage Event page information
* @package    Event
* @author     V-INFOTECH
* @version    1.0
*/

class Event extends Admin_API_Controller
{
    ...
    */    
	...
	{
        parent::__construct();
        $this->load->model(array('events/event_model','admin/login_model'));
    ...
    */
    public function index_post()
    {
        ...
        $Return['ServiceName']  =   'admin_api/event';
        $Return['Data']         =   array();
        $Data                   =   $this->post_data;
        ...
        if(isset($Data) && $Data!=NULL )
        {
            ...
            $SortBy     = !empty($Data['SortBy'])?$Data['SortBy']:"LastActivity";
            ...
            $Return['Data']['results']          = $this->event_model->admin_event_listing($Input,$PageNo,$PageSize,0,FALSE,TRUE);
            $Return['Data']['total_records']    = $this->event_model->admin_event_listing($Input,'','',0,TRUE,TRUE);             
        }else{
    ...
    */
    ...
    {
        $Return['ResponseCode']='200';
        $Return['Message']= lang('event_success_delete');
        $Return['ServiceName']='admin_api/event/delete_event';
        $Return['Data']=array(); 
        $Data=$this->post_data;
        
        /* Validation - starts */
        $validation_rule        =    $this->form_validation->_config_rules['api/event/delete_event'];
        ...
        { 
            $event_guid = !empty($Data['EventGUID'])?$Data['EventGUID']:'';
            $action_type = 'Delete';
            $event_id = get_detail_by_guid($event_guid,14);
            $status = $this->event_model->delete($event_id, 1,$this->UserID);
        }
    ...
    /**
    * Function Name: feature_events
    * @param LoginSessionKey
    * @param EventGUID
    * Description: 
    */
    public function feature_event_post() 
    {
        $Return['ResponseCode']='200';
        $Return['Message']= lang('event_success_feature');
        $Return['ServiceName']='admin_api/event/feature_event';
        $Return['Data']=array(); 
        $Data=$this->post_data;
        
        /* Validation - starts */
        $validation_rule        =    $this->form_validation->_config_rules['api/event/feature_event'];
        ...
        { 
            $event_guid = !empty($Data['EventGUID'])?$Data['EventGUID']:'';
            $is_featured = !empty($Data['IsFeatured'])?$Data['IsFeatured']:'0';
            $event_id = get_detail_by_guid($event_guid,14);
            $status = $this->event_model->feature($event_id, $is_featured,$this->UserID);

            if(!empty($is_featured)){
                $Return['Message']= lang('event_success_feature');
            }else{
                $Return['Message']= lang('event_remove_feature');
            }
        }
