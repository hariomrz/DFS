    ...
    public function option_voter_list_post() {
        ...
        if (isset($data)) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $option_guid = $data['OptionGUID'];
                $row = $this->polls_model->get_single_row("OptionID", POLLOPTION, array('OptionGUID' => $option_guid));
                if($row['OptionID']) {
                    $page_no = isset($data['PageNo']) ? $data['PageNo'] : 1;
                    $page_size = isset($data['PageSize']) ? $data['PageSize'] : PAGE_SIZE;                
                    $return['TotalRecords'] = 0;
                    if ($page_no == '1') {
                        $return['TotalRecords'] = $this->polls_model->get_poll_option_votes($row['OptionID'], TRUE, $page_no, $page_size);                                
                    }
                    $return['Data'] = $this->polls_model->get_poll_option_votes($row['OptionID'], FALSE, $page_no, $page_size);                
                } else {
                    $return['ResponseCode'] = self::HTTP_PRECONDITION_FAILED;
                    $return['Message'] = sprintf(lang('valid_value'), "poll option");
                }
