...
class Users extends Common_API_Controller
{
    ...
    public function search_user_n_group_get()
    {
        ...
        } else
        {
            ...
            $remove_groups = array();
            $page_no = isset($data['PageNo']) ? $data['PageNo'] : '1' ;
            $page_size= isset($data['PageSize']) ? $data['PageSize'] : '10' ;//there must be a default limit 
            ...
            $return['Data'] = $this->user_model->search_user_n_group($search_keyword, $user_id, $remove_users, $remove_groups, $group_id, $formal, $page_no, $page_size);
        }
        $this->response($return);
    }
                ...
                elseif ($data['Type'] == 'TagLine') {
                    ...
                    */
                    ...
                    if ($this->form_validation->run() == FALSE)  { 
                        ...
                        $dob = safe_array_key($data, 'DOB');
                        $dob = $this->user_model->format_dob($dob);
                        
                        $location = array();
                        $location['City']           = safe_array_key($data, 'City');
                        $location['Country']        = safe_array_key($data, 'Country');
                        $location['State']          = safe_array_key($data, 'State');
                        $location['StateCode']      = safe_array_key($data, 'StateCode');
                        $location['CountryCode']    = safe_array_key($data, 'CountryCode');
                        $location_data  = update_location($location);
                        ...
                        $this->user_model->save_user_info($user_id, $loc);
                        //Update Logins Analytic
                        if($data[AUTH_KEY] && $dob != "0000-00-00") {
                            $this->user_model->update_login_analytic_age_group($user_id, $data[AUTH_KEY]);                                
                        }

                        $user['FirstName']  = safe_array_key($data, 'FirstName');
                        $user['LastName']   = safe_array_key($data, 'LastName');
                        $user['Gender']     = safe_array_key($data, 'Gender');
                        $this->user_model->save_user_info($user_id, $user, USERS);
                        if (CACHE_ENABLE) {
                            $this->cache->delete('user_profile_' . $user_id);
                        }
                    }                    
    ...
    /**
     * [delete_work_experience_post Used to delete user work experience]
     */
    function delete_work_experience_post() {
        $return = $this->return;
        $data = $this->post_data;
        $user_id = $this->UserID;
        ...
        if (isset($data)) {
            $validation_rule    =   array(                                
                                        array(                                
                                            'field' => 'WorkExperienceGUID',
                                            'label' => 'work experience GUID',
                                            'rules' => 'trim|required'
                                        )
                                    );
            $this->form_validation->set_rules($validation_rule); 
            if($this->form_validation->run() == FALSE)  {
                ...
                $work_experience_guid  = safe_array_key($data, 'WorkExperienceGUID');               
                $this->user_model->delete_work_experience($user_id, $work_experience_guid);
            }
        } else {
            $return['ResponseCode'] = self::HTTP_BAD_REQUEST;
            $return['Message'] = lang('input_invalid_format');
    ...
    /**
     * [delete_education_post Used to delete user education]
     */
    function delete_education_post() {
        $return = $this->return;
        $data = $this->post_data;
        $user_id = $this->UserID;
        ...
        if (isset($data)) {
            ...
            if($this->form_validation->run() == FALSE)  {
                ...
                $education_guid  = safe_array_key($data, 'EducationGUID');               
                $this->user_model->delete_education($user_id, $education_guid);
            }
        } else {
            $return['ResponseCode'] = self::HTTP_BAD_REQUEST;
            $return['Message'] = lang('input_invalid_format');
