...
class Common_Model extends CI_Model {
    ...
    public $current_db;
    public $top_contributor_ids = array();
    function __construct() {
        parent::__construct();
        
        $this->current_db = $this->db;
        if(IS_ARCHIVE_DB == 1) {
            $this->current_db	= $this->load->database('db_archive', TRUE);
        }
    }
    ...
    function load_archive_db_instance() {
        $this->current_db	= $this->load->database('db_archive', TRUE);
    }
    ...
    function get_device_type_id($device_type) {
        $device = array('Native' => 1, 'IPhone' => 2, 'AndroidPhone' => 3, 'Ipad' => 4, 'AndroidTablet' => 5, 'OtherMobileDevice' => 6, 'WindowsPhone' => 7, 'WindowsTablet' => 8);
        $device_type_id = '';
    ...
    function get_resolution_id($resolution) {
        $resolution_data = array('Low' => 1, 'Medium' => 2, 'High' => 3, 'Desktop' => 4);
        $resolution_id = '';
        if(isset($resolution_data[$resolution])) {
            $resolution_id = $resolution_data[$resolution];
        } else {
    ...
    /**
     * [is_valid_user Used to check valid user]
     * @param  [string] $user  [Field Value]
     * @param  [string] $type  [Filed name]
     */
    ...
    */
    function get_age_range($age_group_id) {
        $this->db->select('ValueRangeFrom,ValueRangeTo');
        $this->db->from(AGEGROUPS);
        $this->db->where('AgeGroupID', $age_group_id);
        $this->db->limit(1);
        $query = $this->db->get();
        if ($query->num_rows()) {
            $row = $query->row();
    ...
    */
    function get_age_group_id($user_id) {
        $this->db->select("DATE_FORMAT(FROM_DAYS(DATEDIFF('" . get_current_date('%Y-%m-%d') . "',UD.DOB)), '%Y')+0 as Age", false);
    ...
    /**
     * used to set top contributors ids
     */
    function set_top_contributors($tag_id){        
        $row_ids = '';
        if (CACHE_ENABLE) {
            $row_ids = $this->cache->get('ttp_' . $tag_id);
        }
        if(empty($row_ids)){
            $this->db->select('GROUP_CONCAT(UserID) as UserIDs ');
            $this->db->from(TOPCONTRIBUTORS);            
            ...
            if ($query->num_rows() > 0) {
                ...
                $user_ids = $row_ids['UserIDs'];
                if(!empty($user_ids)) {                    
                    $this->top_contributor_ids =  explode(',',$user_ids);
                }
            }
            if (CACHE_ENABLE) {
                $this->cache->save('ttp_' . $tag_id, $user_ids, CACHE_EXPIRATION);
            }
        } else if($row_ids != '-1') {
            $this->top_contributor_ids =  explode(',',$row_ids);
        }
    }
    /**
    ...
    function get_top_contributors() {
        return $this->top_contributor_ids;
    }
