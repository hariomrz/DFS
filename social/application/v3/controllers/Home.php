...
//$csv .= $result['firstname'].' '.$result['lastname'].','.$result['email'].','.$result['resgisdate'].','.$result['lastlogindate']. "\n";
    ...
    function time_diff() {
        ...
        echo $current_date = get_current_date('%Y-%m-%d %H:%i:%s');
        echo "<br>Deadline: 2021-02-25 12:43:18";
        $t1 = strtotime("2021-02-25 12:43:18");
        $t2 = strtotime($current_date);

        echo "<br>Difference: ".$t = $t1 - $t2;

        $minutes = round(abs($t) / 60, 2);
        echo "<br>" . $minutes  . " minutes";
        echo "<br>" . floor($minutes)  . " minutes";
        echo "<br>" . floor($t /60)  . " minutes";
        die();
    ...
    public function test_rule() {
        ...
        $this->restore_model->sync_entity("ACTIVITY", 17);
die;
