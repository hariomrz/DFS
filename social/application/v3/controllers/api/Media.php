...
class Media extends Common_API_Controller {
    ...
    function details_post() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $return['Data'] = $this->media_model->details($user_id, $media_guid);
        }
        $this->response($return);
    }
    ...
    function comments_post(){
        ...
        if($this->form_validation->run('api/media/comments') == FALSE){
            ...
            if($page_no == 1) {
                ...
                $return['TotalReplies'] = $this->media_model->comments($user_id,$media_guid,$page_no,$page_size, TRUE, 1);
    ...
    function delete_post(){
        ...
        if (isset($data)) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $media_details = $this->media_model->delete($user_id,$media_guid);
                $return['Data'] = array();
                if(isset($media_details['album_id']) && !empty($media_details['album_id'])) {
                    $album_id = $media_details['album_id'];
                    ...
                    $media_id = $media_details['MediaID'];
    ...
    /**
     * Used to add gallery media
     */
    function add_gallery_post() {
        $return = $this->return;
        $data = $this->post_data;
        if (isset($data)) {            
            $user_id = $this->UserID;
            ...
            if ($media_count > 0) {
                $this->media_model->add_gallery($user_id, $media_array);
    ...
    function get_gallery_post() {
        ...
        $return['Data'] = $this->media_model->get_gallery($data);   
        $this->response($return);
    }
}