...
class Login extends Common_API_Controller {
    ...
    public function validate_otp_post() {
        ...
        if (isset($data)) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if ($verification == 3) {
                    $return['ResponseCode'] = self::HTTP_PRECONDITION_FAILED;
                    $return['Message'] = lang('otp_expired');
                }
            }
        } else {
            $return['ResponseCode'] = self::HTTP_BAD_REQUEST;
    ...
    function check_apk_ver_post() {
        ...
        $response['image_name'] = 'version11.jpg'; //
        $response['upgrade_required'] = 0;
        $response['upgrade_optional'] = 0;
        $response['apk_url'] = 0;
        $response['device_type'] = $device_type;
        $response['upgrade_required_msg'] = "अपना टैलेंट इंदौर को दिखाने के लिए और टैलंटेड इंदौरवासियों को Follow करने के लिए ऐप अपडेट करें"; //अब भोपू पर किसी को भी भेजे निजी सन्देश.
        $response['upgrade_optional_msg'] = ""; //इस नवरात्री गरबे में झूम \nनए भोपू से अब बढ़ी धूम
        ...
        if (!empty($version) && $version < $app_version) {
            ...
            $response['upgrade_required'] = UP_REQUIRED;
            $response['upgrade_optional'] = UP_OPTIONAL;
        }
