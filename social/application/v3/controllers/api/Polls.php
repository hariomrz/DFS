...
class Polls extends Common_API_Controller
{
    ...
    public function vote_post() {
        ...
        if (isset($data)) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $module_id = 3;
                $flag = $this->polls_model->vote($option_guid, $module_entity_id, $module_id);                                
                if(!$flag) {
                    $return['ResponseCode'] = self::HTTP_PRECONDITION_FAILED;
                    $return['Message'] = sprintf(lang('valid_value'), "poll option");
                }
            }
        } else {
    ...
    /**
     * used to get voter list for particular option
    */
    public function option_voter_list_post() {
        $return = $this->return;
        $data = $this->post_data; // Get post data
        $user_id = $this->UserID; // Get post data
        if (isset($data)) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $row = $this->polls_model->current_db->select('OptionID')->from(POLLOPTION)->where('OptionGUID', $option_guid)->limit(1)->get()->row_array();
                ...
                if($row['OptionID']) {
                    ...
                    if ($page_no == '1') {
                        $return['TotalRecords'] = $this->polls_model->get_poll_option_votes($row['OptionID'], $user_id, TRUE, $page_no, $page_size);                                
                    }            
                    $return['Data'] = $this->polls_model->get_poll_option_votes($row['OptionID'], $user_id, FALSE, $page_no, $page_size);                
                } else {
                    $return['ResponseCode'] = self::HTTP_PRECONDITION_FAILED;
                    $return['Message'] = sprintf(lang('valid_value'), "poll option");
                }
    ...
    public function create_post() { 
        ...
        if (isset($data)) {
            ...
            if ($this->form_validation->run('api/polls/add') == FALSE) {
                ...
                $poll_description = $data['Description'];                
                $duration = !empty($data['Duration']) ? $data['Duration'] : ""; //it will be in hour            

                /* --Converted date from clients timezone to UTC-- */
                $expiry_date_time = "";
                if (!empty($duration)) {
                    $duration = $duration/24; //convert in day
                    $expiry_date_time = get_current_date('%Y-%m-%d %H:%i:%s', $duration, 1);
                } 
                ...
                $poll_data['ExpiryDateTime'] = $expiry_date_time;         
                $poll_id = $this->polls_model->create($poll_data, $options);

                $activity_id = $this->polls_model->add_poll_activity($poll_id, 1, 1, 0, $poll_data, 0, 0, $poll_description);

                $subscribe_users = array();
                $subscribe_users[] = array('ModuleEntityID' => $user_id, 'ModuleID' => 3);                    
                $this->load->model(array('subscribe_model', 'tag/tag_model'));
                $this->subscribe_model->addUpdate($subscribe_users, $activity_id);
                
                $return['Data'] = $this->activity_model->getSingleUserActivity($user_id, $activity_id, 0, false, $user_id, 3);
            }
        } else {
            $return['ResponseCode'] = self::HTTP_BAD_REQUEST;
            $return['Message'] = lang('input_invalid_format');
    ...
    {return;
        ...
        /*
    | Function  - Callback to validate poll options
    | Input     - ARRAY
    | OutPut    - Bool
    */
    public function validate_poll_options($options)
    {
        if(is_array($options))
        {
            if(count($options)>POLL_MAX_CHOICE)
            {
                $this->form_validation->set_message('validate_poll_options', lang('invalid_maximum_options').POLL_MAX_CHOICE);
                return FALSE;
            }
            else if(count($options)<POLL_MIN_CHOICE)
            {
                $this->form_validation->set_message('validate_poll_options', lang('invalid_minimum_options'));
                return FALSE;
            }
            else
            {
        ...
        {
            $this->form_validation->set_message('validate_poll_options', lang('invalid_format_poll_option'));
            return FALSE;
        }
    }
}
