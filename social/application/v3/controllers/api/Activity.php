...
class Activity extends Common_API_Controller {
    ...
    public function get_announcement_post() {
        ...
        $this->activity_model->set_top_contributors(0);
       ...
       // $this->forum_model->set_visible_category_list($user_id);
        if ($module_id == '34' && !$module_entity_id) {
            ...
            $ward_ids = safe_array_key($data, 'WardIds', array());
            if(empty($ward_ids) && $this->LocalityID) {
                ...
                if(!empty($localty["WID"])) {
                    $ward_ids[] = $localty["WID"];
                    $data['WardIds'] = $ward_ids; 
                }
            }

            $return['Data'] = $this->activity_model->get_newsfeed_announcements($user_id, false, $data);
        }
        $this->response($return);
    }
    ...
    public function index_post() {
        ...
        if ($dummy_users_only) {
          ...
          /*  $return['TotalFavouriteRecords'] = 0;
           ...
           // $return['TotalFavouriteRecords'] = 0;
            ...
            //commented as archive db is not active or available right now
            /*if (count($activity) == 0 || count($activity) < $page_size) { //check for archival script.
       ...
       // $return['Welcome'] = !empty($rules['Welcome']) ? $rules['Welcome'] : '';
        ...
        $return['AdminUserGUID']  = $this->activity_model->get_user_guid_by_user_ids(array(ADMIN_USER_ID)); //ADMIN_USER_GUID;

        //$return = $this->setActivityDetailsData($return, $activity_guid);
       // $return['ActivityGUID'] = '13c00d86-cc78-ae27-f6c7-06e6b9d42941';
        
        //$this->benchmark->mark('feed_end');
        //log_message('error', "Activity Response Time => ".$this->benchmark->elapsed_time('feed_start', 'feed_end'));
        $this->response($return);
    }
    ...
    function public_feed_post() {
        ...
        $tag_id = safe_array_key($data, 'TagID', ''); 
        $post_type = safe_array_key($data, 'PostType', 0); 
        $page_no = isset($data['PageNo']) ? $data['PageNo'] : 1;
        $page_size = isset($data['PageSize']) ? $data['PageSize'] : PAGE_SIZE;
        $module_id = safe_array_key($data, 'ModuleID', 0); 
        $module_entity_guid = safe_array_key($data, 'ModuleEntityGUID', 0); 
        $module_entity_id = 0;
        if($module_id && $module_entity_guid) {
            $module_entity_id = get_detail_by_guid($module_entity_guid, 47);
        }
        ...
        if(!empty($tag_id)) {
            $tags[] = $tag_id;
            if($page_no == 1) {
                $return['Tag'] = $this->tag_model->details($tag_id, 0, 0);                
            }
        }
        $this->load->model(array('activity/activity_hide_model'));
        $return['Data'] = $this->activity_model->get_public_feed($activity_guid, $module_id, $module_entity_id, $page_no, $page_size, false, array(), $data, $post_type, $tags);            
        
        if (count($return['Data']) == 0) { //check for archival script.
            $this->activity_model->load_archive_db_instance();                
            $return['Data'] = $this->activity_model->get_public_feed($activity_guid, $module_id, $module_entity_id, $page_no, $page_size, false, array(), $data, $post_type, $tags);        
    ...
    /**
     * used to get city news
     */
    function city_news_post() {
        /* Define variables - starts */
        $return = $this->return;
        $data = $this->post_data;
        $user_id = $this->UserID;
        ...
        if (isset($data)) {  
            ...
            if($user_id) {
                if($page_no == 1) {
                    $this->activity_model->update_city_news_visit_date($user_id);
                    $this->load->model(array('admin/advertise_model'));
                    $banner = $this->advertise_model->get_feed_banner();
                    if(!empty($banner) && isset($banner['ImageName'])) {
            ...
            $this->activity_model->set_top_contributors(0);
            $return['Data'] = $this->activity_model->city_news($page_no, $page_size, $user_id, false, $view_entity_tags, $data);            

            if (count($return['Data']) == 0 || count($return['Data']) < $page_size) { //check for archival script.
                $this->activity_model->load_archive_db_instance();                
                $ar_cnt = $this->activity_model->city_news($page_no, $page_size, $user_id, true, $view_entity_tags, $data);
    ...
    /**
     * used to get daily digest
     */
    function daily_digest_post() {
        /* Define variables - starts */
        $return = $this->return;
        $data = $this->post_data;
        $user_id = $this->UserID;
        if (isset($data)) {  
            ...
            if(!empty($row) && isset($row['DailyDigestDate'])) {
                ...
                $this->tag_model->set_following_tag_list($user_id);
                $this->tag_model->set_muted_tag_list($user_id);
                $daily_digest_date = $row['DailyDigestDate'];
                $result = $this->activity_model->daily_digest($page_no, $page_size, $user_id, false, $view_entity_tags, $daily_digest_date);
                if(!empty($result)) {
                    $return['Data'] =  $result;
     ...
     /**
     * used to get user orientation
     */
    function user_orientation_post() {
        /* Define variables - starts */
        $return = $this->return;
        $data = $this->post_data;
        $user_id = $this->UserID;
        if (isset($data)) {  
            ...
            $this->tag_model->set_following_tag_list($user_id);
            $this->tag_model->set_muted_tag_list($user_id);
            $return['Data'] =  $this->activity_model->user_orientation($user_id);           
        } else {
            $return['ResponseCode'] = self::HTTP_BAD_REQUEST;
            $return['Message'] = lang('input_invalid_format');
        }
    ...
    /**
     * used to get story user
     */
    function story_user_post() {
        /* Define variables - starts */
        $return = $this->return;
        $data = $this->post_data;
        $user_id = $this->UserID;
        if (isset($data)) {  
            ...
            $return['Data'] =  $this->activity_model->story_user($user_id, $ward_id);           
        } else {
            $return['ResponseCode'] = self::HTTP_BAD_REQUEST;
            $return['Message'] = lang('input_invalid_format');
        }
     ...
     * used to get story user
     */
    function user_story_post() {
        /* Define variables - starts */
        $return = $this->return;
        $data = $this->post_data;
        $logged_in_user_id = $this->UserID;
        if (isset($data)) {  
            $this->form_validation->set_rules('UserGUID', 'user guid', 'trim|required');
            if ($this->form_validation->run() == FALSE) {
                $error = $this->form_validation->rest_first_error_string();
                $return['ResponseCode'] = self::HTTP_PRECONDITION_FAILED;
                $return['Message'] = $error;
                ...
                $this->load->model(array('activity/activity_hide_model'));
                $this->activity_hide_model->set_user_hide_activity($logged_in_user_id);
                $this->favourite_model->set_user_favourite($logged_in_user_id);
                $this->flag_model->set_user_flagged($logged_in_user_id);
                $this->activity_model->set_top_contributors(0);
                $this->user_model->set_friend_followers_list($logged_in_user_id);
                $this->tag_model->set_following_tag_list($user_id);
                $this->tag_model->set_muted_tag_list($user_id);
                ...
                if($user_id) {
                    ...
                    $return['Data'] = $this->activity_model->user_story($user_id, $ward_id, $logged_in_user_id, $page_no, $page_size, false, 1);                
                } else {
                    $return['ResponseCode'] = self::HTTP_PRECONDITION_FAILED;
                    $return['Message'] = sprintf(lang('valid_value'), "user id");
                }                    
            }        
        } else {
            $return['ResponseCode'] = self::HTTP_BAD_REQUEST;
     ...
     */
    function get_unread_city_news_count_post() {
        /* Define variables - starts */
        $return = $this->return;
        $data = $this->post_data;
        $user_id = $this->UserID;
         
        $return['TotalRecords'] = $this->activity_model->get_unread_city_news_count($user_id);            
    ...
    function createWallPost_post() {
        ...
        if (isset($data)) {            
            ...
            $stripped_content = strip_tags($data['PostContent'], '<img><iframe>');
            if(empty($stripped_content) && empty($data['Media'])) {                
                $return['ResponseCode'] = self::HTTP_PRECONDITION_FAILED;
            ...
            $poll_description   = isset($data['PollDescription']) ? $data['PollDescription'] : '';
            $poll_options       = isset($data['PollOptions']) ? $data['PollOptions'] : array();
            $is_poll_exist      = ($poll_description) ? 1 : 0;
            $delete_poll        = isset($data['DeletePoll']) ? $data['DeletePoll'] : 0;
            if (!empty($poll_description)) {
                $this->can_create_poll = $this->polls_model->can_create_poll();
                if($this->can_create_poll) {
                    if(is_array($poll_options)) {
                        if(count($poll_options) != POLL_MAX_CHOICE) {                        
                            $return['ResponseCode'] = self::HTTP_PRECONDITION_FAILED;
                            $return['Message'] = lang('invalid_poll_options');
                            $this->response($return);                    
                        }
                    } else { 
                        $return['ResponseCode'] = self::HTTP_PRECONDITION_FAILED;
                        $return['Message'] = lang('invalid_format_poll_option');
                        $this->response($return);                    
                    }
                }
            }
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (isset($data['ActivityGUID']) && !empty($data['ActivityGUID'])) {
                    ...
                    $existing_activity_details = get_detail_by_guid($activity_guid, 0, 'ActivityID,IsPollExist', 2);
                    $activity_id = $existing_activity_details['ActivityID'];
                    $is_poll_exist = $existing_activity_details['IsPollExist'];
                    $is_edit = TRUE;
                    if($delete_poll && $is_poll_exist) {
                        $is_poll_exist = 0;
                        $this->polls_model->delete_activity_poll($activity_id);
                    }
                }
                ...
                */
                ...
                $module_id          = isset($data['ModuleID']) ? $data['ModuleID'] : 3;
                $module_entity_guid = isset($data['ModuleEntityGUID']) ? $data['ModuleEntityGUID'] : get_detail_by_id($user_id, 3);
                if($module_id == 47) {
                    $quiz = get_detail_by_guid($module_entity_guid, $module_id, 'QuizID,SponsorID,TotalPost,MaximumPost', 2);
                    if($quiz) {
                        $module_entity_id   =  $quiz['QuizID'];
                        ...
                        if($quiz['MaximumPost'] <= $quiz['TotalPost']) {
                            $return['ResponseCode'] = self::HTTP_PRECONDITION_FAILED;
                            $return['Message'] = lang('max_post_created');
                            $this->response($return);
                        }
                ...
                $is_media_exist     = ($media) ? '1' : '0';
                $delete_link        = isset($data['DeleteLink']) ? $data['DeleteLink'] : 0;
                ...
                $notify_all = isset($data['NotifyAll']) ? $data['NotifyAll'] : 0;
                $facts = isset($data['Facts']) ? $data['Facts'] : '';
                $params = isset($data['Params']) ? $data['Params'] : array();
                $contest_date = isset($data['ContestDate']) ? $data['ContestDate'] : '';
                $is_intro = isset($data['IsIntro']) ? $data['IsIntro'] : 0;
                $summary = isset($data['Summary']) ? $data['Summary'] : '' ;
                $is_city_news = isset($data['IsCityNews']) ? $data['IsCityNews'] : 0 ;
                $params['IsCityNews'] = $is_city_news;
                $is_show_on_news_feed = isset($data['IsShowOnNewsFeed']) ? $data['IsShowOnNewsFeed'] : 0 ;
                ...
                if ($module_id == 3 && $user_id != $module_entity_id) {
                    $is_modified_date = 0;
                    $value = $this->privacy_model->get_value($module_entity_id, 'default_post_privacy');
                    if ($visibility == 0) {
                        $visibility = 1;
                    }
               ...
               /* if ($module_id == 3 && check_blocked_user($user_id, $module_id, $module_entity_id)) {
                    $can_post = FALSE;
                }
                if ($module_id == 18) {
                    $can_post = $this->page_model->get_page_post_permission($user_id, $module_entity_guid);
                }
                if ($module_id == 1) {
                    $permission = check_group_permissions($user_id, $module_entity_id, FALSE);
                    $can_post = $permission['CanPostOnWall'];
                ...
                if ($can_post) {
                    ...
                    $activity_details = $this->activity_model->createPost($post_content, $module_id, $module_entity_id, $user_id, $is_media_exist, $media_cout, $visibility, $commentable, $module_entity_owner, $notify_all, $links, $files_count, $entity_tags, $post_as_module_id, $post_as_module_entity_id, $post_title, $post_type, $media, $files, $activity_id, $is_anonymous, $status, $publish_post, $analytic_login_id, $facts, $params, $contest_date, $is_featured,$summary,$is_poll_exist, $is_show_on_news_feed);
                    $activity_id = $activity_details['ActivityID'];
                    if(!empty($ward_ids)) {
                        $activity_details['UserID'] = $user_id;
                        $this->activity_model->add_activity_ward($activity_id, $ward_ids, 1, $activity_details);
                    }
                    if($this->can_create_poll && !$is_edit && $is_poll_exist==1) {
                        $poll_data = array();
                        $poll_data['Description'] = $poll_description;
                        $poll_data['PostAsModuleID'] = 3;
                        $poll_data['PostAsModuleEntityID'] = $user_id;
                        $poll_data['UserID'] = $user_id;
                        $poll_data['IsAnonymous'] = 0;
                        $poll_data['ExpiryDateTime'] = '';
                        $poll_data['ActivityID'] = $activity_id;
                        $this->polls_model->create($poll_data, $poll_options);
                    }
                    if($delete_link) {
                        $this->activity_model->delete_link($activity_id);
                    }

                    if(!$is_edit && $is_city_news == 1) {
                        $this->activity_model->mark_city_news($activity_id);
                    }
                    
                    $subscribe_action = $activity_details['subscribe_action'];
                    $StatusID = $status;
                    ...
                    if (!empty($media)) {
                        ...
                        $album_id = get_album_id($media_user_id, $album_name, $module_id, $module_entity_id);
                        $this->media_model->updateMedia($media, $activity_id, $media_user_id, $album_id, TRUE, 3, $status, $commentable);                        
                    }

                    //Update FIles for Activity
                    if (!empty($files)) {
                        $check_media_status = TRUE;
                        $album_name = DEFAULT_FILE_ALBUM;
                        $album_id = get_album_id($user_id, $album_name, $module_id, $module_entity_id);
                        $this->media_model->updateMedia($files, $activity_id, $user_id, $album_id, '', '', $status);                        
                    }
                    ...
                    if ($module_id == '3' && $user_id != $module_entity_id) {
                        ...
                        $subscribed_owner[0]['ModuleEntityID'] = $module_entity_id;
                        ...
                        if ($status == 2) {
                            initiate_worker_job('send_post_notification', 
                                    array(
                                        ...
                                        'ExcludedUsers' => $activity_details['notify_users'], 
                                        'PostType' => $post_type, 
                    ...
                    if ($module_id != 3) { // Update last activity date
                    ...
                    $return['Data'] = $this->activity_model->getSingleUserActivity($user_id, $activity_id, $all_activity, FALSE, '', '', $role_id);

                    // Update current history
                    $update_history = array();
                    if (!empty($Return['Data'][0]['Album'])) {
                        $media = json_encode($Return['Data'][0]['Album']);
                        $update_history['Media'] = $media;
                    }
                    if (!empty($Return['Data'][0]['Files'])) {
                        $Files = json_encode($Return['Data'][0]['Files']);
                        $update_history['Files'] = $Files;
                    }

                    if (!empty($update_history) && $status == 2) {
                        $this->activity_model->update_row(ACTIVITYHISTORY, array('StatusID' => 2, 'ActivityID' => $activity_id), $update_history);
                    }
                } else {
    ...
    public function getAllComments_post() {
        ...
        if ($this->form_validation->run('api/activity/getAllComments') == FALSE) { // for web
            ...
            $only_solutions = !empty($Data['OnlySolutions']) ? $Data['OnlySolutions'] : 0;
            ...
            $PostAsModuleEntityID = ($PostAsModuleEntityGUID) ? get_detail_by_guid($PostAsModuleEntityGUID, $PostAsModuleID) : $user_id;
            ...
            switch ($entity_type) {
                ...
                default:
                    ...
                    if ($EntityDetails['ModuleID'] == 34) {
                        $permissions = $this->forum_model->check_forum_category_permissions($user_id, $EntityDetails['ModuleEntityID'], FALSE);
            ...
            if ($EntityDetails || $entity_type == 'RATING') {
                ...
                $include_reply = safe_array_key($Data, 'ir', 0);
                if($this->IsApp==1) {
                    $include_reply = 1;
                }
                $this->user_model->set_friend_followers_list($user_id);
                ...
                $return['Data'] = $this->activity_model->getActivityComments($entity_type, $EntityID, $PageNo, $PageSize, $user_id, $IsOwner, 2, $Flag, $blocked_users, FALSE, '', $PostAsModuleID, $PostAsModuleEntityID, "", $parent_comment_id, $filter, $include_reply, $only_solutions);
                $return['TotalRecords'] = $this->activity_model->getActivityComments($entity_type, $EntityID, '', '', $user_id, $IsOwner, 2, FALSE, $blocked_users, TRUE, '', $PostAsModuleID, $PostAsModuleEntityID, "", $parent_comment_id, $filter, $include_reply, $only_solutions);
                $return['TotalReplies'] = $this->activity_model->getActivityComments($entity_type, $EntityID, '', '', $user_id, $IsOwner, 2, FALSE, $blocked_users, TRUE, '', $PostAsModuleID, $PostAsModuleEntityID, "", $parent_comment_id, $filter, $include_reply, $only_solutions, 1);
            } else {
                $return['ResponseCode'] = self::HTTP_PRECONDITION_FAILED;
    ...
    public function removeActivity_post() {
        ...
        if ($this->form_validation->run('api/activity/removeActivity') == FALSE) { // for web
            ...
            if (empty($activity_id)) {
                ...
                $result = $this->activity_model->removeActivity($activity_id, $user_id, 0, $reason);
                $return['Message'] = $result['Message'];
                $return['ResponseCode'] = $result['ResponseCode'];
            }
        }
    ...
    public function addComment_post() {
        ...
        if ($this->form_validation->run('api/activity/addComment') == FALSE) { // for web
            ...
            $stripped_content = strip_tags($Data['Comment'], '<img><iframe>');
            if(empty($stripped_content) && empty($Data['Media'])) {                
                $return['ResponseCode'] = self::HTTP_PRECONDITION_FAILED;
            ...
            $links = isset($Data['Links']) ? $Data['Links'] : array();
            $delete_link    = isset($Data['DeleteLink']) ? $Data['DeleteLink'] : 0;
            if($delete_link && $comment_id) {
                $this->activity_model->delete_comment_link($comment_id);
            }
            ...
            $CommentData = $this->activity_model->addComment($EntityGUID, $Comment, $Media, $UserID, $is_media_exists, $SourceID, $DeviceID, $entity_owner, $entity_type, $PostAsModuleID, $PostAsModuleEntityID, $parent_comment_id, $is_anonymous, $comment_id, $taged_user, $links);
    ...
    public function deleteComment_post() {
        ...
        if ($this->form_validation->run('api/activity/deleteComment') == FALSE) { // for web
            ...
            $entity_type = isset($data['EntityType']) ? $data['EntityType'] : 'ACTIVITY';
            $reason = isset($data['Reason']) ? strip_tags($data['Reason']) : '';
            $result = $this->activity_model->deleteComment($comment_guid, $user_id, $entity_type, $reason);
            $return['Message'] = $result['Message'];
            $return['ResponseCode'] = $result['ResponseCode'];
        }
        $this->response($return);
    ...
    public function getLikeDetails_post() {
        ...
        if ($this->form_validation->run('api/activity/getLikeDetails') == FALSE) { // for web
            ...
            $blocked_users = $this->activity_model->block_user_list($user_id);
            $return['Data'] = $this->activity_model->getLikeDetails($EntityGUID, $entity_type, $blocked_users, $PageNo, $PageSize, FALSE, $user_id);
            $return['TotalRecords'] = $this->activity_model->getLikeDetails($EntityGUID, $entity_type, $blocked_users, 0, 0, TRUE, $user_id);
            if (!$return['Data']) {
                $return['Data'] = array();
            }
        }
    ...
    public function autoSuggestPostOwner_get() {
        ...
        $Result = $this->activity_model->auto_suggest_post_owner($UserID, $SearchKey, $ModuleID, $ModuleEntityID, $AllActivity, $FilterType);
        $return['Data'] = $Result;
        $this->response($return);
    }
    ...
    public function pin_to_top_post() {return;
        $return = $this->return;
        $data = $this->post_data;
        $entity_guid = isset($data['EntityGUID']) ? $data['EntityGUID'] : '';
        ...
        if ($this->form_validation->run('api/share_post_by_email') == FALSE) {
            ...
            if (count($emails) > 0) {
                foreach ($emails as $mail) {
                    ...
                    $emailDataArr['Subject'] = ucfirst($this->LoggedInName) . " share with you";
    ...
    public function visibility_post() {
        $return = $this->return;
        $data = $this->post_data;
        if (isset($data)) {
            $config = array(
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if($activity_id) {
                    ...
                    if ($page_no == '1') {
                        $return['TotalRecords'] = $this->activity_model->visibility($activity_id, $page_no, $page_size, TRUE);
                    }
            
                    $return['Data'] = $this->activity_model->visibility($activity_id, $page_no, $page_size);
                } else {
                    $return['ResponseCode'] = self::HTTP_PRECONDITION_FAILED;
                    $return['Message'] = sprintf(lang('valid_value'), "activity guid");
                }
    ...
    /**
     * Used to get activity short link
     */
    public function short_link_post() {
        ...
        if (isset($data)) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                // $short_url = '';
                $short_url = '';
                $activity_details = get_detail_by_guid($data['ActivityGUID'], 0, 'ActivityID, ActivityGUID,PostTitle,PostContent,UserID', 2);
                // $activity_details = $this->activity_model->get_activity_details($activity_id, 1);
                if($activity_details) {
                    ...
                    $short_url = SHARE_URL.$short_url;
                    /*
                    if (CACHE_ENABLE) {           
                        $short_url = $this->cache->get('asl'.$activity_id);            
                    }
                    if(empty($short_url)) {
                         $long_url = DOMAIN."/activity/".$data['ActivityGUID'];                      

                         $short_url = get_short_url($long_url);
                        if (CACHE_ENABLE) {
                            $this->cache->save('asl'.$activity_id, $short_url, 2592000);
                        }
                    }   
                    */                
                } else {
    ...
    /**
     * Used to get deleted post
     */
    public function get_deleted_activity_post() {
        $return = $this->return;
        $data = $this->post_data;
        $user_id = $this->UserID;
        if (isset($data)) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $activity_id = get_detail_by_guid($data['ActivityGUID']);
                if($activity_id) {
                    $comment_guid = safe_array_key($data, 'CommentGUID', '');                    
                    $comment_id = 0;
                    if (!empty($comment_guid)) {
                        $comment_id = get_detail_by_guid($comment_guid, 20);
                    }
                    $return['Data'] = $this->activity_model->get_deleted_activity($user_id, $activity_id, $comment_id);                     
                } else {
                    $return['ResponseCode'] = self::HTTP_PRECONDITION_FAILED;
                    $return['Message'] = sprintf(lang('valid_value'), "activity guid");
                }
    ...
    public function show_daily_digest_post() {
        $return = $this->return;
        $row = $this->activity_model->is_daily_digest_exist();
    ...
    public function idea_for_better_indore_post() {
        ...
        if (isset($data)) {  
            ...
            $data['UserID'] = $user_id;
            $data['CountOnly'] = 0;
            $return['Data'] = $this->activity_model->idea_for_better_indore($data);            

            if (count($return['Data']) == 0 || count($return['Data']) < $data['PageSize']) { //check for archival script.
                $this->activity_model->load_archive_db_instance();     
                $data['CountOnly'] = 1;           
                $ar_cnt = $this->activity_model->idea_for_better_indore($data);
