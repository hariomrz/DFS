...
class Ward extends Common_API_Controller {
    ...
    public function list_post() {
        ...
        $data = $this->post_data;
        if (isset($data)) {            
            $return['Data'] = $this->ward_model->get_ward_list($data);
        } else {
    ...
    /**
     * Function Name: trending ward list
     */
    public function get_trending_ward_list_post() {
        $return = $this->return;
        $data = $this->post_data;
        if (isset($data)) {            
            $return['Data'] = $this->ward_model->get_trending_ward_list($data);
        } else {
          $return['ResponseCode'] = self::HTTP_BAD_REQUEST;
          $return['Message'] = lang('input_invalid_format');
        }    
        ...
        */
        ...
        if (isset($data)) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $pinned = $this->ward_model->get_pinned_feature_user_old($data);
                if(!empty($pinned)) {
