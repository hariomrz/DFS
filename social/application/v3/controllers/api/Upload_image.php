...
class Upload_image extends Common_API_Controller {
    ...
    public function index_post() {
        ...
        if ($this->form_validation->run() == FALSE && 0) { 
            ...
            $generateThumb = 0;
            $Data['Type'] = isset($Data['Type'])?$Data['Type']:'wall';
            ...
            if(isset($Data['IsDocument'])) {
                ...
                if($Data['Type'] == 'temp'){
                   ...
                   if ((function_exists("gearman_version") && JOBSERVER == "Gearman") || (extension_loaded('redis') && JOBSERVER == "Rabbitmq")) {
                        ...
                        {   
                            ...
                            $media_data['MediaSectionID'] = $Result['Data']['MediaSectionID'];
                            ...
                            unset($Result['Data']['MediaSectionID']);
                        }
                    } else{
                        $generateThumb = 1;
                        $Result = $this->upload_file_model->uploadImage($Data,$generateThumb);
    ...
    function updatePictureWithoutId_post(){
        ...
        $validation_rule = array(
                                ...
                                array(
                                    'field' => 'MediaGUID',
                                    'label' => 'media guid',
                                    'rules' => 'trim|required|validate_guid[21]'
                                ),
                                array(
                                    'field' => 'ImageData',
                                    'label' => 'Image data',
        ...
        if ($this->form_validation->run() == FALSE) { 
            ...
            $data['Type']        = safe_array_key($data, 'Type', 'profile');
            $data['Type']        = strtolower($data['Type']);  
            $data['CropSize']        = safe_array_key($data, 'CropSize', '');
            $data['IsMediaExisted']  = safe_array_key($data, 'IsMediaExisted', 0);
            $data['ImageName']  = safe_array_key($data, 'ImageName');
            $file_allowed       = array('png','jpg','jpeg','PNG','JPG','JPEG','GIF','gif');
            $image_data         = $data['ImageData'];            
            
            foreach($file_allowed as $farr){
               $image_data = str_replace('data:image/'.$farr.';base64,', '', $image_data);
            }

            $data['ImageData'] = $image_data;            
            $data['FilePath'] = PATH_IMG_UPLOAD_FOLDER.$data['Type'].'/';
            $data['ImageName'] = explode('?', $data['ImageName']);
            $data['ImageName'] = $data['ImageName'][0];

            $this->upload_file_model->uploadProfilePicture($data,$user_id);
        }
        $this->response($Return);
    }
