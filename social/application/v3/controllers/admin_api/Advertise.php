...
class Advertise extends Admin_API_Controller {
    ...
    */
    ...
    public function save_banner_post() {
        $Return['ResponseCode'] = '200';
        $Return['Message'] = lang('success');
        $Return['Data'] = array();
        $Data = $this->post_data;
        ...
        if (!empty($Data)) {
            ...
            if(empty($rawImageBanner)){
                $Return['ResponseCode'] = 511;
                $Return['Message'] = 'Please select banner image';
                return $this->response($Return);
            }

            $fieldArray['Type'] = 'banner';
            $fieldArray['CreatedBy'] = $this->UserID;
            $fieldArray['BlogUniqueID'] = '';
            $fieldArray['BlogTitle'] = '';
            $fieldArray['Advertiser'] = 'bhopu';
            $fieldArray['BannerSource'] = 1;
            $fieldArray['BlogDescription'] = '';

            // Save advertiser data in advertiser master
            $AdvertiserID = $this->advertise_model->save_advertiser('bhopu');

            if ($rawImageBanner) {
                $imageArr = $this->rawImage_convert($rawImageBanner);                    
                $this->load->model('vsocial_model');
                $this->vsocial_model->save_banner_media($this->UserID,$imageArr['Data']['image_name'], $AdvertiserID, '', $SourceID=1, $DeviceID=1, $BannerSize);                    
                $fieldArray['BlogImage'] = $imageArr['Data']['image_name'];
                $fieldArray['BannerSize'] = $BannerSize;
            } 
            ...
            $BlogID = $this->advertise_model->create_banner($fieldArray);   
            if (CACHE_ENABLE) {
                $this->cache->delete('fban');
            }
            $Return['Message'] = 'Banner has been saved successfully';
    ...
    */
    ...
    public function rawImage_convert($image) {
        ...
        if ($image != '') {
            ...
            if (strtolower(IMAGE_SERVER) == 'remote') {//if upload on s3 is enabled 
                ...
                $is_s3_upload = $s3->putObjectFile($image_path, BUCKET, $imageServerPathRemote, S3::ACL_PUBLIC_READ, $headers);
    ...
    public function banner_status_post() {
        ...
        if (isset($Data) && $Data != NULL) {
            ...
            if($Data['Status'] == 3){
                $Return['Message'] = 'Banner has been deleted successfully';
            } else {
                $Return['Message'] = 'Banner status has been changed successfully';
            }
            if (CACHE_ENABLE) {
                $this->cache->delete('fban');
            }
