    ...
    */
    ...
    public function download_users_post()
    {
        ...
        if(isset($Data) && $Data!=NULL )
        {
            ...
            $result = $this->users_model->downloadExcelFile($excelArr);            
            if($result){
    ...
    public function sub_admin_list_post()
    {   
        $return['ResponseCode']='200';
        $return['Message']= lang('success');
        $return['ServiceName']='admin_api/users/sub_admin_list';
        $return['Data']=array();        
        $Data = $this->post_data;
        ...
        $order_by 	= isset($Data['OrderBy']) ? $Data['OrderBy'] : 'DESC' ;
        $user_id    = $this->UserID;
        $return['Data'] = $this->users_model->sub_admin_list($page_no,$page_size,$sort_by,$order_by, 0, $user_id);
        $return['TotalRecords'] = $this->users_model->sub_admin_list($page_no, $page_size, $sort_by, $order_by, 1, $user_id);        
    ...
    public function dummy_user_search_post()
    {
        ...
        $page_size = !empty($data['PageSize']) ? $data['PageSize'] : 20;
        $search = isset($data['SearchKeyword']) ? $data['SearchKeyword'] : '' ;
        
        $return['Data'] = $this->users_model->get_users($page_no, $page_size, true,$search);
    ...
    public function get_communication_post()
    {
        $return = $this->return;       
        $data = $this->post_data;
        if (isset($data)) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $return['Data'] = $this->communication_model->get_history($data);
                if (!empty($return['Data']))
                {
                    foreach ($return['Data'] as $key => $value)
                    {
                        $return['Data'][$key]['Content'] = json_decode($value['Content']);
                    }
                }
                $return['TotalRecords'] = 0;
                if($page_no == 1) {
                    $return['TotalRecords'] = $this->communication_model->get_history($data,true);
                }                
            }
        } else {
    ...
    public function get_random_dummy_details_post()
    {
        ...
        /**
     * [mark_user_as_vip Used to save vip user]
     * @return [array] [Response details]
    */
    function mark_user_as_vip_post() {
        $return = $this->return;
        $data = $this->post_data;
        if (isset($data)) {
            $validation_rule = array(
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $this->users_model->save_vip_user($data);
                $return['Message'] = 'User marked as VIP successfully.';                
            }
        } else {
            $return['ResponseCode'] = self::HTTP_BAD_REQUEST;
            $return['Message'] = lang('input_invalid_format');
    ...
    /**
     * [remove_user_as_vip Used to remove vip user]
     * @return [array] [Response details]
    */
    function remove_user_as_vip_post() {
        $return = $this->return;
        $data = $this->post_data;
        if (isset($data)) {
            $validation_rule = array(
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $user_id = $data['UserID'];                          
                $this->users_model->remove_vip_user($user_id);                        
                $return['Message'] = 'User removed as VIP successfully.';               
            }
        } else {
            $return['ResponseCode'] = self::HTTP_BAD_REQUEST;
            $return['Message'] = lang('input_invalid_format');
    ...
    /**
     * [mark_user_as_association Used to save association user]
     * @return [array] [Response details]
    */
    function mark_user_as_association_post() {
        $return = $this->return;
        $data = $this->post_data;
        if (isset($data)) {
            $validation_rule = array(
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $this->users_model->save_association_user($data);
                $return['Message'] = 'User marked as association successfully.';                
            }
        } else {
            $return['ResponseCode'] = self::HTTP_BAD_REQUEST;
            $return['Message'] = lang('input_invalid_format');
    ...
    /**
     * [remove_user_as_association Used to remove association user]
     * @return [array] [Response details]
    */
    function remove_user_as_association_post() {
        $return = $this->return;
        $data = $this->post_data;
        if (isset($data)) {
            $validation_rule = array(
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $user_id = $data['UserID'];                          
                $this->users_model->remove_association_user($user_id);                        
                $return['Message'] = 'User removed as association successfully.';               
            }
        } else {
            $return['ResponseCode'] = self::HTTP_BAD_REQUEST;
            $return['Message'] = lang('input_invalid_format');
    ...
    /**
     * [get_team_member Used to get team member list]
     * @return [array] [Response details]
    */
    function get_team_member_post() {
        $return = $this->return;
        $data = $this->post_data;
        if (isset($data)) {                        
            $return['Data'] = $this->users_model->get_team_member();
        } else {
            $return['ResponseCode'] = self::HTTP_BAD_REQUEST;
            $return['Message'] = lang('input_invalid_format');
        }
