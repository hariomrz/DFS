...
class Cron extends Common_Controller {
    ...
    function create_image_thumb() { die;
    ...
    function update_user_privacy($key)  { die;
        $this->cron_model->update_user_privacy($key);
    }

    public function do_action() { die;
    ...
    public function remove_unused_tags() { die;
        //remove tags which are not associated with any entity        
        $this->cron_model->remove_unused_tags();
    }
    ...
    public function mailchimp_compaign_report_synch() { die;
    ...
    public function send_new_release_notification() {die;
        ...
        if ($Query->num_rows() > 0) {
            foreach ($Query->result_array() as $Notifications) { 
                //$token = 'eKkU6ggGtlM:APA91bG08aINv6UXbtAo_ghqhPVjx6NUdpShg10lkW0jQLQrjQpJGgeBsvhSgcKJAX0LvX6SmvXh7rMMyGPtOi_hcKyMeH4MwnleY4B61zLoK2j8V0Srv2a9mlBOqBufAf4PSQP51sN_';
    ...
    public function send_message_to_all_user() {     die;
    ...
    public function calculate_top_contributor() {
        ...
        //$this->load->model(array('log/user_activity_log_score_model'));
        //$this->user_activity_log_score_model->calculate_top_contributor(array('all' => $all));
    }

    public function make_old_user_followed_by_admin()
    {
        $this->load->model('user/user_model');
        $this->load->model('follow/follow_model');
        $chunks = $this->user_model->get_all_users();
        $chunks = array_chunk($chunks,300);
        foreach($chunks as $chunk){
            foreach($chunk as $key=>$user)
            {
                $follow_data = array('FollowingID' => getenv('ADMIN_USER_ID'), 'UserID' => $user['UserID']);
