...
function bitly_get_curl($uri) {
  ...
  try {
    ...
    $output = curl_exec($ch);
    if ($output === FALSE) {
        log_message('error', 'Bitly Get Curl: ' . curl_error($ch));
    }
  } catch (Exception $e) {
    log_message('error', 'Error Bitly Get Curl - ' . $e->getMessage());
  }
  return $output;
}
...
function bitly_post_curl($uri, $fields, $header_array = array()) {
  ...
  try {
      ...
      $output = curl_exec($ch);
      if ($output === FALSE) {
          log_message('error', 'Bitly Post Curl: ' . curl_error($ch));
      }
  } catch (Exception $e) {
    log_message('error', 'Error Bitly Post Curl - ' . $e->getMessage());
  }
  return $output;
}
}