...
class Phpthumb {
	...
	// public:
	function GenerateThumbnail($filePath='',$local_path='',$h='',$w='',$is_original=true) {
...
//		}
		...
		if($gif && extension_loaded('imagick'))
		{
			$this->gifResize($filePath,$local_path,$h,$w);
		}
		else
		{
	...
	function gifResize($file_origin,$file_dest,$size_h,$size_w){
	   	$crop_w = 0;
	   	$crop_h = 0;
	   	$crop_x = 0;
	   	$crop_y = 0;
		...
		//$file_dest = '/var/www/html/inclusify/upload/wall/220x220/6a5b1dd02f6ea6e10e6e2763a9c8c335a.gif';
	...
	function RenderToFile($filename) {
		...
		if ($this->RenderOutput()) {
			$ext = explode('.', $renderfilename);
			$ext = end($ext);
			if($ext == 'gif' && extension_loaded('imagick'))
			{
				$image = new Imagick($renderfilename);
			   	$imageContent = $image->getImagesBlob();
			   	
				if (file_put_contents($renderfilename, $imageContent)) {
...
//echo $maxwidth.'x'.$maxheight.'<hr>';
	...
	function ExtractEXIFgetImageSize() {
		...
		$this->source_width  = isset($this->getimagesizeinfo[0]) ? $this->getimagesizeinfo[0] : 0;
		$this->source_height = isset($this->getimagesizeinfo[1]) ? $this->getimagesizeinfo[1] : 0;

		$image_type = isset($this->getimagesizeinfo[2]) ? $this->getimagesizeinfo[2] : 0;
		...
		if (function_exists('exif_thumbnail') && ($image_type == IMAGETYPE_JPEG)) {
