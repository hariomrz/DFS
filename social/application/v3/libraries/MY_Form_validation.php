...
class MY_Form_validation extends CI_Form_validation {
    ...
    // Callback function to validate entity guid
    function validate_entity_id($entity_id, $entity_type="ACTIVITY")
    {       
        if(!empty($entity_id))
        {
            $module_id = '' ; 
            $entity_type = $_POST[$entity_type];
            //echo $entity_id.", ".$entity_type;die;
            switch ($entity_type) 
            {
                case 'ACTIVITY':
                    $module_id = 0;
                ...
                case 'PAGE':
                    ...
                    break;            
                default:
                    $this->set_message('validate_entity_id', lang('valid_value'));
                    return false;
                    break;
            }
            
            $id = get_detail_by_id($entity_id,$module_id);
            if(!empty($id))
            {
                return true;
            }
