	 ...
	 if (!$this->enable)
        ...
        try
        {
            ...
            if (isset($this->config[$this->activate]['no_auth']) && $this->config[$this->activate]['no_auth']) {
                ...
                if($this->config[$this->activate]['SRV'])
                {
                    $dns = "mongodb+srv://{$this->username}:{$this->password}@{$this->hostname}/{$this->database}?retryWrites=true&w=majority";
                }
                else
                {
