...
class Announcement_model extends Common_Model
{
    ...
    public function announcement_list($search_keyword = "", $count_flag = FALSE, $page_no = 1, $page_size = PAGE_SIZE, $sort_by = "CreatedDate", $order_by = "DESC", $list_type = '')
    {
        ...
        $this->db->select('B.BlogID, B.Status, B.CanIgnore, B.BlogGUID, IFNULL(B.Description,"") AS Description', FALSE);
        ...
        if (!empty($res))
        {
            foreach ($res as $key => $value) {
                ...
                if(!empty($value['MediaID'])){
                    $value['ImageName'] = get_detail_by_id($value['MediaID'], 21,'ImageName');
                }

                if($value['URL']=='POST_TAG') {
                    $value['TagName'] = $this->get_tag_name($value['RedirectTo']);
                } else if($value['URL']=='QUESTION_CATEGORY' || $value['URL']=='CLASSIFIED_CATEGORY') {
                    $value['TagName'] = $this->get_tag_category_name($value['RedirectTo']);
                } else if($value['URL']=='QUIZ') {
                    $value['QuizTitle'] = get_detail_by_guid($value['RedirectTo'], 47, 'Title');
                }
    ...
    public function update_media_status($media_id, $blog_id=0, $status=2) {        
        $this->db->where('MediaID',$media_id);
        $update_date = array();
        $update_date['StatusID'] = $status;
        if($blog_id) {
            $update_date['MediaSectionReferenceID'] = $blog_id;
        }
        
        $this->db->update(MEDIA,$update_date);  
        return true;
    }
