 ...
 * 
 */
class Daily_digest_activity_model extends Admin_Common_Model {
    ...
    public function get_activity_list($page_no = 1, $page_size = 10, $filter = array(), $count_only = false) {
        ...
        $daily_digest_date = isset($filter['DailyDigestDate']) ? $filter['DailyDigestDate'] : '';
        $daily_digest_date = $this->format_dob($daily_digest_date); 
        $start_date = isset($filter['StartDate']) ? $filter['StartDate'] : '';

        
        $this->db->select(implode(',', $select_array), false);
        $this->db->select("IFNULL(DDG.Description, '') AS Description, IFNULL(DDG.IsFeatured, 0) AS ShowImage, IFNULL(DDG.DailyDigestID, 0) AS DailyDigestID");
        if(empty($start_date) && !empty($daily_digest_date) && $daily_digest_date!='0000-00-00') {            
            $this->db->from(DIALYDIGEST . ' DDG ');
            $this->db->join(ACTIVITY . ' AATC ', 'AATC.ActivityID = DDG.ActivityID');
            $this->db->where('DDG.DailyDigestDate', $daily_digest_date);
        } else {
            $this->db->from(ACTIVITY . ' AATC ');
            $this->db->join(DIALYDIGEST . ' DDG ', 'DDG.ActivityID = AATC.ActivityID AND DDG.DailyDigestDate="'.$daily_digest_date.'"', 'left');
        }
    ...
    protected function get_select_fields_activity_details($entity, $pefix_tbl, $get_entity_details = false) {
        $activity_detials = array(
            ...
            'ShowImage' => ($entity['ShowImage']) ? $entity['ShowImage'] : 0,
            'DailyDigestID' => ($entity['DailyDigestID']) ? $entity['DailyDigestID'] : 0,
        );
    ...
    function save_daily_digest($data) {
        $daily_digests = safe_array_key($data, 'DailyDigest', array());
        $status = safe_array_key($data, 'Status', 2);
        $daily_digest_date = safe_array_key($data, 'DailyDigestDate');
        $daily_digest_date = $this->format_dob($daily_digest_date); 
        if($daily_digest_date && $daily_digest_date!='0000-00-00') {
            $current_date = get_current_date('%Y-%m-%d %H:%i:%s');
            $insert_data = array();
            foreach($daily_digests as $daily_digest) {
                $activity_id = $daily_digest['ActivityID'];
                $description = $daily_digest['Description'];
                $is_featured = $daily_digest['IsFeatured'];

                $insert_data[] = array(
                    'ActivityID' => $activity_id,
                    'Description' => $description,
                    'IsFeatured' => $is_featured,
                    'DailyDigestDate' => $daily_digest_date,
                    'StatusID' => $status,
                    'CreatedDate' => $current_date
                );
            }
            if ($insert_data) {
                $this->db->where('DailyDigestDate', $daily_digest_date);
                $this->db->delete(DIALYDIGEST);
                
                $this->db->insert_batch(DIALYDIGEST, $insert_data);
            }
        }
    }
    
    function get_daily_digest_list($data) {
        $page_no    = safe_array_key($data, 'PageNo', 1);
        $page_size  = safe_array_key($data, 'PageSize', 20);

        $this->db->select('DailyDigestDate, StatusID AS Status');
        $this->db->from(DIALYDIGEST);
        $this->db->group_by('DailyDigestDate');     
        $this->db->order_by('DailyDigestDate', 'DESC');
    ...
    function delete_daily_digest($data) {
        $daily_digest_date = safe_array_key($data, 'DailyDigestDate');
        if($daily_digest_date && $daily_digest_date!='0000-00-00') {
            $this->db->where('DailyDigestDate', $daily_digest_date);
            $this->db->delete(DIALYDIGEST);
    ...
    function daily_digest_notification($data) {

        $this->db->select("DNL.ID");
        $this->db->from('DailyDigestNotificationLog DNL');
        $this->db->where("DATE(DNL.CreatedDate)='" . get_current_date('%Y-%m-%d') . "'");
        $query = $this->db->get();
        //echo $this->db->last_query();die;
        if ($query->num_rows() > 0) {
            return true;
            ...
            $this->db->insert('DailyDigestNotificationLog', $log_data);            
        }

        /* This is for topic message */
       /* $notification_text = !empty($data['notification_text']) ? $data['notification_text'] : '';   
        $notification_text = trim(strip_tags($notification_text));
        $push_notification = array("EntityID" => "", "ModuleID" => "", "ModuleEntityGUID" => "", "Refer" => "DAILY_DIGEST", "EntityGUID" => "");
        $notification_data = array();
        $notification_data['PushNotification'] = $push_notification;               
        $notification_data['Summary'] = $notification_text;
        $message = 'Aaj ki Chai';
        send_topic_push_notification('daily_digest', $message, 1, $notification_data);
        return true;
        */
        ...
        if ($query->num_rows() > 0) {    
            ...
            $queue_name = 'daily_digest_notification';
            $i = 0;
            foreach ($results as $Notifications) {
                ...
                if($i == 0 || $i == 3) {
                    ...
                    $queue_name = 'daily_digest_notification'.$i;
                }

                initiate_worker_job('daily_digest_notification', $data, '', $queue_name);
