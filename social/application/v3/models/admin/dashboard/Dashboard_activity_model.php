...
class Dashboard_activity_model extends Admin_Common_Model {
    ...
    public function get_activity_list($page_no = 1, $page_size = 10, $filter = array(), $count_only = false) {
        ...
        $exclude_activity_id = isset($filter['ActivityID']) ? $filter['ActivityID'] : 0;
        ...
        $this->db->where_not_in('AATC.ActivityTypeID',array(19, 20, 23, 36, 37, 50));

        if(!empty($exclude_activity_id)) {
            $this->db->where('UAL.ActivityID !=', $exclude_activity_id);
        }
    ...
    public function get_user_details($user_id) {
        ...
        $select_array[] = "CONCAT(U.FirstName, ' ', U.LastName) AS Name, IFNULL(U.ProfilePicture, '') AS ProfilePicture, U.Verified, IFNULL(U.Gender, '') Gender, DATE_FORMAT(U.CreatedDate, '$mysql_date') AS MemberSince,U.StatusID,U.UserGUID, U.Email, U.IsVIP, U.IsAssociation";
        $select_array[] = "IFNULL(UD.Occupation, '') AS Occupation";
        $select_array[] = "IFNULL(UD.UserWallStatus, '') AS About, UD.TotalFollowing, UD.TotalFollowers"; 
        ...
        $select_array[] = "UD.PostCount, UD.CommentCount AS CommnetCount";
        ...
        $select_array[] = "UD.IncomeLevel, UD.NoOfFriendsFB, UD.NoOfFollowersFB, UD.NoOfFollowersTw, UD.NoOfConnectionsIn, U.UserID, 3 AS ModuleID, UD.LocalityID";
...
*/
    ...
    public function get_entity_tags($entity_id, $entity_type, $page_size = 0, $offset = 0, $added_by = NULL) {
        ...
        $this->db->join(TAGS . ' TG', 'TG.TagID = ET.TagID');
        ...
        $tag_list_by_type = array(
            'Normal' => [], 'User_ReaderTag' => [], 'UserProfession' => []
        ); //'Hash' => [], 'ActivityMood' => [], 'ActivityClassification' => [], 'Brand' => [] 
        $tag_types = array(
            1 => 'Normal',  5 => 'User_ReaderTag', 6 => 'UserProfession'
        ); //2 => 'Hash', 3 => 'ActivityMood', 4 => 'ActivityClassification', 7 => 'Brand'
        if($entity_type == "ACTIVITY") {
            ...
            $tag_list_by_type['Question'] = array('Name'=>'question', 'text'=>'question', 'TagID'=>$question_tag_id, 'TagType'=>1, 'AddedBy'=>1, 'IsExist' => 0, 'CCategory' => []);
            $tag_list_by_type['Custom'] = array('Name'=>'classified', 'text'=>'classified', 'TagID'=>$classified_tag_id, 'TagType'=>1, 'AddedBy'=>1, 'IsExist' => 0, 'CCategory' => []);        
        }
        
        foreach ($tag_entities as $tag_entity) {
            if($tag_entity['TagType'] == 1 && $tag_entity['TagID']==$question_tag_id) {
                $tag_list_by_type['Question']['IsExist'] = 1;
                $tag_list_by_type['Question']['CCategory'] = array();//$this->tag_model->get_entity_tag_category($entity_type, $entity_id, $question_tag_id);
                continue;
            }
            if($tag_entity['TagType'] == 1 && $tag_entity['TagID']==$classified_tag_id) {
                $tag_list_by_type['Custom']['IsExist'] = 1;
                $tag_list_by_type['Custom']['CCategory'] = $this->tag_model->get_entity_tag_category($entity_type, $entity_id, $classified_tag_id);
                continue;
            }
    ...
    protected function set_entity_join($filter) {
        ...
        $this->db->join(QUIZ . ' QUZ_PRL ', 'QUZ_PRL.QuizID = UAL.ModuleEntityID  AND UAL.ModuleID = 47 ', 'left');
    ...
    protected function set_entity_conds($filter) {
        ...
        $activity_type_filter = isset($filter['ActivityTypeFilter']) ? $filter['ActivityTypeFilter'] : 0;
        ...
        if($is_verified != 2) {
            $this->db->where(" IF(UAL.ActivityTypeID != 20, (AATC.Verified IS NULL OR AATC.Verified = $is_verified), 1)", NULL, FALSE);
        }
        
        if($is_verified != 2) {
            $this->db->where(' IF(UAL.ActivityTypeID = 20, ( (PCE.Verified IS NULL OR PCE.Verified = '.$is_verified.') AND ( PCE.StatusID IS NULL OR  PCE.StatusID = 2) ), 1) ', NULL, FALSE);
        } else {
            $this->db->where('( ( PCE.StatusID IS NULL OR  PCE.StatusID = 2) ) ', NULL, FALSE);
        }
        if($lastLogID) {
        ...
        if ($GetEntityType == 'ALL') {
            ...
            /* if(ENVIRONMENT=="production") {
                ...
                $not_allowed_activity_types = array(
                    2, 3, 4, 13, 18, 19, 21, 22, 27, 28, 29, 30, 31, 32, 33
                );
            }
            */
            $not_allowed_activity_types = array(
            ...
            if($activity_type_filter == 1) {
                $not_allowed_activity_types = array(
                    2, 3, 4, 13, 18, 19, 20, 21, 22, 27, 28, 29, 30, 31, 32, 33
                );
            }
            if($activity_type_filter == 2) {
                $not_allowed_activity_types = array(
                    1, 8, 2, 3, 4, 13, 18, 19, 21, 22, 27, 28, 29, 30, 31, 32, 33
                );
            }
        ...
        $entities_deleted_conds = "
            IF(AATC.ActivityID != 0, 
                ...
                (EFC.StatusID IS NULL OR EFC.StatusID = 2) AND
    ...
    protected function get_select_arr($filter, $page_no) {
        ...
        $select_array[] = $this->get_entity_details_select(47, 'QUZ_PRL');  //Activity QUIZ
    ...
    protected function apply_filter($filter) {
       ...
       // $end_date = isset($filter['EndDate']) ? $filter['EndDate'] : '';
        ...
        $user_id = isset($filter['UserID']) ? $filter['UserID'] : 0;
    ...
    protected function iterate_entities($entities, $filter) {
        ...
        foreach ($entities as $entity) {
            ...
            $activity = $this->get_select_fields_activity_details($entity, 'AATC', TRUE);
            

            $popup_data = $this->activity_front_helper_model->get_notification_popup_data($activity['ActivityID'], $activity['UserID']);
            ...
            //$activity['notificationTitle']  = $popup_data['notificationTitle'];
           ...
           // }
            ...
            $activity['EditPostContent'] = nl2br($activity['EditPostContent']);
            $activity['OriginalContent'] = nl2br($activity['OriginalContent']);

            $new_entity['activity'] = $activity;
    ...
    protected function get_user_details_select($pefix_usr_tbl, $prefix_url_tbl) {
        return "CONCAT($pefix_usr_tbl.FirstName, ' ', $pefix_usr_tbl.LastName) AS {$pefix_usr_tbl}_UserName, $pefix_usr_tbl.ProfilePicture {$pefix_usr_tbl}_ProfilePicture, "
                . "$pefix_usr_tbl.UserID {$pefix_usr_tbl}_UserID, $pefix_usr_tbl.UserGUID {$pefix_usr_tbl}_UserGUID, $pefix_usr_tbl.IsVIP {$pefix_usr_tbl}_IsVIP, $pefix_usr_tbl.IsAssociation {$pefix_usr_tbl}_IsAssociation, $prefix_url_tbl.Url  {$prefix_url_tbl}_UserProfileURL, {$pefix_usr_tbl}.AccountSuspendTill {$pefix_usr_tbl}_AccountSuspendTill";
    }

    protected function get_activity_details_select($pefix_tbl) {
        return "$pefix_tbl.ActivityID {$pefix_tbl}_ActivityID, $pefix_tbl.PostContent {$pefix_tbl}_PostContent, $pefix_tbl.OriginalContent {$pefix_tbl}_OriginalContent, $pefix_tbl.ActivityTypeID {$pefix_tbl}_ActivityTypeID, "
    ...
    protected function get_comment_details_select($pefix_tbl) {
        return "$pefix_tbl.NoOfLikes {$pefix_tbl}_NoOfLikes, $pefix_tbl.NoOfReplies {$pefix_tbl}_NoOfReplies, $pefix_tbl.PostComment {$pefix_tbl}_PostComment, "
                ...
                . "$pefix_tbl.IsFileExists {$pefix_tbl}_IsFileExists, $pefix_tbl.Verified {$pefix_tbl}_Verified, $pefix_tbl.Solution {$pefix_tbl}_Solution";
    }
    ...
    protected function get_select_fields_user_details($entity, $pefix_usr_tbl, $prefix_url_tbl) {
        return array(
            ...
            'IsVIP' => !empty($entity[$pefix_usr_tbl . '_IsVIP']) ? $entity[$pefix_usr_tbl . '_IsVIP'] : '',
            'IsAssociation' => !empty($entity[$pefix_usr_tbl . '_IsAssociation']) ? $entity[$pefix_usr_tbl . '_IsAssociation'] : '',
        );
    }
    ...
    protected function get_select_fields_activity_details($entity, $pefix_tbl, $get_entity_details = false) {
        $activity_detials = array(
            ...
            'OriginalContent' => ($entity[$pefix_tbl . '_OriginalContent']) ? $entity[$pefix_tbl . '_OriginalContent'] : '',
        ...
        $activity_detials['IsNotificationSent'] = 0;
        $activity_detials['IsIdea']    = 0;
        $activity_detials['IsRelated'] = 0;
        //$activity_detials['IsCityNews']     = 0;
        if($pefix_tbl != 'PA') {
            $activity_detials['IsPined'] = $this->is_pined($activity_detials['ActivityID']);
            $activity_detials['IsNotificationSent'] = $this->is_notification_sent($activity_detials['ActivityID']);
           
            $activity = $this->activity_front_helper_model->get_activity_details($activity_detials['ActivityID']);
            $activity_detials = array_merge($activity_detials, $activity);

            // $activity_detials['IsCityNews'] = $this->is_city_news($activity_detials['ActivityID']);
        }
        ...
        $activity_detials['EditOriginalContent'] = $this->activity_model->parse_tag_edit($activity_detials['OriginalContent'], $activity_detials['ActivityID']);
        $activity_detials['OriginalContent'] = $this->activity_model->parse_tag($activity_detials['OriginalContent'], $activity_detials['ActivityID']);
    ...
    protected function is_notification_sent($activity_id) {
    ...
    public function is_pined($activity_id) {
        $this->db->select('ActivityID');
        $this->db->from(PINTOTOP);
        $this->db->where('ActivityID', $activity_id);
        $this->db->limit(1);
        $query = $this->db->get();
        if ($query->num_rows()) {
    ...
    protected function is_city_news($activity_id) {
        $this->db->select('ActivityID');
    ...
    function remove_from_city_news($activity_id) {
        $this->db->where('ActivityID', $activity_id);
        $this->db->delete(CITYNEWS);

        $this->db->set('IsCityNews', 0);                                       
        $this->db->where('ActivityID', $activity_id);
    ...
    protected function get_select_fields_comment_details($entity, $pefix_tbl) {
        $comment_details = array(
            ...
            'Verified' => ($entity[$pefix_tbl . '_Verified']) ? $entity[$pefix_tbl . '_Verified'] : '',
        );
    ...
    protected function get_activity_dependent_entities($entities) {
        ...
        $select_array[] = $this->get_entity_details_select(47, 'QUZ');
    ...
    /**
     * [story_visibility Get the story visibility details]
     * @param  [int]       $activity_id   [Activity ID]
     * @return [Array]                    [activity visibility details]
     */
    function story_visibility($activity_id) {        
        ...
        $this->db->from(STORYWARD . ' SW');
        $this->db->join(WARD . ' W', 'W.WardID=SW.WardID');
        $this->db->where('SW.ActivityID', $activity_id);
        $this->db->order_by('W.Number', 'ASC');            
        $query = $this->db->get();
        return $query->result_array();
    }

    function add_story($activity_id, $ward_ids) { 
        $this->db->select('ActivityID');   
        $this->db->from(STORY);       
        $this->db->where('ActivityID', $activity_id);
        $this->db->limit(1);        
        $query = $this->db->get();
        if($query->num_rows() == 0) {
            $this->db->insert(STORY, array('ActivityID' => $activity_id, 'CreatedDate' => get_current_date('%Y-%m-%d %H:%i:%s')));
        }
        $this->add_story_ward($activity_id, $ward_ids);
    }

    function add_story_ward($activity_id, $ward_ids) { 
        if(in_array(1, $ward_ids)) {
            $ward_ids = array(1);
        }
        ...
        if (!empty($activity_wards)) {            
            $this->db->insert_on_duplicate_update_batch(STORYWARD, $activity_wards);
    ...
    function remove_story($activity_id) { 
        $this->db->where('ActivityID', $activity_id);
