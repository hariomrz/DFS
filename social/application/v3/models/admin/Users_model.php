...
class Users_model extends Admin_Common_Model {
    ...
    public function save_vip_user($data) {
        $user_id = $data['UserID'];  
        $about = safe_array_key($data, 'About');

        $this->db->set('IsVIP', 1);
        $this->db->set('IsAssociation', 0);
        $this->db->where('UserID', $user_id);
        $this->db->update(USERS);

        if(!empty($about))  {
            $this->db->set('UserWallStatus', $about);
            $this->db->where('UserID', $user_id);
            $this->db->update(USERDETAILS);
        }
       // $ward_id = $this->get_user_ward_id($user_id);
        if (CACHE_ENABLE) {
            //$this->cache->delete('vpu');
            $this->cache->delete('user_profile_' . $user_id);
        }
    }

    public function remove_vip_user($user_id) {
        $this->db->set('IsVIP', 0);
        $this->db->where('UserID', $user_id);
        $this->db->update(USERS);
    ...
    public function save_association_user($data) {
        $user_id = $data['UserID'];  
        $about = safe_array_key($data, 'About');

        $this->db->set('IsAssociation', 1);
        $this->db->set('IsVIP', 0);
        $this->db->where('UserID', $user_id);
        $this->db->update(USERS);
       ...
       // $ward_id = $this->get_user_ward_id($user_id);
        if (CACHE_ENABLE) {
            //$this->cache->delete('assu');
            $this->cache->delete('user_profile_' . $user_id);
        }
    }

    public function remove_association_user($user_id) {
        $this->db->set('IsAssociation', 0);
        $this->db->where('UserID', $user_id);
        $this->db->update(USERS);
    ...
    public function get_user_ward_id($user_id) {
        $this->db->select('L.WardID');
...
//        $this->subquery->end_subquery('IPAddressCount');
    ...
    function get_users($page_no = 1, $page_size = 20, $only_users = false, $search = '', $is_dummy = true) {
        $users = [];
        ...
        if ($is_dummy) {
            $this->db->join(USERROLES . ' UR', 'UR.UserID = U.UserID AND UR.RoleID IN (1,2,3,6)');
    ...
    /**
     * [sub_admin_list Used to get  user list]
     * @param  [int]    $page_no   [Page number]
     * @param  [int]    $page_size [Page Size]
     * @param  [string] $sort_by   [Sort by]
     * @param  [string] $order_by  [Order By]
     * @return [array]             [User details]
     */
    public function sub_admin_list($page_no = 1, $page_size = 10, $sort_by = 'U.CreatedDate', $order_by = 'DESC', $count_only = false, $user_id=0) {
        
        $this->db->select("U.FirstName, U.LastName, U.UserID, U.UserGUID, CONCAT(IFNULL(U.FirstName,''), ' ',IFNULL(U.LastName,'')) as Name");
        $this->db->select('IF(U.ProfilePicture="","user_default.jpg",U.ProfilePicture) as ProfilePicture', FALSE);        
        $this->db->select('P.Url as ProfileURL', FALSE);
        $this->db->from(USERS . ' U');
        $this->db->join(USERDETAILS . ' UD', 'UD.UserID = U.UserID', 'left');
        $this->db->join(USERROLES . ' UR', 'UR.UserID = U.UserID AND UR.RoleID IN (1,3,6)');
        $this->db->join(PROFILEURL . " as P", "P.EntityID = U.UserID and P.EntityType = 'User'", "LEFT");
        if (!$count_only) {
            $this->db->limit($page_size, get_pagination_offset($page_no, $page_size));
        }
        $this->db->where_not_in('U.StatusID', array(3,4));
       // $this->db->order_by($sort_by, $order_by);
       // $this->db->order_by('U.UserID= '.$user_id, $order_by, false);
        $this->db->order_by("FIELD(U.UserID, 145, $user_id)", $order_by, false);
        $this->db->order_by('U.UserID', 'RANDOM');
        //ORDER BY (UserID = 5) DESC, RAND()
        $this->db->group_by('U.UserID');
        $query = $this->db->get();
        //echo $this->db->last_query();die;
        if ($count_only) {
    ...
    public function getProfile($user_id) {
        ...
        $this->db->select('AdminGender, U.Gender AS  UserGender, AdminRelationWithName, U.AccountSuspendTill', FALSE);
        
        $this->db->select('UD.IncomeLevel, UD.IsDOBApprox, UD.LocalityID, UD.PostCount AS ActivityCount, UD.CommentCount, UD.LikeRecievedCount, UD.QuestionCount AS UQC');
        $this->db->select('IFNULL(U.ProfileCover,"") as ProfileCover', FALSE);
        $this->db->select('IFNULL(UD.AdminMartialStatus,"") as MartialStatus,IFNULL(TIMESTAMPDIFF(YEAR, `RelationWithDOB`, NOW()), 0) AS RelationWithAge, RelationWithDOB', FALSE);
        $this->db->select('P.Url as ProfileURL', FALSE);
        $this->db->select('U.Location AS location', FALSE);
       ...
       /* $user_comment_count_query = "(SELECT COUNT(PC.UserID)  FROM " . POSTCOMMENTS . " PC WHERE PC.UserID = $user_id) AS CommentCount";
        ...
        if (!empty($results)) {
            ...
            $this->load->model(array('admin/dashboard/dashboard_activity_model','activity/activity_front_helper_model'));

            //$results['URC'] = $this->activity_front_helper_model->user_response_count($user_id);
            //$results['UQC'] = $this->activity_front_helper_model->user_question_count($user_id);
           ...
           // $results['InterestPercentage'] = $InterestPercentage['new_interests_data'];
            ...
            $results['Locality'] = array('Name' => '', 'HindiName'=>'', 'ShortName'=>'',  'LocalityID' => 0, 'IsPollAllowed' => 1, 'WName'=>'', 'WNumber'=>'', 'WID'=>'', 'WDescription'=>'');
            if($results['LocalityID']) {
                $this->load->model(array('locality/locality_model'));
                $results['Locality'] = $this->locality_model->get_locality($results['LocalityID']);
            }
            if(empty($results['Locality']['LocalityID'])) {
                $results['Locality']['LocalityID'] = 0;
            }
            unset($results['LocalityID']);
    ...
    function get_total_user_who_mentioned_prefered_category() {
    ...
    function get_popular_prefered_category() {
        $this->db->select("count(UTC.TagID) AS cnt, UTC.TagID, T.Name");
        $this->db->from(USERTAGCATEGORY . "  UTC");
        $this->db->join(TAGS . ' T', 'T.TagID = UTC.TagID');
        $this->db->group_by('UTC.TagID ');
    ...
    function get_team_member() {
        ...
        $this->db->from(USERS . ' U');   
        $this->db->where_in('U.UserID', array(1, 145, 563, 1554, 2342, 10161, 10662, 11003, 11769, 12255, 13557, 14145, 14151, 14155, 26211, 27046, 27359, 29783, 30446, 35559, 35560));
