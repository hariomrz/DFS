...
class Flags_model extends Common_Model
{
    ...
    public function get_list($entity_type = 'ARTICLE', $start_offset = 0, $end_offset = "", $start_date = "", $end_date = "", $search_keyword = "", $sort_by = "", $order_by = "")
    {
        ...
        switch ($entity_type):
            ...
            case 'ACTIVITY':
                if (!empty($search_keyword))
                {
                    ...
                    $this->db->or_like('A.PostSearchContent', $search_keyword);
                    $this->db->or_like('CONCAT(U.FirstName, " ", U.LastName)', $search_keyword);
                }

                $this->db->where('F.EntityType', $entity_type);
                $this->db->where('A.StatusID', 2);
                $this->db->select('A.ActivityID,A.ActivityGUID as activity_guid, A.PostContent as activity_post, A.CreatedDate as created_date,A.PostSearchContent');
                $this->db->join(ACTIVITY . ' A', 'A.ActivityID=F.EntityID');

                $this->db->select('U.FirstName, U.LastName');
                $this->db->join(USERS . ' U', 'U.UserID=A.UserID', 'LEFT');

               // $this->db->select('M.ImageUrl,M.MediaID,M.ImageName');
              //  $this->db->join(MEDIA . ' M', 'M.MediaSectionReferenceID=A.ActivityID', 'LEFT');
        ...
        $temp_q = $tempdb->get();  
        $results['total_records'] = $temp_q->num_rows(); //echo $tempdb->last_query();die;
