...
class Skills_model extends Admin_Common_Model
{
    ...
    function skill_count_by_category($category_id)
    {
        ...
        if (!empty($category_id_array))
        {
            ...
            if (!empty($skill_data))
            {
                $this->db->select('SUM(TotalEndorsement) as TotalEndorsement');
                $this->db->from(ENTITYSKILLS);
                $this->db->where_in('SkillID', $skill_data);
                $query = $this->db->get();
                $results = $query->row_array();
                $Return['EndorsementsCount'] = $results['TotalEndorsement'];
            }
        }
        return $Return;
    }
    ...
    function manage_endorsements($user_id, $skillID_array, $new_skill_id)
    {
        ...
        if ($query->num_rows() > 0)
        {
            ...
            foreach ($results as $item)
            {
                ...
                $this->db->select('COUNT(ModuleEntityID) as TotalEndorsement');
                ...
                if ($query->num_rows())
                {
                    ...
                    $TotalEndorsement = $results_row['TotalEndorsement'];
                }
                ...
                $data_insert_array['TotalEndorsement'] = $TotalEndorsement;
            ...
            $entity_skill_profile = get_data('COUNT(TotalEndorsement) as ProfileCount', ENTITYSKILLS, array('SkillID' => $new_skill_id), '1', '');
