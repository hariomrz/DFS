...
class Login_model extends Common_Model {
        ...
        if (isset($user_data) && !empty($user_data)) {
            if ($is_from_signup == 1) {
               ...
               // $this->load->model('timezone/timezone_model');
                ...
                $is_super_admin = $this->user_model->is_super_admin($user_data['UserID']);
                $is_admin = 0;
                $can_create_all_ward_post = 0;
                if(empty($is_super_admin)) {
                    $is_admin = $this->user_model->is_super_admin($user_data['UserID'], 1);                    
                } 
                if($is_super_admin || $is_admin) {
                    $can_create_all_ward_post = 1;
                } else {
                    $can_create_all_ward_post = $this->user_model->is_super_admin($user_data['UserID'], 2);
                }
    ...
    public function get_user_rights($user_id) {
        ...
        if (empty($rightsArr)) {
            ...
            foreach ($results as $right) {
                if (!in_array($right['RightID'], $rightsArr)) {
    ...
    public function send_otp($phone_no, $is_bot=0) {
        $otpData = array();
        ...
        $this->db->trans_start();        
        $otp = unique_random_string(MANAGEOTP, 'OtpCode', array(), 'nozero', 4);
        $otpData["OtpCode"] = $otp;
        ...
        if(empty($is_bot)) {
            if(ACTIVE_SMS_GATEWAY == "msg91") {
                ...
                $result = send_msg91_sms($sms_data);
                $this->db->where(array('ManageOtpID' => $manage_otp_id));
                $this->db->update(MANAGEOTP, array('Response' => $result));
    ...
    public function check_otp($otp_code, $mobile) {
        ...
        if ($otp_code == $user_record['OtpCode']) {
            if($time > $now) {
