...
class Friend_model extends Common_Model {
    ...
    function __construct() {
        parent::__construct();        
        $this->load->model(array('users/user_model', 'notification_model', 'activity/activityrule_model'));
    }
        ...
        if (!$this->settings_model->isDisabled(10)) {
            ...
            if ($status == '1') { //adding friend directly, gets called if the user comes from an invite link or signups via an invite link
                if ($this->checkFriendStatus($user_id, $FriendID) != '1' && $user_id != $FriendID) {
                    ...
                    $f1['Status'] = 1;
                    $f1['CreatedDate'] = $current_date_time;
                    $f1['AcceptedDate'] = $current_date_time;

                    $f2['UserID'] = $FriendID;
                    $f2['FriendID'] = $user_id;
                    $f2['RequestedBy'] = $user_id;
                    $f2['Status'] = 1;
                    $f2['CreatedDate'] = $current_date_time;
                    $f2['AcceptedDate'] = $current_date_time;
                    ...
                    initiate_worker_job('add_notification', array('NotificationTypeID' => 17, 'SenderID' => $user_id, 'ReceiverIDs' => array($FriendID), 'RefrenceID' => $FriendID, 'Parameters' => $parameters, 'ExtraParams' => array()), '', 'notification');
                ...
                if ($sql->num_rows()) {
                    ...
                    if ($result->RequestedBy == $user_id) {
                        ...
                        $this->db->where('UserID', $user_id)
                                ...
                                ->update(FRIENDS, array('Status' => '1', 'AcceptedDate' => $current_date_time));
                    ...
                    if ($newSql->num_rows() > 0) {
                        ...
                        $parameters[0]['ReferenceID'] = $user_id;
                        $parameters[0]['Type'] = 'User';
                        initiate_worker_job('add_notification', array('NotificationTypeID' => 16, 'SenderID' => $user_id, 'ReceiverIDs' => array($FriendID), 'RefrenceID' => $user_id, 'Parameters' => $parameters, 'ExtraParams' => array()), '', 'notification');
                        return 4;
                    }
                }
            }
    ...
    function checkFriendStatus($user_id, $FriendID) {
        $sql = $this->db->limit(1)->get_where(FRIENDS, array('UserID' => $user_id, 'FriendID' => $FriendID));
        if ($sql->num_rows()) {
            $data = $sql->row();
            if ($data->Status == '1') {
    ...
    function get_total_users_count($search_key, $user_id, $type, $UID, $module_id = 0, $module_entity_id = 0) {
		...
		from Modules m, " . USERS . " u left join " . USERDETAILS . " ud on ud.UserID = u.UserID " . $frnd1 . " where " . $cond . " u.StatusID = 2 AND u.UserID != '" . $UsrID . "' AND ud.UserID!='" . $UsrID . "' " . $frnd2 . " " . $searck);
        return $sql->num_rows();
    }
     ...
     */
    function connections($user_id, $type, $uid, $search_key, $page_no, $page_size, $viewingUserID = 0, $count_only = FALSE) {
        $frnd_cnt = 0;
        $permission = 0;
        if ($type == 'Friends') {
    ...
    function get_newsfeed_tagging_records($search_key, $user_id, $type, $page_no = PAGE_NO, $page_size = PAGE_SIZE, $Status = '', $UID = '', $module_id = 0, $module_entity_id = 0, $AdvanceSearch = array(), $CountOnly = 0, $exclude_ids = array(), $ward_id=0) {
        ...
        $this->db->select("U.FirstName, U.LastName, U.UserGUID, U.UserID, 3 as ModuleID");        
        $this->db->select('IFNULL(U.ProfilePicture,"user_default.jpg") as ProfilePicture', FALSE);
        $this->db->from(USERS . ' U');        
        if(!empty($ward_id)) {
        ...
        $this->db->where_not_in('U.StatusID', array(3, 4));
        if (!empty($search_key)) {
            $this->db->where("(U.FirstName like '%" . $search_key . "%' or U.LastName like '%" . $search_key . "%' or concat(U.FirstName,' ',U.LastName) like '%" . $search_key . "%')");
        }
        if ($CountOnly) {
            $query = $this->db->get();
            return $query->num_rows();            
    ...
    function get_all_user($search_key, $user_id, $type, $page_no = PAGE_NO, $page_size = PAGE_SIZE, $Status = '', $UID = '', $module_id = 0, $module_entity_id = 0, $AdvanceSearch = array(), $CountOnly = 0, $sort_by = '', $exclude_ids = array()) {
        ...
        if (isset($AdvanceSearch['AgeGroup']) && $AdvanceSearch['AgeGroup']) {
            $AgeRange = $this->get_age_range($AdvanceSearch['AgeGroup']);
        }
        if ($AdvanceSearch) {
        ...
        /* elseif ($type == 'Members' && $module_id == '1')
          ...
          $this->db->where($privacy_condition, NULL, FALSE);
            ...
            /* $this->db->join(USERPRIVACY . ' UP', 'UP.UserID=u.UserID AND UP.PrivacyLabelKey="search"', 'left');            
              $privacy_condition = "
              IF(UP.Value='everyone',true,
              IF(UP.Value='network', u.UserID IN(SELECT F2.FriendID FROM Friends F JOIN Friends F2 ON F.FriendID = F2.UserID WHERE F.UserID = " . $user_id . " AND F2.Status='1' AND F.Status='1' GROUP BY F2.FriendID),
              IF(UP.Value='friend',u.UserID IN(SELECT FriendID FROM Friends WHERE UserID=" . $user_id . " AND Status=1),TRUE))
              )
              ";
              $this->db->where($privacy_condition, NULL, FALSE);
             * 
        ...
        //echo $this->db->last_query();die;
        $Users = $Query->result_array();
        $ModuleSettings = get_module_settings(true);
        $i = 0;
        $key = 0;
        $row['Members'] = array();
        foreach ($Users as $value) {
...
//        $sub_query = "FR.UserID IN (SELECT FriendID FROM " . FRIENDS . " WHERE UserID = " . $current_user_id . " AND FriendID NOT IN ($not_in_subquery) AND Status=1)";
    ...
    function get_interest($user_id) {
        ...
        $interest = array();//$this->user_model->get_interest($user_id, false, false);
    ...
    function grow_user_network($user_id, $categories, $page_no, $page_size, $user_guid, $interest = array()) {
        ...
        $this->db->select('U.UserID,U.FirstName,U.LastName,U.UserGUID,U.ProfilePicture,U.ProfileCover,P.Url as ProfileURL, UD.UserWallStatus as About, UD.TagLine as TagLine');
        ...
        $this->db->select('IFNULL(CM.CountryName,"") as CountryName', FALSE);
        $this->db->select('IFNULL(CM.CountryCode,"") as CountryCode', FALSE);
        if ($friends_only) {
        ...
        $this->db->join(COUNTRYMASTER . ' CM', 'CM.CountryID = S.CountryID', 'left');
        //$this->db->join(COUNTRYMASTER . ' CM', 'CM.CountryID=UD.CountryID', 'left');
        ...
        if ($query->num_rows()) {
            foreach ($query->result_array() as $r) {
                if (empty($r['TagLine'])) {
                    $r['TagLine'] = lang('default_tagline');
                }
                $uid = $r['UserID'];
                unset($r['UserID']);
                $City = trim($r['CityName']);
                $State = trim($r['StateName']);
                $StateCode = trim($r['StateCode']);
                $Country = trim($r['CountryName']);
                $CountryCode = trim($r['CountryCode']);
                ...
                $r['Location'] = array('City' => $City, 'State' => $State, 'Country' => $Country, 'Location' => $Location, 'StateCode' => $StateCode, 'CountryCode' => $CountryCode);
