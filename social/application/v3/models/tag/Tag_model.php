...
class Tag_model extends Common_Model {
    ...
    protected $tag_following_list = array();
    protected $muted_tag_list = array();
    ...
    public function save_entity_tag($tags_data, $entity_type, $entity_id, $user_id, $is_front_end = 0, $return_ids = false, $forDummyUser = false, $isCheckOldTags = false) {
        ...
        $question_tag_id = $this->is_tag_exist('question', 1);
    ...
    public function get_trending_tags($data) {         
        ...
        if(empty($tags)) {
            ...
            $this->db->select('T.TagID, T.Name, T.IsSubTagExist, WT.WardID, WT.WardTrendingTagID as TID', false);
        ...
        if(!empty($tags) && !$is_follow_disabled) {
            
            $this->set_following_tag_list($user_id);
            $following_tag = $this->get_following_tag_list(); 
        ...
        if(ENVIRONMENT=="production") {
            $tag_data = array(
                array(
                    ...
                    'Description' => '',
                    'Banner' => '',
                    'TagCategory' => $this->question_top_tags(20, $user_id)
                ),
                array(
    ...
    public function get_activity_tags($tag_type_id = 1, $entity_type = 'ACTIVITY', $entity_id) {
                ...
                if (isset($activity_cache['t'])) {
                    ...
                    foreach ($tags as $key => $value) {
                        ...
                        if(!$is_follow_disabled) {
                            ...
                            if(isset($tags[$key]['IsSubTagExist']) && $tags[$key]['IsSubTagExist'] == 1) {
                                $tags[$key]['IsFollow'] = 2;
                            }
                           // unset($tags[$key]['IsSubTagExist']);
                        }
                        ...
                        $tags[$key]['IsMute'] = 0;
                        if(in_array($value['TagID'], $muted_tag)) {
                            $tags[$key]['IsMute'] = 1;
                        }
                    }
                    return $tags;
        ...
        $entity_type_cond_list = implode(",", $entity_type_cond);
        $this->current_db->select('DISTINCT T.TagID, T.Name,T.Name as TooltipTitle, T.IsSubTagExist, ET.AddedBy', false);
     ...
     */
    function question_top_tags($tag_id=20, $user_id=0) {
        if(API_VERSION != "v4"){
            ...
            if($tag_id == 20) {
                //6,8,34,46,71,76,173,240,662,732,817,903,960,985,986,987,1116,1119,1127,1131,1133,1144,1357,1360,1361,1366,1374,1376,1384,1387,1462,1485,1580,1581,1582,1583,1584,1585,1587,1589,1590,1591,1592,1593,1594,1595,1596,1597,1626,1627,1631,1634,1636,1637,1638,1639,1640,1641,1643,1647,1648,1651,1654,1656
                $this->db->where_in('TagID', array(6,8,34,46,71,76,173,240,659,662,732,817,903,960,985,986,987,1116,1119,1127,1131,1133,1144,1357,1360,1361,1366,1374,1376,1384,1387,1462,1485,1580,1581,1582,1583,1584,1585,1587,1589,1590,1591,1592,1593,1594,1595,1596,1597,1626,1627,1631,1634,1636,1637,1638,1639,1640,1641,1643,1647,1648,1651,1654,1656));
    ...
    function top_preferred_tags($tag_id=20) {
        ...
        if($tag_id == 20) {
            //6,8,34,46,71,76,173,240,662,732,817,903,960,985,986,987,1116,1119,1127,1131,1133,1144,1357,1360,1361,1366,1374,1376,1384,1387,1462,1485,1580,1581,1582,1583,1584,1585,1587,1589,1590,1591,1592,1593,1594,1595,1596,1597,1626,1627,1631,1634,1636,1637,1638,1639,1640,1641,1643,1647,1648,1651,1654,1656
            //$this->db->where_in('TagID', array(6,8,34,46,71,76,173,240,662,732,817,903,960,985,986,987,1116,1119,1127,1131,1133,1144,1357,1360,1361,1366,1374,1376,1384,1387,1462,1485,1580,1581,1582,1583,1584,1585,1587,1589,1590,1591,1592,1593,1594,1595,1596,1597,1626,1627,1631,1634,1636,1637,1638,1639,1640,1641,1643,1647,1648,1651,1654,1656));
            $this->db->where_in('TagID', array(34, 71, 76, 240, 817, 903, 960, 985, 986, 987, 1116, 1131, 1133, 1144, 1360, 1366, 1374, 1376, 1384, 1462));
        }
        $this->db->order_by('DisplayOrder', 'ASC');
        //$this->db->order_by('Name', 'ASC');
     ...
     */
    function details($tag_id, $user_id=0, $return_sub_tag=1) {
        $this->db->select('TagID, Name, IsSubTagExist');
        $this->db->select('IFNULL(DisplayName,"") AS DisplayName', FALSE);
        $this->db->select('IFNULL(Description,"") AS Description', FALSE);
        $this->db->select('IFNULL(Icon,"") AS Icon', FALSE);
        ...
        if ($query->num_rows()) {
            $tag = $query->row_array();
            if($tag['IsSubTagExist']) {
                $tag['SubTags'] = array();
                $tag['IsFollow'] = 2;
                if($return_sub_tag) {
                    $tag['SubTags'] = $this->get_sub_tags($tag_id, $user_id);
                }                
            } else {
                if($user_id) {
                    $is_follow_disabled = $this->settings_model->isDisabled(11);
                    if(!$is_follow_disabled) {
                        $tag['IsFollow'] = $this->is_follow($user_id, $tag_id);
                    }                    
                } 
            }
            $tag['IsMute'] = $this->is_mute($user_id, $tag_id);
            unset($tag['IsSubTagExist']);
        }
        return $tag;
    }

    function get_sub_tags($tag_id, $user_id) {
        ...
        if ($query->num_rows()) {
            ...
            if($user_id) {
                $is_follow_disabled = $this->settings_model->isDisabled(11);
                $this->set_following_tag_list($user_id);
                $this->set_muted_tag_list($user_id);
                ...
                foreach ($sub_tag as $key => $value) {
                        ...
                        $sub_tag[$key]['IsMute'] = 0;
                        if(in_array($value['TagID'], $muted_tag)) {
                            $sub_tag[$key]['IsMute'] = 1;
                        }                   
    ...
    function sync_user_profession_tag_as_user_tag() { die; // this script is not correct and having issue
        $this->current_db->select('T.TagID, T.Name, ET.EntityTagID, ET.EntityID', false);
        $this->current_db->from(TAGS . ' T');
        $this->current_db->join(ENTITYTAGS . ' ET', "ET.TagID = T.TagID AND ET.EntityType = 'USER' AND ET.StatusID=2");
        $this->current_db->where('T.TagType', 6);
        $this->current_db->order_by('T.TagID','ASC');
        $query = $this->current_db->get();
        //echo $this->current_db->last_query();die;
        $tag = array();
        if ($query->num_rows()) {
            $user_profession_tags = $query->result_array();
            foreach ($user_profession_tags as $key => $value) {
                $name = trim($value['Name']);
                $tag_id = $value['TagID'];
                $entity_tag_id = $value['EntityTagID'];

                $entity_id = $value['EntityID'];
                $entity_type = 'USER';
               
                //check tag category name exit in tag table or not
                $this->current_db->select('TagID');
                $this->current_db->from(TAGS);
                $this->current_db->where('LOWER(Name)', strtolower($name),NULL,FALSE);
                $this->current_db->where('TagType', 5);
                $query = $this->current_db->get();
                if ($query->num_rows()) {
                    $tag_data = $query->row_array();
                    $existing_tag_id = $tag_data['TagID'];

                    $this->current_db->select('TagID');
                    $this->current_db->from(ENTITYTAGS);
                    $this->current_db->where('EntityType', $entity_type);
                    $this->current_db->where('EntityID', $entity_id, FALSE);
                    $this->current_db->where('TagID', $existing_tag_id, FALSE);
                    $this->current_db->limit(1);
                    $query = $this->current_db->get();
                    if ($query->num_rows()) {
                        $this->current_db->where('EntityTagID', $entity_tag_id);
                        $this->current_db->delete(ENTITYTAGS);
                    } else {
                        $this->current_db->where('EntityTagID', $entity_tag_id);
                        $this->current_db->update(ENTITYTAGS, array('TagID' => $existing_tag_id));
                    }

                } else {
                    $this->current_db->where('TagID', $tag_id);
                    $this->current_db->update(TAGS, array('TagType' => 5));
     ...
     */
    function top_contribution_tags($data) {
        $page_no = safe_array_key($data, 'PageNo', 1);
        $page_size = safe_array_key($data, 'PageSize', 10);

        $exclude_tag_ids = array(870, 871, 6, 20, 173, 573, 1363, 1541);

        $this->db->select('TP.TagID, SUM(TP.TotalPoint) AS PT, T.Name');
        $this->db->select('IFNULL(T.DisplayName,"") AS DisplayName', FALSE);
        $this->db->select('IFNULL(T.Description,"") AS Description', FALSE);
        $this->db->select('IFNULL(T.Icon,"") AS Icon', FALSE);
        $this->db->select('IFNULL(T.Banner,"") AS Banner', FALSE);
        $this->db->from(TOPCONTRIBUTORS.' TP');
        $this->db->join(TAGS . ' T', "T.TagID = TP.TagID");
        $this->db->where_not_in('T.TagID', $exclude_tag_ids);
        if ($page_size) {
            $this->db->limit($page_size, $this->get_pagination_offset($page_no, $page_size));
        }
        $this->db->order_by('PT', 'DESC');
        $this->db->group_by("TP.TagID");
        $query = $this->db->get();
        $tag = array();
        if ($query->num_rows()) {
     ...
     * @param int $tag_id 
     */
    public function unfollow($user_id, $tag_id){
        // delete entry from follow table
        $this->db->where("UserID",$user_id);
        $this->db->where("TagID",$tag_id);
        $this->db->delete(TAGFOLLOW);

        //update followers count for tag
        $this->db->set('TotalFollowers', 'TotalFollowers-1', FALSE);
        $this->db->where('TagID',$tag_id);
        $this->db->update(TAGS);

        if (CACHE_ENABLE) {
            $this->cache->delete('utagflw_' . $user_id);
        }
    }
     ...
     * @param int $tag_id 
     */
    public function follow($user_id, $tag_id){
        ...
        $this->db->insert(TAGFOLLOW, $insert_data);
     ...
     * @param  [int] $user_id   [User Id]
     * @param  [int] $tag_id   [following tag Id]
     * @return [int]           [return follow Status]
     */
    function is_follow($user_id, $tag_id) {
        $this->db->select('TagFollowID');
        $this->db->from(TAGFOLLOW);
        $this->db->where("UserID",$user_id);
        $this->db->where("TagID",$tag_id);
        $this->db->limit(1);
        $query = $this->db->get();
    ...
    /**
	 * [set_following_tag_list used to assign following tag list in variable]
	 * @param type $user_id
	 */
	function set_following_tag_list($user_id) {
		$this->tag_following_list = $this->fetch_tag_following_list($user_id, true);
	}

	/**
	 * [get_following_tag_list used to return following tag list]
	 * @return type
	 */
	function get_following_tag_list() {
		return $this->tag_following_list;
    }
     ...
     * @return type
     */
	function fetch_tag_following_list($user_id, $array=false) {
        $following_data = '';
        if (CACHE_ENABLE) {
            $following_data = $this->cache->get('utagflw_' . $user_id);
        ...
        if (empty($following_data)) {
            //Get Following List
            $this->db->select(' GROUP_CONCAT(TF.TagID) as following_id');
            $this->db->from(TAGFOLLOW.' TF');
            $this->db->join(TAGS.' T','T.TagID=TF.TagID');
            $this->db->where('TF.UserID', $user_id);
            $this->db->order_by('TF.TagID', 'ASC');
            $result = $this->db->get();
            ...
            if (CACHE_ENABLE) {
                $this->cache->save('utagflw_' . $user_id, $following_data, CACHE_EXPIRATION);
            }
        }
     ...
     * @param int $tag_id 
     */
    public function unmute($user_id, $tag_id){
        // delete entry from mute table
        $this->db->where("UserID",$user_id);
        $this->db->where("TagID",$tag_id);
        $this->db->delete(TAGMUTE);

        //update mute count for tag
        $this->db->set('TotalMute', 'TotalMute-1', FALSE);
        $this->db->where('TagID',$tag_id);
        $this->db->update(TAGS);

        if (CACHE_ENABLE) {
            $this->cache->delete('utagmut_' . $user_id);
        }
    }
     ...
     * @param int $tag_id 
     */
    public function mute($user_id, $tag_id){
        // insert entry in mute table
        $insert_data = array(
                "UserID" => $user_id,
                "TagID" => $tag_id,
                "CreatedDate" => get_current_date('%Y-%m-%d %H:%i:%s')
            );
        $this->db->insert(TAGMUTE, $insert_data);
    ...
    /**
     * [is_mute This is used to check the status of mute]
     * @param  [int] $user_id   [User Id]
     * @param  [int] $tag_id   [mute tag Id]
     * @return [int]           [return mute Status]
     */
    function is_mute($user_id, $tag_id) {
        if(empty($user_id)) {
            return 0; // Not mute
        }
        $this->db->select('TagMuteID');
        $this->db->from(TAGMUTE);
        $this->db->where("UserID",$user_id);
        $this->db->where("TagID",$tag_id);
        $this->db->limit(1);
        $query = $this->db->get();
    ...
    /**
	 * [set_muted_tag_list used to assign muted tag list in variable]
	 * @param type $user_id
	 */
	function set_muted_tag_list($user_id) {
		$this->muted_tag_list = $this->fetch_tag_muted_list($user_id, true);
	}

	/**
	 * [get_muted_tag_list used to return muted tag list]
	 * @return type
	 */
	function get_muted_tag_list() {
		return $this->muted_tag_list;
    }
     ...
     * @return type
     */
	function fetch_tag_muted_list($user_id, $array=false) {
        $muted_data = '';
        if (CACHE_ENABLE) {
            $muted_data = $this->cache->get('utagmut_' . $user_id);
            $muted_data = trim($muted_data);   
        }                
        ...
        if (empty($muted_data)) {
            //Get Following List
            $this->db->select(' GROUP_CONCAT(TF.TagID) as muted_id');
            $this->db->from(TAGMUTE.' TF');
            $this->db->join(TAGS.' T','T.TagID=TF.TagID');
            $this->db->where('TF.UserID', $user_id);
            $this->db->order_by('TF.TagID', 'ASC');
            $result = $this->db->get();
            ...
            $muted_data = -1;
            if ($result->num_rows()) {
                $mute_row = $result->row_array();
                if (!empty($mute_row['muted_id'])) {
                    $muted_data = $mute_row['muted_id'];                        
                }
            }
            if (CACHE_ENABLE) {
                $this->cache->save('utagmut_' . $user_id, $muted_data, CACHE_EXPIRATION);
            }
        }
        ...
        if($muted_data == -1) {
            $muted_data = '';
        } else if (!empty($muted_data)) {
            $arr = explode(',', $muted_data);
            $arr = array_unique($arr);
        }
    ...
    /**
     * Used to get browse topic tags
     */
    function browse_topic($user_id) {
        $is_follow_disabled = $this->settings_model->isDisabled(11);
        $this->set_following_tag_list($user_id);
        $following_tag = $this->get_following_tag_list(); 
        $following_tag[] = 0;

        $include_tag_ids = array(71, 76, 173, 240, 659, 732, 817, 960, 986, 987, 1116, 1131, 1133, 1144, 1360, 1361, 1366, 1387, 1462);
        
        $include_tag_ids = array_diff($include_tag_ids, $following_tag);
        $include_tag_ids[] = 0;
        ...
        $this->db->select('IFNULL(Banner,"") AS Banner', FALSE);
        $this->db->from(TAGS);
        $this->db->where_in('TagID', $include_tag_ids);
        
        $this->db->order_by('TagID', 'RANDOM');
        //$this->db->order_by('Name', 'ASC');
        $query = $this->db->get();
        $tags = array();
        if ($query->num_rows()) {
    ...
    /**
     * Function for get most followed tag list
     * Parameters : Data array
     * Return : Tags array + total count
     */
    public function get_top_followed($post_data) {
        ...
        $select_array[] = "T.TagID";
        $select_array[] = "T.Name";
        $select_array[] = "T.TotalFollowers";

        $this->db->select(implode(',', $select_array), FALSE);

        $this->db->from(TAGS . "  T ");
        $this->db->where('T.TagType', 1);      
        $this->db->where('T.TotalFollowers > ', 0);       
        ...
        $order_by_field = isset($post_data['OrderByField']) ? $post_data['OrderByField'] : 'Name';
        ...
        if(!in_array($order_by_field, array_keys($allowed_order_by_fields))) {
            $order_by_field_db = $allowed_order_by_fields['TagID'];
        } else {
     ...
     * Return : Tags array + total count
     */
    public function muted_list($post_data) {
        ...
        $select_array[] = "T.TagID";
        $select_array[] = "T.Name";
        $select_array[] = "T.TotalMute";

        $this->db->select(implode(',', $select_array), FALSE);

        $this->db->from(TAGS . "  T ");
        $this->db->where('T.TagType', 1);      
        $this->db->where('T.TotalMute > ', 0);       
