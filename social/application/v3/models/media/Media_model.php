...
class Media_model extends Common_Model {
    ...
    function get_entity_media($module_id, $module_entity_id, $current_user = 0, $page_no = 1, $page_size = 4) {
        ...
        $sql = "
                SELECT MediaType,ModuleID,UserGUID,FirstName,LastName,ProfilePicture, ProfileURL, ConversionStatus, AlbumID, MediaGUID, MediaID, ImageName,ImageThumbName, Caption, MediaSectionID, MediaSectionReferenceID, CreatedDate, NoOfComments, NoOfLikes, NoOfShares, UserID, Flaggable, IsCommentable, MediaFolder  FROM (
                    " . $query1 . "
                    UNION ALL
                    " . $query2 . "
                    UNION ALL
    ...
    function get_event_media($module_id, $module_entity_id, $current_user = 0, $page_no = '', $page_size = '') {
        ...
        $sql = "
                SELECT MediaType,ModuleID,UserGUID,FirstName,LastName,ProfilePicture, ProfileURL, ConversionStatus, AlbumID, MediaGUID, MediaID, ImageName,ImageThumbName, Caption, MediaSectionID, MediaSectionReferenceID, CreatedDate,ActivityGUID,Params, ANoOfComments, ANoOfLikes, ANoOfShares,MNoOfComments, MNoOfLikes, MNoOfShares, UserID, Flaggable, IsCommentable, MediaFolder  FROM (
                    " . $query1 . "
                    UNION ALL
                    " . $query2 . "
                    UNION ALL
    ...
    function get_media_details($user_id, $media_guid, $paging = '', $type = 'Any', $activity_id = 0) {
        ...
        /* Start */
        $get_album = $this->db->select('AlbumID, MediaSectionID, MediaSectionReferenceID')->from(MEDIA)->where('MediaGUID', $media_guid)->get();
        ...
        $media_count = 0;
        $media_section_id = 0;
        $media_section_reference_id = 0;
        $data['MediaIndex'] = 1;
        $is_wall_album = FALSE;
        if ($get_album->num_rows()) {
            $album = $get_album->row_array();
            $album_id = $album['AlbumID'];
            $media_section_id = $album['MediaSectionID'];
            $media_section_reference_id = $album['MediaSectionReferenceID'];
        }
        ...
        } else if ($media_section_id == '6') {
            $this->db->where('M.MediaSectionReferenceID', $media_section_reference_id);
        ...
        $this->db->select('M.AlbumID,M.Resolution,M.MediaID,M.ImageName,M.ImageThumbName,M.Caption,M.MediaSectionID,M.MediaSectionReferenceID,M.CreatedDate, M.NoOfComments,M.NoOfLikes,M.NoOfShares,M.UserID, M.Flaggable,M.IsCommentable');
        ...
        if ($query->num_rows()) {
            ...
            $data['ImageThumbName'] = $media['ImageThumbName'];
    ...
    function get_filepath_by_guid($media_guid) {
        $this->db->select('M.ImageName,M.ImageThumbName,MS.MediaSectionAlias');
        $this->db->from(MEDIA . ' M');
    ...
    function comments($user_id, $media_guid, $page_no, $page_size, $count_only = FALSE, $only_reply=0) {
        ...
        if ($count_only) {
            ...
            return $this->activity_model->getActivityComments($data['entity_type'], $entity_id, $page_no, $page_size, $user_id, $is_owner, 2, TRUE, array(), TRUE, '', 3, 0, '', 0, "", 0, 0, $only_reply);
        }
        return $this->activity_model->getActivityComments($data['entity_type'], $entity_id, $page_no, $page_size, $user_id, $is_owner, 2, TRUE, array(), FALSE, '', 3, 0, '', 0, "",1);
    ...
    function delete($user_id, $media_guid) {
        ...
        if ($entity_details['MediaSectionID'] == 1 || $entity_details['MediaSectionID'] == 5) {
            ...
            $this->remove_profile_pic($entity_details);
        }
        ...
        if ($entity_details['entity_type'] == 'ACTIVITY') {
            ...
            $this->db->update(ACTIVITY);
        } else if(isset($entity_details['MediaSectionID']) && $entity_details['MediaSectionID'] == 3) {
            
            $activity_query = $this->db->get_where(ACTIVITY, array('ActivityID' => $entity_details['MediaSectionReferenceID']));
            if ($activity_query->num_rows()) {
                ...
                if ($activity_data['Params']) {
                    ...
                    if ($params['count']) {
                        ...
                        $this->db->where('ActivityID', $entity_details['MediaSectionReferenceID']);
                        $this->db->update(ACTIVITY);
                    }
                }
                initiate_worker_job('activity_cache', array('ActivityID' => $entity_details['MediaSectionReferenceID'])); 
            }
        }
        
        if(!empty($entity_details['album_id'])) {
    ...
    function share_details($media_guid) {
        ...
        $this->db->select('M.ImageName,M.ImageThumbName,M.Caption,M.Resolution');
        $this->db->select('MS.MediaSectionAlias as MediaFolder');
        $this->db->from(MEDIA . ' M');
        $this->db->join(MEDIASECTIONS . ' MS', 'MS.MediaSectionID=M.MediaSectionID', 'LEFT');
        $this->db->where('M.MediaGUID', $media_guid);
        ...
        if ($entity_details['entity_type'] == 'ACTIVITY') {
            $activity_query = $this->db->limit(1)->get_where(ACTIVITY, array('ActivityGUID' => $entity_details['entity_guid']));
            if ($activity_query->num_rows()) {
                $activity_data = $activity_query->row_array();
            ...
            $media_query = $this->db->limit(1)->get_where(MEDIA, array('MediaGUID' => $entity_details['entity_guid']));
            if ($media_query->num_rows()) {
    ...
    function get_media_entity($media_guid, $user = 0, $album = 0) {
        ...
        $this->current_db->select('M.MediaID, M.ModuleID, M.ImageName,M.ImageThumbName, M.Resolution, M.Flaggable, M.MediaSectionID,M.MediaSectionReferenceID,M.UserID,M.AlbumID');
        $this->current_db->where('M.MediaGUID', $media_guid);
        $this->current_db->limit(1);
        $query = $this->current_db->get(MEDIA . ' M');
        if ($query->num_rows()) {
            $row = $query->row_array();
            $count_query = $this->current_db->get_where(MEDIA, array('MediaSectionID' => $row['MediaSectionID'], 'MediaSectionReferenceID' => $row['MediaSectionReferenceID'], "StatusID" => 2));
            ...
            $return['ImageName'] = $row['ImageName'];
            $return['ImageThumbName'] = $row['ImageThumbName'];
    ...
    function check_permission($user_id, $media_guid) {
        ...
        $this->current_db->from(MEDIA);
        $this->current_db->where('MediaGUID', $media_guid);
        $this->current_db->where('StatusID!=3', NULL, FALSE);
        $this->current_db->limit(1);
    ...
    function updateMedia($Media, $entity_id, $user_id = 0, $AlbumID = 0, $deleteRemaining = false, $MediaSectionID = 0, $status = FALSE, $commentable = 1) {
        ...
        if ($Media) {
            ...
            if ($deleteRemaining) {
                ...
                $this->db->select('GROUP_CONCAT(MediaID) as MediaIDs');
                ...
                if ($query->num_rows()) {
                    ...
                    if(!empty($media_ids)) {                    
                        $delete_media_ids =  explode(',',$media_ids);                    
                ...
                $query->free_result();
                if($MediaSectionID == 3 && !empty($delete_media_ids)) {
                    //revert point for deleted media
                    initiate_worker_job('revert_media_point', $delete_media_ids,'','point');
                }
            }
    ...
    function get_all_media_details($user_id, $media_guid, $paging = '', $type = 'Any', $activity_id = 0) {
        ...
        $this->db->select('U.UserGUID, U.FirstName, U.LastName, U.ProfilePicture', FALSE);
        $this->db->select('PU.Url as ProfileURL', FALSE);
        $this->db->select('M.AlbumID,M.MediaID,M.ImageName,M.ImageThumbName,M.Caption,M.MediaSectionID,M.MediaSectionReferenceID,M.CreatedDate, M.NoOfComments,M.NoOfLikes,M.NoOfShares,M.UserID, M.Flaggable,M.IsCommentable');
        $this->db->select('MS.MediaSectionAlias as MediaFolder');
        $this->db->from(MEDIA . ' M');
        $this->db->join(MEDIAEXTENSIONS . ' ME', 'ME.MediaExtensionID=M.MediaExtensionID', 'LEFT');
        $this->db->join(MEDIATYPES . ' MT', 'MT.MediaTypeID=ME.MediaTypeID', 'LEFT');
    ...
    function get_all_media($EntityID, $data = array(), $UserID = '') {
        ...
        $this->db->select('M.MediaGUID,M.ImageName,M.ImageThumbName,M.VideoLength,M.Resolution', FALSE);
        $this->db->from(MEDIA . ' M');
        $this->db->where('M.MediaSectionID', 3);
    ...
    public function get_search_photo($user_id, $keyword, $posted_by, $tag, $posted_by_users, $tag_in_users, $page_no, $page_size, $count_only = false, $sort_by = '', $order_by = '') {
        ...
        $sort_network = array();
        if ($sort_by == 'Network' && $sort_friends) {
        ...
        $this->db->from(MEDIA . ' M');
        $this->db->join(MEDIAEXTENSIONS . ' ME', 'M.MediaExtensionID=ME.MediaExtensionID');
        $this->db->join(MEDIASECTIONS . ' MS', 'MS.MediaSectionID=M.MediaSectionID');
        $this->db->join(USERS . ' U', 'U.UserID=M.UserID', 'left');
        $this->db->_protect_identifiers = FALSE;
        $this->db->select("M.NoOfLikes");
        ...
        } else if ($posted_by == 'Friend' && !empty($friends_only)) {
            $this->db->where_in("M.UserID", $friends_only);
        } else if ($posted_by == 'My Follows' && !empty($friends_follow)) {
            $this->db->where_in("M.UserID", $friends_follow);
    ...
    function get_media_type_posts_media($ActivityID) {
        $this->db->select('M.CreatedDate,M.Resolution,M.MediaGUID,M.ImageName,M.NoOfLikes,M.NoOfComments,M.OriginalName,M.Caption,M.Description', false);
        $this->db->from(MEDIA . ' M');
        $this->db->where("M.MediaSectionReferenceID", $ActivityID);
        $this->db->where("M.MediaSectionID", 3);
        $query = $this->db->get();
        $num_rows = $query->num_rows();
        if ($num_rows) {
     ...
     * @return [array]              [Media details]
     */
    function details($user_id, $media_guid) {
        $data = array();
        $this->load->model(array('activity/activity_model', 'users/user_model'));
                
        $this->current_db->select('M.MediaGUID, M.Location, M.MediaID, IFNULL(M.ImageName,"") ImageName, IFNULL(M.NoOfComments, 0) NoOfComments, IFNULL(M.NoOfLikes,0) NoOfLikes, IFNULL(M.CreatedDate,"") CreatedDate, MT.Name as MediaType, M.MediaSectionID, M.MediaSectionReferenceID, M.IsCommentable', FALSE);
        ...
        if ($query->num_rows()) {
            ...
            $is_media = 1;
            if ($media['MediaSectionID'] == 3) {
                $countQ = $this->current_db->where(array('MediaSectionID' => $media['MediaSectionID'], 'MediaSectionReferenceID' => $media['MediaSectionReferenceID'], 'StatusID' => 2))->order_by('CreatedDate', 'DESC')->get(MEDIA);
                $media_count = $countQ->num_rows();
                $this->current_db->select('Privacy as Visibility', false);
                $this->current_db->select('A.PostContent,A.ActivityGUID,A.ModuleEntityOwner,A.ActivityID,A.CreatedDate,A.NoOfComments,A.NoOfLikes,A.NoOfShares,A.UserID,A.IsCommentable,A.Flaggable');
                $this->current_db->select('ATY.FlagAllowed');
                $this->current_db->from(ACTIVITY . ' A');
                $this->current_db->join(ACTIVITYTYPE . ' ATY', 'A.ActivityTypeID=ATY.ActivityTypeID', 'left');
                $this->current_db->where('A.ActivityID', $media['MediaSectionReferenceID']);
                $this->current_db->limit(1);
                $actQ = $this->current_db->get();
                if ($actQ->num_rows()) {
                    $activity = $actQ->row_array();
                }
                if ($media_count == 1) {
            ...
            $data['Locality']               = (object) [];
            $entity = $this->user_model->get_user_details($data['UserID']);
            if ($entity) {
            ...
            if (!$is_media) {
                ...
                $data['NoOfLikes']      = $activity['NoOfLikes'];
                $data['PostContent']    = $activity['PostContent'];
                $data['IsLike']         = $this->activity_model->is_liked($activity['ActivityID'], 'Activity', $user_id, 3, $user_id);
                if($data['NoOfComments'] > 0) {
                    $data['NoOfComments'] = $data['NoOfComments'] + $this->activity_model->get_entity_comment_reply_count($activity['ActivityID'], 'ACTIVITY');
                }
                ...
                */
                ...
                $data['IsLike'] = $this->activity_model->is_liked($data['MediaID'], 'Media', $user_id, 3, $user_id);
                if($data['NoOfComments'] > 0) {
                    $data['NoOfComments'] = $data['NoOfComments'] + $this->activity_model->get_entity_comment_reply_count($data['MediaID'], 'MEDIA');
                }
               /* $data['Flaggable'] = $media['Flaggable'];
                $data['IsOwner'] = 0;
                $data['IsFlagged'] = 0;
            ...
            unset($data['MediaSectionID']);
            unset($data['MediaSectionReferenceID']);
            unset($data['MediaID']);
            unset($data['UserID']);
     ...
     * @return boolean
     */
    public function add_gallery($user_id, $data) {        
        $media = array();
        foreach ($data as $temp) {
            $media_array = array(
                'MediaGUID' => $temp['MediaGUID'],
                'ModuleEntityID' => $user_id,
                'MediaSectionReferenceID' => $user_id,
                'ModuleID' => 3,
                'StatusID' => 2,
    ...
    /**
     * Get gallery media list
     * @param int $album_id
     * @return array
     */
    public function get_gallery($data, $count_only=0) {
        ...
        $this->current_db->select('M.MediaGUID, M.MediaID, IFNULL(M.ImageName,"") ImageName, IFNULL(M.NoOfComments, 0) NoOfComments, IFNULL(M.NoOfLikes,0) NoOfLikes, IFNULL(M.CreatedDate,"") CreatedDate, MT.Name as MediaType', FALSE);
        $this->current_db->select('U.FirstName, U.LastName, U.UserID, U.UserGUID, U.IsVIP, U.IsAssociation');
        $this->current_db->select('IFNULL(U.ProfilePicture, "") as ProfilePicture', FALSE);
        $this->current_db->select('IFNULL(M.ConversionStatus,"") AS ConversionStatus, IFNULL(M.VideoLength,"") AS VideoLength', FALSE);
        $this->current_db->select('IFNULL(M.Description, "") as Description', FALSE);
        ...
        $this->current_db->where('M.MediaSectionID', 14);
        $this->current_db->where('M.UserID', $user_id);
        ...
        if ($count_only) {
            ...
            $this->load->model(array(                    
                'users/user_model', 'activity/activity_model'
            ));
