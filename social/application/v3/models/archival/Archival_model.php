...
class Archival_model extends Common_Model {
    ...
    */
    ...
    /**
     * Used to archive activity details
     */
    protected function activity_details($activity_id) {
        try {         
            ...
            $this->db->where('ActivityID', $activity_id);
            $this->db->order_by('ActivityDetailsID', 'ASC');           
            $query = $this->db->get();
            $results = $query->result_array();
            if($results) {
                $this->db_archive->insert_on_duplicate_update_batch(ACTIVITYDETAILS, $results);
            }            
            $query->free_result();
            ...
            if($this->can_delete_from_source_db) {             
                $this->db->where('ActivityID', $activity_id);            
                $this->db->delete(ACTIVITYDETAILS);
            }
            $this->check_transaction($this->db_archive);
            
        } catch (Exception $e) {
            $this->db_archive->trans_rollback();
            throw new Exception("Issue with archive details for activity ".$activity_id);
        }
    }
