...
class Notification_model extends Common_Model {
    ...
    function __construct() {
        ...
        $this->email_notification_modules = array('1', '2', '10', '14', '18', '19', '23', '25', '29', '30', '33', '34', '36', '47');
    }
    ...
    function get_activity_summary($NotificationTypeID, $RefrenceID, $CustomParams, $user_id = 0) {
        ...
        switch ($NotificationTypeID) {
            case 49:
                $data['Summary'] = get_detail_by_id($RefrenceID, 21, 'Description');
                $data['Summary'] = preg_replace('/(^)?(<br\s*\/?>\s*)+$/', '', $data['Summary']);
                return $data;    
                break;
            case 2:
            case 21:
            ...
            case 110:
                ...
                $data['Summary'] = $this->polls_model->get_poll_description($RefrenceID);                    
                $data['Summary'] = preg_replace('/(^)?(<br\s*\/?>\s*)+$/', '', $data['Summary']);
                return $data;    
                break;
            default:
    ...
    function get_notification_link_phone($NotificationTypeID, $ReferenceID, $user_id, $ToUserID, $PageID = 0, $NotificationTypeKey, $entity_type, $CustomParams = '') {
        if ($NotificationTypeKey == 'make_group_admin' || $NotificationTypeKey == 'make_event_admin' || $NotificationTypeKey == 'make_page_admin') {
            ...
            $EntityGUID = get_guid_by_id($user_id, 3);
            return array("ModuleID" => 3, "ModuleEntityGUID" => $EntityGUID, "EntityID" => $user_id, "EntityGUID" => $EntityGUID, "Refer" => "USER");
        ...
        } elseif (in_array($NotificationTypeID, array(1, 2, 3, 18, 19, 20, 21, 54, 55, 65, 73, 82, 84, 85, 87, 90, 110, 155, 158, 159, 160, 161, 162))) {
            $this->load->helper('activity');
            $entity = get_single_activity_url_phone($ReferenceID);

            if ($NotificationTypeID==158 && $CustomParams) {
                $entity['CommentGUID'] = '';
                $comment_temp = json_decode($CustomParams);
                $comment_id = $comment_temp->Comment;
                if (!empty($comment_id)) {
                    $entity['CommentGUID'] = get_detail_by_id($comment_id, 20);
                }
            }
            return $entity;
        ...
        } elseif ($NotificationTypeKey == 'media_message') {
            $EntityGUID = get_detail_by_id($ReferenceID, 21);
            return array("ModuleID" => 21, "ModuleEntityGUID" => $EntityGUID, "EntityID" => $ReferenceID, "EntityGUID" => $EntityGUID, "Refer" => "MEDIA");
        } elseif (in_array($NotificationTypeID, array(163, 164))) {
            $custom_temp = json_decode($CustomParams);
            $quiz_id = $custom_temp->QuizID;
            $quiz_guid = '';
            if (!empty($quiz_id)) {
                $quiz_guid = get_detail_by_id($quiz_id, 47);
            }
            return array("ModuleID" => 47, "ModuleEntityGUID" => $quiz_guid, "EntityID" => $quiz_id, "EntityGUID" => $quiz_guid, "Refer" => "QUIZ");
        } elseif ($NotificationTypeID == 165) {
            $quiz_guid = '';
            if (!empty($ReferenceID)) {
                $quiz_guid = get_detail_by_id($ReferenceID, 47);
            }
            return array("ModuleID" => 47, "ModuleEntityGUID" => $quiz_guid, "EntityID" => $ReferenceID, "EntityGUID" => $quiz_guid, "Refer" => "QUIZ");
        } else {
            ...
            if ($NotificationTypeID==113 && $CustomParams) {
                $entity['CommentGUID'] = '';
                $comment_temp = json_decode($CustomParams);
                $comment_id = $comment_temp->Comment;
                if (!empty($comment_id)) {
                    $comment_data = get_detail_by_id($comment_id, 20, 'EntityType, EntityID', 2);

                    if($comment_data['EntityType'] == 'MEDIA') {
                        $EntityGUID = get_detail_by_id($comment_data['EntityID'], 21);
                        return array("ModuleID" => 21, "ModuleEntityGUID" => $EntityGUID, "EntityID" => $comment_data['EntityID'], "EntityGUID" => $EntityGUID, "Refer" => "MEDIA");
                    }
                }
            }
    ...
    function get_notification_link($NotificationTypeID, $ReferenceID, $user_id, $ToUserID, $PageID = 0, $NotificationTypeKey, $entity_type, $CustomParams) {
        ...
        if ($NotificationTypeKey == 'make_group_admin' || $NotificationTypeKey == 'make_event_admin' || $NotificationTypeKey == 'make_page_admin') {
            ...
            return get_entity_url($user_id) . '/connections';
        } elseif ($NotificationTypeID == '110' || $NotificationTypeID == '112') {
            $this->load->helper('activity');
            return get_single_activity_url($ReferenceID, 'Polls');
        } elseif (in_array($NotificationTypeID, array(163, 164, 165))) {
            return site_url('quiz');
        } elseif ($NotificationTypeID == '84' || $NotificationTypeID == '85' || $NotificationTypeID == '82' || $NotificationTypeID == '1' || $NotificationTypeID == '2' || $NotificationTypeID == '3' || $NotificationTypeID == '18' || $NotificationTypeID == '19' || $NotificationTypeID == '20' || $NotificationTypeID == '21' || $NotificationTypeID == '65' || $NotificationTypeID == '55' || $NotificationTypeID == '54' || $NotificationTypeID == '90' || $NotificationTypeID == '87' || $NotificationTypeID == '158') {
            $this->load->helper('activity');
        ...
        foreach ($ToUser as $ToUserID) {
            ...
            if ($ToUserID) {
                if ($user_id != $ToUserID || $forcely_add == 1) {
                    ...
                    if ($send_email) {
                        if (in_array($NotificationTypeID, array(1, 2, 4, 5, 16, 17, 18, 19, 21, 47, 22, 23, 25, 26, 27, 28, 29, 30, 31, 32, 46, 48, 51, 52, 55, 56, 58, 60, 61, 62, 63, 64, 65, 67, 68, 70, 81, 82, 84, 85, 97, 98, 99, 100, 101, 102, 103, 104, 105, 106, 107, 108, 109, 115, 116, 118, 125, 126, 130, 131, 132, 133, 134, 135, 136, 137, 138, 145, 146, 149, 152, 155, 159, 160, 161, 162, 165))) {
    ...
    function send_email_notification($notification_data, $original_type = 0, $n_type_id = '') {
        ...
        //log_message("error", "user_id => ".$notification_data['ToUserID']."  template_key => ".$template_key."  ModuleID => ".$notification_type_data['ModuleID']);
        ...
        if (!empty($template_key) && $notification_settings['send_notification']) {
            ...
            switch ($template_key) {
                case 'quiz_winners_announced':
                    $testing = true;
                    $title = get_detail_by_id($notification_data['RefrenceID'], 47, 'Title'); 
                    $email_data['Subject'] = "The winners are announced for the quiz ".$title;
                    $notification_data['Summary'] = '';
                    break;
                case 'poll_result':
                    $testing = true;
                    ...
                    $this->load->model(array('polls/polls_model'));
                    $notification_data['Summary'] = $this->polls_model->get_poll_description($notification_data['RefrenceID'], 1);
                    break;
                case 'post_removed';
                    $testing = true;                    
                    $email_data['Subject'] = "Your post has been deleted by admin";
                    break;
                case 'comment_removed';
                    $testing = true;                    
                    $email_data['Subject'] = "Your comment has been deleted by admin";
                    break;  
                case 'follower':
                    $testing = true;
                    $email_data['Subject'] = $from_user_detail['FullName'] . " is now following you";
                    break;
                case 'admin_edit_post';
                    $testing = false;                    
                    $this->load->helper('activity');
                    $entity_type = 'post';
                    $activity_data = get_detail_by_id($notification_data['RefrenceID'], 0, 'PostTitle, UserID', 2);
                    $title = trim($activity_data['PostTitle']);                    
                    $replace = '#content#';
                    if(empty($title)) {
                        $replace = ' "#content#"';
                    }
                    $notification_data['Summary'] = $notification_type_data['PushNotificationText'];                        
                    $notification_data['Summary'] = strtr($notification_data['Summary'], array($replace => $title));
                    $email_data['Subject'] = "Post edited for better understanding";
                    $source= 7;
                    ...
                    $communication_data = array(
                        'Type' => 1,
                        'ActivityID' => $notification_data['RefrenceID'],
                        'CreatedDate' => $current_date_time,
                        'ModifiedDate' => $current_date_time
                    );   
                    $communication_id = $this->communication_model->add_communication($communication_data);
                    ...
                    $communication_history_data['Content'] = json_encode(array('PushNotificationTitle' => $email_data['Subject'], 'PushNotificationText' => $notification_data['Summary']), JSON_UNESCAPED_UNICODE);
                    $communication_history_data['UserID'] = $notification_data['ToUserID'];
                    $communication_history_data['CreatedDate'] = $current_date_time;
                    $this->communication_model->add_histrory($communication_history_data);                   
                    break;     
                case 'possible_solution';
                case 'solution';
                        $testing = true;
                        ...
                        if ($CustomParams) {
                            $comment_temp = json_decode($CustomParams);
                            $from_admin = $comment_temp->FromAdmin;
                        }
                        ...
                        $notification_data['Summary'] = $notification_type_data['PushNotificationText'];                        
                        $notification_data['Summary'] = strtr($notification_data['Summary'], array($replace => $title));
                        $email_data['Subject'] = "Thank you for your help";
                        $source= 3;
                        if($notification_data['NotificationTypeID'] == 160) {
                            $source= 4;
                            $email_data['Subject'] = "You make Indore proud";
                        }
                    ...
                    case 'question_ready';
                        ...
                        $entity_type = 'post';
                        $notification_data['Summary'] = $notification_type_data['PushNotificationText'];
                        $email_data['Subject'] = "Help on the way";
                        $source= 5;
                ...
                case 'post_message';
                    ...
                    $email_data['StatusMessage'] = "";
                    $this->load->helper('activity');
                    $activity_data = get_detail_by_id($notification_data['RefrenceID'], 0, 'PostTitle,PostType,PostContent,NoOfLikes,ModuleID,UserID,IsMediaExist,ActivityID,ModuleEntityID,ParentActivityID,Params,NoOfComments,PostAsModuleID,PostAsModuleEntityID', 2);
                    $activity_data['PostContent'] = $this->activity_model->parse_tag_html($activity_data['PostContent']);
                    $notification_data['Summary'] = $activity_data['PostContent'];
                    $activity_data['Link'] = get_single_activity_url($notification_data['RefrenceID']);
                    $activity_data['Album'] = $this->activity_model->get_albums($notification_data['RefrenceID'], $user_detail['UserID']);
                    ...
                    if($activity_data['PostType'] == 2) {
                        $email_data['Subject'] = $from_user_detail['FullName'] . " asked a new question.";
                    }
                    break;
                case 'flag_post':
                    $testing = true;
                ...
                case 'media_comment':
                    ...
                    $email_data['Subject'] = $from_user_detail['FullName'] . " commented on your photo";
                    
                    $comment_data = $this->activity_model->get_last_comment($notification_data['RefrenceID'], $from_user_detail['UserID'], 'MEDIA');
                    $comment_data['PostComment'] = $this->activity_model->parse_tag_html($comment_data['PostComment']);
                    $notification_data['Summary'] = $comment_data['PostComment'];
                ...
                case 'endorse_new_skill':
                    $testing = false;
                    $parameters = $notification_data['Parameters'];
                    $skill_id = $parameters[1]['ReferenceID'];
                    $skill_name = $this->skills_model->get_skill_name($skill_id);
                    
                    $email_data['Subject'] = $from_user_detail['FullName'] ." has endorsed you for ".$skill_name;
                    break;               
                default:
                    return FALSE;
                    break;
            ...
            if ($testing) {
                ...
                //log_message("error","notification_settings => ".json_encode($notification_settings));
                if ($notification_settings['send_mobile_notification']) {
                    //log_message("error","SendPushMsg ToUserID =>".$notification_data['ToUserID']." Subject => ".$email_data['Subject']." notifications => ".json_encode(notification_data));
    ...
    function get_user_notification_settings($user_id, $no_cache = false) {
        ...
        $order = array('2' => '1', '1' => '2', '18' => '3', '14' => '4', '10' => '5', '25' => '6', '23' => '7', '26' => '8', '29' => '9', '33' => '10', '34' => '11', '19' => '12', '36' => '13', '30' => '14', '47' => '15');
        $cache_support = false;
    ...
    /**
     * Function Name: update_notification_settings
     * @param user_id
     * @param all_notifications
     * @param email_notifications
     * @param mobile_notifications
     ...
     * Description: set notifications of particular user
     */
    function update_user_notification_settings($user_id, $notifications = array()) {
        if (CACHE_ENABLE) {
            $this->cache->delete('user_notification_setting' . $user_id);
        }            
    ...
    public function get_new_notifications($user_id, $page_no = PAGE_NO, $page_size = PAGE_SIZE, $CountFlag = false, $All = false, $TotalRecords = 0, $status_filter = "", $list = false) {
        ...
        $this->db->from(NOTIFICATIONS . ' N');
        $this->db->join(NOTIFICATIONTYPES . ' NT', 'N.NotificationTypeID=NT.NotificationTypeID and NT.StatusID=2', 'left');
        $this->db->join(USERS . ' U', 'N.UserID=U.UserID', 'left', FALSE);
        ...
        if (!$list) {
            ...
            $this->db->group_by("IF(NT.NotificationTypeKey='follower',N.NotificationID,'')");
        ...
        if ($Notification->num_rows()) {
            foreach ($Notification->result_array() as $result) {
                ...
                $summary_data = $this->get_activity_summary($result['NotificationTypeID'], $result['RefrenceID'], $result['CustomParams'], $user_id);
                $summary_data['Summary'] = strip_tags($summary_data['Summary']);
                if($result['NotificationTypeID'] == 110 || $result['NotificationTypeID'] == 155) {
                   $arr['Summary'] = $summary_data['Summary'];
                } else {
                   // $arr['Summary'] = substr($summary_data['Summary'], 0, 100);
                   $arr['Summary'] = mb_substr($summary_data['Summary'], 0, 100,'UTF-8');
                }
               // $arr['Summary'] = $summary_data['Summary'];
                        ...
                        if ($params) {
                            foreach ($params as $param) {
                                ...
                                if ($param_details['Type'] == 'EntityType') {
                                    ...
                                    $arr[$key][] = array('FirstName' => $this->activity_front_helper_model->get_activity_title($param_details['ReferenceID'], ''), 'LastName' => '', 'ProfilePicture' => '', 'ProfileURL' => '', 'ModuleID' => 0, 'ModuleEntityGUID' => 0);
                                } else if ($param_details['Type'] == 'QuestionOption') {
                                    $quiz_question_option = $this->get_question_option_details($param_details['ReferenceID'], $user_id);
                                    $arr[$key][] = array('FirstName' => $quiz_question_option['Title'], 'LastName' => '', 'ProfilePicture' => '', 'ProfileURL' => '', 'ModuleID' => '', 'ModuleEntityGUID' => '', 'Point' => $quiz_question_option['Point']);
                                } else if ($param_details['Type'] == 'QuizQuestion') {
                                    $quiz_question = $this->get_question_details($param_details['ReferenceID']);
                                    $arr[$key][] = array('FirstName' => $quiz_question['Title'], 'LastName' => '', 'ProfilePicture' => '', 'ProfileURL' => '', 'ModuleID' => 47, 'ModuleEntityGUID' => $quiz_question['QuestionGUID']);
                                } else if ($param_details['Type'] == 'Quiz') {
                                    $quiz      = get_detail_by_id($param_details['ReferenceID'], 47, 'Title, QuizGUID', 2);
                                    $arr[$key][] = array('FirstName' => $quiz['Title'], 'LastName' => '', 'ProfilePicture' => '', 'ProfileURL' => '', 'ModuleID' => 47, 'ModuleEntityGUID' => $quiz['QuizGUID']);
                                } else if ($param_details['Type'] == 'RequestNote') {
                                    $arr[$key][] = $arr[$key][] = array('FirstName' => $this->get_request_note($param_details['ReferenceID'], $user_id), 'LastName' => '', 'ProfilePicture' => '', 'ProfileURL' => '', 'ModuleID' => 0, 'ModuleEntityGUID' => 0);
                                } elseif ($param_details['Type'] == 'time') {
                                    $arr[$key][] = array('time' => $param_details['ReferenceID']);
                ...
                if ($result['NotificationTypeID'] == 85) {
                    ...
                    if ($activity_data['ModuleID'] == 3 && $activity_data['ModuleEntityID'] == $user_id) {
                        $arr['NotificationText'] = strtr($arr['NotificationText'], array("#p2#'s" => 'your')); 
                    }
                }

                if (in_array($result['NotificationTypeID'], array(159, 160, 162))) {
                    $post_title = get_detail_by_id($result['RefrenceID'], 0, 'PostTitle');
                    $post_title = trim($post_title);
                    $replace = '#content#';
                    if(empty($post_title)) {
                        $replace = ' "#content#"';
                    }
                    $arr['NotificationText'] = strtr($arr['NotificationText'], array($replace => $post_title));                     
                }
    ...
    function post_notification($data) {
        ...
        $post_type = !empty($data['PostType']) ? $data['PostType'] : 0;
        $mentions[] = $sender_id;
        if ($sender_id && $sender_name && $activity_id) {
            ...
            if($post_type == 2) {
                $subject = $sender_name . " asked a new question.";
            }
            ...
            $post_title = !empty($data['PostTitle']) ? trim($data['PostTitle']) : ''; 
            if (!empty($post_title))  {
            ...
            $notification_data['PushNotification'] = $data['PushNotification'];
            ...
            if(!empty($ward_ids)) {
                ...
                $this->db->where_in('L.WardID', $ward_ids);
            }
            if (!empty($mentions)) {
            ...
            if ($total_notification > 0) {
                ...
                log_message('error', 'Total Post Notification  - ' . $total); 
                $queue_name = 'post_notification';
                ...
                //$communication_history_data['AdminCommunicationID'] = $communication_id;
               // $communication_history_data['Source'] = 6;              
               // $communication_history_data['Content'] = json_encode(array('PushNotificationTitle' => $subject, 'PushNotificationText' => $notification_data['Summary']));
                
                $i = 0;
                $notification_data['Subject'] = $subject;
                foreach ($results as $userdata) {
                    $notification_data['Result'] = $userdata; 

                    if($i == 0 || $i == 3) {
                        $i = 0;
                        $queue_name = 'post_notification';
                    } else {
                        $queue_name = 'post_notification'.$i;
                    }
                    
                    initiate_worker_job('process_post_notification', $notification_data, '', $queue_name);
    ...
    function process_post_notification($data) {
        ...
        foreach ($results as $userdata) {
            ...
            $data['ToUserID'] = $userdata['UserID'];
            $data['DeviceTypeID'] = $userdata['DeviceTypeID'];
            if (trim($data['UserID']) == trim($data['ToUserID'])) {
                continue;
            }
            //SendPushMsg($userdata['UserID'], $message, $data);

            send_push_notification($token, $message, 1, $data);
        }
        return true;
    }
    ...
    function get_notification_type_data($notification_type_id) {
        ...
        if (CACHE_ENABLE) {
            ...
            if (!is_array($notification_type_data)) {
                $notification_type_data = array();
            }
        }
        ...
        if (empty($notification_type_data)) {
            ...
            $this->db->select('ModuleID,NotificationTypeKey,PushNotificationText');
            ...
            if ($module_query->num_rows()) {
                ...
                $push_notification_text = $module_row->PushNotificationText;
                $notification_type_data = array('NotificationTypeKey' => $notification_type_key, 'ModuleID' => $module_id, 'PushNotificationText'=> $push_notification_text);
            }
        ...
        if ($after_process) {
            ...
            if (!empty($matches[1])) {
                ...
                foreach ($matches[1] as $match) {
                    $mention = $this->activity_model->get_mention($match);
    ...
    function get_question_details($question_id) {
        $this->db->select('QuestionGUID, Title');
        $this->db->from(QUESION);
        $this->db->where('QuestionID', $question_id, false);
        $this->db->limit(1);
        $query = $this->db->get();
        //echo $this->db->last_query(); die;
        $result = array('QuestionGUID' => '', 'Title' => '');
        if ($query->num_rows()) {
    ...
    function get_question_option_details($option_id, $user_id) {
        $this->db->select('QO.Title, UA.Point');
        $this->db->from(QUESIONOPTION.' QO');
        $this->db->join(USERANSWER.' UA',"UA.OptionID=QO.OptionID AND UA.UserID=".$user_id);

        $this->db->where('QO.OptionID', $option_id, false);
        $this->db->limit(1);
        $query = $this->db->get();
        //echo $this->db->last_query(); die;
        $result = array('Point' => '', 'Title' => '');
        if ($query->num_rows()) {
            $result = $query->row_array();
        }
        return $result;
