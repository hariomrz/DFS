...
class Activity_wall_model extends Common_Model {
    ...
    public function get_activity_query(
        ...
        if ($module_id == 3) {
            $page_list = array();//$this->page_model->get_liked_pages_list($entity_id);
    ...
    public function build_query(
        ...
        if ($field == 'ALL') {
            $this->current_db->select(
                ...
                . 'ATY.FlagAllowed,ATY.ShareAllowed,ATY.FavouriteAllowed,U.FirstName,U.LastName,U.UserGUID,U.ProfilePicture, U.IsVIP, U.IsAssociation'
            );
        } else {
            $this->current_db->select($field);
        }
        ...
        $compiled_query = $this->current_db->_compile_select();  //echo $compiled_query;  echo '==============================================';
    ...
    public function activity_conditions(
        ...
        if ($exclude_ids) {
            $this->current_db->where_not_in('A.ActivityID', explode(',', $exclude_ids));
        }
        ...
        if(empty($extra_params['EntityGUID']) && $module_id == 34) {
            ...
            $this->current_db->where($forum_category_exclude_cnds, null, false);
        }
        ...
        if ($privacy_condition) {
            ...
            if ($module_id == 3) {
                ...
                $this->current_db->where($pc, null, false);
            } else {
        ...
        if ($feed_user)
        {
            ...
            $this->current_db->where('A.ModuleEntityOwner', '0');
        }

        if ($as_owner)
        {
            $this->current_db->where('A.ModuleEntityOwner', '1');
        }
        ...
        $this->current_db->where('ATY.StatusID', '2');
        if(empty($activity_guid)) {
           // $this->db->where('A.LocalityID', $this->LocalityID);
        }
        
        $entity_id = $this->current_db->escape_str($entity_id);
        ...
        if ($activity_guid)
        {
            $this->current_db->where('ActivityGUID', $activity_guid);
        }
        else
        {
            $this->current_db->where($mention_condition, null, false);
        }
        
        
        $this->current_db->where("(A.UserID='" . $current_user . "' OR A.Privacy IN(" . implode(',', $is_relation) . ") OR (SELECT ActivityID FROM " . MENTION . " WHERE ModuleID='3' AND StatusID = 2 AND ModuleEntityID='" . $current_user . "' AND ActivityID=A.ActivityID LIMIT 1) is not null)", NULL, FALSE);
        $this->current_db->where_in('A.ActivityTypeID', $activity_type_allow);
        $this->current_db->where_not_in('U.StatusID', array(3, 4));
        ...
        if ($module_id != 47)
        {
            ...
            $this->current_db->where_not_in('A.ActivityTypeID', $exclude_activity_types);
        }
    ...
    public function apply_filter(
        ...
        if (!$this->settings_model->isDisabled(28) && $filter_type != 7)
        {
            ...
            if(empty($extra_params['onlyFeedQuery'])) {
                $this->current_db->select("R.ReminderGUID,R.ReminderDateTime,R.CreatedDate as ReminderCreatedDate,R.Status as ReminderStatus", FALSE);
                $this->current_db->select("IF(R.ReminderDateTime<'" . get_current_date('%Y-%m-%d %H:%i:%s') . "',1,0) as SortByReminder", false);
            }
            ...
            } else
            {
                if (!$activity_guid)
                {
                    $this->current_db->where("(R.Status IS NULL OR R.Status='ACTIVE')");
                }
            }
        ...
        if (($filter_type == 1 || $filter_type === 'Favourite') && $filter_type != '0') {
            $this->current_db->join(FAVOURITE . ' F', 'F.EntityID=A.ActivityID  AND F.EntityType="ACTIVITY"', 'JOIN');
            $this->current_db->where('F.UserID', $current_user);
            ...
            //$this->current_db->where('F.UserID',$current_user);
            $this->current_db->where('F.StatusID', '2');
            $this->current_db->where('A.Flaggable!=0', null, false);
            $this->current_db->group_by('F.EntityID');
        }

        if ($filter_type == 7) {
            $this->current_db->where('A.StatusID', '19');
            $this->current_db->where('A.DeletedBy', $current_user);
        } else if ($filter_type == 4 && !$this->settings_model->isDisabled(43)) {
            $this->current_db->_protect_identifiers = FALSE;
            $this->current_db->join(ARCHIVEACTIVITY . " AA", "AA.ActivityID=A.ActivityID AND AA.Status='ARCHIVED' AND AA.UserID='" . $current_user . "'", "join");
            $this->current_db->_protect_identifiers = TRUE;
        } else if ($filter_type == 10) {
            $this->current_db->where('A.StatusID', '10');
            $this->current_db->where('A.UserID', $current_user);
        } else if ($filter_type == 11) {
            $this->current_db->where('A.IsFeatured', '1');
            $this->current_db->where('A.StatusID', '2');
        } else {
            $this->current_db->where("IF(A.UserID='" . $current_user . "',A.StatusID IN(1,2,10),A.StatusID=2)", null, false);
        }

        if (!$activity_guid && $filter_type != 4 && !$this->settings_model->isDisabled(43)) {
            $this->current_db->where("A.ActivityID NOT IN (SELECT ActivityID FROM " . ARCHIVEACTIVITY . " WHERE Status='ARCHIVED' AND UserID='" . $current_user . "')", NULL, FALSE);
        }
        ...
        if ($search_key) {
            ...
            $this->current_db->where('(U.FirstName LIKE "%' . $search_key . '%" OR U.LastName LIKE "%' . $search_key . '%" OR CONCAT(U.FirstName," ",U.LastName) LIKE "%' . $search_key . '%" OR A.PostContent LIKE "%' . $search_key . '%" OR A.ActivityID IN(SELECT EntityID FROM PostComments WHERE EntityType="Activity" AND StatusID=2 AND PostComment LIKE "%' . $search_key . '%"))', NULL, FALSE);
        }
        ...
        if ($post_type) {
            if (is_array($post_type)) {
                ...
                //$this->current_db->where('A.PostType', $post_type);
            }
        }
    ...
    public function sort_conditions($filter_type, $sticky_ids, $feed_sort_by, $count_only, $module_id) {
        ...
        if($module_id == 34) {
            if($sticky_ids) {
                $this->current_db->_protect_identifiers = false;
                $this->current_db->order_by('FIELD(A.ActivityID,' . implode(',', $sticky_ids) . ') DESC');
                $this->current_db->_protect_identifiers = true;
            }


            $this->current_db->order_by('A.StickyDate', 'DESC');
        }

        if ($feed_sort_by == 1) {
            $this->current_db->order_by('A.ModifiedDate', 'DESC');
        } else if ($feed_sort_by == 2) {
        ...
        /* else if ($feed_sort_by == 4){
            ...
            $this->current_db->order_by('A.IsFeatured DESC,(A.NoOfComments+A.NoOfLikes+A.NoOfViews) DESC,A.CreatedDate DESC');
        }
        */
    }
    ...
    public function get_activities(
        ...
        //echo $activity_query; echo '==============================================================';die;
        $result = $this->current_db->query($activity_query);
