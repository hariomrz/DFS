...
class Activity_model extends Common_Model {
    ...
    /**
     * [get_community_announcements Get announcement posts for community]          
     * @param  [int]    $current_user          [Current User ID]
     * @param  [bool]    $only_id          [only id require or details]
     * @return [Array/string]                    [ActivityIDs or details based on value of $only_id]
     */
    function get_community_announcements($current_user,$only_id=false) {
        $role_id=2;
        $module_id = 3;
        ...
        $this->load->model('sticky/sticky_model', 'activity/activity_hide_model');
        ...
        if($only_id) {
            ...
            $this->db->select('A.*,ATY.ViewTemplate,ATY.Template,ATY.LikeAllowed,ATY.CommentsAllowed,ATY.ActivityType,ATY.ActivityTypeID,ATY.FlagAllowed,ATY.ShareAllowed,ATY.FavouriteAllowed,U.FirstName,U.LastName,U.UserGUID,U.ProfilePicture, U.IsVIP, U.IsAssociation');
        }
    ...
    /**
     * [get_newsfeed_announcements Get announcement posts for newsfeed]          
     * @param  [int]    $current_user          [Current User ID]
     * @param  [bool]    $only_id          [only id require or details]
     * @return [Array/string]                    [ActivityIDs or details based on value of $only_id]
     */
    function get_newsfeed_announcements($current_user,$only_id=false, $extra_params=array()) {     
        $ward_ids = array();
        if(isset($extra_params['WardIds']) && !empty($extra_params['WardIds'])) {
        ...
        if(!empty($ward_ids)) {
            ...
            $this->db->from(PINTOTOP.' P');
            $this->db->where_in('P.WardID',$ward_ids);
            $this->db->order_by('P.WardID', 'DESC');
            $this->db->limit(1);  
            $query = $this->db->get();
            if($query->num_rows()) {
        ...
        $exclude_id_chunks = array_chunk($hide_activity,2);
        if(!empty($exclude_id_chunks)) {
            $this->db->group_start();
            foreach ($exclude_id_chunks as $chunk_key => $chunk_arr) {
                $this->db->where_not_in('A.ActivityID', $chunk_arr);
            }
            $this->db->group_end();
        }
    ...
    /**
     * [get_announcements Get announcement posts]     
     * @param  [int]    $module_id        [Module ID]
     ...
     * @return [Array/string]                    [ActivityIDs or details based on value of $only_id]
     */
    function get_announcements($module_id, $module_entity_id,$current_user,$only_id=false) {
     ...
     */
    function get_user_sticky_posts($user_id,$module_id,$module_entity_id) {
        $sticky_id = array();
    ...
    public function get_photos_count($activity_id) {
        $this->current_db->select('COUNT(M.MediaID) as MediaCount, GROUP_CONCAT(DISTINCT(MT.Name)) as MediaType');
        $this->current_db->from(MEDIA . ' M');
        ...
        $this->current_db->where('M.MediaSectionID', 3);
        $this->current_db->where('M.MediaSectionReferenceID', $activity_id);
        $this->current_db->where_in('M.StatusID', array(2,10));
    ...
    /**
     * [check_media_pending_status used to get activity media status]
    ...
    function createPost($post_content, $module_id, $module_entity_id, $user_id, $is_media_exist, $media_count, $visibility, $commentable, $module_entity_owner, $notify_all = 0, $links = array(), $files_count = 0,$entity_tags=array(), $post_as_module_id=3, $post_as_module_entity_id=0,$post_title="",$post_type=1,$files=array(),$media=array(),$activity_id=FALSE,$is_anonymous=0,$status=2,$publish_post=FALSE,$analytic_login_id=NULL,$facts='',$extra_params=array(),$contest_date='',$is_featured=0,$summary='',$is_poll_exist=0, $is_show_on_news_feed=0) {      
        ...
        $is_city_news = 0;
        if (isset($extra_params['IsCityNews'])) {
            $is_city_news = $extra_params['IsCityNews'];
            if(!in_array($is_show_on_news_feed,array(0,1))) {
                $is_show_on_news_feed = 0;
            }
            unset($extra_params['IsCityNews']);
        }
        ...
        if(!$activity_id) {
            ...
            $insertArray = array(
                'ActivityGUID' => get_guid(), 'ActivityTypeID' => $activity_type_id, 'UserID' => $user_id, 'ModuleID' => $module_id, 
                'ModuleEntityID' => $module_entity_id, 'Params' => json_encode($params), 'CreatedDate' => $current_date_time, 
                'ModifiedDate' => $current_date_time, 'LastActionDate' => $current_date_time, 'StatusID' => 2, 
                'Privacy' => $visibility, 'IsCommentable' => $commentable, 'PostContent' => $post_content, 'OriginalContent' => $post_content, 'IsMediaExist' => $is_media_exist, 
                ...
                'PromotedDate' => $current_date_time,'Facts'=>$facts,'ContestEndDate' => $contest_date, 'IsFeatured' => $is_featured,'Summary' => $summary,
                'LocalityID' => $this->LocalityID,
                'IsPollExist' => $is_poll_exist,
                'IsShowOnNewsFeed' => $is_show_on_news_feed,
                'IsCityNews' => $is_city_news
            );
            ...
            if($module_id==47) {
                $this->update_quiz_post_count($module_entity_id);
            } else if($module_id==34) {
                $this->update_forum_discussions_count($module_entity_id, $module_id, 1);
            }
            $this->update_user_post_count($user_id);
            
            initiate_worker_job('save_log', array('UserID' => $user_id, 'LogType' => 'Activity', 'EntityGUID' => $insertArray['ActivityGUID'], 'Flag' => FALSE, 'DeviceTypeID' => $this->DeviceTypeID));
            // Save user activity Log                         
            $userActivityLog = array(
                'ModuleID' => $module_id, 'ModuleEntityID' => $activity_id, 'UserID' => $user_id, 'ActivityTypeID' => $activity_type_id, 'ActivityID' => $activity_id, 'PA_ID' => $activity_id,
                'PostAsModuleID'=> $post_as_module_id, 'PostAsModuleEntityID' => $post_as_module_entity_id,
                'EntityID' => $activity_id, 'ParentCommentID' => 0, 'LoginSessionKey' => $this->post_data[AUTH_KEY]
            );
            initiate_worker_job('add_activity_log', $userActivityLog);            

        } else {
            $is_edit = 1;     
            $update_array = array(
                'UserID' => $user_id,'Params' => json_encode($params),'ModifiedDate' => $current_date_time, 
                'LastActionDate' => $current_date_time, 'StatusID' => 2,'Privacy' => $visibility, 'IsCommentable' => $commentable, 
        ...
        if (!empty($matches)) {   
            ...
            foreach ($matches as $match) {
                ...
                if($proceed_mention) {
                    ...
                    if(in_array($match, $matches_content[1])) {   
                        ...
                        $post_content = strtr($post_content, array("{{{$match}}}" => '@'.$match_details[0]));
                    }                         
                }                
            }
            ...
            if(!empty($post_content_updated)) {
                ...
                if(empty($is_edit)) {
                    $updatedData['OriginalContent'] = $post_content;
                }
            }
        ...
        if($module_id == 1 && $post_type == 7) {
            ...
            initiate_worker_job('add_notification', array('NotificationTypeID' => $announcement_notification_id, 'SenderID' => $user_id, 'ReceiverIDs' => $announcement_members, 'RefrenceID' => $activity_id, 'Parameters' => $announcement_param, 'ExtraParams' => array()),'','notification');            
            $send_notification = 0;
        }
        ...
        initiate_worker_job('add_update_relationship_score', array('UserID' => $user_id, 'ModuleID' => $module_id, 'ModuleEntityID' => $module_entity_id, 'Score' => 10));
        ...
        if ($links) {
            foreach ($links as $link) {
                $links_arr[] = $this->add_link($user_id, $activity_id, $link);
            }
        }
        ...
        */
        ...
        if($status==2) {
            ...
            if(!empty($links_arr)) {
                $HistoryData['Links'] = json_encode($links_arr);
            }
    ...
    function add_activity_ward($activity_id, $ward_ids, $is_modified_date = 1, $extra_param=array()) {        
        $is_city_feed = 0;
        if(in_array(1, $ward_ids)) {
            $is_city_feed = 1;
            $ward_ids = array(1);
        ...
        $activity_wards = array();
        if(!empty($ward_ids)) {
            foreach ($ward_ids as $ward_id) {
                $activity_ward = array();
                $activity_ward['ActivityID'] = $activity_id;
                $activity_ward['WardID'] = $ward_id;
                $activity_wards[] = $activity_ward;
            }
            if (!empty($activity_wards)) {            
                $this->db->insert_on_duplicate_update_batch(ACTIVITYWARD, $activity_wards);
                
                $this->db->where_not_in('WardID', $ward_ids);
                $this->db->where('ActivityID', $activity_id);
        ...
        if(!empty($is_modified_date) && !empty($ward_ids)) {
            $modified_date_time = get_current_date('%Y-%m-%d %H:%i:%s', 0.25, 1); // 6 hour
            if (empty($is_city_feed)) {       
                $modified_date_time = get_current_date('%Y-%m-%d %H:%i:%s', 0.5, 1); //12 hour
            }
            //log_message('error', "Add modified_date_time => ".$modified_date_time);
            $this->db->where('ActivityID', $activity_id);
            $this->db->set('ModifiedDate', $modified_date_time);
            $this->db->update(ACTIVITY);   
        }   
        
        $point_data = array('EntityID' => $activity_id, 'EntityType' => 1, 'ActivityTypeID' => 45);
        if($is_city_feed == 1) {
            $point_data['ActivityID'] = $activity_id;
            $point_data['OID'] = 0;
            $point_data['UserID'] = $extra_param['UserID'];
            $point_data['PT'] = 1;
    ...
    /**
     * [activity_cache used to cache activity details]
     * @param  [int] $activity_id   [ActivityID]
     */
    function activity_cache($activity_id) {
        ...
        $cache_data['l']=array();
        $cache_data['s']=array();
        $cache_data['wid']=array();
      
        $this->db->select('ActivityGUID, ActivityID, PostContent, IsMediaExist, IsFileExists, UserID, ModuleID,ModuleEntityID,PostAsModuleID,PostAsModuleEntityID,ActivityTypeID,Params,Privacy,IsCommentable,PostTitle,NoOfLikes,NoOfComments,ParentActivityID,StatusID,TagedUser, Solution, PostType');
        $this->db->from(ACTIVITY);
       // $this->db->where('StatusID','2');
        ...
        if ($result->num_rows() > 0) {
            ...
            // Cache for Tag
            $entity_tags = $this->tag_model->get_activity_tags(1, 'ACTIVITY', $activity_id);
            ...
            $cache_data['StatusID']=$result_activity['StatusID'];
            ...
            $cache_data['Solution']=$result_activity['Solution'];
            $cache_data['PostType']=$result_activity['PostType'];
            $cache_data['TagedUser']=json_decode($result_activity['TagedUser']);
            $cache_data['wid'] = $this->get_activity_ward_ids($activity_id);
    ...
    /**
     * [add_link used to add activity links details]
     * @param  [int] $user_id   [UserID]
     * @param  [int] $activity_id   [ActivityID]
     * @param  [array] $link   [Link details]
     */
    function add_link($user_id, $activity_id, $link) {
        $tags_collection = '';
        if (isset($link['TagsCollection']) && is_array($link['TagsCollection'])) {
        ...
        if ($link['ImageURL']) {
            ...
            $image_raw_data = @file_get_contents($link['ImageURL']);
            if($image_raw_data) {
                $image_data = array('DeviceType' => 'Native', 'ImageData' => $image_raw_data, 'ImageURL' => $link['ImageURL'], 'ModuleID' => '19', 'ModuleEntityGUID' => '', 'Type' => 'wall');
    ...
    function delete_link($activity_id) {
        $this->db->where('ActivityID', $activity_id);
        $this->db->set('StatusID', 3);
        $this->db->update(ACTIVITYLINKS);
    }
     ...
     * @param  [array] $link   [Link details]
     */
    function add_comment_link($user_id, $comment_id, $link) {
        
        if ($link['ImageURL']) {
            $this->load->model('upload_file_model');
            $image_data = array('DeviceType' => 'Native', 'ImageData' => file_get_contents($link['ImageURL']), 'ImageURL' => $link['ImageURL'], 'ModuleID' => '19', 'ModuleEntityGUID' => '', 'Type' => 'wall');
            $linkURL = $this->upload_file_model->saveFileFromURL($image_data);
            $link['ImageURL'] =  $linkURL['Data']['ImageName'];
        }
        $data = array('URL' => $link['URL'], 'Title' => $link['Title'], 'MetaDescription' => $link['MetaDescription'], 'ImageURL' => $link['ImageURL'], 'CommentID' => $comment_id, 'UserID' => $user_id, 'IsCrawledURL' => $link['IsCrawledURL'], 'CreatedDate' => get_current_date('%Y-%m-%d %H:%i:%s'), 'ModifiedDate' => get_current_date('%Y-%m-%d %H:%i:%s'));
        $this->db->insert(COMMENTLINKS, $data);
        $link_id = $this->db->insert_id();
        ...
        return array('CommentLinkID'=>$link_id,
                    'IsLinkExists'=>$IsLinkExists,
                    'LinkDesc'=> $link['MetaDescription'],
                    'LinkImgURL'=>$link['ImageURL'],
                    'LinkTitle'=>$link['Title'],
    ...
    function delete_comment_link($comment_id) {
        $this->current_db->where('CommentID', $comment_id);
        $this->current_db->set('StatusID', 3);
        $this->current_db->update(COMMENTLINKS);
    }
    
    /**
     * [activate_activity used to change pending activity status]     
     * @param  [int] $activity_id   [ActivityID]
     */
    function activate_activity($activity_id) {
        $this->db->set('StatusID', '2');
        $this->db->where('ActivityID', $activity_id);
        $this->db->update(ACTIVITY);
        ...
        if ($query->num_rows()) {
            ...
            initiate_worker_job('send_post_notification', 
                                    array(
                                        ...
                                        'ModuleEntityID' => $row['ModuleEntityID'], 
                                        'AfterProcess' => 1, 
                                        'PostAsModuleID' => 0, 
    ...
    function getActivitiesForCron($user_id, $last_logins = 3) {
    ...
    /**
     * [get_activity_links used to get activity links details]
     * @param  [int] $activity_id   [Activity ID]
     */
    function get_activity_links($activity_id) {
        ...
        if(!isset($activity_cache['l'])) {
            $this->current_db->select('IF(URL is NULL,0,1) as IsLinkExists', false);
            $this->current_db->select('URL as LinkURL,Title as LinkTitle,MetaDescription as LinkDesc,ImageURL as LinkImgURL,TagsCollection as LinkTags');
            $this->current_db->from(ACTIVITYLINKS);
     ...
     * @param  [int] $activity_id   [Activity ID]
     */
    function get_comment_links($comment_id) {
        $result=array();        
        $this->current_db->select('IF(URL is NULL,0,1) as IsLinkExists', false);
        $this->current_db->select('URL as LinkURL,Title as LinkTitle,MetaDescription as LinkDesc,ImageURL as LinkImgURL');
        $this->current_db->from(COMMENTLINKS);
        $this->current_db->where('CommentID', $comment_id);
        $this->current_db->where('StatusID', 2);
        $query = $this->current_db->get();
        if ($query->num_rows()) {
    ...
    /**
     * [get_activity_share_details used to get activity share details]
     * @param  [int] $activity_guid   [Activity GUID]
     */
    function get_activity_share_details($activity_guid) {
    ...
    /**
     * [get_public_feed Get public activity]
     * @param  [string]    $activity_guid  [Activity GUID]
     * @param  [int]       $module_id      [Module ID]
     * @param  [int]       $module_entity_id      [Module Entity ID] 
     * @param  [int]       $page_no        [Page No]
     * @param  [int]       $page_size      [Page Size]
     * @param  [int]       $count_only     [count only] 
     ...
     * @return [Array]                    [Activity array]
     */
    function get_public_feed($activity_guid='',$module_id=0,$module_entity_id=0,$page_no=1,$page_size=10,$count_only=false, $rules = array(), $extra_params = array(), $post_type = 0, $tags=array()) {
        $data = array();

        $view_entity_tags = isset($extra_params['ViewEntityTags']) ? $extra_params['ViewEntityTags'] : 0;
        if($count_only && !empty($extra_params['onlyFeedQuery'])) {
            $this->current_db->select('A.ActivityID');
        } else {
            $this->current_db->select('A.*,ATY.ViewTemplate,ATY.Template,ATY.LikeAllowed,ATY.CommentsAllowed,ATY.ActivityType,ATY.ActivityTypeID,ATY.FlagAllowed,ATY.ShareAllowed,ATY.FavouriteAllowed,U.FirstName,U.LastName,U.UserGUID,U.ProfilePicture, U.IsVIP, U.IsAssociation');
        }
        ...
        $this->current_db->join(USERS . ' U', 'U.UserID=A.UserID');
              
        $this->current_db->where('A.StatusID', '2');
       // $this->current_db->where('A.Privacy', '1');

        

        if($post_type) {
            $this->current_db->where_in('A.PostType',$post_type);
        }
        $is_feed = 1;
        ...
        if($activity_guid) {
            ...
            $modules_allowed = array(3, 47);
            if($module_entity_id) {
                $is_feed = 0;
                $this->current_db->where('A.ModuleID', $module_id);
                $this->current_db->where('A.ModuleEntityID', $module_entity_id);
            } else {
                $this->current_db->where_in('A.ModuleID', $modules_allowed);
            }
            
            $this->current_db->where_in('A.ActivityTypeID',array(1,8,49));
           ...
           /* $forum_category_exclude_cnds = 
            ...
            if($is_feed) {
                $this->current_db->join(ACTIVITYWARD . ' AW', "AW.ActivityID=A.ActivityID AND AW.WardID=1");
        ...
        $feed_sort_by = isset($extra_params['FeedSortBy']) ? $extra_params['FeedSortBy'] : 1;
        ...
        if ($query->num_rows()) {
            ...
            $data = $this->activity_result_filter_model->filter_result_set($feed_result, 1, 0, 0, $role_id, $view_entity_tags, false, $module_entity_id, $module_id, $this, $is_single_activity, 3,  0,  0, $extra_params);   
    ...
    /**
     * [city_news Get city news]
     * @param  [int]       $page_no        [Page No]
     * @param  [int]       $page_size      [Page Size]
     * @param  [int]       $count_only     [count only]  
     * @return [Array]                    [Activity array]
     */
    function city_news($page_no=1,$page_size=10,$user_id,$count_only=false, $view_entity_tags, $extra_params) {
        $data = array();
        $module_id=0;
        $module_entity_id=0;
        $post_type = 0;
        ...
        $this->current_db->from(CITYNEWS . ' C');
        $this->current_db->join(ACTIVITY . ' A', 'A.ActivityID=C.ActivityID AND A.StatusID=2');
        $this->current_db->join(ACTIVITYTYPE . ' ATY', 'A.ActivityTypeID=ATY.ActivityTypeID and ATY.StatusID=2');
        $this->current_db->join(USERS . ' U', 'U.UserID=A.UserID');
        ...
        if(isset($extra_params['WardIds']) && !empty($extra_params['WardIds'])) {
            ...
            if(in_array(1, $ward_ids)) {
                ...
                $this->current_db->join(USERDETAILS . ' UD', 'UD.UserID=U.UserID');
                $this->current_db->join(LOCALITY . ' L', 'L.LocalityID=UD.LocalityID');                
                $this->current_db->join(WARD . ' W', "W.WardID=L.WardID AND W.WardID IN(" . $ward_ids_str . ")");
            }           
        ...
        if ($query->num_rows()) {
            ...
            $role_id=2;
            $is_single_activity = 0;
            $data = $this->activity_result_filter_model->filter_result_set($feed_result, 1, $user_id, 1, $role_id, $view_entity_tags, false, $module_entity_id, $module_id, $this,$is_single_activity);          
    ...
    /**
     * [mark_city_news mark city news]
     * @param  [int]       $activity_id    [Activity ID]
     */
    function mark_city_news($activity_id) {
        ...
        if ($query->num_rows() == 0) {
            ...
            $city_news = array();
            $city_news['ActivityID'] = $activity_id;
            $city_news['AddedDate'] = $current_date_time;                    
            $this->db->insert(CITYNEWS, $city_news);
        }

        $point_data = array('EntityID' => $activity_id, 'EntityType' => 1, 'ActivityTypeID' => 44);        
        $point_data['ActivityID'] = $activity_id;
        $point_data['OID'] = 0;
        $point_data['PT'] = 1;
        initiate_worker_job('add_point', $point_data,'','point');       
    ...
    /**
     * [get_unread_city_news_count get city news unread count]
     * @param  [int]       $user_id    [User ID]
     */
    function get_unread_city_news_count($user_id) {
        $this->db->select('VisitedDate');
        $this->db->from(CITYNEWSVISITBYUSER);
        $this->db->where('UserID', $user_id);
        $this->db->limit(1);
        $query = $this->db->get();
        $visited_date_time = '';
        ...
        if(!empty($visited_date_time)) {
            $this->db->where('AddedDate > ', $visited_date_time);
        }        
        $query = $this->db->get();
        return $query->num_rows();
    ...
    /**
     * [update_city_news_visit_date update visit date]
     * @param  [int]       $user_id    [User ID]
     */
    function update_city_news_visit_date($user_id) {
        $this->db->select('VisitedDate');
        $this->db->from(CITYNEWSVISITBYUSER);
        $this->db->where('UserID', $user_id);
        $this->db->limit(1);
        ...
        if ($query->num_rows() > 0) {
            ...
            $this->db->where('UserID', $user_id);
            $this->db->update(CITYNEWSVISITBYUSER);
        } else {
            $city_news = array();
            $city_news['UserID'] = $user_id;
            $city_news['VisitedDate'] = $visited_date_time;                    
            $this->db->insert(CITYNEWSVISITBYUSER, $city_news);
        }
         
    }

    /**
     * [daily_digest Get city news]
     * @param  [int]       $page_no        [Page No]
     * @param  [int]       $page_size      [Page Size]
     * @param  [int]       $count_only     [count only]  
     * @return [Array]                    [Activity array]
     */
    function daily_digest($page_no=1,$page_size=10,$user_id,$count_only=false, $view_entity_tags, $current_date) {
        $data = array();
        $module_id=0;
        $module_entity_id=0;
        ...
        if($count_only) {
            ...
            $this->db->select('A.*, D.Description, IFNULL(D.IsFeatured, 0) AS ShowImage, D.IsFeatured, ATY.ViewTemplate,ATY.Template,ATY.LikeAllowed,ATY.CommentsAllowed,ATY.ActivityType,ATY.ActivityTypeID,ATY.FlagAllowed,ATY.ShareAllowed,ATY.FavouriteAllowed,U.FirstName,U.LastName,U.UserGUID,U.ProfilePicture, U.IsVIP, U.IsAssociation');
        }
        
        $this->db->from(DIALYDIGEST . ' D');
        $this->db->join(ACTIVITY . ' A', 'A.ActivityID=D.ActivityID AND A.StatusID=2');
        $this->db->join(ACTIVITYTYPE . ' ATY', 'A.ActivityTypeID=ATY.ActivityTypeID and ATY.StatusID=2');
        $this->db->join(USERS . ' U', 'U.UserID=A.UserID');
                   
        if (!$count_only && !empty($page_size)) {
            $this->db->limit($page_size, $this->get_pagination_offset($page_no, $page_size));
        }
        $this->db->where('D.DailyDigestDate', $current_date);
        $this->db->where('D.StatusID', 2);
        $this->db->order_by('D.IsFeatured', 'DESC');
        ...
        if ($query->num_rows()) {
            ...
            $role_id=2;
            $is_single_activity = 0;
            $result = $this->activity_result_filter_model->filter_result_set($feed_result, 1, $user_id, 1, $role_id, $view_entity_tags, false, $module_entity_id, $module_id, $this,$is_single_activity);          
     ...
     * @return [Array]                    [Activity array]
     */
    function user_orientation($user_id) {
        $this->db->select('OrientationCategoryID');
        $this->db->select("IFNULL(ETitle, '') AS ETitle");
        $this->db->select("IFNULL(EDescription, '') AS EDescription");
        $this->db->select("IFNULL(HTitle, '') AS HTitle");
        $this->db->select("IFNULL(HDescription, '') AS HDescription");
        $this->db->from(ORIENTATIONCATEGORY);    
        $this->db->where('StatusID', 2);
        $query = $this->db->get();     
        $result = array();   
        if ($query->num_rows() > 0) {
            ...
            foreach ($result as $key => $res) {
                $result[$key]['Post'] = $this->user_orientation_post(1, 5, $user_id, false, 1, $res['OrientationCategoryID']);
            }
        }   
        return $result;    
    }
    ...
    /**
     * [user_orientation Get user orientation]
     * @param  [int]       $page_no        [Page No]
     * @param  [int]       $page_size      [Page Size]
     * @param  [int]       $count_only     [count only]  
     * @return [Array]                    [Activity array]
     */
    function user_orientation_post($page_no=1,$page_size=2,$user_id,$count_only=false, $view_entity_tags, $orientation_category_id) {
        ...
        if($count_only) {
            ...
            $this->db->select('A.*, A.Summary as Description, ATY.ViewTemplate,ATY.Template,ATY.LikeAllowed,ATY.CommentsAllowed,ATY.ActivityType,ATY.ActivityTypeID,ATY.FlagAllowed,ATY.ShareAllowed,ATY.FavouriteAllowed,U.FirstName,U.LastName,U.UserGUID,U.ProfilePicture, U.IsVIP, U.IsAssociation');
        }
        
        $this->db->from(USERORIENTATION . ' UO');
        $this->db->join(ACTIVITY . ' A', 'A.ActivityID=UO.ActivityID AND A.StatusID=2');
        $this->db->join(ACTIVITYTYPE . ' ATY', 'A.ActivityTypeID=ATY.ActivityTypeID and ATY.StatusID=2');
        $this->db->join(USERS . ' U', 'U.UserID=A.UserID');
        ...
        $this->db->where('UO.OrientationCategoryID', $orientation_category_id);
        $this->db->where('UO.StatusID', 2);
        $this->db->order_by('A.CreatedDate', 'DESC');
     ...
     */
    function story_user($user_id, $ward_id) {
        $ward_ids = array();
        $ward_ids[] = $ward_id;       
        if(!in_array(1, $ward_ids)) {
            $ward_ids[] = 1;
        ...
        $ward_ids_str = implode(',', $ward_ids);
        $ward_ids_str = trim($ward_ids_str, ',');
        $last_two_days = get_current_date('%Y-%m-%d %H:%i:%s', 2);
        

        $this->db->select('COUNT(A.UserID) as StoryCount, Max(`A`.`ActivityID`) AS ActivityID', false);
        $this->db->select('A.ActivityGUID, U.FirstName, U.LastName, U.UserGUID, U.ProfilePicture, U.IsVIP, U.IsAssociation');      
        $this->db->from(STORY . ' S');
        $this->db->join(ACTIVITY . ' A', 'A.ActivityID=S.ActivityID');
        $this->db->join(USERS . ' U', 'U.UserID=A.UserID');
        $this->db->join(STORYWARD . ' SW', "SW.ActivityID=S.ActivityID AND SW.WardID IN(" . $ward_ids_str . ")");
        
        $this->db->where("A.CreatedDate >= '" . $last_two_days . "'", NULL, FALSE);
        $this->db->where("A.StatusID", 2);
     ...
     * @return [Array]                    [Activity array]
     */
    function user_story($user_id, $ward_id, $logged_in_user_id, $page_no=1, $page_size=10, $count_only=false, $view_entity_tags) {
        ...
        if ($query->num_rows() > 0) {
            ...
            $role_id=2;
            $is_single_activity = 0;
            $result = $this->activity_result_filter_model->filter_result_set($feed_result, 1, $logged_in_user_id, 1, $role_id, $view_entity_tags, false, $module_entity_id, $module_id, $this,$is_single_activity);                        
        }   
        return $result; 
    }
   

    function is_daily_digest_exist() {
       // $current_date = get_current_date('%Y-%m-%d', 1);
       $this->db->select("DATE_FORMAT(D.DailyDigestDate,'%d %b %Y') as formatted_date, D.DailyDigestDate"); 
        $this->db->from(DIALYDIGEST . ' D');        
        //$this->db->where('D.DailyDigestDate', $current_date);   
        $this->db->where('D.StatusID', 2);   
        $this->db->order_by('D.DailyDigestDate', 'DESC');  
        $this->db->limit(1);
        $query = $this->db->get();        
        if ($query->num_rows() > 0) {
            $row = $query->row_array();
            return $row;
            //return get_current_date('%d %M %Y', 1);
        }
        return 0;
    }
    ...
    /**
     * [idea_for_better_indore Get activity which are marked for better indore]
     * @param  [Array]       $data        [request data]
     * @return [Array]                    [Activity array]
     */
    function idea_for_better_indore($data) {
        $view_entity_tags = safe_array_key($data, 'ViewEntityTags', 0);
        $user_id = $data['UserID'];
        $page_no = $data['PageNo'];
        ...
        $this->current_db->from(ACTIVITYDETAILS . ' AD');
        $this->current_db->join(ACTIVITY . ' A', 'A.ActivityID=AD.ActivityID AND A.StatusID=2');
        $this->current_db->join(ACTIVITYTYPE . ' ATY', 'A.ActivityTypeID=ATY.ActivityTypeID and ATY.StatusID=2');
        $this->current_db->join(USERS . ' U', 'U.UserID=A.UserID');
        $this->current_db->where('AD.IsIdea', 1);
        /*if(isset($data['WardIds']) && !empty($data['WardIds'])) {
            $ward_ids = $data['WardIds']; 
        ...
        */
        ...
        if ($query->num_rows()) {
            ...
            $role_id=2;
            $is_single_activity = 0;            
            $data = $this->activity_result_filter_model->filter_result_set($feed_result, 1, $user_id, $view_entity_tags, $role_id, 1, false, $module_entity_id, $module_id, $this,$is_single_activity); 
        }
        return $data;
    }
     ...
     * @return [Array]                    [Activity array]
     */
    function getDummyUserActivities($user_id, $page_no, $page_size, $feed_sort_by, $feed_user = 0, $filter_type = 0, $is_media_exists = 2, $search_key = false, $start_date = false, $end_date = false, $show_archive = 0, $count_only = 0, $ReminderDate = array(), $activity_guid = '', $mentions = array(), $entity_id = '', $entity_module_id = '', $activity_type_filter = array(), $activity_ids = array(),$view_entity_tags=1,$role_id=2,$post_type=0,$tags='', $rules = array(), $extra_params= array())
    {
        $this->load->model(array('polls/polls_model','category/category_model','forum/forum_model', 'activity/activity_hide_model'));
       ...
       // $page_list = $this->page_model->get_feed_pages_condition();
        ...
        $activity_type_allow = array(1, 8, 49);//23, 24,
        ...
        $select_array=array();
        $select_array[]='A.*,ATY.ViewTemplate, ATY.Template, ATY.LikeAllowed, ATY.CommentsAllowed, ATY.ActivityType, ATY.ActivityTypeID, ATY.FlagAllowed, ATY.ShareAllowed, ATY.FavouriteAllowed, U.FirstName, U.LastName, U.UserGUID, U.ProfilePicture, U.IsVIP, U.IsAssociation';
    

        $this->db->from(ACTIVITY . ' A');
        $this->db->join(ACTIVITYTYPE . ' ATY', 'A.ActivityTypeID=ATY.ActivityTypeID and ATY.StatusID=2', 'left');
        $this->db->join(USERS . ' U', 'U.UserID=A.UserID', 'left');
        ...
        if(isset($extra_params['WardIds']) && !empty($extra_params['WardIds'])) {
            ...
            $ward_ids_str = implode(',', $ward_ids);
            $ward_ids_str = trim($ward_ids_str, ',');
            $this->db->join(ACTIVITYWARD . ' AW', "AW.ActivityID=A.ActivityID AND AW.WardID IN(" . $ward_ids_str . ")");
        }
        ...
        if ($result->num_rows())
        {
            $dummy_users_only = isset($extra_params['DummyUsersOnly']) ? $extra_params['DummyUsersOnly'] : 0;
            $feed_result = $result->result_array();
            
            $return = $this->activity_result_filter_model->filter_result_set($feed_result, $page_no, $user_id, $filter_type, $role_id, $view_entity_tags, $search_key,$entity_id, $entity_module_id, $this, 0, 3, $dummy_users_only);                       
        }
        return $return;
    }
    
    /**
     * [get_questions_feed Get questions feed]
     * @param  [int]       $page_no        [Page No]
     * @param  [int]       $page_size      [Page Size]
     * @param  [int]       $count_only     [count only]  
     * @return [Array]                    [Activity array]
     */
    function get_questions_feed($user_id, $page_no, $page_size, $count_only = 0, $data = []) {
        $result = array();
        $module_id=0;
        $module_entity_id=0;
        $post_type = 2;

        $time_zone      = 'Asia/Calcutta';//$this->user_model->get_user_time_zone();
        $question_type  = safe_array_key($data, 'QuestionType', 1);
        $feed_sort_by   = safe_array_key($data, 'FeedSortBy');
        $start_date     = safe_array_key($data, 'StartDate');
        $end_date       = safe_array_key($data, 'EndDate');
        $ward_id        = safe_array_key($data, 'WID');
        $feed_user      = safe_array_key($data, 'UserID', array());
        $team_member_id        = safe_array_key($data, 'TID', 0);
        
        if(!empty($start_date)) {
            $start_date = get_current_date('%Y-%m-%d', 0, 0, strtotime($start_date));
        }
        if(!empty($end_date)) {
            $end_date = get_current_date('%Y-%m-%d', 0, 0, strtotime($end_date));
        }
        ...
        if(!empty($team_member_id)) {
            $this->db->join(ACTIVITYMEMBERS . ' AM', "AM.ActivityID=A.ActivityID AND AM.UserID=".$team_member_id);
        }
        ...
        if($question_type == 1) {
            $this->db->where('A.Solution <', 2); 
        } else if($question_type == 2) {
            $this->db->where('A.Solution', 2); 
        }

        if($question_type == 3) {
            $this->db->join(ACTIVITYDETAILS . ' AD', 'AD.ActivityID=A.ActivityID and AD.IsAnswerRequired=1');
        } else {
            $this->db->where("A.ActivityID NOT IN (SELECT ActivityID FROM " . ACTIVITYDETAILS . " WHERE IsAnswerRequired=1)", null, false);
        }
        ...
        if ($count_only) {
            ...
            if ($feed_sort_by == 2) {
                ...
                $this->db->order_by('(A.NoOfComments+A.NoOfCommentReplies+A.NoOfLikes+A.NoOfViews)', 'DESC');
            } else {
                $this->db->order_by('A.ModifiedDate', 'DESC');
            }
            $query = $this->db->get();
            ...
            if ($query->num_rows()) {
                ...
                $role_id=2;
                $is_single_activity = 0;    
                $extra_params = array('ExtraInfo' => 1);     
                
                $this->load->model(array('activity/activity_hide_model'));
                $result = $this->activity_result_filter_model->filter_result_set($feed_result, 1, $user_id, 0, $role_id, 0, false, $module_entity_id, $module_id, $this,$is_single_activity,3,0,0,$extra_params);              
            }
            return $result;
        }
    }
     ...
     */
    function change_activity_status($activity_id, $status_id) {
     ...
     */
    function getSingleUserActivity($user_id, $activity_id, $AllActivity = 0, $html_parse = false, $entity_id = '', $entity_module_id = '',$role_id=2)
    {
        $top_contributors = $this->get_top_contributors();  
        $followers = array();
        ...
        $this->load->model(array('forum/forum_model','contest/contest_model', 'activity/activity_hide_model'));
        ...
        if ($result->num_rows())
        {
            $this->load->model(array('activity/activity_result_filter_model', 'privacy/privacy_model'));            
            foreach ($result->result_array() as $res) {
                ...
                $activity['WIDs']           = $this->get_activity_ward_ids($activity_id);
                ...
                if (in_array($activity_type_id, array(23, 24))) {
                    ...
                    if (!in_array($activity_type_id, array(5, 6, 9, 10, 14, 15))) {
                        $activity['Album'] = $this->get_albums($res_entity_id, $res['UserID'], '', $res_entity_type, 0);
                    }
                    if ($BUsers) {
                ...
                $activity['IsHide']    = $this->activity_hide_model->is_hide($activity_id, $current_user);
                ...
                if ($module_id == 3) {
                    ...
                    $IsAdminGuid = $this->get_user_guid_by_user_ids(array(ADMIN_USER_ID)); 
                ...
                if ($res['ActivityTypeID'] == 1 || $res['ActivityTypeID'] == 7 || $res['ActivityTypeID'] == 11 || $res['ActivityTypeID'] == 12) {                    
                    $activity['PostContent'] = strtr($activity['PostContent'], array('­' => ''));
    ...
    /**
     * [getSingleComment Get comment details]
        ...
        if ($sql->num_rows()) {            
            foreach ($sql->result() as $comment) {
                ...
                if (!empty($parent_comment_id)) {
                    $parent_comment_owner_id = $this->get_comment_owner_id($parent_comment_id);
     ...
     */
    public function getActivityComments($entity_type, $entity_id, $page_no, $page_size, $user_id, $IsOwner, $entity_owner = 2, $Flag = TRUE, $blocked_users = array(), $NumRows = FALSE, $comment_id = 0,$postas_module_id=3,$postas_module_entity_id=0,$search_key='',$parent_comment_id=0,$filter="",$include_reply=0, $only_solutions=0, $only_reply=0) {
        ...
        {
            $activity_row = get_detail_by_id($entity_id, 0, 'ModuleID,ModuleEntityID,Params,ActivityTypeID', 2);            
        ...
        $this->current_db->select('PC.*,U.FirstName,U.LastName,U.UserGUID,U.ProfilePicture,U.UserID, U.IsVIP, U.IsAssociation, SUM(PC.NoOfLikes+PC.NoOfReplies) AS POPULAR');
        $this->current_db->select('IFNULL(UD.Occupation,"") as Occupation', FALSE);
        $this->current_db->select('IFNULL(PU.Url,"") as UserProfileUrl', FALSE);
        $this->current_db->select('L.Name, L.HindiName, L.ShortName, L.LocalityID');
        $this->current_db->from(POSTCOMMENTS . ' PC');
        $this->current_db->join(USERS . ' U', 'PC.UserID=U.UserID', 'left');
        $this->current_db->join(USERDETAILS . ' UD', 'UD.UserID = U.UserID', 'left');
        $this->current_db->join(LOCALITY . ' L', 'L.LocalityID=UD.LocalityID', 'left');
        $this->current_db->join(PROFILEURL . ' PU', 'PU.EntityID=U.UserID AND PU.EntityType="User" ', 'left');
        $this->current_db->where('PC.EntityType', $entity_type);        
        $this->current_db->where('PC.EntityID', $entity_id);

        if($only_reply) {
            $this->current_db->where('PC.ParentCommentID >', 0);
        } else {
            $this->current_db->where('PC.ParentCommentID', $parent_comment_id);
        }
        //if(!empty($parent_comment_id)) {
           // $this->current_db->where('PC.ParentCommentID', $parent_comment_id);
       // }
        ...
        if($comment_id) {
            $this->current_db->where('PC.PostCommentID', $comment_id);
            $this->current_db->limit(1);
        } else {
            $this->current_db->where_in('PC.StatusID', array(2,22));
        }
        
        if($only_solutions == 1) {
            $this->current_db->where('PC.Solution >', 0);  
        }

        if (!empty($blocked_users)) {
            $this->current_db->where_not_in('PC.UserID', $blocked_users);
        }

        if(!empty($filter)) {
            if($filter=='Recent') {
                $this->current_db->order_by('PC.ModifiedDate', 'DESC');   
            } elseif($filter=='Popular') {
                $this->current_db->order_by("POPULAR ASC, PC.StatusID = '22' DESC, PC.PostCommentID DESC");   
            } elseif($filter=='Network') {
                $result = array();
                $this->current_db->order_by("CASE WHEN PC.UserID IN ('" .$friends_list."') THEN 3 WHEN PC.StatusID=22 THEN 2 ELSE 0 END, PC.PostCommentID DESC");
            }
        } else {   
            if($page_size == 1 && $this->IsApp == 1) {
                $this->current_db->order_by("PC.Solution DESC, PC.PostCommentID DESC");   
            } else {
                $this->current_db->order_by("PC.StatusID = '22' DESC, PC.PostCommentID DESC");    
            }            
        }

        $this->current_db->group_by('PC.PostCommentID');        
        if ($Flag && $comment_id == '' && !empty($page_size)) {
            $this->current_db->limit($page_size, $this->get_pagination_offset($page_no, $page_size));
        }
        if($search_key != '') {
            $this->current_db->like('PC.PostComment',$search_key);
        }

        $sql = $this->current_db->get();
        if ($NumRows) {
            return $sql->num_rows();
        }
        $comments = array();
        if ($sql->num_rows()) {
            ...
            foreach ($sql->result() as $comment) {
                ...
                $cmnt['sbdg'] =  0;
                if(in_array($comment->UserID, $top_contributors)) {
                    $cmnt['sbdg'] =  1;
                }

                if($comment->StatusID == 3) {
                    $cmnt['DeleteReason']     = $comment->DeleteReason;
                }
                ...
                if(!$is_follow_disabled) {
                    ...
                    if ($cmnt['IsOwner'] == 1) {
                        $cmnt['IsFollow'] = 2;
                    } else if (in_array($cmnt['UserID'], $followers)) {
                        $cmnt['IsFollow'] = 1;
                    }
                    
                    $IsAdmin = $this->user_model->is_super_admin($cmnt['UserID']);
                    $IsAdminGuid = $this->get_user_guid_by_user_ids(array(ADMIN_USER_ID)); // admin set from config page
                    if($IsAdmin || $cmnt['UserID']==$IsAdminGuid )
                    {
                        $cmnt['IsFollow']=2;
                    }
                }
                ...
                $cmnt['Links'] = $this->get_comment_links($comment->PostCommentID);
    ...
    function get_comment_owner($comment_id) {
        $this->current_db->select('PC.UserID');
        $this->current_db->from(POSTCOMMENTS . ' PC');
        $this->current_db->where('PC.ParentCommentID', $comment_id);
        $query = $this->current_db->get();
        if ($query->num_rows()) {
            $row = $query->row();
            return $row->UserID;
     ...
     */
    function get_comment_reply_count($parent_comment_id,$blocked_users = array()) {        
        $this->current_db->select('COUNT(PC.PostCommentID)as TotalRow');
        $this->current_db->from(POSTCOMMENTS . ' PC');
        $this->current_db->join(USERS . ' U', 'PC.UserID=U.UserID AND U.StatusID=2', 'left');
        $this->current_db->where('PC.ParentCommentID', $parent_comment_id);
        $this->current_db->where('PC.StatusID', '2');
     ...
     */
    function get_activity_comment_count($entity_type, $entity_id, $blocked_users = array())
    {        
        $this->current_db->select('COUNT(PC.PostCommentID)as TotalRow');
        $this->current_db->from(POSTCOMMENTS . ' PC');
        $this->current_db->join(USERS . ' U', 'PC.UserID=U.UserID AND U.StatusID NOT IN (3,4)', 'left');
        $this->current_db->where('PC.EntityType', $entity_type);
        $this->current_db->where_in('PC.StatusID', array(2,22));
        $this->current_db->where('PC.EntityID', $entity_id);
        $this->current_db->where('PC.ParentCommentID', 0);
     ...
     */
    function get_comment_media($comment_id, $live_feed = false, $is_all_media = false) {
        $this->current_db->select('Media.MediaGUID,Media.Resolution,Media.OriginalName,IFNULL(Caption,"") AS Caption,Media.ConversionStatus, IFNULL(ImageName,"") AS ImageName,IFNULL(ImageThumbName,"") AS ImageThumbName,Media.Size,MediaExtension.Name as MediaExtension,MediaType.Name as MediaType');
        $this->current_db->select('MS.MediaSectionAlias as MediaFolder');
        $this->current_db->from(MEDIA . ' AS Media');
        $this->current_db->join(MEDIAEXTENSIONS . " AS MediaExtension", ' MediaExtension.MediaExtensionID = Media.MediaExtensionID', 'left');
        $this->current_db->join(MEDIATYPES . " AS MediaType", ' MediaType.MediaTypeID = MediaExtension.MediaTypeID');
        $this->current_db->join(MEDIASECTIONS . ' MS', 'MS.MediaSectionID=Media.MediaSectionID', 'LEFT');
        $this->current_db->where('MediaType.MediaTypeID != ', '4'); //documents
        $this->current_db->where('Media.MediaSectionReferenceID', $comment_id);
        $this->current_db->where('Media.MediaSectionID', '6');
        $this->current_db->where('Media.StatusID', '2');
        ...
        if ($query->num_rows()) {
            if ($is_all_media) {
                ...
                $row = $query->row();
                if ($live_feed) {
                    $media = array(array('ImageName' => $row->ImageName,'ImageThumbName'=>$row->ImageThumbName, 'Resolution' => $row->Resolution, 'Caption' => $row->Caption, 'ConversionStatus' => $row->ConversionStatus, 'MediaGUID' => $row->MediaGUID, 'MediaFolder' => $row->MediaFolder));
                    ...
                    return array('ImageName' => $row->ImageName,'ImageThumbName'=>$row->ImageThumbName, 'Resolution' => $row->Resolution, 'Caption' => $row->Caption, 'MediaGUID' => $row->MediaGUID, 'MediaFolder' => $row->MediaFolder);
                }
            }
        }
        return array();
    ...
    /**
     * [block_user_list Get list of blocked user IDs for wall]
     * @param  [int]  $user_id   [User ID]
     * @param  [int]  $module_id [Module ID]
    ...
    public function block_user_list($user_id, $module_id = 3) {
        ...
        if(empty($arr))
        {
            ...
            {
                $this->current_db->where('(UserID=' . $user_id . ' OR EntityID=' . $user_id . ')', NULL, FALSE);
            } else
            {
                $this->current_db->where('ModuleEntityID', $user_id);
            }
            $this->current_db->where('ModuleID', $module_id);
            $this->current_db->where('StatusID', '2');
            $query = $this->current_db->get(BLOCKUSER);
            //echo $this->db->last_query();
            if ($query->num_rows())
            {
     ...
     */
    function is_activity_user($user_id, $activity_id) {
        $this->db->where('ActivityID', $activity_id);
        $this->db->where('UserID', $user_id);
        $this->db->where('StatusID', '2');
        $this->db->limit(1);
     ...
     */
    function get_albums($entity_id, $user_id, $album_guid = '', $entity_type = 'Activity', $page_size = 0, $search_key='') {
        $album_data=array();
        if ($this->IsApp == 1) {
        ...
        if(empty($album_data)) {
            $this->current_db->select('M.MediaID, M.Resolution, A.AlbumID,M.MediaGUID, M.NoOfComments, M.NoOfLikes, M.NoOfShares, M.ImageName,M.ImageThumbName, M.Caption, M.AlbumID, A.AlbumName, A.AlbumGUID, A.AlbumType, A.ModuleID, A.ModuleEntityID, MT.Name as MediaType');
            $this->current_db->select('IFNULL(M.ConversionStatus,"") AS ConversionStatus,IFNULL(M.VideoLength,"") AS VideoLength', FALSE);
            $this->current_db->select('P.Url as AlbumProfileURL', false);
            $this->current_db->select('MS.MediaSectionAlias as MediaFolder'); /*added by gautam*/
            $this->current_db->select('G.GroupName, G.GroupGUID');
            $this->current_db->from(MEDIA . ' M');
            $this->current_db->join(ALBUMS . ' A', 'M.AlbumID=A.AlbumID AND A.AlbumType != "DOCUMENT" ');
            $this->current_db->join(PROFILEURL . ' P', 'P.EntityID=A.UserID AND P.EntityType="User" ', 'LEFT');
            $this->current_db->join(MEDIAEXTENSIONS . ' ME', 'ME.MediaExtensionID=M.MediaExtensionID', 'LEFT');
            $this->current_db->join(MEDIATYPES . ' MT', 'MT.MediaTypeID=ME.MediaTypeID', 'LEFT');
            $this->current_db->join(MEDIASECTIONS . ' MS', 'MS.MediaSectionID=M.MediaSectionID', 'LEFT');/*added by gautam*/
            $this->current_db->join(GROUPS . ' G', 'A.ModuleID = 1 AND A.ModuleEntityID = G.GroupID', 'LEFT');
            
            if (!$album_guid) {
                if ($entity_type == 'Activity') {
                    $this->current_db->join(ACTIVITY . ' ACT', 'M.MediaSectionReferenceID=ACT.ActivityID AND M.MediaSectionID=3', 'left');
                    $this->current_db->where('ACT.IsMediaExist', '1');
                    $this->current_db->where('ACT.ActivityID', $entity_id);
                    //$this->current_db->where('A.UserID',$user_id);
                } else if ($entity_type == 'Ratings') {
                    $this->current_db->join(RATINGS . ' RT', 'M.MediaSectionReferenceID=RT.RatingID AND M.MediaSectionID=8', 'left');
                    $this->current_db->where('RT.UserID', $user_id);
                    $this->current_db->where('RT.RatingID', $entity_id);
                } else {
                    $this->current_db->where('M.MediaID', $entity_id);
                }
            } else {
                $this->current_db->where('A.AlbumGUID', $album_guid);
            }
            $this->current_db->where_not_in('M.StatusID', array(3,21));
            $this->current_db->order_by('M.CreatedDate', 'DESC');

            $tempdb = clone $this->current_db;
            //$temp_q = $tempdb->get();
            $total_media = $tempdb->count_all_results();

            if($page_size)
            {
                $this->current_db->limit($page_size);
            }
            ...
            if ($query->num_rows()) {
                ...
                foreach ($query->result() as $row) {
                    ...
                    $media[] = array(
                        ...
                        'ImageThumbName' => $row->ImageThumbName,
                        'Caption' => $row->Caption,
    ...
    /**
     * [get_activity_media Get the activity media details]
     * @param  [int]       $entity_id        [Entity ID]
     * @param  [int]       $user_id        [User ID]
     * @param  [string]    $album_guid     [Album GUID]
     * @param  [string]    $entity_type     [Entity Type]
     ...
     */
    function get_activity_media($entity_id, $user_id, $album_guid = '', $entity_type = 'Activity', $page_no = 1, $page_size = 4) {
        $album_data=array();
        ...
        if ($query->num_rows()) {
            ...
            foreach ($query->result() as $row) {
                ...
                if($row->NoOfComments > 0) {
                    $media_detail['NoOfComments'] = $media_detail['NoOfComments'] + $this->get_entity_comment_reply_count($row->MediaID, 'MEDIA'); 
                }
     ...
     */
    function removeChildActivity($activity_id, $activity_type_ids)
    {
    ...
    /**
     * [restoreActivity used to restore deleted activity details]
     * @param  [int]       $activity_id        [Activity ID]
     * @param  [int]       $user_id        [User ID]
     */
    function restoreActivity($activity_id, $user_id) {
        $this->db->where('ActivityID', $activity_id);
        $this->db->limit(1);
        ...
        if ($result->num_rows()) {
            ...
            if (checkPermission($user_id, $row->ModuleID, $row->ModuleEntityID, 'Sticky', 3, $user_id) || checkPermission($user_id, 3, $row->UserID)) {
                ...
                if ($users) {
                    ...
                    initiate_worker_job('add_notification', array('NotificationTypeID' => 85, 'SenderID' => $user_id, 'ReceiverIDs' => $users, 'RefrenceID' => $activity_id, 'Parameters' => $parameters, 'ExtraParams' => array()),'','notification');                    
                }

                if ($mentions) {
                    $parameters[0]['ReferenceID'] = $user_id;
                    $parameters[0]['Type'] = 'User';
                    initiate_worker_job('add_notification', array('NotificationTypeID' => 84, 'SenderID' => $user_id, 'ReceiverIDs' => $mentions, 'RefrenceID' => $activity_id, 'Parameters' => $parameters, 'ExtraParams' => array()),'','notification');                    
                }
            }
        }
    }
     ...
     */
    function removeActivity($activity_id, $user_id, $explicit_status_id = 0, $reason='') {
        $row = get_detail_by_id($activity_id, 0, 'ModuleID, ModuleEntityID, UserID, ActivityTypeID, ParentActivityID, StatusID', 2);            
        if (!empty($row)) {
            $this->load->model('users/user_model');
            $is_super_admin=$this->user_model->is_super_admin($user_id, 1);

            if (($user_id == $row['UserID']) || $is_super_admin) {

                if (in_array($row['ActivityTypeID'], array(9, 10, 14, 15))) {
                ...
                $this->db->where('ActivityID', $activity_id);
                $this->db->update(ACTIVITY, array('StatusID' => $status_id, 'DeletedBy' => $user_id, 'DeleteReason' => $reason));
                if($row['ModuleID']==47) {
                    $this->update_quiz_post_count($row['ModuleEntityID'], -1);
                ...
                if ($user_id != $row['UserID'] && $is_super_admin) {
                    $parameters[0]['ReferenceID'] = ADMIN_USER_ID;
                    $parameters[0]['Type'] = 'User';
                    $parameters[1]['ReferenceID'] = 1;
                    $parameters[1]['Type'] = 'EntityType';
                    initiate_worker_job('add_notification', array('NotificationTypeID' => 73, 'SenderID' => ADMIN_USER_ID, 'ReceiverIDs' => array($row['UserID']), 'RefrenceID' => $activity_id, 'Parameters' => $parameters, 'ExtraParams' => array()),'','notification');                    
                }
                
                $point_data = array('EntityID' => $activity_id, 'EntityType' => 1);
                initiate_worker_job('revert_point', $point_data,'','point');
     ...
     */
    function checkLikeStatus($activity_guid, $user_id, $postas_module_id, $postas_module_entity_id)
    {
    ...
    function is_liked($entity_id, $entity_type, $user_id, $postas_module_id=3, $postas_module_entity_id=0,$status = 2)
    {
        ...
        $entity_type = strtoupper($entity_type);                
        $this->current_db->select('EntityID');
        $this->current_db->where('EntityID', $entity_id);
        
        if ($entity_type == 'ACTIVITY' || $entity_type == 'COMMENT') {
            $this->current_db->where('UserID', $user_id);
            $this->current_db->where('PostAsModuleID', $postas_module_id);
            $this->current_db->where('PostAsModuleEntityID', $postas_module_entity_id);
        } else {
            $this->current_db->where('UserID', $user_id);
        }
        $this->current_db->where('EntityType', $entity_type);
        $this->current_db->where('StatusID', $status);
        $this->current_db->limit(1);
        $query = $this->current_db->get(POSTLIKE);
     ...
     */
    function addActivity($module_id, $module_entity_id, $activity_type_id, $user_id, $activity_id = 0, $post_content = '', $CommentSettings = '', $VisibleFor = '', $Params = array(), $is_media_exist = '0', $module_entity_owner = 0, $post_as_module_id = 3, $post_as_module_entity_id = 0, $is_file_exists = 0,$post_title='',$post_type=1,$activity_guid='')
    {
        ...
        if($activity_guid == '') {
            ...
            if($activity_type_id == 2) {
                $data = array(
                    ...
                    'PostAsModuleID' => $post_as_module_id, 'PostAsModuleEntityID' => $user_id,
                    'EntityID' => $activity_id, 'ParentCommentID' => 0, 'LoginSessionKey' => $this->post_data[AUTH_KEY]
                );
                initiate_worker_job('add_activity_log', $data);
            }
     ...
     */
    function change_privacy($activity_id, $privacy)
    {
        $this->db->where('ActivityID', $activity_id);
     ...
     */
    function getActivityOwnerdetails($activity_id)
    {
        $Name = '';
        $user_id = '';
        $UserGUID = '';
    ...
    /**
     * [get_tagged_user Used to get @tagged user]
     * @param  [string] $entity_type  [entity type]
     * @param  [int] $entity_id     [entity id]
     * @return [array]           [tagged user details] 
     */
    function get_tagged_user($entity_type, $entity_id) {
        if ($entity_type == 'ACTIVITY') {
            $select = 'PostContent';
            $table = ACTIVITY;
        ...
        $tagged_user = array();
        if(!empty($content)) {
            preg_match_all('/{{([0-9.a-zA-Z\s:]+)}}/', $content, $matches);
            $mentions = array();
            if (!empty($matches[1])) {
                foreach ($matches[1] as $match) {
     ...
     */
    function addComment($entity_guid, $Comment, $Media, $user_id, $is_media_exists = 0, $SourceID = 1, $DeviceID = 1, $entity_owner = 0, $entity_type = 'ACTIVITY', $postas_module_id = 3, $postas_module_entity_id=0, $parent_comment_id=0, $is_anonymous=0, $comment_id=0, $taged_user='', $links=array()) {        
        if(!$postas_module_entity_id) {
            $postas_module_entity_id=$user_id;
        }
        ...
        if ($entity_type == 'ACTIVITY') {
            $entity = get_detail_by_guid($entity_guid, 0, "ModuleEntityOwner, ActivityID, ActivityTypeID, Params, ModuleID, ModuleEntityID, Privacy, UserID, IsPromoted", 2);
            if(!empty($entity)){
                if ($entity['ActivityTypeID'] == '23' || $entity['ActivityTypeID'] == '24') {
                    $entity_params = json_decode($entity['Params'], true);
                    $entity_guid = $entity_params['MediaGUID'];
                    $entity_type = 'MEDIA';
                }
                $activity_id = $entity_id = $parent_activity_id = $entity['ActivityID'];
                $module_id = $entity['ModuleID'];
                $params = $entity['Params'];
                $activity_type_id = $entity['ActivityTypeID'];
                $module_entity_id = $entity['ModuleEntityID'];
                $module_entity_owner = $entity['ModuleEntityOwner'];
                $is_promoted = $entity['IsPromoted'];
                $parent_owner_id = $entity['UserID'];
            }
        }
        ...
        if (!empty($entity_type)) {            
            switch ($entity_type) {
                ...
                case 'ALBUM':
                    ...
                    break;
                case 'MEDIA':
                    $this->db->select('M.MediaID, M.UserID, M.AlbumID, M.MediaSectionID, M.MediaSectionReferenceID, ME.MediaTypeID');
                    ...
                    $media_type_id  = $res['MediaTypeID'];
                    $parent_owner_id = $res['UserID'];
                    if($res['MediaSectionID'] == 3) {
                        $parent_activity_id = $res['MediaSectionReferenceID'];
                    }
        ...
        initiate_worker_job('add_update_relationship_score', array('UserID' => $user_id, 'ModuleID' => $module_id, 'ModuleEntityID' => $module_entity_id, 'Score' => 4));
        ...
        if(IS_ARCHIVE_DB == 1) {
            ...
            $this->restore_model->sync_entity($entity_type, $entity_id);
            if(!empty($parent_comment_id)) {
                $this->restore_model->sync_entity("COMMENT", $parent_comment_id);
            }
        }
        ...
        if(empty($comment_id)) {
            ...
            if(!empty($activity_id)) {
                initiate_worker_job('set_promotion', array('ActivityID' => $activity_id, 'EntityID' => 0, 'EntityType' => ''));             
            }
                       
            $user_log_data = array(
                'ModuleID' => 3, 'ModuleEntityID' => $user_id, 'UserID' => $user_id, 'ActivityID' => $activity_id, 'PA_ID' => $parent_activity_id, 'OID' => $parent_owner_id, 
                'ActivityTypeID' => 20, 'ParentCommentID' => $parent_comment_id, 'PostAsModuleID' => 3,
                'PostAsModuleEntityID' => $user_id, 'EntityID' => $comment_id, 'LoginSessionKey' => $this->post_data[AUTH_KEY]
            );
            initiate_worker_job('add_activity_log', $user_log_data);
                        
            //nofify admin dashboard for update.
            //notify_node('updateAdminDashboard', array('EntityID' => $comment_id));
            
            $this->updateCommentCount($entity_id, $entity_type, $Count, $parent_comment_id, $album_id);
            $this->update_user_comment_count($user_id);
            
            $DeviceID = isset($DeviceID) ? $DeviceID : $this->DeviceTypeID;
            initiate_worker_job('save_log', array('UserID' => $user_id, 'LogType' => 'Comment', 'EntityGUID' => $data['PostCommentGUID'], 'Flag' => FALSE, 'DeviceTypeID' => $DeviceID));            
        } else {   
            $Isedit=true;
            $data = array(
                        'EntityType'        => $entity_type,
            ...
            if ($entity_type == 'ACTIVITY' && $user_id != $entity['UserID']) {                
                $parameters[0]['ReferenceID'] = $user_id;
                $parameters[0]['Type'] = 'User';                
                initiate_worker_job('add_notification', array('NotificationTypeID' => 127, 'SenderID' => $user_id, 'ReceiverIDs' => array($entity['UserID']), 'RefrenceID' => $data['EntityID'], 'Parameters' => $parameters, 'ExtraParams' => array('Comment' => $comment_id)),'','notification');
            }
        }
        ...
        if ($links) {
            foreach ($links as $link) {
                $links_arr[] = $this->add_comment_link($user_id, $comment_id, $link);
            }
        }
        ...
        if (!empty($matches[1])) {
            foreach ($matches[1] as $match) {
                ...
                $Comment = strtr($Comment, array($match => $mention_id));
            }
        ...
        if(!empty($comment_id)) {
            ...
            $this->add_notification_reply($comment_id,$user_id); 
        }       
        ...
        $data['MentionPages'] = $mentions_page;
        $data['Tagged'] = $tagged;
        $data['ModuleID'] = $module_id;
        $data['ModuleEntityID'] = $module_entity_id;
        $data['ActivityTypeID'] = $activity_type_id;
        $data['Params'] = $params;
        $data['MediaTypeID'] = $media_type_id; 
        $data['Entity'] = $entity;
        $data['IsEdit'] = $Isedit;
        $data['IsPromoted'] = $is_promoted;
        initiate_worker_job('send_comment_notification', $data, '', 'notification');
        ...
        if(IS_ARCHIVE_DB == 1) {
            $this->current_db = $this->db;
        }
        $comment_data =  $this->getActivityComments($data['EntityType'], $data['EntityID'], 1, 1, $data['UserID'], 1,2,TRUE,array(),FALSE,$comment_id,$postas_module_id,$postas_module_entity_id,'',$parent_comment_id);
        if(isset($comment_data[0]['Ar'])) {
            $comment_data[0]['Ar'] = 0;
        }
        return $comment_data;
    }
    ...
    /**
     * [send_comment_notification Used to send comment notification]
    ...
    function send_comment_notification($data) {
        ...
        $mentions_page = $data['MentionPages'];
        $tagged = $data['Tagged'];
        $Isedit = $data['IsEdit'];
        $is_promoted = $data['IsPromoted'];
        $post_as_module_entity_id = $data['PostAsModuleEntityID'];
        $post_as_module_id = $data['PostAsModuleID'];
        /* if (!empty($mentions_page)) {
            $this->load->model('pages/page_model');
            foreach ($mentions_page as $m_p) {
                ...
                initiate_worker_job('add_notification', array('NotificationTypeID' => 70, 'SenderID' => $user_id, 'ReceiverIDs' => $admins, 'RefrenceID' => $entity_id, 'Parameters' => $parameters, 'ExtraParams' => array('Comment' => $comment_id)),'','notification');                
                initiate_worker_job('subscribe_email', array('UserID' => $user_id, 'EntityID' => $entity_id, 'SubscribeAction' => 'page_tagged_comment', 'SendEmail' => true, 'TaggedEntity' => $m_p, 'CommentID' => $comment_id));                
            }
        }
        */
        
        $block_user_list1 = $this->block_user_list($module_entity_id, $module_id);
        $block_user_list2 = array();
        if($module_id == 3 && $user_id != $module_entity_id) {
            $block_user_list2 = $this->block_user_list($user_id, 3);
        }
        $block_user_list = array_unique(array_merge($block_user_list1, $block_user_list2));
        $notification_user_list = '';
        if ($entity_type == 'ACTIVITY') {
            $notification_user_list = $this->notification_model->get_notification_user_list($entity_id, $user_id);
            $post_tagged_users = $this->get_tagged_user('ACTIVITY', $entity_id);

            if ($notification_user_list) {
                $parameters[0]['ReferenceID'] = $user_id;
                $parameters[0]['Type'] = 'User';

                $parameters[1]['ReferenceID'] = 1;
                $parameters[1]['Type'] = 'EntityType';
                if ($activity_type_id == 5 || $activity_type_id == 6 || $activity_type_id == 13) {
                    $params = json_decode($params, true);
                    $album_id = get_detail_by_guid($params['AlbumGUID'], 13);
                    $parameters[1]['ReferenceID'] = 2;
                    $parameters[2]['ReferenceID'] = $album_id;
                    $parameters[2]['Type'] = 'Album';
                }
                $notification_type_id = 2;
            }
        } else if ($entity_type == 'MEDIA') {
            $notification_user_list = $this->getMediaOwner($entity_id);            
            if ($notification_user_list) {
                $parameters[0]['ReferenceID'] = $user_id;
                $parameters[0]['Type'] = 'User';
                $parameters[1]['Type'] = 'EntityType';
                ...
                } elseif ($media_type_id == 2 || $media_type_id == 3) {
            ...
            $notification_user_list[] = $this->rating_model->get_rating_owner($entity_id);
            if ($notification_user_list) {
                $parameters[0]['ReferenceID'] = $user_id;
                $parameters[0]['Type'] = 'User';
                $notification_type_id = 63;
            }
        }
        ...
        $tagged_notify = array();
        if ($notification_user_list) {
            foreach ($notification_user_list as $key => $val) {              
                

                if( !empty($comment_owner_id) && ($comment_owner_id==$val)) {
                    unset($notification_user_list[$key]);
                }

                if (in_array($val, $tagged)) {
                    unset($notification_user_list[$key]);
                }
                if (in_array($val, $block_user_list)) {
                    unset($notification_user_list[$key]);
                }
                ...
                if( !empty($user_id) && ($user_id==$val)) {
                    unset($notification_user_list[$key]);
                }
            }
            if (isset($notification_type_id) && $Isedit==false ) {
                if ($post_as_module_id == 18  && isset($module_entity_id)) {
                    $parameters[0]['ReferenceID'] = $post_as_module_entity_id;
                    $parameters[0]['Type'] = 'Page';
                }
                initiate_worker_job('add_notification', array('NotificationTypeID' => $notification_type_id, 'SenderID' => $user_id, 'ReceiverIDs' => $notification_user_list, 'RefrenceID' => $data['EntityID'], 'Parameters' => $parameters, 'ExtraParams' => array('Comment' => $comment_id)),'','notification');                
            }
            if ($tagged_notify) {
                initiate_worker_job('add_notification', array('NotificationTypeID' => 55, 'SenderID' => $user_id, 'ReceiverIDs' => $tagged_notify, 'RefrenceID' => $data['EntityID'], 'Parameters' => array($parameters[0]), 'ExtraParams' => array('Comment' => $comment_id)),'','notification');
            }
        }
        if ($tagged) {
            foreach ($tagged as $k => $v) {
                ...
                if (in_array($v, $notification_user_list) || in_array($v, $tagged_notify)) {
                    unset($tagged[$k]);
                }
                if (isset($entity['UserID']) && isset($entity['Privacy'])) {
                    if (!in_array($entity['Privacy'], $this->isRelation($entity['UserID'], $v, true))) {
                        unset($notification_user_list[$k]);
                    }
                }
            }
            if ($tagged) {
                ...
                if(isset($entity)) {
                    if ($entity['ActivityTypeID'] == '23' || $entity['ActivityTypeID'] == '24') {
                        initiate_worker_job('add_notification', array('NotificationTypeID' => 132, 'SenderID' => $user_id, 'ReceiverIDs' => $tagged, 'RefrenceID' => $entity['ActivityID'], 'Parameters' => $parameters, 'ExtraParams' => array('Comment' => $comment_id)),'','notification');                        
                    } else {
                        initiate_worker_job('add_notification', array('NotificationTypeID' => 21, 'SenderID' => $user_id, 'ReceiverIDs' => $tagged, 'RefrenceID' => $data['EntityID'], 'Parameters' => $parameters, 'ExtraParams' => array('Comment' => $comment_id)),'','notification');                       
                    }
                }
            }
        }
        ...
        if ($comment_owner_id) {
            $parameters[0]['ReferenceID'] = $user_id;
            $parameters[0]['Type'] = 'User';
            initiate_worker_job('add_notification', array('NotificationTypeID' => 113, 'SenderID' => $user_id, 'ReceiverIDs' => array($comment_owner_id), 'RefrenceID' => $data['EntityID'], 'Parameters' => $parameters, 'ExtraParams' => array('Comment' => $comment_id)),'','notification');            
        }

        if ($entity_type == 'ACTIVITY') {
            set_last_activity_date($data['EntityID'],0,TRUE,$is_promoted);
            if ($module_id != 3) {
                set_last_activity_date($module_entity_id, $module_id, TRUE, $is_promoted);
            }
        }
    }
    
    /**
     * [add_notification_reply Used to send notification to all user who are replied on comment]
     * @param  [int] $comment_id [comment ID]
     * @param  [int] $user_id    [User ID]
     */
    function add_notification_reply($comment_id,$user_id) {
       ...
       if($query->num_rows()>0) {
            ...
            if (!empty($notification_user_list)) {
                $parameters[0]['ReferenceID'] = $user_id;
                $parameters[0]['Type'] = 'User';
                initiate_worker_job('add_notification', array('NotificationTypeID' => 121, 'SenderID' => $user_id, 'ReceiverIDs' => explode(',',$notification_user_list), 'RefrenceID' => $comment_id, 'Parameters' => $parameters, 'ExtraParams' => array()),'','notification');                
            }            
        }        
    }
    
    /**
     * [get_comment_owner_id Used to get comment owner]
     * @param  [int] $comment_id [comment id]
     * @return [int]             [comment owner id]
     */
    function get_comment_owner_id($comment_id) {
        $this->current_db->select('UserID');
        $this->current_db->from(POSTCOMMENTS);
     ...
     * @return [Array]      [Users]
     */
    function get_entity_log_users($entity_type, $entity_guid, $search_key, $page_size, $page_no, $CountOnly = 0)
    {
     ...
     * @return [type]               [description]
     */
    function updateShareCount($entity_type, $entity_id, $Count = 1)
    {
     ...
     * @return [type]               [description]
     */
    function updateCommentCount($entity_id, $entity_type, $Count = 1,$parent_comment_id=0, $album_id=0)
    {
        ...
        if($parent_comment_id==0) {
            ...
            if ($entity_type == "ACTIVITY" && $Count == 1) {
                // $modified_date_time = get_current_date('%Y-%m-%d %H:%i:%s');
               // $this->db->where('ModifiedDate <= ', $modified_date_time);
               // $this->db->set('ModifiedDate', $modified_date_time);
            }
            //log_message('error', "modified_date_time => ".$modified_date_time);
            $this->db->update($table_name);
        } else {            
            ...
            if(in_array($entity_type, array('ACTIVITY', 'MEDIA'))) {
                $set_field = 'NoOfCommentReplies';
                $this->db->where($condition);
                $this->db->set($set_field, "$set_field+($Count)", FALSE);
                $this->db->update($table_name);
            }
        ...
        if($entity_type == 'MEDIA' && !empty($album_id)) {
            $set_field = 'MediaCommentCount';
            $this->db->where('AlbumID', $album_id);
            $this->db->set($set_field, "$set_field+($Count)", FALSE);
            $this->db->update(ALBUMS);
        }
     ...
     */
    function deleteComment($CommentGUID, $user_id, $entity_type = 'ACTIVITY', $reason='') {
        $entity_type = strtoupper($entity_type);
        $this->db->select('PC.UserID as CommenterID, PC.ParentCommentID, PC.PostCommentID, PC.EntityID, PC.IsMediaExists, PC.EntityType, PC.NoOfReplies');
        
        $this->db->from(POSTCOMMENTS . ' PC');
        switch ($entity_type) {            
            case 'MEDIA':
                $this->db->select('M.UserID, M.ModuleID, M.MediaID AS ModuleEntityID, ME.MediaTypeID, M.AlbumID');
                $this->db->join(MEDIA . ' M', 'M.MediaID=PC.EntityID', 'left');
                $this->db->join(MEDIAEXTENSIONS . ' AS ME', 'ME.MediaExtensionID=M.MediaExtensionID', 'left');
                break;
            case 'RATING':
                $this->db->select('R.UserID,R.ModuleID,R.ModuleEntityID');
                $this->db->join(RATINGS . ' R', 'R.RatingID=PC.EntityID', 'left');
                break;
            case 'BLOG':
                $this->db->select('B.BlogID AS ModuleEntityID');
                $this->db->join(BLOG . ' B', 'B.BlogID=PC.EntityID', 'left');
                //23
                break;
            default:                
                $this->db->select('A.UserID,A.ModuleID,A.ModuleEntityID');
        ...
        if ($query->num_rows()) {
            ...
            if (($comment_owner_id == $user_id) || ($user_id == $row->UserID) || ($user_id == $row->CommenterID) || $is_super_admin) {
                $this->db->where('PostCommentID', $comment_id);
                $this->db->update(POSTCOMMENTS, array('StatusID' => 3, 'DeletedBy' => $user_id, 'DeleteReason' => $reason));
                
                $Comments = $row; //$this->getCommentDetails($CommentGUID);                
                $count = -1;
                if(!empty($row->ParentCommentID) && $row->ParentCommentID!=0 ) {
                    ...
                    if(in_array($entity_type, array('ACTIVITY', 'MEDIA'))) {                        
                        $this->db->where($condition);
                        $this->db->where('NoOfCommentReplies > ', 0);
                        $this->db->set('NoOfCommentReplies', 'NoOfCommentReplies-1', FALSE);
                        $this->db->update($table_name);
                    }

                } else {
                    $this->db->where('ParentCommentID', $Comments->PostCommentID);
                    $this->db->update(POSTCOMMENTS, array('StatusID' => 3, 'DeletedBy' => $user_id, 'DeleteReason' => $reason));

                    $count = -(1 + $Comments->NoOfReplies);
                    if (strtoupper($Comments->EntityType) == 'ACTIVITY') {
                        $set_field = 'NoOfCommentReplies';

                        $this->db->set('NoOfComments', 'NoOfComments-1', FALSE);
                        $this->db->set($set_field, "$set_field+(-$Comments->NoOfReplies)", FALSE);
                ...
                if($entity_type == 'MEDIA' && isset($row->AlbumID) && !empty($row->AlbumID)) {
                    $set_field = 'MediaCommentCount';
                    $this->db->where('AlbumID', $row->AlbumID);
                    $this->db->set($set_field, "$set_field+($count)", FALSE);
                    $this->db->update(ALBUMS);
                }
                ...
                if ($user_id != $row->CommenterID && $is_super_admin) {
                    $notify_params = array('Comment' => $comment_id);
                    $parameters[0]['ReferenceID'] = ADMIN_USER_ID;
                    $parameters[0]['Type'] = 'User';
                    $parameters[1]['Type'] = 'EntityType';
                    if ($entity_type == 'ACTIVITY') {   
                    ...
                    initiate_worker_job('add_notification', array('NotificationTypeID' => 158, 'SenderID' => ADMIN_USER_ID, 'ReceiverIDs' => array($row->CommenterID), 'RefrenceID' => $entity_id, 'Parameters' => $parameters, 'ExtraParams' => $notify_params),'','notification');                    
                }

                $point_data = array('EntityID' => $comment_id, 'EntityType' => 2, 'ParentID' => $row->ParentCommentID);
                initiate_worker_job('revert_point', $point_data,'','point');
     ...
     */
    function isRelation($user_id, $current_user, $arr = false, $activity_guid = 0) {
        if ($activity_guid) {
     ...
     * 
     */
    function getLikeDetails($entity_guid, $entity_type, $blocked_users = array(), $page_no = 0, $page_size = 0, $NumRows = FALSE, $user_id = 0)
    {
        $result = array();
       ...
       // $friends = $friend_followers_list['Friends'];
        ...
        } else if ($entity_type == 'MEDIA') { // Added condition to fetch like detail of activity type media
            $entity_id = get_detail_by_guid($entity_guid, 21, "MediaID", 1);
        } else if ($entity_type == 'BLOG') { // Added condition to fetch like detail of activity type media
            $entity_id = get_detail_by_guid($entity_guid, 24, "BlogID", 1);
        }

        $this->current_db->select('U.UserID,U.UserGUID,U.FirstName,U.LastName,IF(U.ProfilePicture="","user_default.jpg",U.ProfilePicture) as ProfilePicture, PU.Url AS ProfileURL', false);
        $this->current_db->from(POSTLIKE. ' PL');
        $this->current_db->join(USERS . ' U', 'U.UserID=PL.UserID', 'left');
        $this->current_db->join(PROFILEURL . " as PU", "PU.EntityID = U.UserID and PU.EntityType = 'User'", "LEFT");
        $this->current_db->where('PL.EntityType', $entity_type);
        $this->current_db->where('PL.EntityID', $entity_id);
        $this->current_db->where('PL.StatusID', '2');
        if (!empty($blocked_users)) {
            $this->current_db->where_not_in('PL.UserID', $blocked_users);
        }
        ...
        $this->current_db->order_by("CASE WHEN U.UserID = ".$user_id." THEN 2 WHEN U.UserID IN ('" . $friends_list . "') THEN 1 ELSE 0 END DESC");
        $this->current_db->_protect_identifiers = TRUE;
        $this->current_db->order_by('PL.ModifiedDate', 'DESC');
        ...
        if ($query->num_rows()) {
            ...
            foreach ($query->result_array() as $r) {
                ...
                $result[$i]['Locality']               = (object) [];
                $result[$i]['Occupation'] = '';
                $entity = $this->user_model->get_user_details($r['UserID']);
                if ($entity) {                    
                    $result[$i]['Occupation'] = isset($entity['Occupation']) ? $entity['Occupation'] : '';
                    $result[$i]['Locality']   = $entity['Locality'];
                }
                ...
                if(!$is_follow_disabled) {
                    ...
                    if ($r['UserID'] == $user_id) {
                        $result[$i]['IsFollow'] = 2;
                    } else if (in_array($r['UserID'], $followers)) {
                        $result[$i]['IsFollow'] = 1;
                    }
                    
                    $IsAdmin = $this->user_model->is_super_admin($r['UserID']);
                    $IsAdminGuid = $this->get_user_guid_by_user_ids(array(ADMIN_USER_ID)); 
                    if($IsAdmin == "1" || $r['UserID'] == $IsAdminGuid){
                        $result[$i]['IsFollow'] = 2;
                    }
                }
     ...
     */
    function get_like_count($entity_id, $entity_type = "ACTIVITY", $blocked_users = array(), $status = 2)
    {
        $this->current_db->select('COUNT(PL.PostLikeID) as TotalRow');
        $this->current_db->from(POSTLIKE .' PL');
        $this->current_db->join(USERS . ' U', 'U.UserID=PL.UserID AND U.StatusID=2', 'left');
        $this->current_db->where('PL.EntityType', strtoupper($entity_type));
        $this->current_db->where('PL.EntityID', $entity_id);
        $this->current_db->where('PL.StatusID', $status);
        if (!empty($blocked_users))
        {
            $this->current_db->where_not_in('PL.UserID', $blocked_users);
        }
        $result = $this->current_db->get();
        $count_data=$result->row_array();
        return $count_data['TotalRow'];
    }
     ...
     */
    function sharePost($module_id, $module_entity_id, $user_id, $entity_type, $entity_id, $post_content, $CommentSettings = '', $VisibleFor = '',$post_type=1,$activity_guid='')
    {
        $entity_typeID = 1;
        $is_media_exist = 0;
        $IsFileExist = 0;
        ...
        if($activity_guid == '') {
            if ($entity_type == 'MEDIA') {
                ...
                //$OriginalActivity 
                $OriginalActivity = $this->getOriginalActivity($entity_id, 2);
                $entity_id = $OriginalActivity['ActivityID'];
                $org_activity_type_id = $OriginalActivity['ActivityTypeID'];
                if($org_activity_type_id == 14 || $org_activity_type_id == 15) {
                    $entity_type = 'MEDIA';
                    if ($activity_type_id == 10) {
                        $activity_type_id = 15;
                        $is_media_exist = 1;
        ...
        if($activity_guid == '')
        {
            if ($activity_type_id == 9 || $activity_type_id == 14)
            {
                ...
                $parameters[1]['Type'] = 'EntityType';
                initiate_worker_job('add_notification', array('NotificationTypeID' => 47, 'SenderID' => $user_id, 'ReceiverIDs' => array($module_entity_id), 'RefrenceID' => $activity_id, 'Parameters' => $parameters, 'ExtraParams' => array()),'','notification');
            }
            if ($entity_type == 'MEDIA') {
            ...
            if ($OriginalActivityOwnerDetail['UserID'] != $user_id && $OriginalActivityOwnerDetail['UserID'] != $module_entity_id) {
                initiate_worker_job('add_notification', array('NotificationTypeID' => 48, 'SenderID' => $user_id, 'ReceiverIDs' => array($OriginalActivityOwnerDetail['UserID']), 'RefrenceID' => $activity_id, 'Parameters' => $parameters, 'ExtraParams' => array()),'','notification');                
            }
            if (isset($subscribe_action)) {                
                initiate_worker_job('subscribe_email', array('UserID' => $user_id, 'EntityID' => $entity_id, 'SubscribeAction' => $subscribe_action, 'SendEmail' => true, 'TaggedEntity' => 0, 'CommentID' => 0));                
            }            
        }
     ...
     */
    function getOriginalActivity($activity_id, $response_type=1)
    {
        $this->db->select('ParentActivityID, ActivityTypeID');
        $this->db->where('ActivityID', $activity_id);
        $query = $this->db->get(ACTIVITY);
        $return_id = $activity_id;
        $activity_type_id = 0;
        if ($query->num_rows()) {
            $row = $query->row();
            $activity_type_id = $row->ActivityTypeID;
            if (!empty($row->ParentActivityID)) {
                $return_id = $row->ParentActivityID;
            } 
        }
        if($response_type == 1) {
            return $return_id;
        } else {
            return array("ActivityID" => $return_id, "ActivityTypeID" => $activity_type_id);
        }
    }
     ...
     */
    function getSharedActivityDetail($ParentActivityID) {
     ...
     * @return [array]              [comment details]
     */
    function getCommentDetails($CommentGUID) {
        $this->db->select('EntityID, IsMediaExists, PostCommentID, EntityType, NoOfReplies');
     ...
     */
    function commentStatus($entity_type, $entity_guid) {        
        $entity_type = strtoupper($entity_type);
        $Return['ResponseCode'] = 412;
        $Return['Message'] = sprintf(lang('valid_value'), "Entity Type");
        if ($entity_type == 'ACTIVITY') {
    ...
    function toggleLike($Data) {
        ...
        $post_as_module_entity_guid = isset($Data['PostAsModuleEntityGUID']) ? $Data['PostAsModuleEntityGUID'] : '0';
        $post_as_module_entity_id=($post_as_module_entity_guid) ? get_detail_by_guid($post_as_module_entity_guid, $post_as_module_id) : $user_id;
        $StatusID = 2;
        $Count = 1;
        $module_id = 3;
        $module_entity_id = '';
        ...
        $activity_id = 0;
        if ($entity_type == 'ACTIVITY') {
            $entity = get_detail_by_guid($entity_guid, 0, "ActivityID, ActivityTypeID, Params, ModuleID, ModuleEntityID, UserID", 2);
            if(!empty($entity)) {
                if ($entity['ActivityTypeID'] == '23' || $entity['ActivityTypeID'] == '24') {
                    $entity_params = json_decode($entity['Params'], true);
                    $entity_guid = $entity_params['MediaGUID'];
        ...
        switch ($entity_type) {
            case 'ACTIVITY':
                $log_type = 'ActivityDislike';
                if($action_type == 'LIKE'){
                    $log_type = 'ActivityLike';
                }
                $type = 'PL';
                break;
            case 'ALBUM':
            ...
            case 'COMMENT':
                ...
                $comment_entity_type = $entity['EntityType'];
                $activity_id = $parent_entity_id = $entity['EntityID'];
                if (strtolower($comment_entity_type) == 'activity') {
                    $module_details = get_detail_by_id($parent_entity_id, 0, 'ModuleID,ModuleEntityID', 2);
                    if ($module_details['ModuleID'] == 18) {
                        $module_entity_id = $module_details['ModuleEntityID'];
                    }
                }
                $log_type = 'CommentDislike';
                if($action_type == 'LIKE'){
                    $log_type = 'CommentLike';
                }
                break;
            case 'PAGE':
                ...
                break;
            case 'MEDIA':
                $this->db->select('MediaID, MediaTypeID, AlbumID');
                $this->db->from(MEDIA . ' AS M');
                $this->db->join(MEDIAEXTENSIONS . ' AS ME', 'ME.MediaExtensionID=M.MediaExtensionID');
                $res = $this->db->where('M.MediaGUID', $entity_guid)->limit(1)->get()->row_array();
                $album_id   = $res['AlbumID'];
                $entity_id  = $res['MediaID'];
                $media_type_id = $res['MediaTypeID'];
                $module_id = 21;
                $module_entity_id = $entity_id;
                $log_type = 'MediaDislike';
                if($action_type == 'LIKE'){
                    $log_type = 'MediaLike';
                }
                $type = 'ML';
        ...
        if(IS_ARCHIVE_DB == 1) {
            $this->load->model(array('archival/restore_model'));  
            $this->restore_model->likes($entity_type, $entity_id);
            $this->restore_model->sync_entity($entity_type, $entity_id);
        }
        ...
        if ($query->num_rows() > 0) {
            $row = $query->row_array();
            $like_status = $row['StatusID'];
            $like_id = $row['PostLikeID'];
            ...
            if($row['StatusID'] == 2 && $action_post_status == 2) {
                ...
                $this->update_dislike_count($entity_id, $entity_type, $dislike_count, $album_id);
                ...
                $this->db->where('PostLikeID', $like_id)
                    ->update(POSTLIKE, array('StatusID' => $StatusID, 'ModifiedDate' => $CreatedDate, 'DeviceTypeID' => $this->DeviceTypeID));
                $this->update_like_count($entity_id, $entity_type, $like_count, $album_id);
            }
            $point_data = array('EntityID' => $like_id, 'EntityType' => 3);
            initiate_worker_job('revert_point', $point_data,'','point');   
        } else {            
            ...
            // Set promotion date if activity is promoted            
            initiate_worker_job('set_promotion', array('ActivityID' => 0, 'EntityID' => $entity_id, 'EntityType' => $entity_type));
            // Insert activity log and its score
            
            $user_log_data = array(
            'ModuleID' => 3, 'ModuleEntityID' => $user_id, 'UserID' => $user_id, 'ActivityID' => $activity_id,
            'ActivityTypeID' => 19, 'ParentCommentID' => 0, 'PostAsModuleID' => 3,
            'PostAsModuleEntityID' => $user_id, 'EntityID' => $PostLikeID, 'LoginSessionKey' => $this->post_data[AUTH_KEY]
            );
            initiate_worker_job('add_activity_log', $user_log_data);                        
        }
        ...
        if ($StatusID == 2) {            
            initiate_worker_job('save_log', array('UserID' => $user_id, 'LogType' => $log_type, 'EntityGUID' => $entity_guid, 'Flag' => FALSE, 'DeviceTypeID' => $this->DeviceTypeID));
            $like_data = array(
                "EntityType" => $entity_type, 
                "EntityID" => $entity_id, 
                "UserID" => $user_id, 
                ...
                "PostAsModuleEntityID" => $post_as_module_entity_id, 
                "PostAsModuleID" => $post_as_module_id,
                "Params" => $params, 
                "MediaTypeID" => $media_type_id, 
                "CommentEntityType" => $comment_entity_type, 
                "ParentEntityID" => $parent_entity_id,
                "LikeStatus" => $like_status);
            initiate_worker_job('send_like_notification', $like_data, '', 'notification');
    ...
    /**
     * [send_like_notification Used to send like notification]
     * @param  [array] $data [Like data]
     */
    function send_like_notification($data) {
        $entity_type = $data['EntityType'];
        $entity_id = $data['EntityID'];
        $user_id = $data['UserID'];
        $module_id = $data['ModuleID'];
        ...
        $params = $data['Params'];
        $media_type_id = $data['MediaTypeID'];
        $comment_entity_type = $data['CommentEntityType'];
        $parent_entity_id = $data['ParentEntityID'];
        $like_status = $data['LikeStatus'];
        if ($entity_type == 'ACTIVITY') {
            $notification_user_list = $this->notification_model->get_notification_user_list($entity_id, $user_id, false);
            $post_tagged_users = $this->get_tagged_user('ACTIVITY', $entity_id);
            if ($notification_user_list) {
                $parameters[0]['ReferenceID'] = $user_id;
                $parameters[0]['Type'] = 'User';
            ...
            $notification_user_list = $this->get_comment_owner_and_tagged_users($entity_id, $user_id);
            $owner = isset($notification_user_list['owner'][0]) ? $notification_user_list['owner'][0] : array();
            $comment_tagged_users = $notification_user_list = $notification_user_list['tagged'];
            if(!empty($owner)) {
                $notification_user_list[] = $owner;
            }
            ...
            if ($comment_entity_type == 'MEDIA') {
                $notification_type_id = 50;
            }
            // get the comment ParentEntity details
            if (!empty($parent_entity_id)) {
                $entity = get_detail_by_id($parent_entity_id, 0, "ModuleID, ModuleEntityID", 2);
                if (!empty($entity))
                {
        ...
        if ($notification_user_list) {
            ...
            if (!empty($module_entity_id)) {
                $ignore_user = $this->block_user_list($module_entity_id, $module_id);
            }
            foreach ($notification_user_list as $key => $value) {
                if (in_array($value, $ignore_user)) {
                    unset($notification_user_list[$key]);
                }

                if (isset($post_tagged_users) && !empty($post_tagged_users)) {
                    if (in_array($notification_user_list[$key], $post_tagged_users)) {
                        $tagged_notify[] = $notification_user_list[$key];
                        unset($notification_user_list[$key]);
                    }
                }
                //unset comment tagged users
                if (isset($comment_tagged_users) && !empty($comment_tagged_users)) {
                    if (in_array($notification_user_list[$key], $comment_tagged_users)) {
                        $comment_tagged_notify[] = $notification_user_list[$key];
                        unset($notification_user_list[$key]);
                    }
                }
            }
            if ($notification_user_list) {
                if(!($notification_type_id=='3'  && $like_status == '2')) {
                    initiate_worker_job('add_notification', array('NotificationTypeID' => $notification_type_id, 'SenderID' => $user_id, 'ReceiverIDs' => $notification_user_list, 'RefrenceID' => $entity_id, 'Parameters' => $parameters, 'ExtraParams' => $notify_params),'','notification');                    
                }
            }
            if ($tagged_notify) {
                initiate_worker_job('add_notification', array('NotificationTypeID' => 54, 'SenderID' => $user_id, 'ReceiverIDs' => $tagged_notify, 'RefrenceID' => $entity_id, 'Parameters' => array($parameters[0]), 'ExtraParams' => array()),'','notification');                
            }
            if ($comment_tagged_notify) {
                initiate_worker_job('add_notification', array('NotificationTypeID' => 140, 'SenderID' => $user_id, 'ReceiverIDs' => $comment_tagged_notify, 'RefrenceID' => $entity_id, 'Parameters' => array($parameters[0]), 'ExtraParams' => array()),'','notification');
            }
        }
    }

    /**
     * [get_comment_owner_and_tagged_users Used to get comment owner and tagged users]
     * @param  [int] $comment_id [comment ID]
     * @param  [int] $user_id    [User ID]
     * @return [array]           [array of comment owner and tagged users ]
     */
    function get_comment_owner_and_tagged_users($comment_id, $user_id) {
     ...
     */
    function getMediaOwner($MediaID) {
        $arr = array();
     ...
     * @return [type]               [description]
     */
    function update_like_count($entity_id, $entity_type, $count = 1, $album_id=0)
    {
        $set_field = "NoOfLikes";
        $table_name = ACTIVITY;
        $condition = array("ActivityID" => $entity_id);
     ...
     * @return [type]               [description]
     */
    function update_dislike_count($entity_id, $entity_type, $count = 1, $album_id=0)
    {
     ...
     */
    function get_activity_details($entity_id, $activity_type_id)
    {
        $data = array();
        if ($activity_type_id == 14 || $activity_type_id == 15) {
            $table = MEDIA;
            $where = 'MediaID';
            $this->current_db->where($where, $entity_id);
            $this->current_db->limit(1);
            $query = $this->current_db->get($table);            
        } else {
            $where = 'A.ActivityID';
            $activity['OriginalActivityProfileURL'] = '';
            $this->current_db->select('M.ModuleName,A.*,ATY.ActivityType,U.FirstName,U.LastName,U.UserGUID,P.Url');
            $this->current_db->from(ACTIVITY . ' A');
            $this->current_db->join(ACTIVITYTYPE . ' ATY', 'A.ActivityTypeID=ATY.ActivityTypeID', 'left');
            $this->current_db->join(USERS . ' U', 'A.UserID=U.UserID', 'left');
            $this->current_db->join(PROFILEURL . ' P', 'A.UserID=P.EntityID', 'left');
            $this->current_db->join(MODULES . ' M', 'M.ModuleID=A.ModuleID', 'left');
            $this->current_db->where('P.EntityType', 'User');
            $this->current_db->where($where, $entity_id);
            $this->current_db->limit(1);
            $query = $this->db->get();
        }
     ...
     */
    function approveFlagActivity($user_id, $activity_id)
    {
        $this->db->select('ModuleID,ModuleEntityID');
        $this->db->from(ACTIVITY);
        $this->db->where('ActivityID', $activity_id);
     ...
     */
    function autoSuggestPostOwner($user_id, $search_key, $module_id = 0, $module_entity_id = 0, $AllActivity = 0, $filter_type = 0)
    {
        if (!$module_entity_id)
        {
     ...
     */
    function getLikeName($activity_id, $user_id, $entity_owner = 2, $blocked_users = array(), $entity_type = 'ACTIVITY')
    {
        $pagerow = array();
     ...
     * @return [array]            [Comment details]
     */
    function get_last_comment($activity_id, $user_id,$entity_type='ACTIVITY') {
        $this->db->select('*');
        $this->db->from(POSTCOMMENTS);
        $this->db->where('EntityType',$entity_type);
        $this->db->where('EntityID', $activity_id);
     ...
     * @return [array]                [Activity Details]
     */
    function get_recent_activities($user_id, $current_user_id)
    {
        $inner_sql = array();
    ...
    function parse_tag($post_content,$activity_id=0,$is_tag_link=1)
    {
        ...
        if(!isset($activity_cache['mtd'])) {
            ...
            if (!empty($matches[1])) { 
                foreach ($matches[1] as $match) {
                    $this->current_db->select('ModuleID,ModuleEntityID,Title');
                    $this->current_db->where('MentionID', $match);
                    $this->current_db->limit(1);
                    $result = $this->current_db->get(MENTION);
                    if ($result->num_rows()) {
            ...
            foreach ($matches[1] as $match) {
                if($activity_cache && isset($activity_cache['mtd'])) {
                    foreach($activity_cache['mtd'] as $key=>$val) {                        
                        ...
                        if($match == $key) {
                            if($this->IsApp == 1){
                                if(!in_array($this->AppVersion, array('v1', 'v2'))) {                            
                                    $post_content = strtr($post_content, array('{{' . $match . '}}' => '{{'.$title.':'.$module_entity_id.':'.$module_id.'}}'));
                                } else {
                    ...
                    if ($result->num_rows()) {
                        ...
                        if($this->IsApp == 1){
                            if(!in_array($this->AppVersion, array('v1', 'v2'))) {
                                $post_content = strtr($post_content, array('{{' . $match . '}}' => '{{'.$row['Title'].':'.$row['ModuleEntityID'].':'.$row['ModuleID'].'}}'));
                            } else {
     ...
     * @return [string]               [Parsed Post Content]
     */
    function parse_tag_html($post_content) {
        preg_match_all('/{{([0-9]+)}}/', $post_content, $matches);
        if (!empty($matches[1])) {
            foreach ($matches[1] as $match) {
                $this->current_db->select('ModuleID,ModuleEntityID,Title');
     ...
     */
    function live_feed($user_id, $page_no = PAGE_NO, $page_size = 20, $count_only = false)
    {
...
//INNER JOIN (SELECT MAX(GM.JoinedAt) as JoinedAt FROM ".GROUPMEMBERS." GM ".$cond.") PLJ ON PLJ.JoinedAt=GM.JoinedAt
     ...
     * @return [array]               [Live feed details]
     */
    function get_single_live_feed($user_id, $from_user_id, $type, $entity_guid)
    {
        $time_zone = get_user_time_zone($user_id);
        $current_utc_date = get_current_date('%Y-%m-%d');
        $friend_follower = $this->user_model->gerFriendsFollowersList($user_id, true, 1);
...
//INNER JOIN (SELECT MAX(GM.JoinedAt) as JoinedAt FROM ".GROUPMEMBERS." GM ".$cond.") PLJ ON PLJ.JoinedAt=GM.JoinedAt
        ...
        if ($entity_type == "USER")
        {
            ...
            if ($user_id != $entity_id)
            {
                ...
                if($this->settings_model->isDisabled(10)) { // If friend module is disabled
                    $entity_details['ShowFollowBtn'] =  1;
                    $entity_details['follow'] =  'Follow';
                    ...
                    $this->db->where('F.StatusID', 2);
                    $this->db->where('F.UserID', $user_id);
                    $this->db->where('F.TypeEntityID', $entity_id);
                    
                    $query = $this->db->get();
                    $follow_result = $query->row_array(); 
                    if($follow_result) {
                        $entity_details['ShowFollowBtn'] = $follow_result['ShowFollowBtn'];
                        $entity_details['follow'] = $follow_result['follow'];
                    }
                }
                if(!$this->settings_model->isDisabled(10)) { // If friend module is not disabled                                    
     ...
     * @return boolean
     */
    function is_tagged($activity_id, $user_id) {
        $activity_cache=array();
        if(CACHE_ENABLE && $activity_id) {
            $activity_cache=$this->cache->get('activity_'.$activity_id); 
            if(!empty($activity_cache)) {
     ...
     * @return boolean
     */
    function set_user_tagged($user_id) {
     ...
     */
    function remove_tags($activity_guid, $user_id) {
    ...
    function flag_users_detail($user_id, $entity_id, $entity_type) {
        ...
        if ($query->num_rows()) {
            ...
            foreach ($users as $value) {
                ...
                $value['FlagReason'] = strtr($value['FlagReason'], array(',' => ', '));
                if ($privacy_details['Label']) {
                    foreach ($privacy_details['Label'] as $privacy_label) {
                        if ($privacy_label['Value'] == 'view_profile_picture' && !in_array($privacy_label[$privacy], $users_relation)) {
    ...
    function get_activity_files($activity_id) {
        ...
        $this->current_db->select('Media.MediaGUID,Media.OriginalName,Media.ImageName,Media.Size,MediaExtension.Name as MediaExtension,MediaType.Name as MediaType');
        $this->current_db->from(MEDIA . ' AS Media');
        $this->current_db->join(MEDIAEXTENSIONS . " AS MediaExtension", ' MediaExtension.MediaExtensionID = Media.MediaExtensionID', 'left');
        $this->current_db->join(MEDIATYPES . " AS MediaType", ' MediaType.MediaTypeID = MediaExtension.MediaTypeID');
        $this->current_db->where('MediaType.MediaTypeID', '4'); //documents
        $this->current_db->where('Media.MediaSectionReferenceID', $activity_id);
        $this->current_db->where_in('Media.StatusID', array(2,10));
        $sql = $this->current_db->get();
     ...
     * @return [Array]                    [Activity array]
     */
    function get_newsfeed_files($user_id, $page_no = 1, $page_size = 20, $count_only = 0, $search_key = false, $entity_id = '', $entity_module_id = '', $show_archive = 0, $feed_sort_by = 1, $feed_user = 0, $filter_type = 0, $is_media_exists = 2, $is_file_exists = 1, $start_date = false, $end_date = false, $reminder_date = array(), $activity_guid = '', $mentions = array(), $activity_type_filter = array()) {
        $this->load->model('polls/polls_model');
    ...
    */
    ...
    {    
        ...
        if ($result->num_rows()) {
            ...
            foreach ($result->result_array() as $res)
            {
                ...
                $activity['ShareDetails'] = array('Title' => 'Activity', 'Summary' => $activity['PostContent'], 'Image' => '', 'Link' => site_url() . 'activity/' . $activity['ActivityGUID']);
                if (isset($activity['Album'][0]['Media'][0]))
                {
                    $share_image = IMAGE_SERVER_PATH . 'upload/';
    ...
    function get_seen_list($user_id, $entity_type, $entity_id, $page_no, $page_size, $count_only=false)
    {
        ...
        $this->db->select('U.UserGUID,U.FirstName,U.LastName,U.ProfilePicture, U.UserID, "3" as ModuleID', false);
        $this->db->select('IFNULL(UD.Occupation,"") as Occupation', FALSE);
        $this->db->select('L.Name, L.HindiName, L.ShortName, L.LocalityID');
        
        $this->db->from(ENTITYVIEW.' EV');        
        $this->db->join(USERS.' U','U.UserID=EV.UserID');
        $this->db->join(USERDETAILS . ' UD', 'U.UserID=UD.UserID');        
        ...
        $this->db->_protect_identifiers = FALSE;
        $this->db->order_by("CASE WHEN U.UserID = ".$user_id." THEN 2 ELSE 0 END DESC");
        $this->db->_protect_identifiers = TRUE;
        $this->db->order_by('EV.ModifiedDate', 'DESC');
        $query = $this->db->get();
        if($query->num_rows()) {   
            $followers = array();
            $is_follow_disabled = $this->settings_model->isDisabled(11);
            ...
            foreach($query->result_array() as $key=>$val) {
                ...
                if(!$is_follow_disabled) {
                    ...
                    if ($val['UserID'] == $user_id) {
                        $val['IsFollow'] = 2;
                    } else if (in_array($val['UserID'], $followers)) {
                        $val['IsFollow'] = 1;
                    } 

                    $IsAdmin = $this->user_model->is_super_admin($r['UserID']);
                    $IsAdminGuid = $this->get_user_guid_by_user_ids(array(ADMIN_USER_ID)); // admin set from config page
                    if($IsAdmin || $val['UserID']==$IsAdminGuid )
                    {
                        $val['IsFollow']=2;
                    }
                }
    ...
    function cache_all_activity($page_no, $page_size)
    {
        $this->db->select('ActivityID');
        $this->db->from(ACTIVITY);
        $this->db->where('StatusID',2);
    ...
    function get_popular_discussions($user_id,$module_id, $module_entity_id, $activity_type_id,$page_no,$page_size)
    {
        $blocked_users = $this->activity_model->block_user_list($user_id, 3);
        $activity_ids = array();
     ...
     */
    function get_popuplar_activity_of_module($module_id, $module_entity_guid, $activity_type_id, $time_duration = 30){
     ...
     * @return [JSON] [JSON Object]
     */
    function request_question_answer($activity_guid, $module_id, $request_by, $request_to, $note, $status = 'PENDING'){
        ...
        if (!empty($request_to))
        {
            ...
            if(!empty($requested_user)){
                ...
                initiate_worker_job('add_notification', array('NotificationTypeID' => $notification_type_id, 'SenderID' => $request_by, 'ReceiverIDs' => $requested_user, 'RefrenceID' => $activity_id, 'Parameters' => $parameters, 'ExtraParams' => array()),'','notification');                
            }
            return true;
        }
    }
    ...
    function add_activity_history($activity_id_previous,$data,$previously_tagged,$status,$publish_post) {
        ...
        if($status==2) {
            ...
            if (isset($mentions) && !empty($mentions)) {
                ...
                $parameters[0]['ReferenceID'] = $user_id;
                $parameters[0]['Type'] = 'User';
                initiate_worker_job('add_notification', array('NotificationTypeID' => 114, 'SenderID' => $user_id, 'ReceiverIDs' => $mentions, 'RefrenceID' => $activity_id, 'Parameters' => $parameters, 'ExtraParams' => array()),'','notification');                
            }
            ...
            if($activity_id_previous && !$publish_post) {
                ...
                if(!empty($NotificationUserList)) {      
                    ...
                    if (isset($NotificationUserList) && !empty($NotificationUserList)) {
                        ...
                        $parameters[0]['ReferenceID'] = $user_id;
                        $parameters[0]['Type'] = 'User';
                       // initiate_worker_job('add_notification', array('NotificationTypeID' => 119, 'SenderID' => $user_id, 'ReceiverIDs' => $NotificationUserList, 'RefrenceID' => $activity_id, 'Parameters' => $parameters, 'ExtraParams' => array()),'','notification');                        
                    }
    ...
    function change_activity_version($activity_guid,$history_id,$user_id) {   
        $this->db->select('UserID, ActivityID, ModuleID, ModuleEntityID');
        $this->db->where('ActivityGUID', $activity_guid);        
        $this->db->limit(1);
        $result = $this->db->get(ACTIVITY);                 
        if ($result->num_rows()) {
    ...
    function add_comment_history($comment_id_previous,$data,$previously_tagged,$status) {   
        ...
        if($status==2) {
            ...
            if (!empty($previously_tagged)) {
                ...
                $parameters[0]['ReferenceID'] = $user_id;
                $parameters[0]['Type'] = 'User';
                initiate_worker_job('add_notification', array('NotificationTypeID' => 120, 'SenderID' => $user_id, 'ReceiverIDs' => $previously_tagged, 'RefrenceID' => $comment_id, 'Parameters' => $parameters, 'ExtraParams' => array()),'','notification');                
            }
        }
     ...
     * @return [array]            [Comment details]
     */
    function get_request_detail($activity_id, $user_id) {
        $this->db->select('*');
    ...
    /**
     * [get_activity_versions Get activity history details]
     * @param  [string] $activity_guid [Activity GUID]
     * @param  [int] $history_id     [History ID]
     * @param  [int] $user_id     [User ID]
     * @return [array]            [Activity history details]
     */
     ...
     * @return [Array]                    [Activity array]
     */
    function similar_forum_discussion($entity_id, $module_id, $page_no, $page_size, $current_user,$count_only = false, $m_entity_id = '', $entity_module_id = '',$post_type,$activity_guid) {
        $this->load->model(array('tag/tag_model'));
        $e_module_id = $module_id;
        ...
        if ($result->num_rows())
        {
            ...
            foreach ($result->result_array() as $res)
            {
                ...
                {
                    $activity['PostTitle']= $this->activity_front_helper_model->get_activity_title('',$temp_post_content);
                }
                //$activity['ActivityLink'] = site_url() . 'activity/' . $activity['ActivityGUID'];
     ...
     * @return [Array]                    [Activity array]
     */
    function similar_discussion_posts($entity_id, $module_id, $page_no, $page_size, $current_user,$count_only = false, $m_entity_id = '', $entity_module_id = '',$post_type,$activity_guid)
    {
        $this->load->model(array('tag/tag_model'));
        $e_module_id = $module_id;
        $e_entity_id = $entity_id;
     ...
     * @param  [int]       $user_id        [User ID]
     * @param  [int]       $entity_id      [Module Entity ID]     
     * @param  [int]       $remove_for_all [HIDE All announcement or not ]     
     */
    function hide_announcement($user_id,$entity_id,$remove_for_all) {
        if($remove_for_all) {
     ...
     * @return [Array]
     */
    function get_popular_tags($module_id,$module_entity_id,$page_no,$page_size) {
    ...
    function set_featured_post($user_id, $module_id, $module_entity_id, $activity_id, $need_check = true) {
        if ($need_check) {
            ...
            if ($this->user_model->is_super_admin($user_id)) {
                $check = true;
            }
        } else {
            $check = true;
     ...
     * @return [type]               [description]
     */
    function update_forum_discussions_count($module_entity_id, $module_id, $count = 1) {
    ...
    function get_total_module_posts($module_id,$module_entity_id) {
     ...
     * @return Array
     */
    function get_welcome_question($user_id,$activity_rule_id) {
        $activity = array();
        $this->db->select('ActivityID');
     ...
     * @return Array
     */
    function check_user_commented_on_activity($activity_id,$user_id) {
    ...
    function activity_recent_participants($activity_id,$user_id) {
        $result['Data']=array();
    ...
    function get_allowed_activity_type($type,$module_id,$filter='',$enable=1) {
        ...
        if($type == 'wall') {
            if($module_id == '1') {
                ...
                $activity_type_allow = array(1, 8, 9, 10, 5, 6, 14, 15, 25, 7, 11, 12, 26, 36, 37, 38, 39, 49,50); //23, 24,
            } else if($module_id == '14') {
                $activity_type_allow = array(11, 5, 6);// 23, 24
            } else if($module_id == '18') {
                $activity_type_allow = array(1, 8, 12, 5, 6, 25);//23, 24,
                ...
                $activity_type_allow = array(49,50);
            } else {
            ...
            if($module_id == '3') {
                ...
                $activity_type_allow = array(1,8);//23, 24,
            }
    ...
    function is_valid_activity_type($activity_type_id) {
        $r = true;
    ...
    /**
     * [visibility Get the activity visibility details]
     * @param  [int]       $activity_id   [Activity ID]
     * @param  [int]       $page_no       [Page No]
     * @param  [int]       $page_size     [Page Size]
     * @return [Array]                    [activity visibility details]
     */
    function visibility($activity_id, $page_no = PAGE_NO, $page_size = PAGE_SIZE,$count_flag=FALSE) {        
        $this->current_db->select('IFNULL(W.Name,"") as WName', FALSE);
        $this->current_db->select('IFNULL(W.WardID,"") as WID', FALSE);
        $this->current_db->select('IFNULL(W.Number,"") as WNumber', FALSE);
        $this->current_db->select('IFNULL(W.Description,"") as WDescription', FALSE);            
        $this->current_db->from(ACTIVITYWARD . ' AW');
        $this->current_db->join(WARD . ' W', 'W.WardID=AW.WardID');
        $this->current_db->where('AW.ActivityID', $activity_id);
        $this->current_db->order_by('W.Number', 'ASC');
        if($count_flag) {
            $query = $this->current_db->get();
            return $query->num_rows();
        }
     ...
     /**
     * [get_activity_ward_ids Get the activity ward ids]
     * @param  [int]       $activity_id   [Activity ID]
     * @return [Array]                    [activity ward ids]
     */
    function get_activity_ward_ids($activity_id) {
        $ward_ids = array();
        if(CACHE_ENABLE && $activity_id) {
            $activity_cache=$this->cache->get('activity_'.$activity_id);
            if(isset($activity_cache['wid'])) {
                $ward_ids = $activity_cache['wid'];
            }
        }
        if(empty($ward_id)) {
            $this->current_db->select('GROUP_CONCAT(WardID) as WardIDs');
            $this->current_db->from(ACTIVITYWARD);
            $this->current_db->where('ActivityID', $activity_id);
            $query = $this->current_db->get();

            if ($query->num_rows() > 0) {
                $row_ids=$query->row_array();
                if(!empty($row_ids['WardIDs'])) {
                    $ward_ids=  explode(',',$row_ids['WardIDs']);
                }
            }
        }
        return $ward_ids;
    ...
    /**
     * [get_deleted Get deleted posts]          
     * @param  [int]    $current_user          [Current User ID]
     * @param  [int]    $activity_id          [Activity ID]
     * @return [Array]                    [Activity details]
     */
    function get_deleted_activity($user_id, $activity_id, $comment_id=0) {     
        $role_id=2;
        $module_id = 3;        
        $entity_id = $user_id;
        $this->load->model(array('polls/polls_model','activity/activity_hide_model'));
        $this->db->select('A.*,ATY.ViewTemplate,ATY.Template,ATY.LikeAllowed,ATY.CommentsAllowed,ATY.ActivityType,ATY.ActivityTypeID,ATY.FlagAllowed,ATY.ShareAllowed,ATY.FavouriteAllowed,U.FirstName,U.LastName,U.UserGUID,U.ProfilePicture, U.IsVIP, U.IsAssociation');   
        $this->db->from(ACTIVITY.' A');
        $this->db->join(ACTIVITYTYPE . ' ATY', 'A.ActivityTypeID=ATY.ActivityTypeID');
        $this->db->join(USERS . ' U', 'U.UserID=A.UserID');                
        ...
        if($query->num_rows()) {
            $feed_result = $query->result_array();
            $return = $this->activity_result_filter_model->filter_result_set($feed_result, 1, $user_id, 0, $role_id, 1, false, $entity_id, $module_id, $this, 0, 3, 0, $comment_id);
        }
        return $return;
    }
    ...
    function get_entity_comment_reply_count($entity_id, $entity_type='ACTIVITY') {
        $this->current_db->select('COUNT(PC.PostCommentID) as TotalRow');
        $this->current_db->from(POSTCOMMENTS . ' PC');
        $this->current_db->join(USERS . ' U', 'PC.UserID=U.UserID AND U.StatusID NOT IN (3,4)', 'left');
        $this->current_db->where('PC.EntityType', $entity_type);        
    ...
    /**
     * [update_quiz_post_count description]
     * @param  [int]    $quiz_id   [quiz Id]
     * @param  int      $count      [Count increment/decrement]
     */
    function update_quiz_post_count($quiz_id, $count = 1) {
        $set_field = "TotalPost";
        $condition = array("QuizID" => $quiz_id);
        $this->db->where($condition);
        $this->db->set($set_field, "$set_field+($count)", FALSE);
        $this->db->set('ModifiedDate', get_current_date('%Y-%m-%d %H:%i:%s'));
        $this->db->update(QUIZ);
    ...
    function get_user_guid_by_user_ids($UserIDs =array())
    {
             $result = $this->db->select("*",FALSE)
                            ->from(USERS." AS U")
                            ->where_in('U.UserID',$UserIDs)->get()->row_array();
                            //echo $this->db->last_query();
