...
class Cron_model extends Common_Model {
    ...
    function ward_user_count() {
        $today_date = get_current_date('%Y-%m-%d', 1);
        $this->db->select("COUNT(L.WardID) as TotalUser");
        $this->db->select('IFNULL(W.WardID,"") as WardID', FALSE);        
        $this->db->from(USERS. ' U');
        $this->db->join(USERDETAILS. ' UD', 'UD.UserID=U.UserID');
        $this->db->join(LOCALITY . ' L', 'L.LocalityID=UD.LocalityID');
        $this->db->join(WARD . ' W', 'W.WardID=L.WardID');
        $this->db->like('U.CreatedDate', $today_date);
        $this->db->group_by('L.WardID');
        $query = $this->db->get();        
        //echo $this->db->last_query();die;
        if($query->num_rows()) {
            foreach ($query->result_array() as $result) {                    
                $total_user = $result['TotalUser'];
                $ward_id = $result['WardID'];  
                if($ward_id) {
                    $this->db->select('PollAnalyticsID');
                    $this->db->from(WARDUSERCOUNT);
    ...
    function ward_engagement($no_of_days=1) {
        log_message('error', 'V3 Ward Engagement => '.get_current_date('%Y-%m-%d %H:%i:%s'));
        for ($d = 1; $d <= $no_of_days; $d++) {
            $today_date = get_current_date('%Y-%m-%d', $d);
            $this->db->select("COUNT(A.ActivityID) as TotalPost");
            $this->db->select('AW.WardID');        
            $this->db->from(ACTIVITYWARD. ' AW');
            $this->db->join(ACTIVITY. ' A', 'A.ActivityID=AW.ActivityID');
            //$this->db->where('AW.WardID!=',1);
            $this->db->like('A.CreatedDate', $today_date);
            $this->db->group_by('AW.WardID');
            $this->db->order_by('AW.WardID');
            $query = $this->db->get();        
           // echo $this->db->last_query();die;
            $total_post_arr = array();
            if($query->num_rows()) {
                $result = $query->result_array();
                $total_post_arr = array_column($result, NULL, 'WardID');
            }

            $this->db->select("COUNT(PC.PostCommentID) as TotalComment");
            $this->db->select('AW.WardID');        
            $this->db->from(ACTIVITYWARD. ' AW');
            $this->db->join(POSTCOMMENTS. ' PC', 'PC.EntityID=AW.ActivityID AND PC.EntityType="ACTIVITY"');
            //$this->db->where('PC.ParentCommentID', 0);
            //$this->db->where('AW.WardID!=',1);
            $this->db->like('PC.CreatedDate', $today_date);
            $this->db->group_by('AW.WardID');
            $this->db->order_by('AW.WardID');
            $query = $this->db->get();    
            ...
            if($query->num_rows()) {
                $result = $query->result_array();
                $total_comment_arr = array_column($result, NULL, 'WardID');
            }  

            $this->db->select("COUNT(PC.PostCommentID) as TotalComment");
            $this->db->select('AW.WardID');        
            $this->db->from(ACTIVITYWARD. ' AW');
            $this->db->join(MEDIA. ' M', 'M.MediaSectionReferenceID=AW.ActivityID AND M.MediaSectionID=3 AND M.StatusID NOT IN (3, 21)');
            $this->db->join(POSTCOMMENTS. ' PC', 'PC.EntityID=M.MediaID AND PC.EntityType="MEDIA"');
            //$this->db->where('PC.ParentCommentID', 0);
            //$this->db->where('AW.WardID!=',1);
            $this->db->like('PC.CreatedDate', $today_date);
            $this->db->group_by('AW.WardID');
            $this->db->order_by('AW.WardID');
            $query = $this->db->get();    
            $total_media_comment_arr = array();
            if($query->num_rows()) {
                $result = $query->result_array();
                $total_media_comment_arr = array_column($result, NULL, 'WardID');
            }  
            
            $this->db->select("COUNT(PL.PostLikeID) as TotalLike");
            $this->db->select('AW.WardID');
            $this->db->from(ACTIVITYWARD. ' AW');
            $this->db->join(POSTLIKE. ' PL', 'PL.EntityID=AW.ActivityID AND PL.EntityType="ACTIVITY"');            
            //$this->db->where('AW.WardID!=',1);
            $this->db->like('PL.CreatedDate', $today_date);
            $this->db->group_by('AW.WardID');
            $this->db->order_by('AW.WardID');
            $query = $this->db->get();    
            ...
            if($query->num_rows()) {
                $result = $query->result_array();
                $total_like_arr = array_column($result, NULL, 'WardID');
            }  

            $ward_engagements = array();
            foreach ($total_post_arr as $result) {      
                /* if($result['WardID'] == 1) {
                    continue;
                }
                */
                $ward_engagement = array();              
                $ward_engagement['TotalPost'] = $result['TotalPost'];
                $ward_engagement['WardID'] = $result['WardID'];
                $ward_engagement['CreatedDate'] = $today_date;
                $ward_engagement['TotalComment'] = 0;
                $ward_engagement['TotalPostLike'] = 0;
                if(!empty($total_comment_arr[$result['WardID']]))  {
                    $ward_engagement['TotalComment'] = $total_comment_arr[$result['WardID']]['TotalComment'];
                    unset($total_comment_arr[$result['WardID']]);
                } 
                if(!empty($total_media_comment_arr[$result['WardID']]))  {
                    $ward_engagement['TotalComment'] = $ward_engagement['TotalComment'] + $total_media_comment_arr[$result['WardID']]['TotalComment'];
                    unset($total_media_comment_arr[$result['WardID']]);
                } 
                if(!empty($total_like_arr[$result['WardID']]))  {
                    $ward_engagement['TotalPostLike'] = $total_like_arr[$result['WardID']]['TotalLike'];
                    unset($total_like_arr[$result['WardID']]);
                }              
                $ward_engagements[$result['WardID']] = $ward_engagement;
            }

            foreach ($total_comment_arr as $result) {      
                /* if($result['WardID'] == 1) {
                    continue;
                }
                */
                $ward_engagement = array();              
                $ward_engagement['TotalPost'] = 0;
                $ward_engagement['WardID'] = $result['WardID'];
                $ward_engagement['CreatedDate'] = $today_date;
                $ward_engagement['TotalComment'] = $result['TotalComment'];
                $ward_engagement['TotalPostLike'] = 0;  
                if(!empty($total_media_comment_arr[$result['WardID']]))  {
                    $ward_engagement['TotalComment'] = $ward_engagement['TotalComment'] + $total_media_comment_arr[$result['WardID']]['TotalComment'];
                    unset($total_media_comment_arr[$result['WardID']]);
            ...
            foreach ($total_media_comment_arr as $result) {      
                /* if($result['WardID'] == 1) {
                    continue;
                }
                */
            ...
            foreach ($total_like_arr as $result) {      
                /* if($result['WardID'] == 1) {
                    continue;
                }
                */
                ...
                $ward_engagement['CreatedDate'] = $today_date;
                $ward_engagement['TotalComment'] = 0;
                $ward_engagement['TotalPostLike'] = $result['TotalLike'];                 
                            
                $ward_engagements[$result['WardID']] = $ward_engagement;
            }

            if (!empty($ward_engagements)) { 
                $ward_engagements = array_values($ward_engagements);           
                //print_r($ward_engagements);
                $this->db->insert_on_duplicate_update_batch(WARDENGAGEMENT, $ward_engagements);
            }            
        }
    }

    function user_subscribe_to_topic($page_no=1, $page_size=900, $ward_data=array()) {
        ...
        if(!empty($ward_data)) {
            $this->db->join(USERDETAILS . ' UD ', 'UD.UserID = U.UserID');
            $this->db->join(LOCALITY . ' L', 'L.LocalityID=UD.LocalityID');
            $this->db->join(WARD . ' W', 'W.WardID=L.WardID AND W.WardID='.$ward_data['WID']);
        }
        ...
        if ($query->num_rows() > 0) { 
            ...
            $k = 0;
            $this->push_notification->removeDevice();
            foreach ($results as $result) {
                $token = $result['DeviceToken'];                
                $k = 1;
                $this->push_notification->addDevice($token);                               
            }
            
            if($k == 1) {
                $this->push_notification->subscribeToTopic($topic_pre_fix.'_daily_digest');
                $this->push_notification->subscribeToTopic($topic_pre_fix.'_all_ward');
                if(!empty($ward_data)) {
                    $ward_number = $ward_data['WNumber'];
                    $this->push_notification->subscribeToTopic($topic_pre_fix.'_ward'.$ward_number);
                }
            }            
            ++$page_no;
            sleep(2);
            $this->user_subscribe_to_topic($page_no, $page_size, $ward_data);
        }
    }

    function ward_user_subscribe_to_topic() {
        ...
        $this->db->where('W.WardID', 3);

        $this->db->order_by('W.Number', 'ASC');
        $query = $this->db->get();
        if ($query->num_rows()) {
            $ward_results = $query->result_array();
            foreach ($ward_results as $ward_result) {
                $this->user_subscribe_to_topic(1, 900, $ward_result);
                sleep(1);                
            }
        }        
    }
