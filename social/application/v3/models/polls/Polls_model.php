...
class Polls_model extends Common_Model {
     ...
     */
    function create($insert, $options) {
        $poll_id = 0;
        if (!empty($insert)) {
     ...
     * @Output 	- boolean
     */
    function add_options($options, $poll_id) {
        $option_data = array();
     ...
     */
    function get_poll_by_activity_id($activity_id, $user_id) {
        $this->current_db->select('P.PollID, P.PollGUID, P.Description, P.UserID', FALSE);      
        $this->current_db->select('IFNULL(P.ExpiryDateTime,"") AS ExpiryDateTime', FALSE);        
        $this->current_db->from(POLL . ' P');
        $this->current_db->where('P.ActivityID', $activity_id);
        $this->current_db->where_in('P.Status', array('ACTIVE'));
     ...
     * @return array              [poll data]
     */
    function get_poll_description($activity_id, $update_reminder_status=0) {
        $this->current_db->select('P.PollID, P.Description', FALSE);        
        $this->current_db->from(POLL . ' P');
        $this->current_db->where('P.ActivityID', $activity_id);
        $this->current_db->where_in('P.Status', array('ACTIVE'));
        $this->current_db->limit(1);
        ...
        if ($query->num_rows()) {            
            ...
            $poll_id = $poll['PollID'];  
            if($update_reminder_status == 1) {
                $this->db->set('IsReminderSent', 1);
     ...
     */
    function is_voted($poll_id, $module_id, $module_entity_id) {
        $this->current_db->select("OptionID");
        $this->current_db->from(POLLOPTIONVOTES);
        $this->current_db->where(array('PollID' => $poll_id, 'ModuleID' => $module_id, 'ModuleEntityID' => $module_entity_id));
        $this->current_db->limit(1);
        $query = $this->current_db->get();
        if ($query->num_rows() > 0) {
     ...
     * @return boolean              [description]
     */
    function get_poll_options($poll_id, $module_id, $module_entity_id, $is_anonymous = 0, $voted_option=0) {
        $this->current_db->select('PO.OptionGUID, PO.OptionID, PO.Value, PO.NoOfVotes');
        $this->current_db->from(POLLOPTION . ' PO');
        $this->current_db->where('PO.PollID', $poll_id);
        $this->current_db->where('PO.Status', 'ACTIVE');
        $query = $this->current_db->get();
        $poll_option = array();
     ...
     */
    function is_poll_expired($option_guid) {
     ...
     */
    function vote($option_guid, $module_entity_id, $module_id) {
        $this->current_db->select("OptionID, PollID");
        $this->current_db->from(POLLOPTION);
        $this->current_db->where(array('OptionGUID' => $option_guid));
        $this->current_db->limit(1);
        $query = $this->current_db->get();
        if ($query->num_rows()) {
            ...
            if (!$this->is_voted($poll_id, $module_id, $module_entity_id)) {
                $poll_data = $this->current_db->select('UserID, ActivityID')->from(POLL)->where('PollID', $poll_id)->limit(1)->get()->row();
                $created_user_id = $poll_data->UserID;
                $activity_id = $poll_data->ActivityID;

                if(IS_ARCHIVE_DB == 1) {
                    $this->load->model(array('archival/restore_model'));  
                    $this->restore_model->poll($activity_id);
                    $this->restore_model->sync_entity("ACTIVITY", $activity_id);
                }
     ...
     * @return [type]                    [description]
     */
    function get_poll_option_votes($option_id, $user_id, $count_flag = FALSE, $page_no = '', $page_size = '') {
        $this->current_db->select("CONCAT(IFNULL(U.FirstName,''), ' ',IFNULL(U.LastName,'')) as Name, U.UserGUID, U.UserID");
        $this->current_db->select('IFNULL(U.ProfilePicture,"") as ProfilePicture', FALSE);
        $this->current_db->select('IFNULL(UD.HouseNumber,"") as HouseNumber', FALSE);
        $this->current_db->select('IFNULL(UD.Occupation,"") as Occupation', FALSE);
        $this->current_db->select('IFNULL(UD.Address,"") as Address', FALSE);
        $this->current_db->select('UD.LocalityID, POV.CreatedDate');
        $this->current_db->from(POLLOPTIONVOTES . " POV");
        $this->current_db->join(USERS . " U", "U.UserID=POV.ModuleEntityID AND POV.ModuleID=3");
        $this->current_db->join(USERDETAILS . ' UD', 'UD.UserID = U.UserID');
        $this->current_db->where('POV.OptionID', $option_id);   
        $this->current_db->_protect_identifiers = FALSE;
        $this->current_db->order_by('CASE WHEN U.UserID = '.$user_id.' THEN 1 ELSE 0 END DESC', FALSE);
        $this->current_db->_protect_identifiers = TRUE;
        $this->current_db->order_by('POV.CreatedDate', 'DESC');
        ...
        if (empty($count_flag)) { // check if array needed
            ...
            if ($query->num_rows()) {
                ...
                foreach ($query->result_array() as $result) {    
                    ...
                    $data['Locality'] = array('Name' => '', 'HindiName'=>'', 'ShortName'=>'',  'LocalityID' => 0, 'IsPollAllowed' => 1, 'WName'=>'', 'WNumber'=>'', 'WID'=>'');
            ...
            return $this->current_db->get()->num_rows();
        }
    }
    ...
    function send_poll_result_notificaton() {
        //$select_date = get_current_date('%Y-%m-%d', 3);
        $select_date = get_current_date('%Y-%m-%d %H:%i:%s');
        $this->db->select('PollID, ActivityID');
        $this->db->from(POLL);
        $this->db->where('Status', 'ACTIVE');
        $this->db->where('ExpiryDateTime <', $select_date);   
        $this->db->where('IsReminderSent', 0);
            ...
            foreach ($query->result_array() as $result) {                    
                ...
                if ($voter_query->num_rows()) {
                    ...
                    if (!empty($voter_row['UserIDs'])) {
                        //log_message('error',  'send_poll_result_notificaton => '.$voter_row['UserIDs']);
                        $user_ids = explode(',', $voter_row['UserIDs']);
                    }
                    if(!empty($user_ids)) {
                        initiate_worker_job('add_notification', array('NotificationTypeID' => 155, 'SenderID' => 91, 'ReceiverIDs' => $user_ids, 'RefrenceID' => $activity_id, 'Parameters' => array(), 'ExtraParams' => array()),'','notification');
                    }
                }        
            }                
        }
