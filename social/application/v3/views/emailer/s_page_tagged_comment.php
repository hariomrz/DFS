            ...
            <td style="padding:10px 0px; width:100%;" class="mob-padding">
                <p style="color:#444444; font-size:14px;"> 
                  <span style="color:#444444; font-weight:bold;">
                    <?php echo $data['EntityDetails']['Title'].' tagged in comments' ?>
                  </span>
                </p>
            </td>
          </tr>
                    ...
                    <table width="100%" border="0" cellspacing="0" cellpadding="0" style="background-color:#F4F4F4; border-top:1px solid #E5E5E5;">
                      ...
                      <td style="padding:20px 0;" class="mob-padding">
                        ...
                        <td style="padding:0 20px 0 0; vertical-align:top;">
                          <?php if($data['user_details']['ProfilePicture']){ ?>
                              <img width="48px" height="48px" src="<?php echo IMAGE_SERVER_PATH.'upload/profile/220x220/'.$data['user_details']['ProfilePicture'] ?>"  />
                            <?php } else { ?>
                              <img width="48px" height="48px" src="<?php echo IMAGE_SERVER_PATH.'upload/profile/220x220/user_default.jpg' ?>"  />
                            <?php } ?>
                        </td>
                        <td>
                          <p style="font-size:16px; color:#444444; font-weight:500; display:block; margin:0;"><?php echo $data['user_details']['FirstName'].' '.$data['user_details']['LastName'] ?></p>
                          <p style="font-size:14px; color:#444444; margin:5px 0">
                            <?php
                                $this->load->model('activity/activity_model');
                                echo html_substr($this->activity_model->parse_tag_html($data['CommentDetails']['PostComment']),50);
                            ?>
                          </p>
                          <a href="<?php echo get_single_activity_url(get_detail_by_guid($data['activity_data']['ActivityGUID'],0,'ActivityID',1)) ?>" style="color:#00529F; font-size:14px; font-weight:500;"><?php echo lang('view_comment') ?></a>
                        </td>
                        </tr>
                        </table>
                      </td>
