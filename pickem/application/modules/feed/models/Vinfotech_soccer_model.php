			...
			foreach ($league_data as $league)
			{
				...
				if(!empty($season_array['response']['data']))
				{
					...
					foreach ($season_array['response']['data'] as $key => $value) 
					{
                        ...
                        if ($upcoming_day < $tmp_scheduled_date) 
						{
                        	continue;
                        }
			...
			foreach ($current_game as $season_game)
			{
		        ...
		        $this->db->trans_start();
					...
					if(!empty($match_info))
					{
						...
						if(isset($match_info['score']) && $match_info['score'] != "" && $match_info['score'] != "[]")
						{
							$season_update_array['score_data'] = $match_info['score'];
						}

						if(isset($match_info['winning_team_id']) && $match_info['winning_team_id'] != "")
						{
							$season_update_array['winning_team_id'] = @$team_ids[$match_info['winning_team_id']];
						}
