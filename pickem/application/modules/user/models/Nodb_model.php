...
class Nodb_model extends CI_Model {
    ...
    public function send_email_cd($to, $subject = "", $message = "", $from_email = '', $from_name = '' )
	{
        ...
        /* configuration and sending email */
        $config = array();
        $config['smtp_host'] = getenv('CD_SMTP_HOST');
        $config['smtp_user'] = getenv('CD_SMTP_USER');
        $config['smtp_pass'] = getenv('CD_SMTP_PASS');
        $config['smtp_port'] = getenv('CD_SMTP_PORT');
        $config['protocol']  = getenv('CD_PROTOCOL');
        $config['smtp_crypto'] = getenv('CD_SMTP_CRYPTO');
        $config['mailpath']  = '';
        $config['mailtype']  = 'html';
        $config['charset']   = 'utf-8';
        $config['wordwrap']  = TRUE;
